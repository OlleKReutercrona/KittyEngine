...
namespace TGA
{
	...
	namespace FBX
	{
		...
		int GatherSkeletonInternal(FbxNode* aNode, FBXSkeleton& outSkeleton, std::string& outMessage,
		                           int someParentIdx = -1)
		{
			// Check if this node is a skeleton type.
			int myIdx = someParentIdx;
			if (aNode->GetNodeAttribute() && aNode->GetNodeAttribute()->GetAttributeType() && aNode->GetNodeAttribute()
			                                                                                       ->GetAttributeType() == FbxNodeAttribute::eSkeleton)
			{
				...
				if (outSkeleton.BoneNameToIndex.find(bone.Name) != outSkeleton.BoneNameToIndex.end())
				{
					...
					sprintf_s(stringBuffer, 256, "Found more than one bone with the same name! Name was %s",
					          bone.Name.c_str());
		...
		int GatherSkeletonData(FbxNode* aNode, FBXSkeleton& outSkeleton, std::string& outMessage,
		                       int someParentIdx = -1)
		{
		...
		void GetElementMappingdata(FbxLayerElementTemplate<FbxVector4>* anElement, int aFbxContolPointIdx,
		...
		std::string MatrixToString(const Matrix& matrix)
		{
			...
			sprintf_s(buffer, 256,
			          "M11: \t%.3f\t M12: \t%.3f\t M13: \t%.3f\t M14: \t%.3f\t \nM21: \t%.3f\t M22: \t%.3f\t M23: \t%.3f\t M24: \t%.3f\t \nM31: \t%.3f\t M32: \t%.3f\t M33: \t%.3f\t M34: \t%.3f\t \nM41: \t%.3f\t M42: \t%.3f\t M43: \t%.3f\t M44: \t%.3f\t"
			          , matrix.Data[0], matrix.Data[1], matrix.Data[2], matrix.Data[3], matrix.Data[4], matrix.Data[5],
			          matrix.Data[6], matrix.Data[7]
			          , matrix.Data[8], matrix.Data[9], matrix.Data[10], matrix.Data[11], matrix.Data[12],
					...
					if (FbxTexture* texture = FbxCast<FbxTexture>(channelProperty.GetSrcObject<FbxTexture>(0)))
					{
						if (const FbxFileTexture* fileTexture = FbxCast<FbxFileTexture>(texture))
						{
							...
							if (startPos != std::string::npos)
								result.Name = result.RelativePath.substr(
									startPos + 1, result.RelativePath.size() - startPos);
							else
		...
		void GetMaterialData(const FbxSurfaceMaterial* aMaterial, FBXMaterial& outMaterial)
		{
			...
			outMaterial.VectorDisplacement = GetMaterialChannelData(
				aMaterial, FbxSurfaceMaterial::sVectorDisplacementColor);
	...
	bool FBXImporter::LoadModel(const std::string& someFilePath, FBXModel& outModel)
	{
		...
		//set fbx importer to convert to meters
		...
		if (fbxImporter->Import(fbxScene))
		{
			...
			for (FbxNode* mdlMeshNode : mdlMeshNodes)
			{
				...
				if (fbxMesh->GetElementBinormalCount() == 0 || fbxMesh->GetElementTangentCount() == 0)
				{
					...
					{
						myLastError = "Failed to generate Tangent/Binormal data for the mesh " + std::string(
							fbxMesh->GetName()) + "!";
						fbxImporter->Destroy();
						return false;
					}
			...
			//for (auto& bone : mdlSkeleton.Bones)
			//{
			//	bone.BindPoseInverse.Data[12] *= 0.01f;
			//	bone.BindPoseInverse.Data[13] *= 0.01f;
			//	bone.BindPoseInverse.Data[14] *= 0.01f;
			//}
			...
			if (!bImporterError)
			{
				...
				for (FbxNode* mdlMeshNode : mdlMeshNodes)
				{
					...
					if (hasBones)
					{
						...
						{
							FbxSkin* fbxSkin = reinterpret_cast<FbxSkin*>(fbxMesh->GetDeformer(
								deformIdx, FbxDeformer::eSkin));
							...
							for (int clusterIdx = 0; clusterIdx < fbxSkin->GetClusterCount(); clusterIdx++)
							{
								...
								fbxsdk::FbxAMatrix bindPoseInverseTransform = linkTransform.Inverse() *
									meshBindTransform * rootTransform;
								...
								FBX::ConvertFBXMatrixToArray(bindPoseInverseTransform,
								                             mdlSkeleton.Bones[jointIndex].BindPoseInverse.Data);
					...
					for (int meshMaterialIndex = 0; meshMaterialIndex < mdlMeshNode->GetMaterialCount() ||
					     meshMaterialIndex == 0; meshMaterialIndex++)
					{
						...
						if (bHasMaterials)
						{
							for (int sceneMaterialIndex = 0; sceneMaterialIndex < fbxScene->GetMaterialCount();
							     sceneMaterialIndex++)
							{
						...
						for (int p = 0; p < fbxMeshPolygonCount; p++)
						{
							...
							for (int v = 0; v < 3; v++)
							{
								...
								FBX::GetElementMappingdata(fbxNormalElement, fbxControlPtIndex, polygonIndex,
								                           fbxNormal);

								FbxVector4 fbxTangent;
								FbxGeometryElementTangent* fbxTangentElement = fbxMesh->GetElementTangent(0);
								FBX::GetElementMappingdata(fbxTangentElement, fbxControlPtIndex, polygonIndex,
								                           fbxTangent);

								FbxVector4 fbxBinormal;
								FbxGeometryElementBinormal* fbxBinormalElement = fbxMesh->GetElementBinormal(0);
								FBX::GetElementMappingdata(fbxBinormalElement, fbxControlPtIndex, polygonIndex,
								                           fbxBinormal);
									...
									switch (colElement->GetMappingMode())
									{
										case FbxGeometryElement::eByControlPoint:
										{
											switch (fbxNormalElement->GetReferenceMode())
											{
												case FbxGeometryElement::eDirect:
												{
													fbxColors[col] = colElement->GetDirectArray().GetAt(
														fbxControlPtIndex);
												}
												break;
												case FbxGeometryElement::eIndexToDirect:
												{
													const int Idx = colElement->GetIndexArray().
													                            GetAt(fbxControlPtIndex);
										...
										case FbxGeometryElement::eByPolygonVertex:
										{
											switch (colElement->GetReferenceMode())
											{
												...
												case FbxGeometryElement::eIndexToDirect:
												{
													const int Idx = colElement->GetIndexArray().GetAt(
														windedPolygonIndex);
													fbxColors[col] = colElement->GetDirectArray().GetAt(Idx);
												}
												break;
								...
								if (hasBones)
								{
									...
									typedef std::unordered_multimap<unsigned int, std::pair<size_t, float>>::iterator
										MMIter;

									// Then we use equal range to get all the data stored for this specific control point.
									std::pair<MMIter, MMIter> values = ControlPointWeights.equal_range(
										fbxControlPtIndex);
	...
	bool FBXImporter::LoadAnimation(const std::string& someFilePath, const std::vector<std::string>& BoneNames,
	                                std::vector<FBXAnimation>& outAnimationVector)
	{
		FbxImporter* fbxImporter = FbxImporter::Create(Internals::Get().fbxManager, "");
		...
		if (fbxImporter->Import(fbxScene))
		{
			...
			if (!skeletonRoot)
			{
				skeletonRoot = fbxScene->FindNodeByName(FbxString("RootNode"));
			}

			std::vector<std::string> animNodeNames;
			for (int j = 0; j < fbxScene->GetNodeCount(); j++)
			{
				const ::FbxNode* node = fbxScene->GetNode(j);
				animNodeNames.push_back(node->GetName());
			}
			...
			{
				int index = 0;
				while (const FbxAnimStack* fbxAnimStack = fbxScene->GetSrcObject<FbxAnimStack>(index))
				{
					auto& outAnimation = outAnimationVector.emplace_back();
					...
					if (const FbxTakeInfo* fbxTakeInfo = fbxScene->GetTakeInfo(animStackName))
					{
						...
						for (FbxLongLong t = startFrame; t <= endFrame && !bImporterError; t++)
						{
							...
							for (size_t j = 0; j < BoneNames.size() && !bImporterError; j++)
							{
								...
								FBX::ConvertFBXMatrixToArray(localFrameTransform,
								                             outAnimation.Frames[localFrameCounter].LocalTransforms[j].
								                             Data);
								FBX::ConvertFBXMatrixToArray(globalFrameTransform,
								                             outAnimation.Frames[localFrameCounter].GlobalTransforms[j].
								                             Data);
							}
