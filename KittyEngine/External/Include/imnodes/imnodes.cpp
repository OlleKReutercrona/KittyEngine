...
inline float GetDistanceToPolyline(
    const ImVec2& pos,
	ImVec2* polyline,
	const int num_points)
{
	...
	{
		const ImVec2 p0 = polyline[i];
		const ImVec2 p1 = polyline[i + 1];
		const ImVec2 closest = ImLineClosestPoint(p0, p1, pos);
		const float  dist = ImLengthSqr(pos - closest);
		min_dist = ImMin(min_dist, dist);
	}
	return ImSqrt(min_dist);
}
...
void GetPolylinePoints(ImVec2 startPos, ImVec2 endPos, ImVec2* points)
{
    if (startPos.x > endPos.x)
    {
        ImSwap(startPos, endPos);
    }

    float posDistance = endPos.x - startPos.x;
    float horizontalExtension = ImClamp(posDistance * 0.5f, 0.0f, 25.0f);

    points[0] = startPos;
    points[1] = startPos + ImVec2(horizontalExtension, 0);
    points[2] = endPos - ImVec2(horizontalExtension, 0);
    points[3] = endPos;
    ...
    const ImNodesAttributeType start_type)
{
    ...
    if (rectangle.Overlaps(lrect))
    {
        ...
        case ImLinkShape::Polyline:
	    {
            ImVec2 points[4];
            GetPolylinePoints(start, end, points);
            
			return RectangleOverlapsLineSegment(rectangle, points[0], points[1]) ||
				   RectangleOverlapsLineSegment(rectangle, points[1], points[2]) ||
                   RectangleOverlapsLineSegment(rectangle, points[2], points[3]);
	    }
        }
...
void BoxSelectorUpdateSelection(ImNodesEditorContext& editor, ImRect box_rect)
{
    ...
    for (int link_idx = 0; link_idx < editor.Links.Pool.size(); ++link_idx)
    {
        if (editor.Links.InUse[link_idx])
        {
            ...
            // Test
            if (RectangleOverlapsLink(editor.LinkShape, box_rect, start, end, pin_start.Type))
            {
                editor.SelectedLinkIndices.push_back(link_idx);
            }
        }
...
void ClickInteractionUpdate(ImNodesEditorContext& editor)
{
    ...
    case ImNodesClickInteractionType_LinkCreation:
    {
        ...
        case ImLinkShape::Polyline:
        {
            ...
            ImVec2 points[4];
            GetPolylinePoints(start_pos, end_pos, points);
                
            GImNodes->CanvasDrawList->AddPolyline(points, 4, GImNodes->Style.Colors[ImNodesCol_Link], false, GImNodes->Style.LinkThickness);
            break;
        }
        }
    ...
    const ImObjectPool<ImPinData>&  pins)
{
    ...
    for (int idx = 0; idx < links.Pool.Size; ++idx)
    {
        ...
        case ImLinkShape::Bezier:
	    {
	        const CubicBezier cubic_bezier = GetCubicBezier(start_pin.Pos, end_pin.Pos, start_pin.Type, GImNodes->Style.LinkLineSegmentsPerLength);
            const ImRect link_rect = GetContainingRectForCubicBezier(cubic_bezier);

            if (link_rect.Contains(GImNodes->MousePos))
            {
                const float distance = GetDistanceToCubicBezier(GImNodes->MousePos, cubic_bezier, cubic_bezier.NumSegments);
	    ...
	    {
            ImVec2 points[4];
            GetPolylinePoints(start_pin.Pos, end_pin.Pos, points);

            const float distance = GetDistanceToPolyline(GImNodes->MousePos, points, 4);
            if (distance < GImNodes->Style.LinkHoverDistance && distance < smallest_distance)
            {
                smallest_distance = distance;
                link_idx_with_smallest_distance = idx;
...
void DrawLink(ImNodesEditorContext& editor, const int link_idx)
{
    ...
    case ImLinkShape::Polyline:
	{
	    ImVec2 startPos = start_pin.Pos;
		ImVec2 endPos = end_pin.Pos;

	    ImVec2 points[4];
	    GetPolylinePoints(startPos, endPos, points);

	    GImNodes->CanvasDrawList->AddPolyline(points, 4, link_color, false, GImNodes->Style.LinkThickness);
    	break;
	}
    }
...
static void MiniMapDrawLink(ImNodesEditorContext& editor, const int link_idx)
{
    ...
    case ImLinkShape::Polyline:
    {
        ...
        ImVec2 points[4];
        GetPolylinePoints(ScreenSpaceToMiniMapSpace(editor, start_pin.Pos), ScreenSpaceToMiniMapSpace(editor, end_pin.Pos), points);
...
ImNodesStyle::ImNodesStyle()
    : GridSpacing(24.f), NodeCornerRounding(4.f), NodePadding(8.f, 8.f), NodeBorderThickness(1.f),
      ...
      Colors(),LinkShape(ImLinkShape::Bezier)
{
}
...
void BeginNodeEditor(float aScaling)
{
    ...
    ImGui::BeginGroup();
    {
    	...
    	ImGui::BeginChild(
            "scrolling_region",
            ImGui::GetContentRegionAvail() / GImNodes->scale,
            true,
		...
		GImNodes->CanvasDrawList->_FringeScale = 1.0f / GImNodes->scale;
...
void EndNodeEditor()
{
        ...
        MouseInCanvas() && !IsMiniMapHovered())
    {
        ...
        {
            GImNodes->HoveredLinkIdx = ResolveHoveredLink(editor.LinkShape, editor.Links, editor.Pins);
        }
    }
