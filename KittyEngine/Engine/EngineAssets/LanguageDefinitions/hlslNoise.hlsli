...
{  return float4( taylorInvSqrt(r.x), taylorInvSqrt(r.y), taylorInvSqrt(r.z), taylorInvSqrt(r.w) ); }
...
//
#define OCTAVES   		1		// 7
#define SWITCH_TIME 	600000.0		// seconds
	...
	for( int i=-1; i<=1; i++ ) 
{
		...
		o = 0.5 + 0.41*sin( currentTime + 6.2831*o );	
		float2 r = g - f + o;
...
//@function voronoiEX
float voronoiEX(float2 x, float time, float function)
{
    const float t = 0.0f;
    //const float function = mod(t, 4.0);
    const bool multiply_by_F1 = mod(t, 8.0) >= 4.0;
...
float fbm(float4 v) 
{float w=0.,d=.5;float4 y=float4(100,100,100,100);for(int x=0;x<NUM_OCTAVES;++x)w+=d*snoise(v),v=v*2.+y,d*=.5;return w;}
