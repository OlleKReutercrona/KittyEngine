...
float4 Multiply(float4 a, float4x4 b)
{
    return mul(a, b);
}
...
float3 Multiply(float3x3 a, float3 b)
{
    return mul(a, b);
}

//@function MulVector3Matrix
float3 Multiply(float3 a, float3x3 b)
{
    return mul(a, b);
}
...
float3 AssembleFloat3(float4 a)
{
    return a.xyz;
}

//@function float4from3
float4 AssembleFloat4(float3 a, float w)
{
    return float4(a, w);
}

//@function Sample2D
float4 Sample2D(float2 uv, Texture2D texture)
{
    return texture.Sample(internalSampler, uv);
}
...
//
	...
	return float3(1.0f);// ambientdiffuse + ambientspecular;
}
