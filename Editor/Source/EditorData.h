...
struct PerformanceData {
	...
	float LowestFps() {
		return 1.0f / highestTime;
	}
	float HighestFps() {
		return 1.0f / lowestTime;
	}
...
struct GameObjectData {
	...
	bool SelectObject(int anId) {
		if (IsGameObjectSelected(anId)) return false;
...
struct ActiveData {
	char editorState =
		0;	// 0 = active, 1 = fullscreen editor, 2 = fullscreen game
};
...
enum class DebugRenderLevel : char { eNone = 0, eBasic = 1, eFull = 2 };
