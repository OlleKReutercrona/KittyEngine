...
namespace KE_EDITOR {
	...
	std::string ComponentInspector::StructureData(
		const char* aDirtyData,
		KE::SphereColliderComponent* aColliderComponent) {
		ImGui::Text(aDirtyData);

		// split aDirtyData by semicolon such that each element is a string that
		...
		for (int i = 0; i < splitData.size(); i++) {
			...
			// try to find the typename, which is the first word in the string
			// that is not a keyword
			while (true) {
				size_t splitPoint = data.find_first_of(' ');

				if (data.find_first_of("<") < splitPoint &&
					data.find_first_of(">") > splitPoint) {
		...
		size_t byteOffset = 17;	 // 17 because 8 is from vtable pointer in
								 // component, 8 is from GameObject reference in
								 // component, and 1 from bool in component
		for (int i = 0; i < typeNames.size(); i++) {
			...
			if (_stricmp(typeNames[i].c_str(), "Vector4f") == 0) {
				Vector4f* vec =
					(Vector4f*)(((char*)aColliderComponent) + byteOffset);
