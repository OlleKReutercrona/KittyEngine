...
KE_EDITOR::EditorInspectionSystem::EditorInspectionSystem() {
	...
	AddType<unsigned int>([](const char* variableName, void* variable) -> bool {
		return ImGui::DragScalar(variableName, ImGuiDataType_U32,
								 static_cast<unsigned int*>(variable));
	});
	AddType<CommonUtilities::Vector2<unsigned int>>(
		[](const char* variableName, void* variable) -> bool {
			return ImGui::DragScalarN(variableName, ImGuiDataType_U32,
									  static_cast<unsigned int*>(variable), 2);
		});
	AddType<CommonUtilities::Vector3<unsigned int>>(
		[](const char* variableName, void* variable) -> bool {
			return ImGui::DragScalarN(variableName, ImGuiDataType_U32,
									  static_cast<unsigned int*>(variable), 2);
		});
	AddType<CommonUtilities::Vector4<unsigned int>>(
		[](const char* variableName, void* variable) -> bool {
			return ImGui::DragScalarN(variableName, ImGuiDataType_U32,
									  static_cast<unsigned int*>(variable), 2);
		});
	...
	AddType<Matrix3x3f>([](const char* variableName, void* variable) -> bool {
		...
		bool hit = false;
		for (int i = 0; i < 3; i++) {
			hit |= ImGui::DragFloat3(rowNames[i],
									 static_cast<float*>(variable) + i * 3);
		}
		return hit;
	});
	AddType<Matrix4x4f>([](const char* variableName, void* variable) -> bool {
		...
		for (int i = 0; i < 4; i++) {
			hit |= ImGui::DragFloat3(rowNames[i],
									 static_cast<float*>(variable) + i * 4);
		}
		return hit;
	});
}
...
void KE_EDITOR::EditorInspectionSystem::Inspect(
	void* object, KE_SER::Data& serializationData) {
	if (myInspectionFunctions.find(serializationData.typeName) ==
		myInspectionFunctions.end()) {
		return;
	}

	char* variable = ((char*)object) + serializationData.offset;
	myInspectionFunctions[serializationData.typeName](
		BeautifyMemberName(serializationData.name).c_str(), variable);
}

const std::string& KE_EDITOR::EditorInspectionSystem::BeautifyMemberName(
	const std::string& aMemberName) {
	if (beautifiedMemberNames.find(aMemberName) ==
		beautifiedMemberNames.end()) {
		...
		beautifiedName = std::regex_replace(
			beautifiedName, std::regex("([a-z])([A-Z])"), "$1 $2");
		...
		beautifiedName = std::regex_replace(
			beautifiedName, std::regex("([a-zA-Z])([0-9])"), "$1 $2");
