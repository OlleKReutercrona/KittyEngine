...
KE_EDITOR::EditorFileInteraction KE_EDITOR::EditorFile::Display(
	KE::TextureLoader* aTextureLoader) const {
	return BeginDisplayBox(aTextureLoader);
}

KE_EDITOR::EditorFileInteraction KE_EDITOR::EditorFile::BeginDisplayBox(
	KE::TextureLoader* aTextureLoader) const {
	...
	if (ImGui::BeginChild(myPath.c_str(),
						  ImVec2(64.0f, 64.0f) + ImVec2(0.0f, 40.0f), false,
...
KE_EDITOR::EditorFileInteraction KE_EDITOR::EditorFile::DisplayIcon(
	KE::TextureLoader* aTextureLoader) const {
	...
	if (iconTex->myMetadata.myTextureFormat != KE::TextureFormat::Texture2D) {
		aTextureLoader->ProjectCubemap(
			myIconPath, KE::CubemapProjectionType::Equirectangular);
		KE::RenderTarget* rt =
			KE_GLOBAL::blackboard.Get<KE::Graphics>()->GetRenderTarget(9);

		ImGui::Image(rt->GetShaderResourceView(), {64.0f, 64.0f});
	}

	ImGui::ImageButton(myPath.c_str(), iconTex->myShaderResourceView.Get(),
					   ImVec2(64.0f, 64.0f));
	...
	if (ImGui::BeginDragDropSource()) {
		if (myType == EditorFileType::eTexture) {
			...
			if (!ImGui::SetDragDropPayload("texturePath", buffer,
										   sizeof(buffer))) {
				ImGui::BeginTooltip();
				ImGui::ImageButton(iconTex->myShaderResourceView.Get(),
								   ImVec2(64.0f, 64.0f));
				ImGui::SameLine();
			...
			if (!ImGui::SetDragDropPayload("modelPath", buffer,
										   sizeof(buffer))) {
				ImGui::BeginTooltip();
	...
	if (ImGui::IsItemHovered()) {
		if (ImGui::IsMouseDoubleClicked(0)) {
			return EditorFileInteraction::eLeftDoubleClick;
		}
		if (ImGui::IsMouseClicked(0)) {
			return EditorFileInteraction::eLeftClick;
		}
		if (ImGui::IsMouseDoubleClicked(1)) {
			return EditorFileInteraction::eRightDoubleClick;
		}
		if (ImGui::IsMouseClicked(1)) {
			return EditorFileInteraction::eRightClick;
		}
