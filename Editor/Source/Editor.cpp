...
namespace KE_EDITOR {
	Editor::Editor()
		:  // Engine/Source/Graphics/Shaders
	...
	void Editor::Init(KE::Window* aWindow, KE::Timer* aTimer,
		...
		ImGuiHandler::Init(myWindow->myHWnd, myGraphics->GetDevice().Get(),
						   myGraphics->GetContext().Get(), this);
	...
	void Editor::Update() {
		...
		if (ImGui::IsKeyReleased(ImGuiKey_F8)) {
			mySceneManager->GetCurrentScene()->ToggleDrawDebugFlag(
				KE::SceneDrawFlags::eDrawPhysX);
		}
		...
		if (ImGui::IsKeyPressed(ImGuiKey_F1)) {
			myData.activeData.editorState = myData.activeData.editorState == 1
												? 0
												: 1;  // set fullscreen editor
		}
		if (ImGui::IsKeyPressed(ImGuiKey_F2)) {
			myData.activeData.editorState = myData.activeData.editorState == 2
												? 0
												: 2;  // set fullscreen editor

			myGraphics->GetRenderTarget(0)->SetShouldRenderDebug(
				myGraphics->GetRenderTarget(1)->ShouldRenderDebug());
		}
		...
		if (myData.activeData.editorState > 0) {
			myGraphics->AppendRenderTargetQueue(
				-1, myData.activeData.editorState == 1 ? KE_DEBUG_CAMERA_INDEX
													   : KE_MAIN_CAMERA_INDEX);
			if (myData.activeData.editorState == 1) {
				// ImGuiHandler::DisplayVFXSequence(nullptr,
				// KittyFlagTypes::eBeginWindow);
				ControlCamera(myGraphics->GetCameraManager().GetCamera(
					KE_DEBUG_CAMERA_INDEX));

				myGraphics->GetCameraManager().SetHighlightedCamera(
					KE_DEBUG_CAMERA_INDEX);
			} else {
				myGraphics->GetCameraManager().SetHighlightedCamera(
					KE_MAIN_CAMERA_INDEX);
		...
		if (pressedNode) {
			if (openNodePopup) {
				ImGui::OpenPopup("createWindowPopup");
			}
		...
		const std::vector<KE::GameObject*>& newGameObjects =
			gom.GetNewGameObjects();
		...
		if (camIndex >= 0) {
			...
			if (ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left) &&
				...
				KE::GameObject* hoveredObject =
					GetHoveredGameObject(gameObjects, vmpf);
				hoveredObject = hoveredObject
									? hoveredObject
									: GetHoveredGameObject(
										  newGameObjects,
										  vmpf);  // if no regular object
												  // exists, check new objects

				if (hoveredObject) {
					myData.gameObjectData.selectedGameObjects.clear();
					myData.gameObjectData
						.selectedGameObjects[hoveredObject->myID] = true;
					myData.gameObjectData.lastSelectedGameObject =
						hoveredObject->myID;
				} else {
					myData.gameObjectData.selectedGameObjects.clear();
	...
	void Editor::ExtractLogChildren(const KE::LoggedVar& log,
		...
		// But if text is concantinated with ms ImGui's label system breaks as
		// ms is a changing variable (it will cause the node to collapse every
		// frame)

		// text += ": " + std::to_string(log.myTime) + "ms" + "##" +
		// log.myName.c_str();
	...
	void Editor::RegisterLambdaWindow(const std::string& aName,
	...
	void Editor::ReceiveDragDrop(const std::string& aPath) {
		...
		KE_LOG("Received File: %s", myData.dragDropData.files.back().c_str());
	}
	...
	Vector2i Editor::GetViewportMousePos(int* anOutCamIndex) {
		...
		for (auto& wData : ImGuiHandler::windowData) {
			POINT topLeft = {(LONG)wData.second.aTopLeftX,
							 (LONG)wData.second.aTopLeftY};
			POINT bottomRight = {(LONG)wData.second.aBottomRightX,
								 (LONG)wData.second.aBottomRightY};

			ScreenToClient(myWindow->myHWnd, &topLeft);
			ScreenToClient(myWindow->myHWnd, &bottomRight);

			if (aX >= topLeft.x && aX <= bottomRight.x && aY >= topLeft.y &&
				aY <= bottomRight.y) {
				// translate mouse coordinates to 0,0 at top left of window, and
				// 1,1 at bottom right of window
				aX -= topLeft.x;
				aY -= topLeft.y;
	...
	KE::Box Editor::GetModelBoundsExtremes(
		...
		const Matrix3x3f rot =
			aTransform
				.GetCUMatrix();	 // truncate to a 3x3 matrix since Box handles
								 // positioning, we only want rot and scale!
	...
	KE::GameObject* Editor::GetHoveredGameObject(
		...
		for (int m = 0; m < aGameObjectList.size(); m++) {
			...
			if (aGameObjectList[m]->TryGetComponent<KE::ModelComponent>(
					objModelComponent)) {
				const KE::ModelData* modelData =
					objModelComponent->GetModelData();
			...
			if (aGameObjectList[m]->TryGetComponent<KE::SkeletalModelComponent>(
					objSkeletalModelComponent)) {
				const KE::SkeletalModelData* modelData =
					objSkeletalModelComponent->GetModelData();
				for (const auto& subMesh : modelData->myMeshList->myMeshes) {
					objectBounds.push_back(&subMesh.myBounds);
				}
			}

			if (objectBounds.size() == 0) {
				continue;
			}

			KE::Box boundsExtreme = GetModelBoundsExtremes(
				objectBounds, aGameObjectList[m]->myWorldSpaceTransform);

			Vector3f hitPoint;
			if (EditorIntersectBox(boundsExtreme, castRay, hitPoint)) {
				float distance =
					(hitPoint - cam->transform.GetPosition()).Length();

				hitData.push_back(
					{distance, aGameObjectList[m], meshList, skeletalMeshList});
			}
		}
		...
		if (hitData.size() > 0) {
			...
			// if (hitData.empty()) { return nullptr; } //just in case we delete
			// everything

			////sort
			// std::sort(hitData.begin(), hitData.end(), [](const
			// SelectionHitData& a, const SelectionHitData& b)
			//{
			...
			for (const auto& hit : hitData) {
				if (hit.distance > workingDistance) {
					continue;
				}
				if (hit.meshList &&
						RayIntersectsModel(hit.meshList, castRay,
										   hit.object->myWorldSpaceTransform) ||
					hit.skeletalMeshList &&
						RayIntersectsModel(hit.skeletalMeshList, castRay,
										   hit.object->myWorldSpaceTransform)) {
	...
	bool Editor::PointInsideTriangle(Vector3f p0, Vector3f p1, Vector3f p2,
									 Vector3f point) {
		p0 -= point;
		p1 -= point;
		p2 -= point;
	...
	bool Editor::RayIntersectsModel(KE::MeshList* aMeshList, const Rayf& aRay,
									const Transform& aTransform) {
		for (const auto& mesh : aMeshList->myMeshes) {
			for (int i = 0; i < mesh.myIndices.size(); i += 3) {
				...
				Vector4f p0 = {mesh.myVertices[index0].x,
							   mesh.myVertices[index0].y,
							   mesh.myVertices[index0].z, 1.0f};
				Vector4f p1 = {mesh.myVertices[index1].x,
							   mesh.myVertices[index1].y,
							   mesh.myVertices[index1].z, 1.0f};
				Vector4f p2 = {mesh.myVertices[index2].x,
							   mesh.myVertices[index2].y,
							   mesh.myVertices[index2].z, 1.0f};
				...
				Vector3f hitPoint;
				if (!IntersectionPlaneRay(p, aRay, hitPoint)) {
					continue;
				}
				if (!PointInsideTriangle(p0.xyz(), p1.xyz(), p2.xyz(),
										 hitPoint)) {
					continue;
				}
				return true;
	...
	bool Editor::RayIntersectsModel(KE::SkeletalMeshList* aMeshList,
									const Rayf& aRay,
									const Transform& aTransform) {
		for (const auto& mesh : aMeshList->myMeshes) {
			for (int i = 0; i < mesh.myIndices.size(); i += 3) {
	...
	bool Editor::HighlightObject(KE::GameObject* aGameObject) {
		...
		KE::ModelComponent* objModelComponent;
		if (aGameObject->TryGetComponent<KE::ModelComponent>(
				objModelComponent)) {
	...
	void Editor::ControlCamera(KE::Camera* aCamera) {
		...
		if (!myWindow->IsCursorEnabled()) {
			...
			if (scrollDelta) {
				...
				cameraScrollSpeedMultiplier =
					std::clamp(cameraScrollSpeedMultiplier, 0.1f, 5.0f);
			}
			...
			if (input.IsKeyHeld(KE::KEY_MOV_FW)) {
				aCamera->transform.TranslateLocal(
					Vector3f(0.0f, 0.0f, cameraSpeed));
			}
			...
			if (input.IsKeyHeld(KE::KEY_MOV_LF)) {
				aCamera->transform.TranslateLocal(
					Vector3f(-cameraSpeed, 0.0f, 0.0f));
			}
			...
			if (input.IsKeyHeld(KE::KEY_MOV_UP)) {
				aCamera->transform.TranslateLocal(
					Vector3f(0.0f, cameraSpeed, 0.0f));
			}
	...
	void Editor::SaveMeowFile() {
		...
		aMeowFile.windowSettings.consoleHeight =
	...
	void Editor::LoadMeowFile() {
		...
		myWindow->SetWindowDims((int)aMeowFile.windowSettings.windowX,
		...
		rect.right = (LONG)(aMeowFile.windowSettings.consoleX +
							aMeowFile.windowSettings.consoleWidth);
		rect.bottom = (LONG)(aMeowFile.windowSettings.consoleY +
							 aMeowFile.windowSettings.consoleHeight);
