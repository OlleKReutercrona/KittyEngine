...
namespace KE_EDITOR {
	...
	bool GUIEditor::DisplayDebugOptions() {
		...
		ImGui::Checkbox("On: Square grid | Off: Rectangle grid",
						&myGUIHandler->gridSquareCells);
	...
	void GUIEditor::DisplaySelectedGuiElement() {
		...
		KE::eProgressionDirection progressionDirection =
			mySelectedGUIElement->myProgressionDirection;
		...
		if (ImGui::BeginPopup("ChangeNamePopup")) {
			...
			if (ImGui::InputText("New Name", newNameBuffer,
		...
		if (ImGui::Button("Duplicate")) {
			Vector2f resolution = {
				(float)myGUIHandler->myGraphics->GetRenderSize().x,
				(float)myGUIHandler->myGraphics->GetRenderSize().y};
			KE::GUIElement* duplicateElement =
				&mySelectedGUIScene->DuplicateGUIElement(*mySelectedGUIElement,
														 resolution);
			...
			myGUIHandler->AssignSpriteToGUIElement(
				*duplicateElement, myGUIHandler->myGraphics,
				{mySelectedGUIElement->myBox.myWidth,
				 mySelectedGUIElement->myBox.myHeight},
				mySelectedGUIElement->mySpriteBatch.myData.myTexture->myMetadata
					.myFilePath);

			duplicateElement->UpdateSpriteScaleAndPosition(
				{resolution.x, resolution.y}, false);

			if (mySelectedGUIElement->hasText) {
				duplicateElement->hasText = true;
				myGUIHandler->SetGUIText(duplicateElement,
										 mySelectedGUIElement->myText);
			}

			mySelectedGUIElement = duplicateElement;
			mySelectedElementIndex = static_cast<int>(
				mySelectedGUIScene->GetGUIElements().size() - 1);
		}
		...
		ImGuiHandler::DisplayTexture(&mySelectedGUIElement->myDisplayTexture,
									 {64, 64});
		ImGuiHandler::DisplayTexture(&mySelectedGUIElement->mySecondaryTexture,
									 {64, 64});

		mySelectedGUIElement->Reset();

		ImGui::Checkbox("Hide at start",
						&mySelectedGUIElement->shouldHideAtStart);
		ImGui::Checkbox("Show", &mySelectedGUIElement->shouldDraw);

		ImGui::Text("Event Name: %s",
					mySelectedGUIElement->myEvent.myEventName.c_str());
		...
		if (ImGui::InputTextWithHint("##eventName", "Event Name", eventBuffer,
									 64,
									 ImGuiInputTextFlags_EnterReturnsTrue)) {
			...
			myGUIHandler->RegisterAssignedEvent(eventBuffer,
												mySelectedGUIElement);
		...
		static int currentItem =
			0;	// This will store the index of the currently selected item
		...
		if (ImGui::BeginCombo("Select Event", events[currentItem].c_str())) {
			for (int i = 0; i < events.size(); i++) {
				...
				if (ImGui::Selectable(events[i].c_str(), isSelected)) {
					currentItem = i;  // Update the current item index when an
									  // item is selected
					myGUIHandler->RegisterAssignedEvent(events[i],
														mySelectedGUIElement);
				}

				// Set the initial focus when opening the combo (scrolling to
				// the current item)
				if (isSelected) {
		...
		// ImGuiHandler::DisplayEnumSlider("Type",
		// &mySelectedGUIElement->myType, KE::eGUIElementType::Count);
		ImGuiHandler::DisplayEnumSlider("AlignType",
										&mySelectedGUIElement->myAlignType,
										KE::eAlignType::Count);
		ImGuiHandler::DisplayEnumSlider(
			"ProgressionDirection",
			&mySelectedGUIElement->myProgressionDirection,
			KE::eProgressionDirection::Count);
		...
		if (width != mySelectedGUIElement->myBox.myWidth ||
			...
			const Vector2i resolution =
				myGUIHandler->myGraphics->GetRenderSize();
			mySelectedGUIElement->UpdateSpriteScaleAndPosition(
				{(float)resolution.x, (float)resolution.y}, false);

			myGUIHandler->SetGUIText(mySelectedGUIElement,
									 mySelectedGUIElement->myText);
		...
		if (mySelectedGUIElement->hasText) {
			...
			std::string displayText = mySelectedGUIElement->myText.length() > 0
										  ? mySelectedGUIElement->myText
										  : "Text";
			if (ImGui::InputTextWithHint(
					"##newText", displayText.c_str(), textBuffer, 64,
					ImGuiInputTextFlags_EnterReturnsTrue)) {
				text = textBuffer;
			...
			ImGui::ColorEdit4("Text Colour",
							  &mySelectedGUIElement->myTextColour.x);

			mySelectedGUIElement->mySpriteBatch.myTextStyling.text
				.horizontalAlign = KE::TextAlign::Center;
			ImGuiHandler::EditTextStyling(
				mySelectedGUIElement->mySpriteBatch.myTextStyling);

			myGUIHandler->SetGUIText(mySelectedGUIElement, text);
		}

		KE::DebugRenderer* debugRenderer =
		...
		// Anchor point
		{
			...
			debugRenderer->RenderScreenSpaceLine(left, right,
												 Vector4f(1, 0, 0, 0.5f));
			debugRenderer->RenderScreenSpaceLine(up, down,
												 Vector4f(1, 0, 0, 0.5f));
			debugRenderer->RenderScreenSpaceLine(leftUp, rightDown,
												 Vector4f(1, 0, 0, 0.5f));
			debugRenderer->RenderScreenSpaceLine(leftDown, rightUp,
												 Vector4f(1, 0, 0, 0.5f));
			debugRenderer->RenderScreenSpaceLine(
				position, mySelectedGUIElement->myBox.myScreenPosition,
				Vector4f(0, 1, 0, 0.5f));
		}
		...
		// Draw selected GUIElement highlight box
		{
			...
			const Vector2f leftUp = {
				mySelectedGUIElement->myBox.myScreenPosition.x - offset,
				mySelectedGUIElement->myBox.myScreenPosition.y -
					mySelectedGUIElement->myBox.myHeight - offset};
			const Vector2f rightUp = {
				mySelectedGUIElement->myBox.myScreenPosition.x +
					mySelectedGUIElement->myBox.myWidth + offset,
				mySelectedGUIElement->myBox.myScreenPosition.y -
					mySelectedGUIElement->myBox.myHeight - offset};
			const Vector2f rightDown = {
				mySelectedGUIElement->myBox.myScreenPosition.x +
					mySelectedGUIElement->myBox.myWidth + offset,
				mySelectedGUIElement->myBox.myScreenPosition.y + offset};
			const Vector2f leftDown = {
				mySelectedGUIElement->myBox.myScreenPosition.x - offset,
				mySelectedGUIElement->myBox.myScreenPosition.y + offset};
			debugRenderer->RenderScreenSpaceLine(leftUp, rightUp,
												 Vector4f(1, 1, 1, 1.0f));
			debugRenderer->RenderScreenSpaceLine(rightUp, rightDown,
												 Vector4f(1, 1, 1, 1.0f));
			debugRenderer->RenderScreenSpaceLine(rightDown, leftDown,
												 Vector4f(1, 1, 1, 1.0f));
			debugRenderer->RenderScreenSpaceLine(leftDown, leftUp,
												 Vector4f(1, 1, 1, 1.0f));
		}
	}

	void GUIEditor::RenameNotUniqueElementName(KE::GUIScene* aGuiScene,
											   std::string& aOutElementName) {
		// Check if name is unique
		int numberOfSameNames = 0;
		for (auto& element : aGuiScene->GetGUIElements()) {
			std::string existingNameWithoutNumber =
				std::regex_replace(element.myName, std::regex("_[0-9]"), "");
			std::string newNameWithoutNumber =
				std::regex_replace(aOutElementName, std::regex("_[0-9]"), "");
	...
	void GUIEditor::RenameNotUniqueSceneName(KE::GUIHandler* aGuiHandler,
											 std::string& aOutElementName) {
		// Check if name is unique
		int numberOfSameNames = 0;
		for (auto& scene : aGuiHandler->myGUISceneMap) {
			std::string existingNameWithoutNumber =
				std::regex_replace(scene.first, std::regex("_[0-9]"), "");
			std::string newNameWithoutNumber =
				std::regex_replace(aOutElementName, std::regex("_[0-9]"), "");
			if (existingNameWithoutNumber == newNameWithoutNumber) {
				numberOfSameNames++;
	...
	bool GUIEditor::DisplayGuiSceneElements(KE::GUIScene* aGuiScene) {
		...
		if (ImGui::BeginPopup("ChangeNamePopup")) {
			...
			if (ImGui::InputText("New Name", newNameBuffer,
				...
				// Update the element's name when Enter is pressed
				std::string newName = newNameBuffer;
				auto sceneToRename = myGUIHandler->myGUISceneMap.extract(
					mySelectedGUIScene->myName);
		...
		if (ImGui::Button("Delete")) {
			if (mySelectedGUIScene != nullptr) {
				...
				myGUIHandler->RemoveGUIScene(
					mySelectedGUIScene->GetSceneName());
		...
		if (ImGui::InputTextWithHint("##newGuiElement", "New GUIElement",
									 elementInputBuffer, 64,
									 ImGuiInputTextFlags_EnterReturnsTrue)) {
			...
			aGuiScene->CreateGUIElement(size, offset, resolution,
										// KE::eGUIElementType::Decoration,
										false, KE::eAlignType::Center);
			...
			myGUIHandler->AssignSpriteToGUIElement(
				aGuiScene->GetGUIElements().back(), myGUIHandler->myGraphics,
				{100.f, 100.f}, "Data\\EngineAssets\\KEDefault_c.dds");

			mySelectedGUIElement = &aGuiScene->GetGUIElements().back();
			mySelectedElementIndex =
				static_cast<int>(aGuiScene->GetGUIElements().size() - 1);
		}
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			if (i < elements.size()) {
				if (ImGui::Selectable((elements[i].myName).c_str(),
			...
			if (ImGui::BeginDragDropSource()) {
				ImGui::SetDragDropPayload("DRAGGABLE_ELEMENT", &guiPayload,
										  sizeof(guiPayload));
			...
			if (ImGui::BeginDragDropTarget()) {
				if (const ImGuiPayload* payload =
						ImGui::AcceptDragDropPayload("DRAGGABLE_ELEMENT")) {
					auto* received = (GUIDragDropPayload*)payload->Data;
					int payloadIndex = received->elementIndex;

					if (received->sourceScene != aGuiScene) {
						myGUIHandler->MoveElementBetweenScenes(
							received->sourceScene->GetSceneName(),
							aGuiScene->GetSceneName(), payloadIndex);
					} else {
	...
	void GUIEditor::Render() {
		if (myGUIHandler != nullptr) {
			...
			if (ImGui::InputTextWithHint(
					"##newGuiScene", "New GUIScene", sceneInputBuffer, 64,
					ImGuiInputTextFlags_EnterReturnsTrue)) {
	...
	void GUIEditor::SetSceneActive(const KE::GUIScene* aScene,
								   const bool aActive) const {
		if (aActive) {
	...
	void GUIEditor::OnReceiveEvent(ES::Event& aEvent) {
		if (const KE::InputEvent* event =
				dynamic_cast<KE::InputEvent*>(&aEvent)) {
			const Vector2f mousePosition = {
				static_cast<float>(event->myMousePosition.x),
				static_cast<float>(event->myMousePosition.y)};
			...
			if ((event->myInputType == KE::eInputType::LeftClick ||
				...
				if (!lockSelect) {
					...
					if (myGUIHandler->IsMouseOverGUI(mousePosition)) {
						KE::GUIElement* element =
							myGUIHandler->GetGUIElementFromMousePosition(
								mousePosition);
