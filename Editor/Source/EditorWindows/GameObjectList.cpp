...
void KE_EDITOR::GameObjectList::Render() {
	const std::vector<KE::GameObject*>& _gameObjectList =
		KE_GLOBAL::editor->mySceneManager->GetCurrentScene()
			->GetGameObjectManager()
			.GetGameObjects();
	const std::vector<KE::GameObject*>& _newGameObjectList =
		KE_GLOBAL::editor->mySceneManager->GetCurrentScene()
			->GetGameObjectManager()
			.GetNewGameObjects();

	const std::vector<KE::GameObject*>& gameObjectList =
		_gameObjectList.size() > 0 ? _gameObjectList : _newGameObjectList;
	...
	if (ImGui::BeginChild("Object List Child", {0, 0},
		...
		if (ImGui::BeginPopup("ObjectListPopup")) {
			if (ImGui::MenuItem("Create Empty")) {
				int id = KE_GLOBAL::editor->mySceneManager->GetCurrentScene()
							 ->GetGameObjectManager()
							 .GenerateUniqueID();
...
bool KE_EDITOR::GameObjectList::GameObjectCacheDirty(
	const std::vector<KE::GameObject*>& aGameObjectList) {
	// return false;
	if (/*gameObjectDataCache.size()*/ lastGameObjectCount !=
		aGameObjectList.size()) {
...
void KE_EDITOR::GameObjectList::CreateGameObjectCache(
	const std::vector<KE::GameObject*>& aGameObjectList) {
	gameObjectDataCache.clear();

	for (size_t i = 0; i < aGameObjectList.size(); i++) {
		GameObjectDataCache newCache;
		newCache.assembledName =
			FormatString("(%i) : %s", aGameObjectList[i]->myID,
						 aGameObjectList[i]->GetName().c_str());
		newCache.objectName = aGameObjectList[i]->GetName();
		newCache.searchName = newCache.assembledName;
		std::transform(newCache.searchName.begin(), newCache.searchName.end(),
					   newCache.searchName.begin(), ::tolower);

		gameObjectDataCache[aGameObjectList[i]->myID] = newCache;
	}
}

void KE_EDITOR::GameObjectList::CalculateGameObjectFilter(
	const std::vector<KE::GameObject*>& aGameObjectList,
	const std::string& aNameFilter) {
	for (size_t i = 0; i < aGameObjectList.size(); i++) {
		// if (aGameObjectList[i]->myParent != nullptr) { continue; } //skip
		// children

		if (aNameFilter.size() > 0) {
			size_t found =
				gameObjectDataCache[aGameObjectList[i]->myID].searchName.find(
					aNameFilter);
			if (found != std::string::npos) {
				gameObjectDataCache[aGameObjectList[i]->myID].matchesFilter =
					true;
				gameObjectDataCache[aGameObjectList[i]->myID]
					.filterMatchCharacterStart = (int)found;
				gameObjectDataCache[aGameObjectList[i]->myID]
					.filterMatchCharacterEnd =
					(int)found + (int)aNameFilter.size();
			} else {
				gameObjectDataCache[aGameObjectList[i]->myID].matchesFilter =
					false;
				gameObjectDataCache[aGameObjectList[i]->myID]
					.filterMatchCharacterStart = 0;
				gameObjectDataCache[aGameObjectList[i]->myID]
					.filterMatchCharacterEnd = 0;
			}

			gameObjectDataCache[aGameObjectList[i]->myID]
				.childrenMatchesFilter = false;
			for (KE::GameObject* child : aGameObjectList[i]->GetChildren()) {
				if (gameObjectDataCache[child->myID].searchName.find(
						aNameFilter) != std::string::npos) {
					gameObjectDataCache[aGameObjectList[i]->myID]
						.childrenMatchesFilter = true;
				}
			}
		} else {
...
void KE_EDITOR::GameObjectList::DisplayListEntry(KE::GameObject* aGameObject) {
	...
	bool selected =
		KE_GLOBAL::editor->myData.gameObjectData.IsGameObjectSelected(id);
