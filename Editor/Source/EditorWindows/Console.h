...
namespace KE_EDITOR {
	...
	public:
		...
		void ClearMessages() {
			...
			myMessagePopups.clear();
		};
		inline const std::vector<EditorConsoleMessage>& GetMessages() const {
			return myMessageHistory;
		}

		void LogMessage(const char* aChannel, const char* aFile,
						unsigned int aLine, const char* aFunction,
						const char* aMessage, ...);
	};
	...
	private:
	public:
		EditorConsole(EditorWindowInput aStartupData = {})
