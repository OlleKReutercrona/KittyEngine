...
#define IS_IMAGE(x)                                                      \
	((x) == ".png" || (x) == ".PNG" || (x) == ".dds" || (x) == ".DDS" || \
	 (x) == ".jpg" || (x) == ".JPG" || (x) == ".jpeg" || (x) == ".JPEG")
#define IS_CUBEMAP(x)                            \
	((x).find("cubemap") != std::string::npos || \
	 (x).find("Cubemap") != std::string::npos || \
	 (x).find("CubeMap") != std::string::npos)

#define IS_MODEL(x) \
	((x) == ".fbx" || (x) == ".FBX" || (x) == ".obj" || (x) == ".OBJ")

KE_EDITOR::EditorAssetBrowser::EditorAssetBrowser(
	EditorWindowInput aStartupData)
	: myEngineFileWatcher(
		  L"../Engine/",
		  [this](const std::wstring& path, const filewatch::Event change_type) {
			  ProcessFileChangeEngine(path, change_type);
		  }),
	  myBinFileWatcher(L"Data", [this](const std::wstring& path,
...
void KE_EDITOR::EditorAssetBrowser::ProcessFileChangeEngine(
	const std::wstring& path, const filewatch::Event change_type) {
	// std::wcout << path << L" : ";
	switch (change_type) {
		case filewatch::Event::modified:
			if (path.ends_with(L".hlsl")) {
				// const std::string file = std::string(path.begin(),
				// path.end()); //this way gives a warning convert to string in
				// a safer way, as the path is a wstring
...
void KE_EDITOR::EditorAssetBrowser::ProcessFileChangeBin(
	const std::wstring& path, const filewatch::Event change_type) {
	// std::wcout << path << L" : ";
	switch (change_type) {
...
void KE_EDITOR::EditorAssetBrowser::RecursiveFolderTreeGen(
	const std::filesystem::path& aPath, FileTreeNode* aNode) {
	...
	for (auto& entry : std::filesystem::directory_iterator(aPath)) {
		...
		if (entry.is_directory()) {
			...
			aNode->myFolders.emplace_back(
				EditorFileType::eFolder, path.string(), fileName.string(),
				"../Editor/EditorAssets/Icons/Folder.png");

			RecursiveFolderTreeGen(entry.path(), &aNode->myChildren.back());
		} else {
			if (IS_IMAGE(fileName.extension())) {
				if (IS_CUBEMAP(fileName.string())) {
					aNode->myFiles.emplace_back(
						EditorFileType::eCubemap, path.string(),
						fileName.string(),
						"../Editor/EditorAssets/Icons/CubemapIcon.png");
				} else {
					aNode->myFiles.emplace_back(
						EditorFileType::eTexture, path.string(),
						fileName.string(), path.string());
				}
			} else if (IS_MODEL(fileName.extension())) {
				std::string modelThumbnailPath =
					"Data/InternalAssets/ModelThumbnails/" + fileName.string() +
					".dds";
				...
				aNode->myFiles.emplace_back(EditorFileType::eModel,
											path.string(), fileName.string(),
											modelThumbnailPath);
			} else if (fileName.extension() == ".kittyVFX") {
				aNode->myFiles.emplace_back(
					EditorFileType::eVFXSequence, path.string(),
					fileName.string(),
					"../Editor/EditorAssets/Icons/KittyVFXIcon.png");
			} else if (fileName.extension() == ".scritch") {
				aNode->myFiles.emplace_back(
					EditorFileType::eScript, path.string(), fileName.string(),
					"../Editor/EditorAssets/Icons/ScritchIcon.png");
			} else {
				aNode->myFiles.emplace_back(
					EditorFileType::eUnknown, path.string(), fileName.string(),
					"../Editor/EditorAssets/Icons/File.png");
			}
		}
	}
}
...
void KE_EDITOR::EditorAssetBrowser::GenerateEditorFiles() {
	myFiles["..."] = EditorFile(EditorFileType::eFolder, "...", "...",
								"../Editor/EditorAssets/Icons/Return.png");
...
void KE_EDITOR::EditorAssetBrowser::RenderFileGrid(
	const ImVec2& aContentRegionSize) {
	const float iconSize = 64.0f;
	const float textVArea = 40.0f;

	const unsigned int columnCount =
		(unsigned int)(aContentRegionSize.x / (iconSize + 10.0f));
	if (columnCount == 0) {
		return;
	}
	...
	if (ImGui::InputText("Search", buffer, 64)) {
		...
		for (auto& c : myFileFilter) {
			if (std::isupper(c)) {
				caseSensitive = true;
			}  // this is evil ass visual studio style implementation :(
		}
	}
	...
	if (globalFilter && !myFileFilter.empty()) {
		for (auto& [fPath, file] : myFiles) {
			if (!PassesFilter(file.GetName(), myFileFilter, caseSensitive)) {
				continue;
			}
	...
	for (int i = 0; i < myFileTree.myActiveNode->myFolders.size(); i++) {
		...
		if (!PassesFilter(folder.GetName(), myFileFilter, caseSensitive)) {
			continue;
		}
			...
			case EditorFileInteraction::eLeftDoubleClick: {
				switch (file.GetType()) {
					case EditorFileType::eVFXSequence: {
						KE::VFXManager& mgr =
							KE_GLOBAL::editor->myGraphics->GetVFXManager();
						...
						ImGuiHandler::vfxEditorActiveSequenceIndex =
							mgr.GetVFXSequenceFromName(fileName);
						break;
					}
					case EditorFileType::eScript: {
						KE::ScriptManager* mgr =
							KE_GLOBAL::blackboard.Get<KE::ScriptManager>(
								"scriptManager");
						std::string fileName = file.GetName();
						// remove extension
						fileName =
						...
						if (script) {
							auto scriptEditorWindows =
								KE_GLOBAL::editor
									->GetEditorWindowsOfType<NodeEditor>();
							bool found = false;
					...
					default: {
						std::filesystem::path filePath =
							std::filesystem::absolute(myCurrentPath);
						filePath /= file.GetPath();

						ShellExecuteA(NULL, "open", filePath.string().c_str(),
									  NULL, NULL, NULL);
						break;
					}
				}
				...
				switch (file.GetType()) {
					case EditorFileType::eModel: {
						if (ImGui::BeginTooltip()) {
							...
							RenderModelThumbnail(KE_GLOBAL::editor->myGraphics,
												 file.GetPath(), false, &rt);
							if (rt) {
								ImGui::Image(rt->GetShaderResourceView(),
...
bool KE_EDITOR::EditorAssetBrowser::PassesFilter(const std::string& aFileName,
