...
void KE_EDITOR::SelectedObject::Update() {
	...
	if (selectedID != INT_MIN) {
		mySelectedObject = KE_GLOBAL::editor->mySceneManager->GetCurrentScene()
							   ->GetGameObjectManager()
							   .GetGameObject(selectedID);
	} else {
...
void KE_EDITOR::SelectedObject::Render() {
	...
	ImGui::BeginChild("Selected Object Child", ImVec2(0, 0),
					  ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_Border);
	ImGui::Text(mySelectedObject->GetName().c_str());
	ImGui::SameLine();
	ImGui::Text(
		"( Layer: %s )",
		KE::Collision::GetLayerAsString(mySelectedObject->myLayer).c_str());
	...
	ImGuiHandler::DisplayTransform(&mySelectedObject->myTransform,
								   eSeparatorAbove);
