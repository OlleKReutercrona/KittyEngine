...
void KE_EDITOR::MaterialEditor::Render() {
	...
	changed = ImGui::ColorEdit3("Albedo", &materialParameters.albedo.x)
				  ? true
				  : changed;
	changed =
		ImGui::SliderFloat("Metallic", &materialParameters.metallic, 0.0f, 1.0f)
			? true
			: changed;
	changed = ImGui::SliderFloat("Roughness", &materialParameters.roughness,
								 0.0f, 1.0f)
				  ? true
				  : changed;
	// changed = ImGui::SliderFloat("Ambient Occlusion", &materialParameters.ao,
	// 0.0f, 1.0f) ? true : changed;
	changed =
		ImGui::SliderFloat("Emission", &materialParameters.emission, 0.0f, 1.0f)
			? true
			: changed;
	...
	if (ImGui::Button("Create Material") || changed) {
		...
		auto& texLoader =
			KE_GLOBAL::blackboard.Get<KE::Graphics>()->GetTextureLoader();
		texLoader.SaveDataMaterial(intermediatePath, newMaterial);
		madeMaterial = texLoader.LoadDataMaterial(intermediatePath, true);

		std::string thumbnailPath =
			std::format("Data/InternalAssets/MaterialThumbnails/{}.dds",
						materialParameters.materialName);

		texLoader.RenderThumbnail(madeMaterial, thumbnailPath);
		thumbnailTexture =
			texLoader.GetTextureFromPath(thumbnailPath, false, true);
