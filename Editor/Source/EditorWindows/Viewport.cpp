...
void KE_EDITOR::Viewport::Render() {
	...
	region = GetAspectRegion(
		region, (float)gfx->GetRenderWidth() / (float)gfx->GetRenderHeight());
	...
	ImGuiHandler::windowData[myRenderTargetIndex] = {
		vMinWin.x, vMinWin.y, vMaxWin.x, vMaxWin.y, camera->GetIndex()};

	ImDrawList* tDrawList = ImGui::GetWindowDrawList();
	// ImGui::Image(renderTarget->GetShaderResourceView(), region);
	tDrawList->AddImage(renderTarget->GetShaderResourceView(), vMinWin,
						vMaxWin);

	if (auto* console = KE_GLOBAL::editor->GetEditorWindow<EditorConsole>()) {
		console->RenderPopups(screenCursorPos + ImVec2(4.0f, 2.0f),
							  tDrawList);  // temp!
	}

	constexpr float h = 24.0f, w = 182.0f, wo = 5.0f, ho = 0.0f;

	tDrawList->AddRectFilled(ImVec2(vMaxWin.x - w, vMinWin.y),
							 ImVec2(vMaxWin.x, vMinWin.y + h),
							 ImColor(0.0f, 0.0f, 0.0f, 0.78f), 8.0f,
	...
	if (drawDebug) {
		ImVec4 viewportBounds = {vMinWin.x, vMinWin.y, vMax.x - 8, vMax.y - 27};

		if (KE_GLOBAL::editor->myData.debugRenderData.renderNavmesh ==
			DebugRenderLevel::eFull) {
			ImGuiHandler::DisplayNavmesh(
				&KE_GLOBAL::editor->mySceneManager->GetCurrentScene()
					 ->GetNavmesh(),
				camera, viewportBounds);
		}

		if (KE_GLOBAL::editor->myData.debugRenderData.renderSkeleton ==
			DebugRenderLevel::eFull) {
			for (auto& gameObject :
				 KE_GLOBAL::editor->mySceneManager->GetCurrentScene()
					 ->GetGameObjectManager()
					 .GetGameObjectsWithComponent<
						 KE::SkeletalModelComponent>()) {
		...
		int& selectedGameObject =
			KE_GLOBAL::editor->myData.gameObjectData.lastSelectedGameObject;
		if (selectedGameObject != INT_MIN) {
			KE::GameObject* gameObject =
				gameObjectManager->GetGameObject(selectedGameObject);

			if (!gameObject) {
				selectedGameObject = INT_MIN;
			} else {
				// ugly!
				DirectX::XMMATRIX oldWorldSpaceTransform =
					gameObject->myWorldSpaceTransform.GetMatrix();
				ImGuiHandler::DisplayGizmo(&gameObject->myWorldSpaceTransform,
										   camera, viewportBounds);
				DirectX::XMMATRIX newWorldSpaceTransform =
					gameObject->myWorldSpaceTransform.GetMatrix();

				// apply the changes between old and new to
				// gameObject->myTransform
				DirectX::XMMATRIX delta =
					newWorldSpaceTransform *
					DirectX::XMMatrixInverse(nullptr, oldWorldSpaceTransform);
				gameObject->myTransform.SetMatrix(
					delta * gameObject->myTransform.GetMatrix());
		...
		const bool anyHandleClicked = ImGuiHandler::DisplayCustomHandles(
			camera, viewportBounds);  // returns true if any handle was pressed
		const bool arbitraryGizmoUsed = ImGuiHandler::DisplayArbitraryGizmo(
			&ImGuiHandler::arbitraryGizmo, camera, viewportBounds);

		if (!arbitraryGizmoUsed &&
			ImGui::IsMouseClicked(ImGuiMouseButton_Left) && !anyHandleClicked) {
