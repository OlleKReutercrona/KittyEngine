...
void KE_EDITOR::ConsoleInternal::LogMessage(const char* aChannel,
											const char* aFile,
											unsigned int aLine,
	...
	if (!myMessageHistory.empty()) {
		...
		if (lastMSG.channel == c && lastMSG.message == message &&
			lastMSG.data.file == file && lastMSG.data.function == function &&
			lastMSG.data.line == aLine)	 // if the last message was the same,
	...
	if (myChannelFlags[c]) {
		myMessagePopups.push_back(std::pair<int, float>(
			static_cast<int>(myMessageHistory.size() - 1), popupDuration));
	}
}
...
void KE_EDITOR::EditorConsole::Render() {
	...
	if (ImGui::Checkbox("Log", (bool*)&console.myChannelFlags["log"])) {
	}
	ImGui::SameLine();
	if (ImGui::Checkbox("Warning", (bool*)&console.myChannelFlags["warning"])) {
	}
	ImGui::SameLine();
	if (ImGui::Checkbox("Error", (bool*)&console.myChannelFlags["error"])) {
	}
	ImGui::SameLine();
	if (ImGui::BeginMenu("Other")) {
		for (auto& channel : console.myChannelFlags) {
			if (channel.first != "log" && channel.first != "warning" &&
				channel.first != "error") {
				if (ImGui::Checkbox(channel.first.c_str(),
									(bool*)&channel.second)) {
				}
			}
		}
	...
	ImGui::PushStyleColor(ImGuiCol_ChildBg,
						  (ImU32)ImColor(0.0f, 0.0f, 0.0f, 0.5f));
	if (ImGui::BeginChild("Message Log", ImGui::GetContentRegionAvail() -
											 ImVec2(0.0f, 25.0f))) {
		ImGui::PopStyleColor();
		int i = 0;
		for (auto& message : console.myMessageHistory) {
			if (console.myChannelFlags[message.channel] == 0) {
				continue;
			}
			...
			if (ImGui::BeginChild("ConsoleChild", ImVec2(0.0f, 0.0f),
								  ImGuiChildFlags_AutoResizeY)) {
				bool b = ImGui::TreeNode("Details:");
				ImGui::SameLine();
				ImGui::TextWrapped("(%d)", message.logCount);
				if (b) {
					ImGui::TextWrapped("Function: %s",
									   message.data.function.c_str());
				...
				ImGui::PushStyleColor(
					ImGuiCol_Text, console.myChannelColours[message.channel]);
			...
			ImGui::GetWindowDrawList()->AddRect(
				...
				console.myChannelColours[message.channel], 5.0f, 0, 2.0f);
	...
	char buf[128] = {0};
	if (ImGui::InputTextWithHint("##ConsoleInput", "Console Command", buf, 128,
	...
	if (ImGui::Button("Clear Log")) {
		console.myMessageHistory.clear();
		console.myMessagePopups.clear();
	}
}

void KE_EDITOR::EditorConsole::RenderPopups(const ImVec2& aPosition,
											ImDrawList* aDrawList) {
	auto& console = KE_GLOBAL::console;
	...
	for (int i = 0; i < console.myMessagePopups.size(); i++) {
		...
		ImGui::PushStyleColor(ImGuiCol_Text,
							  console.myChannelColours[ogMessage.channel]);
		...
		aDrawList->AddRectFilled(
			min + ImVec2(-4.0f, -2.0f), max + ImVec2(8.0f, 4.0f),
			inside ? (ImU32)ImColor(0.25f, 0.25f, 0.25f, 0.75f)
				   : (ImU32)ImColor(0.0f, 0.0f, 0.0f, 0.75f),
			5.0f, ImDrawFlags_RoundCornersRight);
