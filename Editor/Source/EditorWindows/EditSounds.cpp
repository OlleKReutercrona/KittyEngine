...
namespace KE_EDITOR {
	...
	void EditSounds::Render() {
		...
		if (ImGui::TreeNode("SFX")) {
			...
			for (int i = 0; i < (int)sound::SFX::Count; i++) {
				const char* name =
					KE::GlobalAudio::EnumParser(static_cast<sound::SFX>(i));
				int volume =
					static_cast<int>(KE::GlobalAudio::sfxData[i].volume * 100);

				if (ImGui::SliderInt(name, &volume, static_cast<int>(min),
									 static_cast<int>(max))) {
	...
	void EditSounds::Save() {
		...
		// Save to file.
		if (std::filesystem::exists(soundSettings) &&
			std::filesystem::is_regular_file(soundSettings) &&
			std::filesystem::status(soundSettings).permissions() ==
				std::filesystem::perms::_File_attribute_readonly) {
			std::filesystem::permissions(
				soundSettings, std::filesystem::perms::owner_write,
