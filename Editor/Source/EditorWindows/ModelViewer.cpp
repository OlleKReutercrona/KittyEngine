...
bool KE_EDITOR::ModelViewer::HoveringOverRender() const {
	...
	return mousePos.x > lastRenderRectMin.x &&
		   ...
		   mousePos.y > lastRenderRectMin.y && mousePos.y < lastRenderRectMax.y;
}

void KE_EDITOR::ModelViewer::Init() {
	myModelData.myMeshList =
		&KE_GLOBAL::editor->myModelLoader->Load("Data/Assets/testModel.fbx");
	auto& rr = myModelData.myRenderResources.emplace_back();
	rr.myMaterial = KE_GLOBAL::editor->myTextureLoader->GetDefaultMaterial();
	rr.myPixelShader = KE_GLOBAL::editor->myShaderLoader->GetPixelShader(
		SHADER_LOAD_PATH "Model_Deferred_PS.cso");
	rr.myVertexShader = KE_GLOBAL::editor->myShaderLoader->GetVertexShader(
		SHADER_LOAD_PATH "Model_Deferred_VS.cso");
	myModelData.myTransform = &myModelTransform.GetMatrix();

	myModelViewerGround.myMeshList = &KE_GLOBAL::editor->myModelLoader->Load(
		"Data/EditorAssets/modelViewerGroundPlane.fbx");
	auto& grr = myModelViewerGround.myRenderResources.emplace_back();
	grr.myMaterial = KE_GLOBAL::editor->myTextureLoader->GetCustomMaterial(
		"Data/EditorAssets/Prototype/Light/texture_08.png");
	grr.myPixelShader = KE_GLOBAL::editor->myShaderLoader->GetPixelShader(
		SHADER_LOAD_PATH "ModelViewer_Ground_PS.cso");
	grr.myVertexShader = KE_GLOBAL::editor->myShaderLoader->GetVertexShader(
		SHADER_LOAD_PATH "Model_Deferred_VS.cso");
	...
	myLightManager.Init(
		graphics->GetDevice().Get(), &graphics->GetShaderLoader(),
		{graphics->GetRenderWidth(), graphics->GetRenderHeight()});
	myLightManager.AssignCubemap(
		KE_GLOBAL::editor->myTextureLoader->GetCubemapFromPath(
			"Data/EngineAssets/CubeMap_Skansen.dds"));
	auto* dirLight = (KE::DirectionalLightData*)myLightManager.CreateLightData(
		KE::eLightType::Directional);
...
void KE_EDITOR::ModelViewer::Update() {
	...
	auto* cam =
		graphics->GetCameraManager().GetCamera(KE_DEBUG_CAMERA_INDEX - 1);
	...
	if (HoveringOverRender()) {
		...
		const auto rightClickDelta =
			ImGui::GetMouseDragDelta(ImGuiMouseButton_Right);
		if (rightClickDelta.x == 0 && rightClickDelta.y == 0) {
			lastRightClickDelta = {};
		}

		// pos += transform.GetRight() * -(rightClickDelta.x -
		// lastRightClickDelta.x) * 0.1f;
	...
	transform.GetMatrix() *=
		Matrix4x4f::CreateRotationAroundX(xAngle * KE::DegToRadImmediate);
	transform.GetMatrix() *=
		Matrix4x4f::CreateRotationAroundY(yAngle * KE::DegToRadImmediate);
...
void KE_EDITOR::ModelViewer::Render() {
	if (ImGui::BeginTable("ModelViewerTable", 2, ImGuiTableFlags_Resizable)) {
		...
		if (ImGui::BeginChild("imageChild")) {
			...
			if (ImGui::BeginDragDropTarget()) {
				if (const ImGuiPayload* payload =
					...
					myModelData.myMeshList =
						&KE_GLOBAL::editor->myModelLoader->Load(
			...
			auto* cam = graphics->GetCameraManager().GetCamera(
				KE_DEBUG_CAMERA_INDEX - 1);
			cam->SetPerspective(size.x, size.y, 90.0f * KE::DegToRadImmediate,
								0.1f, 100.0f);

			switch (myRenderType) {
				case RenderType::DeferredModel: {
					graphics->SetViewport(static_cast<int>(size.x),
				...
				case RenderType::VFXModel: {
					...
					RenderCustom(
						RenderType::VFXModel,
						KE_GLOBAL::editor->myShaderLoader->GetVertexShader(
				...
				case RenderType::FullscreenQuad: {
					RenderCustom(
						...
						KE_GLOBAL::editor->myShaderLoader->GetPixelShader(
							SHADER_LOAD_PATH "SDFRendering_PS.cso"),
						myModelData.myRenderResources[0].myMaterial, {});

					float aspect =
						static_cast<float>(graphics->GetRenderWidth()) /
						static_cast<float>(graphics->GetRenderHeight());
					if (customAspect) {
						aspect = customWidth / customHeight;
					}

					ImGui::Image(myRenderTarget->GetShaderResourceView(),
								 GetAspectRegion(size, aspect));
					lastRenderRectMin = ImGui::GetItemRectMin();
					lastRenderRectMax = ImGui::GetItemRectMax();
		...
		ImGuiHandler::DisplayEnumSlider("Mode", &myRenderType,
										RenderType::Count);
...
void KE_EDITOR::ModelViewer::RenderCustom(RenderType aType,
										  KE::VertexShader* aVS,
										  KE::PixelShader* aPS,
										  KE::Material* aMaterial,
	...
	switch (aType) {
		case RenderType::DeferredModel: {
			myBuffer->SetAsActiveTarget(graphics->GetContext().Get(),
										myBuffer->GetDepthStencilView());

			{
				const KE::BasicRenderInput input(nullptr, cam->GetViewMatrix(),
			...
			}
			{
				const KE::BasicRenderInput input(nullptr, cam->GetViewMatrix(),
												 cam->GetProjectionMatrix());
				graphics->GetDefaultRenderer()->RenderModel(
					input, myModelViewerGround);
			}

			{
				const KE::BasicRenderInput input(
					...
					myLightManager.myDirectionalLightCamera
						.GetProjectionMatrix(),
					aVS,
				...
				graphics->SetRasterizerState(
					KE::eRasterizerStates::FrontfaceCulling);
				graphics->GetDefaultRenderer()->RenderModel(input, myModelData);
				graphics->GetDefaultRenderer()->RenderModel(
					input, myModelViewerGround);
			...
			graphics->GetContext()->PSSetShaderResources(
				15, 1,
				myLightManager.GetCubemap()
		...
		case RenderType::VFXModel: {
			...
			graphics->SetDepthStencilState(
				KE::eDepthStencilStates::ReadOnlyLess);
		...
		case RenderType::FullscreenQuad: {
			myRenderTarget->MakeActive(false);
			myBuffer->SetAllAsResources(
				graphics->GetContext().Get(),
				4u);  // retain 0, 1, 2, 3 for input textures
