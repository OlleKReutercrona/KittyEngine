...
#pragma region ModelThumbnail
	...
	bool SaveImage(const std::string& aFilePath, KE::Graphics* aGraphics,
				   const int size, KE::RenderTarget& rt) {
		std::string fileNameBase = GetFileNameFromPath(aFilePath);
		std::string finalName =
			"Data/InternalAssets/ModelThumbnails/" + fileNameBase + ".dds";

		return aGraphics->GetTextureLoader().SaveTextureToFile(
			finalName, rt.GetShaderResourceView(), size, size);
	}

	bool RenderModelThumbnail(KE::Graphics* aGraphics,
							  const std::string& aFilePath, bool aSave,
							  KE::RenderTarget** aOutRenderTarget) {
		...
		if (ml->myMeshes.size() == 0) {
			// KE_ERROR("Thumbnail Generation failed for empty mesh %s.",
			// aFilePath.c_str());
			return false;
		}
		...
		if (modelData.myRenderResources.size() == 0) {
			...
			wd.myVertexShader = aGraphics->GetShaderLoader().GetVertexShader(
				SHADER_LOAD_PATH "Model_Tooltip_VS.cso");
			wd.myPixelShader = aGraphics->GetShaderLoader().GetPixelShader(
				SHADER_LOAD_PATH "Model_Tooltip_PS.cso");
		}
		if (modelData.myMeshList->myMaterialNames.size() > 0) {
			modelData.myRenderResources.back().myMaterial =
				aGraphics->GetTextureLoader().GetMaterialFromPath(
					modelData.myMeshList->myMaterialNames[0]);
		} else {
			modelData.myRenderResources.back().myMaterial =
				aGraphics->GetTextureLoader().GetDefaultMaterial();
		}

		Vector3f average = modelData.myMeshList->myMeshes[0].myBounds.min +
						   modelData.myMeshList->myMeshes[0].myBounds.max;
		average /= 2.0f;

		Vector3f dims = modelData.myMeshList->myMeshes[0].myBounds.max -
						modelData.myMeshList->myMeshes[0].myBounds.min;
		...
		transform.RotateWorld(
			{0.0f, (pRot.x + rotOff) * KE::DegToRadImmediate, 0.0f});
		if (!aSave) {
		...
		if (!rtInit) {
			rt.Init(aGraphics->GetDevice().Get(), aGraphics->GetContext().Get(),
					size, size);
			rtInit = true;
		}
		...
		aGraphics->GetDefaultRenderer()->RenderModel(
			{nullptr, cam.GetViewMatrix(), cam.GetProjectionMatrix()},
			modelData);
		...
		if (aSave) {
			return SaveImage(modelData.myMeshList->myFilePath, aGraphics, size,
							 rt);
		}
...
#pragma region VFXCurveEdit
	...
	int VFXCurveEdit::EditPoint(size_t curveIndex, int pointIndex,
								ImVec2 value) {
		(*curveData)[curveIndex].myData[pointIndex] =
			Vector2f(value.x, value.y);
		SortValues(curveIndex);
		for (size_t i = 0; i < GetPointCount(curveIndex); i++) {
			if ((*curveData)[curveIndex].myData[i].x == value.x) return (int)i;
		}
		return pointIndex;
	}
	...
	bool VFXCurveEdit::IsPointVisible(size_t aCurveIndex, size_t aPointIndex) {
		// if (curveData->at(aCurveIndex).myType ==
...
#pragma region VFXSequenceInterface
	void VFXSequenceInterface::DisplayParticleEmitter(
		int anIndex, ImDrawList* aDrawList, const ImRect& rc,
		...
		KE::ParticleEmitter& emitter =
			mySequence->myParticleEmitters[stamp.myEffectIndex].myEmitter;

		myCurveEdits[anIndex].mMin = ImVec2(0.0f, 0.f);
		myCurveEdits[anIndex].mMax = ImVec2((float)mySequence->myDuration, 1.f);

		aDrawList->AddText(ImVec2(legendRect.Min.x + 30, legendRect.Min.y),
						   0xFFFFFFFF, "Bursts");
		...
		ImCurveEdit::Edit(myCurveEdits[anIndex], rc.Max - rc.Min,
						  ImGui::GetID(FormatString("o%i n%i", anIndex, 0)),
						  &myCurveEdits[anIndex].editData, &clippingRect, NULL);
	...
	void VFXSequenceInterface::DisplayCurveEditMenu(int anIndex,
													int& curveToEdit) {
		if (ImGui::BeginPopup(
				KE_EDITOR::FormatString("curveEditMenu%i", anIndex))) {
			int currentCurve = (int)myCurveEdits[anIndex]
								   .curveData->at(curveToEdit)
								   .myCurveProfile;

			ImVec2 orgCurPos = ImGui::GetCursorPos();
			for (int i = 0; i < (int)KE::VFXCurveProfiles::Count; i++) {
				if (ImGui::RadioButton(KE::VFXCurveProfileNames[i],
									   currentCurve == i)) {
					myCurveEdits[anIndex]
						.curveData->at(curveToEdit)
						.myCurveProfile = (KE::VFXCurveProfiles)i;
				}
			}
			...
			if (ImGui::MenuItem("Delete Curve")) {
				myCurveEdits[anIndex].RemoveCurve(
					(KE::VFXAttributeTypes)curveToEdit);
			}
			ImGui::EndPopup();
		}
	}

	void VFXSequenceInterface::DisplayVFXMeshInstance(
		int anIndex, ImDrawList* aDrawList, const ImRect& rc,
		const ImRect& legendRect, const ImRect& clippingRect,
		const ImRect& legendClippingRect) {
		KE::VFXTimeStamp& stamp = mySequence->myTimestamps[anIndex];

		// aDrawList->AddRectFilled(clippingRect.Min, clippingRect.Max,
		// IM_COL32_BLACK);

		myCurveEdits[anIndex].mMin = ImVec2(0.0f, 0.f);
		myCurveEdits[anIndex].mMax = ImVec2((float)mySequence->myDuration, 1.f);
		// aDrawList->PushClipRect(legendClippingRect.Min,
		// legendClippingRect.Max, true);

		static int curveToEdit = -1;
		int visibleCurveIndex = 0;
		for (int i = 0; i < (int)KE::VFXAttributeTypes::Count; i++) {
			if (!myCurveEdits[anIndex].curveData->at(i).IsValid()) {
				continue;
			}
			ImVec2 pta(legendRect.Min.x + 30,
					   legendRect.Min.y + visibleCurveIndex * 14.f);
			ImVec2 ptb(legendRect.Max.x,
					   legendRect.Min.y + (visibleCurveIndex + 1) * 14.f);
			aDrawList->AddText(
				pta,
				myCurveEdits[anIndex].IsVisible(i) ? 0xFFFFFFFF : 0x80FFFFFF,
				myCurveEdits[anIndex].curveData->at(i).GetName());
			if (ImRect(pta, ptb).Contains(ImGui::GetMousePos())) {
				if (ImGui::IsMouseClicked(0) && ImGui::IsWindowFocused()) {
					myCurveEdits[anIndex].curveData->at(i).visible =
						!myCurveEdits[anIndex].curveData->at(i).visible;
		...
		if (ImRect(pta, ptb).Contains(ImGui::GetMousePos()) &&
			ImGui::IsMouseClicked(0) && ImGui::IsWindowFocused()) {
			ImGui::OpenPopup(
				KE_EDITOR::FormatString("addAttribute%i", anIndex));
		}

		KE::VFXAttributeTypes typeToAdd = KE::VFXAttributeTypes::Count;
		if (ImGui::BeginPopup(
				KE_EDITOR::FormatString("addAttribute%i", anIndex))) {
			for (KE::VFXAttributeTypes type = (KE::VFXAttributeTypes)0;
				 type < KE::VFXAttributeTypes::Count;
				 type = (KE::VFXAttributeTypes)((int)type + 1)) {
		...
		for (KE::VFXCurveDataSet& cd : *myCurveEdits[anIndex].curveData) {
			for (Vector2f& dd : cd.myData) {
				dd.x = (float)stamp.myStartpoint +
					   ((float)stamp.myEndpoint - (float)stamp.myStartpoint) *
						   dd.x;
			}
		}
		...
		if (ImGui::BeginPopup("pointEditPopup")) {
			...
			ImGui::SetTooltip(
				"%s\nValue: %.3f",
				myCurveEdits[anIndex]
					.curveData->at(myCurveEdits[anIndex].editData.overCurve)
					.GetName(),
				myCurveEdits[anIndex]
					.curveData->at(myCurveEdits[anIndex].editData.overCurve)
					.GetEvaluatedValue(
						static_cast<int>(
							myCurveEdits[anIndex].editData.overCurveT *
							static_cast<float>(
								(stamp.myStartpoint + stamp.myEndpoint))),
						stamp.myStartpoint, stamp.myEndpoint));
		}

		for (KE::VFXCurveDataSet& cd : *myCurveEdits[anIndex].curveData) {
			for (Vector2f& dd : cd.myData) {
				// turn dd.x back from the span [myStartpoint - > myEndpoint] to
				// [0 - > 1]
				dd.x = (dd.x - (float)stamp.myStartpoint) /
					   ((float)stamp.myEndpoint - (float)stamp.myStartpoint);
	...
	int VFXSequenceInterface::GetItemCount() const {
		return static_cast<int>(
			mySequence->myTimestamps
				.size()); /*static_cast<int>(mySequence->myVFXMeshes.size()) +
							 static_cast<int>(mySequence->myParticleEmitters.size());*/
	}

	void VFXSequenceInterface::Get(int index, int** start, int** end, int* type,
								   unsigned int* color) {
	...
	const char* VFXSequenceInterface::GetItemTypeName(int typeIndex) const {
		return typeIndex == (int)KE::VFXType::ParticleEmitter
				   ? "ParticleEmitter"
				   : "VFXMeshInstance";
	}
	...
	const char* VFXSequenceInterface::GetItemLabel(int index) const {
		return FormatString("%s %i",
							mySequence->myTimestamps[index].myType ==
									KE::VFXType::ParticleEmitter
								? "ParticleEmitter"
								: "VFXMeshInstance",
	...
	void VFXSequenceInterface::Add(int type) {
		...
		if (type == (int)KE::VFXType::VFXMeshInstance) {
			...
			ts.myEffectIndex =
				static_cast<int>(mySequence->myVFXMeshes.size()) - 1;
		} else if (type == (int)KE::VFXType::ParticleEmitter) {
			mySequence->AddParticleEmitter();
			ts.myEffectIndex =
				static_cast<int>(mySequence->myParticleEmitters.size()) - 1;
			// ts.myCurveDataSets[(int)KE::VFXAttributeTypes::PARTICLE_BURST].myType
			// = KE::VFXAttributeTypes::PARTICLE_BURST;
			// ts.myCurveDataSets[(int)KE::VFXAttributeTypes::PARTICLE_BURST].myCurveProfile
			// = KE::VFXCurveProfiles::Linear;
			// ts.myCurveDataSets[(int)KE::VFXAttributeTypes::PARTICLE_BURST].myData.push_back({
			// 0.0f, 0.5f });
			// ts.myCurveDataSets[(int)KE::VFXAttributeTypes::PARTICLE_BURST].myData.push_back({
			// 0.0f, 0.5f });
		}
	...
	size_t VFXSequenceInterface::GetCustomHeight(int index) {
		if (!mySequence->myTimestamps[index].myIsOpened) {
			return 0;
		}
		...
		switch (mySequence->myTimestamps[index].myType) {
			...
			case KE::VFXType::VFXMeshInstance: {
				int count = 0;
				for (auto& curve :
					 mySequence->myTimestamps[index].myCurveDataSets) {
					if (curve.IsValid()) {
						count++;
					}
				}
	...
	void VFXSequenceInterface::Del(int index) {
		...
		// case KE::VFXType::ParticleEmitter:
		//{
		//	mySequence->myParticleEmitters.erase(mySequence->myParticleEmitters.begin()
		...
		// }
		//
		// mySequence->myTimestamps.erase(mySequence->myTimestamps.begin() +
		// index);
		//
		// Link(mySequence);
	}
	...
	void VFXSequenceInterface::Duplicate(int index) {
		...
		switch (newTS.myType) {
			case KE::VFXType::ParticleEmitter: {
				mySequence->myParticleEmitters.back() =
					mySequence->myParticleEmitters[oldTS.myEffectIndex];
				break;
			}
			case KE::VFXType::VFXMeshInstance: {
				mySequence->myVFXMeshes.back() =
					mySequence->myVFXMeshes[oldTS.myEffectIndex];
				break;
			}
		}
	...
	void VFXSequenceInterface::CustomDraw(int anIndex, ImDrawList* aDrawList,
										  const ImRect& rc,
		...
		switch (stamp.myType) {
			case KE::VFXType::ParticleEmitter: {
				DisplayParticleEmitter(anIndex, aDrawList, rc, legendRect,
									   clippingRect, legendClippingRect);
				break;
			}
			case KE::VFXType::VFXMeshInstance: {
				DisplayVFXMeshInstance(anIndex, aDrawList, rc, legendRect,
									   clippingRect, legendClippingRect);
				break;
			}
			default:
