...
void KE_EDITOR::ImGuiHandler::Init(HWND hwnd, ID3D11Device* aDevice,
	...
	editor = aEditor;
	myDebugRenderer =
		KE_GLOBAL::blackboard.Get<KE::DebugRenderer>("debugRenderer");
	...
	// set custom style stuff:
	{
		// colours
		{
			...
			colors[ImGuiCol_FrameBgHovered] =
				ImVec4(0.08f, 0.53f, 0.41f, 0.29f);
			...
			colors[ImGuiCol_SliderGrabActive] =
				ImVec4(0.02f, 0.71f, 0.53f, 1.00f);
			...
			colors[ImGuiCol_SeparatorHovered] =
				ImVec4(1.00f, 0.00f, 0.80f, 0.78f);
			colors[ImGuiCol_SeparatorActive] =
				ImVec4(1.00f, 0.96f, 0.00f, 1.00f);
			colors[ImGuiCol_ResizeGrip] = ImVec4(0.08f, 0.53f, 0.41f, 0.20f);
			colors[ImGuiCol_ResizeGripHovered] =
				ImVec4(0.08f, 0.53f, 0.41f, 0.55f);
			colors[ImGuiCol_ResizeGripActive] =
				ImVec4(0.08f, 0.53f, 0.41f, 0.78f);
			...
			colors[ImGuiCol_TabUnfocusedActive] =
				ImVec4(0.08f, 0.53f, 0.45f, 0.51f);
			colors[ImGuiCol_DockingPreview] =
				ImVec4(0.08f, 0.53f, 0.41f, 0.71f);
			...
			colors[ImGuiCol_TextSelectedBg] =
				ImVec4(0.10f, 0.61f, 0.45f, 0.59f);
		...
		// font
		{
			// ImGui::GetIO().Fonts->AddFontFromFileTTF("Data/InternalAssets/Katex.ttf",
			// 16.0f);
		}
	}
}
...
void KE_EDITOR::ImGuiHandler::DisplayProgressBar(float aProgress) {
	...
	ImVec4 lerpedCol =
		ImLerp(ImLerp(noProgress, midProgress, toHalf), yesProgress, toEnd);
...
bool KE_EDITOR::ImGuiHandler::TryBeginWindow(const char* aString,
											 bool* aCondition,
											 ImGuiWindowFlags aWindowFlags) {
...
bool KE_EDITOR::ImGuiHandler::PerformFlagsBegin(const char* aString,
												KittyFlags aFlags,
												bool* aOpenFlag,
												int aInputFlags) {
	if (aFlags & eBeginWindow) {
		if (!TryBeginWindow(aString, aOpenFlag, aInputFlags)) {
			return false;
		}
	}
	if (aFlags & eSeparatorAbove) {
		ImGui::SeparatorText(aString);
	}
...
void KE_EDITOR::ImGuiHandler::PerformFlagsEnd() {
	...
	if (flagStack.back() & eSeparatorBelow) {
		ImGui::Separator();
	}
	if (flagStack.back() & eBeginWindow) {
		ImGui::End();
	}
	...
	if (ImGui::BeginMainMenuBar()) {
		if (ImGui::BeginMenu("File")) {
			if (ImGui::BeginMenu("Levels")) {
				...
				for (int i = 0; i < allScenes.size(); ++i) {
					bool isCurrentLevel =
						editor->mySceneManager->GetCurrentScene() ==
						&allScenes[i];
					bool isStartupLevel =
						editor->mySceneManager->GetStartupScene() ==
						&allScenes[i];

					if (ImGui::BeginChild(FormatString("%i sceneChild", i),
										  ImVec2(256.0f, 16.0f))) {
						if (ImGui::Selectable(allScenes[i].sceneName.c_str(),
											  &isCurrentLevel)) {
							customHandles.clear();
							editor->mySceneManager->SwapScene(
								allScenes[i].sceneID);
					...
					if (ImGui::Checkbox(
							FormatString("Startup##%i sceneActiveStartup", i),
							&isStartupLevel)) {
						editor->mySceneManager->ExportStartupScene(
							&allScenes[i]);
					}
				}
			...
			if (ImGui::BeginMenu("Build Settings")) {
				std::string appName(
					WideStringToNarrow(KE::BuildManager::GetApplicationName()));
				...
				ImVec4 colour = KE::BuildManager::IsBuilding()
		...
		if (ImGui::BeginMenu("View")) {
			if (ImGui::BeginMenu("Debug Rendering")) {
				...
				if (ImGui::BeginMenu("Navmesh")) {
					DebugRenderLevel& renderLevel =
						editor->myData.debugRenderData.renderNavmesh;
					if (ImGui::RadioButton(
							"None", renderLevel == DebugRenderLevel::eNone)) {
						renderLevel = DebugRenderLevel::eNone;
					}
					if (ImGui::RadioButton(
							"Basic", renderLevel == DebugRenderLevel::eBasic)) {
						renderLevel = DebugRenderLevel::eBasic;
					}
					if (ImGui::RadioButton(
							"Full", renderLevel == DebugRenderLevel::eFull)) {
						renderLevel = DebugRenderLevel::eFull;
					}
					ImGui::EndMenu();
				}
				if (ImGui::BeginMenu("Skeleton")) {
					DebugRenderLevel& renderLevel =
						editor->myData.debugRenderData.renderSkeleton;
					if (ImGui::RadioButton(
							"None", renderLevel == DebugRenderLevel::eNone)) {
						renderLevel = DebugRenderLevel::eNone;
					}
			...
			if (ImGui::BeginMenu("Render Layers")) {
				if (ImGui::Button("Regenerate Instancing Data")) {
					for (int i = 0;
						 i < static_cast<int>(KE::eRenderLayers::Count); i++) {
						editor->myGraphics
							->GetRenderLayer(static_cast<KE::eRenderLayers>(i))
							->GenerateInstancingData(
								editor->myGraphics->GetModelData());
					}
				}

				for (int i = 0; i < static_cast<int>(KE::eRenderLayers::Count);
					 i++) {
					ImGui::PushID(i);
					KE::RenderLayer* layer = editor->myGraphics->GetRenderLayer(
						static_cast<KE::eRenderLayers>(i));
					bool active = layer->flags & KE::RenderLayerFlags_Active;
					if (ImGui::Checkbox(
							EnumToString(static_cast<KE::eRenderLayers>(i)),
							&active)) {
					...
					if (ImGui::BeginMenu("Settings")) {
						...
						for (int j = 0; j < KE::RenderLayerFlags_Count; j++) {
							...
							if (ImGui::Checkbox(KE::RenderLayerFlagStrings[j],
												&flag)) {
								layer->flags ^= (1 << j);
							}
						}
						...
						DisplayEnumSlider("DepthStencilState",
										  &layer->mySettings.depthStencilState,
										  KE::eDepthStencilStates::Count);
						DisplayEnumSlider("RasterizerState",
										  &layer->mySettings.rasterizerState,
										  KE::eRasterizerStates::Count);
						DisplayEnumSlider("BlendState",
										  &layer->mySettings.blendState,
										  KE::eBlendStates::Count);
		...
		if (ImGui::BeginMenu("Windows")) {
			...
			for (const auto& lambdaWindowData :
				 editor->myLambdaWindowRegistry) {
				if (ImGui::MenuItem(lambdaWindowData.first.c_str())) {
					editor->myWindows.push_back(
						new LambdaWindow(lambdaWindowData.second));
				}
			}
		...
		ImGui::Text("Draw Calls: %i (%i Light)",
					editor->myGraphics->GetLastDrawCalls(),
					editor->myGraphics->GetLastLightDrawCalls());

		ImGui::SameLine(ImGui::GetWindowWidth() - 200);
		ImGui::Text("FPS: %i (%f MS)", (int)editor->myData.performance.Fps(),
					editor->myData.performance.deltaTime * 1000.f);
		ImGui::SameLine();
...
void KE_EDITOR::ImGuiHandler::DisplayNavmesh(KE::Navmesh* aNavmesh,
											 KE::Camera* aCamera,
											 ImVec4 aViewportBounds) {
	...
	for (KE::NavNode& node : aNavmesh->myNodes) {
		...
		for (int i = 0; i < 3; i++) {
			...
			drawList->AddText(nodePos[i], ImColor(1.0f, 0.0f, 1.0f, 1.0f),
							  FormatString("%i", node.indices[i]));
		}

		{
			...
			if (aCamera->WorldToScreenPoint(pos, clip, viewDims)) {
				ImVec2 imClip = ImVec2(clip.x, clip.y);
				drawList->AddText(upperLeft + imClip,
								  ImColor(1.0f, 1.0f, 1.0f, 1.0f),
								  FormatString("%i", nodeIndex));
			}
		}
...
void KE_EDITOR::ImGuiHandler::DisplayTransform(Transform* aTransform,
											   KittyFlags aFlags) {
	size_t transformID = (size_t)aTransform;

	ImGui::PushID((unsigned int)transformID);
	ImGui::PushStyleColor(ImGuiCol_ChildBg,
						  ImColor(0.12f, 0.12f, 0.12f, 1.0f).Value);
	ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 5.0f);

	ImGui::BeginChild("TransformChild", ImVec2(0.0f, 0.0f),
	...
	if (PerformFlagsBegin("Transform", aFlags)) {
		...
		ImGuizmo::DecomposeMatrixToComponents((float*)&aTransform->GetMatrix(),
											  &pos.x, &rot.x, &scale.x);
		...
		ImGuizmo::RecomposeMatrixFromComponents(
			&pos.x, &rot.x, &scale.x, (float*)&aTransform->GetMatrix());
		...
		if (showMatrix) {
			...
			bool changed = false;
			if (ImGui::DragFloat4("##row1", &matrix.r[0].m128_f32[0])) {
				changed = true;
			}
			if (ImGui::DragFloat4("##row2", &matrix.r[1].m128_f32[0])) {
				changed = true;
			}
			if (ImGui::DragFloat4("##row3", &matrix.r[2].m128_f32[0])) {
				changed = true;
			}
			if (ImGui::DragFloat4("##row4", &matrix.r[3].m128_f32[0])) {
				changed = true;
			}
			if (changed) {
...
bool KE_EDITOR::ImGuiHandler::DisplayCustomHandle(CustomHandle& aCustomHandle,
												  KE::Camera* aCamera,
												  ImVec4 aViewportBounds) {
	ImVec2 windowMousePos = GetMousePosInWindow(ImVec2(0.0f, 0.0f));
	ImVec2 upperLeft = ImVec2(aViewportBounds.x, aViewportBounds.y);
	...
	switch (aCustomHandle.myGizmoType) {
		case ArbitraryGizmo::Type::VECTOR3: {
			...
			Vector2f clip;
			if (!aCamera->WorldToScreenPoint(
					drawPos, clip,
					Vector2f(aViewportBounds.z, aViewportBounds.w))) {
				break;
			}
			...
			const float dist =
				(Vector2f(windowMousePos.x, windowMousePos.y) -
				 Vector2f(upperLeft.x + imClip.x, upperLeft.y + imClip.y))
					.LengthSqr();

			if (dist < nodeRadius * nodeRadius || isUsingData) {
				drawList->AddCircleFilled(upperLeft + imClip, nodeRadius,
										  ImColor(1.0f, 1.0f, 1.0f, 1.0f));
				if (mouseClicked && !isUsingData) {
					arbitraryGizmo.Attach(
						ImGui::GetIO().KeyCtrl, ArbitraryGizmo::Type::VECTOR3,
						aCustomHandle.myData, aCustomHandle.myTransform);
					used = true;
				}
			} else {
				drawList->AddCircle(upperLeft + imClip, nodeRadius,
									ImColor(1.0f, 1.0f, 1.0f, 1.0f));
			}
			drawList->AddText(upperLeft + imClip,
							  ImColor(1.0f, 1.0f, 1.0f, 1.0f),
							  aCustomHandle.myText.c_str());
		...
		case ArbitraryGizmo::Type::TRANSFORM: {
			...
			if (dist < nodeRadius * nodeRadius || isUsingData) {
				...
				if (mouseClicked && !isUsingData) {
					arbitraryGizmo.Attach(
						ImGui::GetIO().KeyCtrl, ArbitraryGizmo::Type::TRANSFORM,
						aCustomHandle.myData, aCustomHandle.myTransform);
					used = true;
				}
			} else {
...
bool KE_EDITOR::ImGuiHandler::DisplayCustomHandles(KE::Camera* aCamera,
												   ImVec4 aViewportBounds) {
	bool anyUsed = false;
	for (int i = 0; i < customHandles.size(); i++) {
		anyUsed =
			DisplayCustomHandle(customHandles[i], aCamera, aViewportBounds)
				? true
				: anyUsed;
	}
	return anyUsed;
...
void KE_EDITOR::ImGuiHandler::DisplayBoundingBox(const Vector3f& aWorldMin,
												 const Vector3f& aWorldMax,
												 KE::Camera* aCamera,
												 ImVec4 aViewportBounds) {}

bool KE_EDITOR::ImGuiHandler::DisplayArbitraryGizmo(ArbitraryGizmo* aGizmo,
													KE::Camera* aCamera,
													ImVec4 aViewportBounds) {
	// Vector3f averagePos;
	...
	for (Transform& trf : aGizmo->myTransforms) {
		averageTransform.SetMatrix(averageTransform.GetCUMatrix() +
								   trf.GetCUMatrix());
	}
	averageTransform.SetMatrix(averageTransform.GetCUMatrix() *
							   (1.0f / (float)aGizmo->myTransforms.size()));

	switch (aGizmo->myType) {
		case ArbitraryGizmo::Type::VECTOR3: {
			Transform deltaMatrix;
			DisplayGizmo(&averageTransform, aCamera, aViewportBounds,
						 &deltaMatrix);
	...
	if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) &&
		!ImGui::GetIO().KeyCtrl && !ImGuizmo::IsOver()) {
...
void KE_EDITOR::ImGuiHandler::DisplayGizmo(Transform* aTransform,
										   KE::Camera* aCamera,
										   ImVec4 aViewportBounds,
										   void* anOutDeltaMatrix) {
	ImGuizmo::SetOrthographic(false);
	ImDrawList* drawList = ImGui::GetWindowDrawList();
	ImGuizmo::SetDrawlist(drawList);

	ImGuizmo::SetRect(aViewportBounds.x, aViewportBounds.y, aViewportBounds.z,
					  aViewportBounds.w);
	...
	drawList->AddRectFilled(
		...
		ImColor(0.0f, 0.0f, 0.0f, 0.78f), 10.0f,
	...
	if (ImGui::RadioButton("Translate", op == opT)) {
		op = opT;
	}
	if (ImGui::RadioButton("Rotate", op == opR)) {
		op = opR;
	}
	if (ImGui::RadioButton("Scale", op == opS)) {
		op = opS;
	}
	ImGui::NewLine();
	if (ImGui::RadioButton("Local", md == mdL)) {
		md = mdL;
	}
	if (ImGui::RadioButton("World", md == mdW)) {
		md = mdW;
	}
	...
	ImGuizmo::Manipulate(&viewMatrix(1, 1), &projectionMatrix(1, 1), op, md,
						 (float*)&transform, (float*)&deltaMatrix);
...
bool KE_EDITOR::ImGuiHandler::BeginDisplayComponent(
	const std::string& aComponentName, KE::Component* aComponent) {
	...
	ImGui::BeginChild("ComponentChild", ImVec2(0.0f, 0.0f),
					  ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_Border);
...
void KE_EDITOR::ImGuiHandler::ComponentDisplayDispatch(
	KE::Component* aComponent) {
	KittyFlags componentFlags = eSeparatorAbove;

	// ImGui::PushID((unsigned long long)aComponent);
	// ImGui::PushStyleColor(ImGuiCol_ChildBg, ImColor(0.12f, 0.12f,
	// 0.12f, 1.0f).Value);
	// ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 5.0f);
	...
	if (dynamic_cast<KE::ModelComponent*>(aComponent)) {
		...
		DisplaySkeletalModelComponent((KE::SkeletalModelComponent*)aComponent,
									  componentFlags);
	...
	// }
		...
		DisplayCameraComponent((KE::CameraComponent*)aComponent,
							   componentFlags);
	} else if (dynamic_cast<KE::BoxColliderComponent*>(aComponent)) {
		DisplayBoxColliderComponent((KE::BoxColliderComponent*)aComponent,
									componentFlags);
	} else if (dynamic_cast<KE::SphereColliderComponent*>(aComponent)) {
		DisplaySphereColliderComponent((KE::SphereColliderComponent*)aComponent,
									   componentFlags);
	} else if (dynamic_cast<KE::PostProcessingComponent*>(aComponent)) {
		DisplayPostProcessingComponent((KE::PostProcessingComponent*)aComponent,
									   componentFlags);
	...
	} else if (auto* shellComp =
				   dynamic_cast<KE::ShellTexturingComponent*>(aComponent)) {
...
void KE_EDITOR::ImGuiHandler::DisplayDecalComponent(
	KE::DecalComponent* aDecalComponent, KittyFlags aFlags) {
	// COMPONENT_NAME(aDecalComponent);
	BEGIN_DISPLAY_COMPONENT(aDecalComponent);

	if (auto* decal = aDecalComponent->myDecalManager->GetDecal(
			aDecalComponent->myDecalIndex)) {
		ImGui::Text("Decal Index: %i", aDecalComponent->myDecalIndex);
		ImGui::DragFloat4("Intensities", &decal->myTextureIntensities.x, 0.1f,
						  0.0f, 1.0f);
...
void KE_EDITOR::ImGuiHandler::DisplayModelComponent(
	KE::ModelComponent* aModelComponent, KittyFlags aFlags) {
...
bool KE_EDITOR::ImGuiHandler::DisplayComponentHeader(
	const std::string& aComponentName, KE::Component* aComponent) {
	...
	ImGui::SameLine(32);
	bool open = ImGui::TreeNodeEx(aComponentName.c_str(),
...
void KE_EDITOR::ImGuiHandler::DisplaySkeletalModelComponent(
	KE::SkeletalModelComponent* aSkeletalModelComponent, KittyFlags aFlags) {
	// COMPONENT_NAME(aSkeletalModelComponent);
	BEGIN_DISPLAY_COMPONENT(aSkeletalModelComponent);

	// if (!BeginDisplayComponent("Skeletal Model Component",
	// aSkeletalModelComponent)) { return; }

	KE::SkeletalModelData* modelData = aSkeletalModelComponent->GetModelData();
	DisplaySkeletalModelData(modelData);

	KE::AnimationPlayer& animationPlayer =
		aSkeletalModelComponent->GetAnimationPlayer();
	ImGui::SeparatorText("Animations");
	for (auto& pair : modelData->myAnimationClipMap) {
		bool current = pair.second == animationPlayer.currentAnimation.myClip;
		if (ImGui::RadioButton(pair.first.c_str(), current)) {
			animationPlayer.PlayAnimation(pair.first);
		}
		if (current) {
			float progress = animationPlayer.currentAnimation.myTime /
							 animationPlayer.currentAnimation.myClip->duration;
			if (ImGui::SliderFloat("##animProgress", &progress, 0.0f, 1.0f,
								   "%.2f")) {
				animationPlayer.currentAnimation.myTime =
					progress *
					animationPlayer.currentAnimation.myClip->duration;
			}
		}
...
void KE_EDITOR::ImGuiHandler::DisplayShadowRendering(
	KE::DeferredLightManager* aLightManager) {
	ImGui::Separator();
	ImGui::Text("Shadow Settings");
	static Vector2i viewportSize = {350, 200};
	static Vector2f nearandfar = {-1000.0f, 500.0f};
	if (ImGui::DragInt2("Shadow Camera viewport size", &viewportSize.x, 1, 10,
						8000)) {
		aLightManager->InitShadowCamera(viewportSize, nearandfar.x,
										nearandfar.y);
	}
	if (ImGui::DragFloat("Shadow Camera near plane", &nearandfar.x, 5.0f, -2000,
						 0.0f)) {
		aLightManager->InitShadowCamera(viewportSize, nearandfar.x,
										nearandfar.y);
	}
	if (ImGui::DragFloat("Shadow Camera far plane", &nearandfar.y, 5.0f, 1.0f,
						 2000.0f)) {
		aLightManager->InitShadowCamera(viewportSize, nearandfar.x,
										nearandfar.y);
	}
	...
	ImGui::Image(aLightManager->myDepthBuffer.GetShaderResourceView(),
				 ImVec2(256, 144));
}

void KE_EDITOR::ImGuiHandler::DisplayLightComponent(
	KE::LightComponent* aLightComponent, KittyFlags aFlags) {
	// COMPONENT_NAME(aLightComponent);
	BEGIN_DISPLAY_COMPONENT(aLightComponent);

	KE::LightComponentData* componentData =
		(KE::LightComponentData*)aLightComponent->GetLightData();
	...
	if (componentData->myLightType == KE::eLightType::Directional) {
		...
		ImGui::DragFloat("Intensity", &dlData->myDirectionalLightIntensity,
						 0.1f, 0.0f, 100.0f);
		ImGui::DragFloat("Ambient Intensity", &dlData->myAmbientLightIntensity,
						 0.1f, 0.0f, 100.0f);

		DisplayShadowRendering(&editor->myGraphics->GetDeferredLightManager());
		// ImGui::Text("Dir Light RT");
		// ImGui::Image(editor->myGraphics->GetRenderTarget(6)->GetShaderResourceView(),
		// ImVec2(256, 144));
		...
		ImGui::DragFloat("Intensity", &plData->myIntensity, 0.1f, 0.0f,
						 1000.0f);
		...
		ImGui::DragFloat("Intensity", &slData->myIntensity, 0.1f, 0.0f,
						 1000.0f);
		...
		ImGui::DragFloat("Inner Angle", &slData->myInnerAngle, 0.1f, 0.0f,
						 outerMax);
		ImGui::DragFloat("Outer Angle", &slData->myOuterAngle, 0.1f, innerMax,
						 179.0f);
	}
...
// void KE_EDITOR::ImGuiHandler::DisplayAudioComponent(KE::AudioComponent*
// aAudioComponent, KittyFlags aFlags)
//{
...
void KE_EDITOR::ImGuiHandler::DisplayCameraComponent(
	KE::CameraComponent* aCameraComponent, KittyFlags aFlags) {
	...
	DisplayEnumSlider("Control Mode", &aCameraComponent->myMode,
					  KE::CameraComponentMode::Count);
	...
	switch (camType) {
		case KE::ProjectionType::Perspective: {
			...
			if (ImGui::DragFloat("Fov", &degFov, 1.0f, 1.0f, 145.0f)) {
				changed = true;
			}
			if (ImGui::DragFloat("Near", &projData.perspective.nearPlane)) {
				changed = true;
			}
			if (ImGui::DragFloat("Far", &projData.perspective.farPlane)) {
				changed = true;
			}

			if (changed) {
				cam->SetPerspective(projData.perspective.width,
		...
		case KE::ProjectionType::Orthographic: {
			...
			bool changed = false;
			if (ImGui::DragFloat("Width", &projData.orthographic.width)) {
				changed = true;
			}
			if (ImGui::DragFloat("Height", &projData.orthographic.height)) {
				changed = true;
			}
			if (ImGui::DragFloat("Near", &projData.orthographic.nearPlane)) {
				changed = true;
			}
			if (ImGui::DragFloat("Far", &projData.orthographic.farPlane)) {
				changed = true;
			}

			if (changed) {
				cam->SetOrthographic(projData.orthographic.width,
	...
	ImGui::Image(
		...
		GetAspectRegion(ImGui::GetContentRegionAvail(),
						(float)editor->myGraphics->GetRenderWidth() /
							(float)editor->myGraphics->GetRenderHeight()));

	END_DISPLAY_COMPONENT();
}

void KE_EDITOR::ImGuiHandler::DisplayBoxColliderComponent(
	KE::BoxColliderComponent* aBoxColliderComponent, KittyFlags aFlags) {
...
void KE_EDITOR::ImGuiHandler::DisplaySphereColliderComponent(
	KE::SphereColliderComponent* aSphereColliderComponent, KittyFlags aFlags) {
	// COMPONENT_NAME(aSphereColliderComponent);
	BEGIN_DISPLAY_COMPONENT(aSphereColliderComponent);

	if (ImGui::DragFloat3("Offset##SphereColOffset",
						  &aSphereColliderComponent->myOffset.x, 0.25f)) {
		aSphereColliderComponent->myCollider->UpdateOffset(
			aSphereColliderComponent->myOffset);
	}

	END_DISPLAY_COMPONENT();
}

void KE_EDITOR::ImGuiHandler::DisplayPostProcessingComponent(
	KE::PostProcessingComponent* aPostProcessingComponent, KittyFlags aFlags) {
...
void KE_EDITOR::ImGuiHandler::DisplayVFXComponent(
	KE::VFXComponent* aVFXComponent, KittyFlags aFlags) {
	...
	for (int i = 0; i < aVFXComponent->GetVFXSequenceCount(); i++) {
		...
		KE::VFXSequence* sequence =
			editor->myGraphics->GetVFXManager().GetVFXSequence(
				vfxSequenceIndex);
		ImGui::TableNextRow();
		...
		ImGui::TableNextColumn();
		if (ImGui::Button("Play")) {
			aVFXComponent->TriggerVFX(i);
		}
		ImGui::TableNextColumn();
		if (ImGui::Button("Loop")) {
			aVFXComponent->TriggerVFX(i, true);
		}
		ImGui::TableNextColumn();
		if (ImGui::Button("Stop")) {
			aVFXComponent->StopVFX(i);
		}
		ImGui::TableNextColumn();
		if (ImGui::Button("Edit")) {
			vfxEditorActiveSequenceIndex = vfxSequenceIndex;
		}
		ImGui::PopID();
	}
	ImGui::EndTable();
...
void KE_EDITOR::ImGuiHandler::DisplayShellTexturingComponent(
	KE::ShellTexturingComponent* aShellComponent, KittyFlags aFlags) {
	...
	if (KE::RenderTarget* rt =
			aShellComponent->shellModelData.Get()->effectsRT) {
		ImGui::PushID("ModificationTex");
		ImGui::Image(rt->GetShaderResourceView(), {256.0f, 256.0f});
		if (ImGui::Button("Reset")) {
			aShellComponent->ClearDisplacement();
		}
		...
		if (ImGui::Button("Save")) {
			...
			gfx->GetTextureLoader().SaveTextureToFile(
				path, rt->GetShaderResourceView(), 1024, 1024);
		}
		...
		if (isEditing) {
			...
			KE::Camera* viewingCamera =
				gfx->GetCameraManager().GetHighlightedCamera();

			auto mp =
				KE_GLOBAL::blackboard.Get<KE::InputHandler>("inputHandler")
					->GetMousePosition();
			Rayf painterRay = viewingCamera->GetRay({(float)mp.x, (float)mp.y});

			Vector3f planeHit =
				painterRay.GetOrigin() +
				painterRay.GetDirection() *
					(-painterRay.GetOrigin().y / painterRay.GetDirection().y);
			...
			if (ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
				...
				gfx->GetShellTexturedRenderer()->RenderDisplacement(
					aShellComponent->shellModelData, {displacement},
					aShellComponent->sharedBounds.min,
					aShellComponent->sharedBounds.max, nullptr,
					(ImGui::GetIO().KeyAlt ? 2 : 1) *
						(ImGui::GetIO().KeyCtrl ? -1 : 1));
			}
		}
...
//
void KE_EDITOR::ImGuiHandler::DisplaySkeleton(KE::GameObject* aGameObject,
											  KittyFlags aFlags) {
	auto& skeletalModel =
		aGameObject->GetComponent<KE::SkeletalModelComponent>();
	// auto& debugRenderer =
	// aGameObject->GetManager().GetScene()->myDebugRenderer;
	auto& worldSpaceTransforms =
		skeletalModel.GetModelData()->myWorldSpaceJoints;
	...
	for (int i = 0; i < bones.size(); i++) {
		...
		customHandles.push_back(
			{nullptr, ArbitraryGizmo::Type::VECTOR3, &v, currentBoneTransform,
			 FormatString("%i - %s", i, bones[i].myName.c_str())});

		int parentIndex = bones[i].myParentIndex;
		if (parentIndex > -1) {
			Transform parentBoneTransform = worldSpaceTransforms[parentIndex];

			myDebugRenderer->RenderLine(currentBoneTransform.GetPosition(),
										parentBoneTransform.GetPosition());
		}
	}
}

void KE_EDITOR::ImGuiHandler::DisplayParticleEmitter(
	KE::ParticleEmitter* anEmitter, KittyFlags aFlags) {
	DisplayTexture(&anEmitter->mySpriteBatch.myData.myTexture,
				   ImVec2(64.0f, 64.0f));

	if (ImGui::RadioButton("Default", anEmitter->mySpriteBatch.myData.myMode ==
										  KE::SpriteBatchMode::Default)) {
		anEmitter->mySpriteBatch.myData.myMode = KE::SpriteBatchMode::Default;
	}

	if (ImGui::RadioButton("Billboard",
						   anEmitter->mySpriteBatch.myData.myMode ==
							   KE::SpriteBatchMode::Billboard)) {
		anEmitter->mySpriteBatch.myData.myMode = KE::SpriteBatchMode::Billboard;
	}

	if (PerformFlagsBegin("Particle Emitter", aFlags)) {
		ImGui::DragFloat2("Burst Time",
						  &anEmitter->mySharedParticleAttributes.burstTimeMin);
		ImGui::DragInt2("Burst Count",
						&anEmitter->mySharedParticleAttributes.burstCountMin);
		ImGui::DragFloat2("Velocity",
						  &anEmitter->mySharedParticleAttributes.velocityMin);
		ImGui::DragFloat2(
			"Acceleration",
			&anEmitter->mySharedParticleAttributes.accelerationMin);
		ImGui::DragFloat2("Life Time",
						  &anEmitter->mySharedParticleAttributes.lifeTimeMin);
		ImGui::DragFloat2("Angle",
						  &anEmitter->mySharedParticleAttributes.angleMin);
		ImGui::DragFloat2(
			"Velocity Factor",
			&anEmitter->mySharedParticleAttributes.horizontalVelocityFactor);
		ImGui::Separator();
		ImGui::Text("Particle Count: %d",
					anEmitter->myParticleCapacity -
						anEmitter->myFreeParticleIndices.size());
		ImGui::Text("Free Particle Count: %d",
					anEmitter->myFreeParticleIndices.size());

		if (ImGui::TreeNode("Colour")) {
			ImGui::ColorEdit4(
				"Start Colour",
				&anEmitter->mySharedParticleAttributes.startColor.x);
			ImGui::ColorEdit4(
				"Mid Colour",
				&anEmitter->mySharedParticleAttributes.midColor.x);
			ImGui::ColorEdit4(
				"End Colour",
				&anEmitter->mySharedParticleAttributes.endColor.x);

			ImGui::TreePop();
		}

		if (ImGui::TreeNode("Size")) {
			ImGui::DragFloat("Start Size",
							 &anEmitter->mySharedParticleAttributes.startSize);
			ImGui::DragFloat("Mid Size",
							 &anEmitter->mySharedParticleAttributes.midSize);
			ImGui::DragFloat("End Size",
							 &anEmitter->mySharedParticleAttributes.endSize);
...
void KE_EDITOR::ImGuiHandler::DisplayPostProcessing(
	KE::PostProcessing* aPostProcessing, KittyFlags aFlags) {
	if (PerformFlagsBegin("Post Processing", aFlags)) {
		if (ImGui::TreeNode("Gaussian Blur settings")) {
			ImGui::Checkbox("Activate Gaussian Blur",
							&aPostProcessing->myGaussianActive);
			ImGui::SliderFloat("Direction##GaussianDirection",
							   &aPostProcessing->myAttributes.gaussianDirection,
							   10.0f, 25.0f);
			ImGui::SliderFloat("Quality##GaussianQuality",
							   &aPostProcessing->myAttributes.gaussianQuality,
							   2.5f, 25.0f);
			ImGui::SliderFloat("Size##GaussianSize",
							   &aPostProcessing->myAttributes.gaussianSize,
							   0.0f, 100.0f);
			ImGui::SliderFloat("Treshold##GaussianTreshold",
							   &aPostProcessing->myAttributes.gaussianTreshold,
							   0.0f, 1.0f);
		...
		if (ImGui::TreeNode("Colour Correction Settings")) {
			ImGui::SliderFloat("Saturation##CCSaturation",
							   &aPostProcessing->myAttributes.saturation, 0.0f,
							   5.0f);
			ImGui::SliderFloat("Exposure##CCExposure",
							   &aPostProcessing->myAttributes.exposure, -5.0f,
							   5.0f);
			ImGui::SliderFloat("Contrast##CCContrast",
							   &aPostProcessing->myAttributes.contrast, -5.0f,
							   5.0f);
			if (ImGui::TreeNode("Tint Settings")) {
				ImGui::ColorPicker4("Tint##CCTint",
									&aPostProcessing->myAttributes.tint.x);
				ImGui::TreePop();
			}
			if (ImGui::TreeNode("BlackPoint Settings")) {
				ImGui::ColorPicker4(
					"BlackPoint##CCBlackPoint",
					&aPostProcessing->myAttributes.blackPoint.x);
				ImGui::TreePop();
			}
			if (ImGui::TreeNode("Colour Correction Settings")) {
				ImGui::ColorPicker3(
					"Colour Correction",
					&aPostProcessing->myAttributes.colourCorrecting.x);
				ImGui::TreePop();
			}
		...
		if (ImGui::TreeNode("Chromatic Aberration Settings")) {
			ImGui::SliderFloat("Strength##CAMultiplier",
							   &aPostProcessing->myAttributes.CAMultiplier,
							   0.0f, 5.0f);
			ImGui::SliderFloat2("Red Offset##CARedOffset",
								&aPostProcessing->myAttributes.CARedOffset.x,
								-5.0f, 5.0f);
			ImGui::SliderFloat2("Green Offset##CAGreenOffset",
								&aPostProcessing->myAttributes.CAGreenOffset.x,
								-5.0f, 5.0f);
			ImGui::SliderFloat2("Blue Offset##CABlueOffset",
								&aPostProcessing->myAttributes.CABlueOffset.x,
								-5.0f, 5.0f);
		...
		if (ImGui::TreeNode("Bloom Settings")) {
			ImGui::SliderInt("Number of downsamples##bloomDownSamples",
							 &aPostProcessing->myNumberOfDownSamples, 1,
							 MAX_NUMBER_OF_DOWNSAMPLES);
			ImGui::SliderFloat("Blending##bloomBending",
							   &aPostProcessing->myAttributes.bloomBlending,
							   0.0f, 1.0f);
			ImGui::SliderFloat("Alpha Treshold##bloomTreshold",
							   &aPostProcessing->myAttributes.bloomTreshold,
							   0.0f, 1.0f);
			ImGui::ColorEdit3(
				"Bloom Sample Pass",
				&aPostProcessing->myAttributes.bloomSampleTreshold.x);

			ImGui::Spacing();
			if (ImGui::TreeNode("Bloom Sample")) {
				static Vector2i bloomWindowSize = {256, 144};
				ImGui::DragInt("Window Width##BloomSampleWindowSize",
							   &bloomWindowSize.x, 5, 0, 512);
				bloomWindowSize.y =
					static_cast<int>((float)bloomWindowSize.x * 0.5625f);
				ImGui::Image(
					aPostProcessing->myPrePostProcessSample
						.GetShaderResourceView(),
					ImVec2((float)bloomWindowSize.x, (float)bloomWindowSize.y));
				ImGui::TreePop();
			}
			ImGui::TreePop();
		}
		...
		if (ImGui::TreeNode("Vignette")) {
			ImGui::SliderFloat("Size##VignetteSize",
							   &aPostProcessing->myAttributes.vignetteSize,
							   0.0f, 1.0f);
			ImGui::SliderFloat(
				"Feather Thickness##VignetteFeatherThickness",
				&aPostProcessing->myAttributes.vignetteFeatherThickness, 0.0f,
				1.0f);
			ImGui::SliderFloat("Intensity##VignetteIntensity",
							   &aPostProcessing->myAttributes.vignetteIntensity,
							   0.0f, 1.0f);
			ImGui::SliderInt("Show Mask##VignetteShowMask",
							 &aPostProcessing->myAttributes.vignetteShowMask, 0,
							 1);
		...
		if (ImGui::TreeNode("ToneMap")) {
			ImGui::SliderFloat("ToneMap Intensity",
							   &aPostProcessing->myAttributes.toneMapIntensity,
							   0.0f, 1.0f);
			ImGui::TreePop();
		}
...
void KE_EDITOR::ImGuiHandler::DisplayShader(KE::Shader** aShader,
											bool aDragSource,
											bool aDragTarget) {
	KE::Shader*& underlyingShader = *aShader;
	if (underlyingShader->myName.empty()) {
		return;
	}

	const size_t lastSlash = underlyingShader->myName.find_last_of("/");
	const std::string beautifiedShaderName = underlyingShader->myName.substr(
		lastSlash + 1, underlyingShader->myName.length() - lastSlash - 5);
	...
	switch (underlyingShader->myShaderType) {
		case KE::Shader::ShaderType::eVertexShader: {
			...
			if (ImGui::BeginDragDropSource()) {
				ImGui::SetDragDropPayload(
					"vertexShaderPayload", underlyingShader->myName.c_str(),
					strlen(underlyingShader->myName.c_str()) + 1);
			...
			if (ImGui::BeginDragDropTarget()) {
				if (const ImGuiPayload* payload =
						ImGui::AcceptDragDropPayload("vertexShaderPayload")) {
					std::string shaderName = (char*)payload->Data;
					KE::Shader* shader =
						editor->myShaderLoader->GetVertexShader(shaderName);
		...
		case KE::Shader::ShaderType::ePixelShader: {
			...
			if (ImGui::BeginDragDropSource()) {
				ImGui::SetDragDropPayload(
					"pixelShaderPayload", underlyingShader->myName.c_str(),
					strlen(underlyingShader->myName.c_str()) + 1);
				ImGui::Text(beautifiedShaderName.c_str());
				ImGui::EndDragDropSource();
			}
			...
			if (ImGui::BeginDragDropTarget()) {
				if (const ImGuiPayload* payload =
						ImGui::AcceptDragDropPayload("pixelShaderPayload")) {
					std::string shaderName = (char*)payload->Data;
					KE::Shader* shader =
						editor->myShaderLoader->GetPixelShader(shaderName);
					*aShader = shader;
				}
				ImGui::EndDragDropTarget();
			}
...
void KE_EDITOR::ImGuiHandler::DisplayModelData(KE::ModelData* aModelData,
											   KittyFlags aFlags) {
	if (PerformFlagsBegin("Model Data", aFlags)) {
		if (ImGui::BeginTabBar("MeshTabBar")) {
			if (ImGui::TabItemButton(FormatString(
					"Mesh: %s:",
					GetFileNameFromPath(aModelData->myMeshList->myFilePath)))) {
				ImGui::OpenPopup("MeshSelectorPopup");
			}

			if (ImGui::BeginDragDropTarget()) {
				if (const ImGuiPayload* payload =
						ImGui::AcceptDragDropPayload("modelPath")) {
					std::string meshPath = (char*)payload->Data;
					KE_LOG("Editor", "DragDrop Mesh Path: %s",
						   meshPath.c_str());
					KE::MeshList& meshList =
						editor->myModelLoader->Load(meshPath);
			...
			for (int i = 0; i < aModelData->myMeshList->myMeshes.size(); i++) {
				std::string materialName =
					aModelData->myMeshList->myMaterialNames[i];
				if (ImGui::BeginTabItem(FormatString("%i", i))) {
					auto& rrToUse =
						aModelData->myRenderResources
							[i >= aModelData->myRenderResources.size() ? 0 : i];

					DisplayMaterial(rrToUse.myMaterial);

					ImGui::SeparatorText("Shaders");
					DisplayShader((KE::Shader**)&rrToUse.myVertexShader, true,
								  true);
					DisplayShader((KE::Shader**)&rrToUse.myPixelShader, true,
								  true);
...
void KE_EDITOR::ImGuiHandler::DisplaySkeletalModelData(
	KE::SkeletalModelData* aModelData, KittyFlags aFlags) {
	if (PerformFlagsBegin("Skeletal Model Data", aFlags)) {
		ImGui::Checkbox("outline", &aModelData->myRenderOutline);
		// ImGui::Combo("Render Layer: ", (int*)&aModelData->myRenderLayer,
		// KE::eRenderLayersStrings,
		// static_cast<int>(KE::eRenderLayers::Count));
		if (ImGui::BeginTabBar("MeshTabBar")) {
			if (ImGui::TabItemButton(FormatString(
					"Mesh: %s:",
					GetFileNameFromPath(aModelData->myMeshList->myFilePath)))) {
			...
			if (ImGui::BeginDragDropTarget()) {
				if (const ImGuiPayload* payload =
						ImGui::AcceptDragDropPayload("skeletalModelPath")) {
					std::string meshPath = (char*)payload->Data;
					KE_LOG("Editor", "DragDrop Mesh Path: %s",
						   meshPath.c_str());

					editor->myModelLoader->LoadSkeletalModel(*aModelData,
															 meshPath);
				}
			...
			for (int i = 0; i < aModelData->myMeshList->myMeshes.size(); i++) {
				...
				if (ImGui::BeginTabItem(FormatString("%i", i))) {
					ImGui::BeginChild(
						"##modelRenderResourceChild", {0, 0},
						ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_Border);
					DisplayMaterial(
						aModelData
							->myRenderResources
								[i >= aModelData->myRenderResources.size() ? 0
																		   : i]
							.myMaterial);

					ImGui::SeparatorText("Shaders");
					DisplayShader(
						(KE::Shader**)&aModelData
							->myRenderResources
								[i >= aModelData->myRenderResources.size() ? 0
																		   : i]
							.myVertexShader,
...
void KE_EDITOR::ImGuiHandler::DisplayTexture(KE::Texture** aTexture,
											 ImVec2 aSize, KittyFlags aFlags) {
	if (PerformFlagsBegin("Texture", aFlags)) {
		bool pressed = ImGui::ImageButton(
			KE::TextureTypeStrings[(int)(*aTexture)->myMetadata.myTextureType]
				.c_str(),
			(*aTexture)->myShaderResourceView.Get(), aSize);

		if (ImGui::BeginDragDropTarget()) {
			if (const ImGuiPayload* payload =
					ImGui::AcceptDragDropPayload("texturePath")) {
				std::string texturePath = (char*)payload->Data;
				KE_LOG("Editor", "DragDrop Texture Path: %s",
					   texturePath.c_str());
				KE::Texture* tex =
					editor->myTextureLoader->GetTextureFromPath(texturePath);
				*aTexture = tex;
			}
			ImGui::EndDragDropTarget();
		}
		...
		if (ImGui::BeginCombo("##texture",
							  (*aTexture)->myMetadata.myFileName.c_str())) {
		...
		if (hovered) {
			...
			ImGui::Image(
				...
				(*aTexture)->myShaderResourceView.Get(), ImVec2(256, 256));
			ImGui::Text(FormatString("%s\n(%i x %i)",
									 (*aTexture)->myMetadata.myFilePath.c_str(),
									 (*aTexture)->myMetadata.myWidth,
									 (*aTexture)->myMetadata.myHeight));
			ImGui::EndTooltip();
		}
...
void KE_EDITOR::ImGuiHandler::DisplayMaterial(KE::Material* aMaterial,
											  KittyFlags aFlags) {
	if (PerformFlagsBegin(
			FormatString("Material: %s", aMaterial->myName.c_str()),
			aFlags | KittyFlagTypes::eSeparatorAbove)) {
		for (int i = 0; i < 4; i++) {
			ImGui::PushID(i);
...
bool KE_EDITOR::ImGuiHandler::DisplayViewport(const std::string& aViewportName,
											  KE::RenderTarget* aRenderTarget,
											  int anIndex, KE::Camera* aCamera,
											  KittyFlags aFlags) {
	if (PerformFlagsBegin(aViewportName.c_str(), aFlags, nullptr,
...
void KE_EDITOR::ImGuiHandler::DisplayVFXSequence(KittyFlags aFlags) {
	if (vfxEditorActiveSequenceIndex == -1) {
		return;
	}
	KE::VFXManager& mgr = editor->myGraphics->GetVFXManager();
	KE::VFXSequence* aVFXSequence =
		mgr.GetVFXSequence(vfxEditorActiveSequenceIndex);
	if (!aVFXSequence) {
		KE_LOG("Closed VFX Sequence %i as it was null",
			   vfxEditorActiveSequenceIndex);
	...
	if (PerformFlagsBegin("VFX Sequence", aFlags, &open)) {
		...
		currentFloat += ImGui::IsKeyPressed(ImGuiKey_LeftArrow)	   ? -1.0f
						: ImGui::IsKeyPressed(ImGuiKey_RightArrow) ? 1.0f
																   : 0.0f;
		currentFloat +=
			KE_GLOBAL::deltaTime * KE::VFX_SEQUENCE_FRAME_RATE * (float)playing;
		currentFloat = KE::Wrap(currentFloat, (float)sqInterface.GetFrameMin(),
								(float)sqInterface.GetFrameMax() - 1);
		current = (int)currentFloat;

		static bool expanded = true;

		ImGui::PushItemWidth(130);
		ImGui::InputInt("Duration (VFX Frames)",
						&sqInterface.mySequence->myDuration);
		ImGui::SameLine();
		float ds = static_cast<float>(sqInterface.mySequence->myDuration) /
				   static_cast<float>(KE::VFX_SEQUENCE_FRAME_RATE);
		if (ImGui::InputFloat("Duration (Seconds)", &ds)) {
			sqInterface.mySequence->myDuration = static_cast<int>(
				ds * static_cast<float>(KE::VFX_SEQUENCE_FRAME_RATE));
		}
		...
		if (ImGui::Button("Preview Settings")) {
			ImGui::OpenPopup("vfxPreviewAttr");
		}
		if (ImGui::BeginPopup("vfxPreviewAttr")) {
			ImGui::Checkbox("Preview", &preview);
			ImGui::DragFloat3("Position", &previewTransform.GetPositionRef().x,
							  0.1f);
			ImGui::SliderInt(
				"Layer", &previewLayer,
		...
		if (ImGui::GetIO().KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_S, false)) {
			mgr.SaveVFXSequence(aVFXSequence);
		}
		ImGui::SameLine();
		if (ImGui::Button("Save")) {
			mgr.SaveVFXSequence(aVFXSequence);
		...
		// begin a table
		if (ImGui::BeginTable("VFX Sequence Table", 3,
							  ImGuiTableFlags_Resizable)) {
			ImGui::TableNextColumn();
			if (ImGui::BeginChild("Sequencer Child")) {
				const int currentCache = current;
				ImSequencer::Sequencer(&sqInterface, &current, 0 /*&expanded*/,
									   &selected, &firstTime,
			...
			ImGui::TableNextColumn();
			if (ImGui::BeginChild("Attribute Child") &&
				sqInterface.mySequence->myTimestamps.size() > selected) {
				const KE::VFXTimeStamp& selectedStamp =
					sqInterface.mySequence->myTimestamps[selected];
				ImGui::Text("Selected: %d", selected);

				switch (selectedStamp.myType) {
					case KE::VFXType::VFXMeshInstance: {
						KE::ModelData* modelData =
							sqInterface.mySequence
								->myVFXMeshes[selectedStamp.myEffectIndex]
								.GetModelData();
						DisplayModelData(modelData);
						break;
					}
					case KE::VFXType::ParticleEmitter: {
						KE::ParticleEmitter* emitter =
							&sqInterface.mySequence
								 ->myParticleEmitters[selectedStamp
														  .myEffectIndex]
								 .myEmitter;
		...
		if (preview) {
			...
			mgr.RenderInspectedVFX(
				vfxEditorActiveSequenceIndex, current, &previewTransform,
				static_cast<KE::eRenderLayers>(previewLayer));
		}
...
std::string KE_EDITOR::ImGuiHandler::BeautifyComponentName(
	const std::string& aUGLYName) {
	if (beautifiedComponentNameMap.contains(aUGLYName)) {
		return beautifiedComponentNameMap[aUGLYName];
	}
	...
	if (componentMarker == std::string::npos) {
		componentMarker = aUGLYName.length();
	}

	// get the substring between the last namespace marker and the component
	// marker, so that "KE::TransformComponent" becomes "Transform"
	std::string beautifiedName = aUGLYName.substr(
		lastNamespaceMarker + 1, componentMarker - lastNamespaceMarker - 1);
