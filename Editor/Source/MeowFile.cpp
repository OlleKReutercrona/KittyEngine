...
namespace KE_EDITOR {
	void MeowFile::Save(const std::string& aFilePath) {
		...
		json["/window/isMaximized"_json_pointer] =
			windowSettings.isWindowMaximized;
		json["/window/isFullscreen"_json_pointer] =
			windowSettings.isWindowFullscreen;
		...
		json["/console/isMaximized"_json_pointer] =
			windowSettings.isConsoleMaximized;

		json["/debugRendering/navmeshLevel"_json_pointer] =
			(char)debugRenderData.renderNavmesh;
		json["/debugRendering/skeletonLevel"_json_pointer] =
			(char)debugRenderData.renderSkeleton;
	...
	void MeowFile::Load(const std::string& aFilePath) {
		...
		// make sure we don't crash if we load a file that doesn't have these
		// values
		if (json.contains("/debugRendering/navmeshLevel"_json_pointer) ==
			false) {
			return;
		}

		debugRenderData.renderNavmesh =
			(DebugRenderLevel)json["/debugRendering/navmeshLevel"_json_pointer];
		debugRenderData.renderSkeleton = (DebugRenderLevel)
			json["/debugRendering/skeletonLevel"_json_pointer];
		...
		for (const auto& windowData : json["editorWindows"]) {
			...
			auto* window =
				KE_GLOBAL::editor->myWindowRegistry[windowName].myCreationFunc(
					{});
