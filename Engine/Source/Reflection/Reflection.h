...
namespace KE_SER {
	...
	struct Data {
		...
		template <typename T>
#ifndef KITTYENGINE_NO_EDITOR
		Data(T& aData, const std::string& aName, const std::string& aType,
			 size_t anOffset) {
	...
	struct SerializerData {
		...
		SerializerData& operator&(const ReflectionPackage<T>& aRefPKG) {
			myData.push_back(Data(aRefPKG.member, aRefPKG.varName,
								  aRefPKG.typeName, aRefPKG.offset));
			return *this;
		}
#else
	...
	public:
		...
		template <typename T>
		static void Deserialize(T& anObject,
								DeserializerData& aDeserializerData) {
			anObject.Serialize(aDeserializerData);
		}
	};

	inline SerializerData MakeSerializerData(
		const DeserializerData& aDeserializerData) {
		SerializerData serIn;
		serIn.myData = aDeserializerData.myData;
		return serIn;
	}

	inline DeserializerData MakeDeserializerData(
		const SerializerData& aSerializerData) {
...
#define PARAM(member)                                            \
	KE_SER::ReflectionPackage((member), sizeof(member), #member, \
							  typeid(member).name(),             \
							  KE_SER::offsetOf(&ReflectedType::member))
#else
