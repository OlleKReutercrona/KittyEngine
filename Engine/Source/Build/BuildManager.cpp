...
void KE::BuildManager::Build() {
	...
	if (!myBuildPath.empty()) {
		...
		// std::filesystem::recursive_directory_iterator(myBinPath))
		//{
		//	numberOfFiles++;
		// }
		...
		for (const auto& dirEntry :
			...
			if (extension._Equal(L".png") || extension._Equal(L".pdb") ||
				extension._Equal(L".fbx.dds") || extension._Equal(L".meow")) {
				ignoredFiles += dirEntry.path().relative_path().wstring();
				ignoredFiles +=
					L"\n";	// <- This is crash and I dont know why DDDDDDD':

				std::wcout << L"Skipping file: "
						   << dirEntry.path().relative_path().wstring()
						   << std::endl;
			...
			if (extension == L".exe") {
				...
				if (filename.find(L"Moggie") != std::wstring::npos ||
					filename.find(L"Hybrid") != std::wstring::npos) {
					ignoredFiles += dirEntry.path().relative_path().wstring();
					ignoredFiles += L"\n";
					...
					//  This is the .exe we want so if we have name we rename
					//  it!!
					dirPath = '\\';
					dirPath += myExeName;
				}
			...
			// std::filesystem::copy_file(dirEntry.path(), copyPath, options,
			// error);
			...
			//	NarrowStrToWideStr(error.message().c_str(),
			//(wchar_t*)outStr.c_str());

			//	std::wcout << L"FAILED TO COPY: " << ("%s", copyPath) << " ::: "
			//<< outStr << std::endl;
		...
		// Copy files
		{
			for (int i = 0; i < pathsToCopy.size(); i++) {
				...
				SetFileAttributes(file.myDirectory.c_str(),
								  GetFileAttributes(file.myDirectory.c_str()) &
				...
				std::filesystem::copy_file(file.myPath, file.myDirectory,
										   options, error);
				...
				if (error.value() > 0) {
					...
					std::wcout << L"FAILED TO COPY: "
							   << ("%s", file.myDirectory) << " ::: " << outStr
							   << std::endl;
		...
		std::cout << "\n\n\n\nSUCCESSFULLY BUILT TO '"
				  << WideStringToNarrow(myBuildPath)
				  << "'\n    Time:" << ("%.4f", timer.UpdateDeltaTime())
				  << " seconds" << std::endl;
...
#pragma warning(pop)
	...
	ShellExecute(NULL, (LPCWSTR)L"open", (LPCWSTR)myBuildPath.c_str(), NULL,
				 NULL, SW_SHOWDEFAULT);
}
...
void KE::BuildManager::ScanAndRemovePNGFromBuild() {
	for (const auto& dirEntry :
		 std::filesystem::recursive_directory_iterator(myBuildPath)) {
		if (dirEntry.path().extension() == ".png") {
			std::cout << "Removing " << dirEntry.path().filename() << " from "
					  << dirEntry << std::endl;
...
const std::wstring KE::BuildManager::BrowseFolder(
	const std::wstring& aSavedPath) {
