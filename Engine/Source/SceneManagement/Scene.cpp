...
KE::Scene::Scene(const int aSceneID, const std::string aSceneName,
				 const int aBuildIndex)
	: sceneID(aSceneID), sceneName(aSceneName), buildIndex(aBuildIndex) {
...
void KE::Scene::Init(Window* aWindow, PrefabHandler* aPFHandler) {
	...
	Camera* mainCamera = aWindow->GetGraphics().GetCameraManager().GetCamera(
		KE_MAIN_CAMERA_INDEX);
	myRaycastHandler.Init(*mainCamera, myCollisionHandler);

	if (myNavmeshPath.size() != 0) {
		bool initSuccess =
			myNavmesh.Init(&myWindow->GetGraphics(), myNavmeshPath);

		if (!initSuccess) {
			KE_ERROR("Scene failed to load Navmesh for file: %s",
					 myNavmeshPath.c_str());
			return;
		}
...
void KE::Scene::DebugDraw(KE::DebugRenderer& aDrawer) {
	...
	if ((myDrawDebugFlags & (int)SceneDrawFlags::eDrawGameObject))
		gameObjectManager.DebugDraw(aDrawer);
	KE::DebugTimeLogger::EndLogVar("Draw GOs");

	KE::DebugTimeLogger::BeginLogVar("Draw PhysX");
	if ((myDrawDebugFlags & (int)SceneDrawFlags::eDrawPhysX))
		myCollisionHandler.DebugDraw(aDrawer);
	KE::DebugTimeLogger::EndLogVar("Draw PhysX");

	KE::DebugTimeLogger::BeginLogVar("Draw Navmesh");
	if ((myDrawDebugFlags & (int)SceneDrawFlags::eDrawNavmesh))
		myNavmesh.DebugRender(&myWindow->GetGraphics());
	if ((myDrawDebugFlags & (int)SceneDrawFlags::eDrawPathfinding))
		myPathfinder.DebugRender(&myWindow->GetGraphics());
	KE::DebugTimeLogger::EndLogVar("Draw Navmesh");
}

KE::GameObject* KE::Scene::AddGameObject(const int anID,
										 const std::string& aName,
										 const Transform& aTransform,
										 const bool isStatic) {
	return gameObjectManager.CreateGameObject(anID, &aName, aTransform,
											  isStatic);
}
