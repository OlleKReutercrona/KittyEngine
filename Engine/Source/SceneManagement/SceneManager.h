...
namespace KE {
	...
	public:
		...
		void SetGameState(eSceneState aState) {
			mySceneState = aState;
		}

		inline Scene* GetStartupScene() {
			return myStartupScene;
		}
		...
		inline void SetActive(const bool aValue) {
			IsActive = aValue;
		}
	...
	private:
		...
		const int CheckSceneForBuildMatch(const std::string& aScene,
										  const int anIndex);
