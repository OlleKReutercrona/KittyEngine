...
KE::KittyMesh NavmeshBuilder::Build(std::vector<Transform>& someTransforms) {
	...
	auto& meshList =
		gfx->GetModelLoader().Load("Data/EngineAssets/NavmeshPlane.fbx");
	...
	// Create triangles from the navmesh plane and save to indices and vertices.
	{
		...
		for (auto& transform : someTransforms) {
			Vector4f vert0World =
				Vector4f(mesh->myVertices[0].x, mesh->myVertices[0].y,
						 mesh->myVertices[0].z, 1.0f) *
				transform.GetCUMatrix();
			Vector4f vert1World =
				Vector4f(mesh->myVertices[1].x, mesh->myVertices[1].y,
						 mesh->myVertices[1].z, 1.0f) *
				transform.GetCUMatrix();
			Vector4f vert2World =
				Vector4f(mesh->myVertices[2].x, mesh->myVertices[2].y,
						 mesh->myVertices[2].z, 1.0f) *
				transform.GetCUMatrix();
			Vector4f vert3World =
				Vector4f(mesh->myVertices[3].x, mesh->myVertices[3].y,
						 mesh->myVertices[3].z, 1.0f) *
				transform.GetCUMatrix();
...
#pragma region OLD
	...
	// for (int i = 0; i < vertices.size(); i++)
	//{
	//     std::cout << "obj " << i / 4 << "World vertex " << i << ": " <<
	//     vertices[i].x << ", " << vertices[i].y << std::endl;
	// }
