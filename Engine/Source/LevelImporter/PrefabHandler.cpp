...
void KE::PrefabHandler::Init(KE::SceneManager* aSceneManager) {
	...
	for (const auto& entry : std::filesystem::directory_iterator(folderPath)) {
		...
		std::string filePath =
			folderPath + '/' + entry.path().filename().string();
...
void KE::PrefabHandler::AddPrefab(const nlohmann::json& aJsonBlock) {
	...
	KE_LOG_CHANNEL("Prefabs", "Created a new prefab with name: %s",
				   prefabName.c_str());
}
...
KE::GameObject* KE::PrefabHandler::Instantiate(const std::string& aPrefab) {
	...
	KE::GameObject* gameObject =
		GOManager.CreateGameObject(GOManager.GenerateUniqueID(), &aPrefab);

	LevelImporter::AddComponents(mySceneManager->GetCurrentScene(), *gameObject,
								 myPostponedObjects[aPrefab].prefabs);
