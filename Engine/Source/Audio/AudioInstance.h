...
#define AudioCompMaxConcurrentSounds (16u)
	...
	public:
		...
		// Debug-only instantiation of a freestanding component. Strictly used
		...
		bool IsPlaying() const {
			return myVoicePairs[0].myIsPlaying;
		}

		void PlayAudioFile(float aVolScalar = 1.0f);
		void Stop(void);

		// Volume is clamped between 0 and 1. Sets the volume for this sound
		// only.
	...
	private:
		struct VoicePair : IXAudio2VoiceCallback {
			...
			inline operator bool() const {
				return mySourceVoice; /* return mySourceVoice && mySubmixVoice;
									   */
			}
			inline void Clear() {
				mySourceVoice = nullptr;
				mySubmixVoice = nullptr; /* memset(this, 0, sizeof(*this)); */
			}
			...
			virtual void OnBufferEnd(
				void* pBufferContext) noexcept override final;
