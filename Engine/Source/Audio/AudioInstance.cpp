...
void KE::AudioInstance::SetData(AudioInstanceData& aData) {
	...
	myAudioFile =
		KE_GLOBAL::audioWrapper.DoesSoundExist(mySoundFileName.c_str());
...
#pragma warning(pop)
		...
		KE_GLOBAL::audioWrapper.AddSoundFromDisk(filePath, myShouldLoop,
												 mySoundFileName.c_str());
		myAudioFile =
			KE_GLOBAL::audioWrapper.DoesSoundExist(mySoundFileName.c_str());
	}
...
void KE::AudioInstance::PlayAudioFile(float aVolScalar) {
	...
	if (nextInLine == -1) {
		KE_WARNING(
			"This audio component has reached the concurrency limit of %i, no "
			"more instances can play at the same time.",
	...
	// Set volume.
	const float channelVolumes[2] = {myVolume * aVolScalar,
									 myVolume * aVolScalar};
...
KE::AudioInstance::VoicePair::VoicePair(AudioInstance* aParent)
