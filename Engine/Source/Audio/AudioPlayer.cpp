...
void KE::Sound::PlayAudioFile(void) {
	...
	if (nextInLine == -1) {
		KE_WARNING(
			"This sound has reached the concurrency limit of %i, no more "
			"instances can play at the same time.",
			AudioCompMaxConcurrentSounds);
		return;
	}
	...
	KE_GLOBAL::audioWrapper.PlaySoundFile(newVoicePair->mySourceVoice,
										  myAudioFile);
}
...
KE::Sound::VoicePair::VoicePair(Sound* aParent)
	: mySourceVoice(nullptr), mySubmixVoice(nullptr), myKillSignal(false) {
	HRESULT hr;

	IXAudio2* IXAudioHandle = KE_GLOBAL::audioWrapper.GetIXAudioHandle();

	if (FAILED(hr = IXAudioHandle->CreateSourceVoice(
				   &mySourceVoice, (WAVEFORMATEX*)&aParent->myAudioFile->myWFX,
				   0, XAUDIO2_DEFAULT_FREQ_RATIO, this))) {
		KE_ERROR("Failed to create a source voice to play sound %s.",
				 aParent->mySoundFileName.c_str());
		return;
	}
}

void KE::Sound::VoicePair::OnBufferEnd(void* pBufferContext) noexcept {
	// Extremely important to remove ourselves from the XAudio graph, but we may
	// not do it from within a callback.
	myKillSignal = true;
	myIsPlaying = false;
}
...
void KE::AudioPlayer::Init(void) {
	...
	if (!fs::exists(audioFPath)) {
		KE_ERROR(
			"You are missing the audio folder on disk. Make sure to copy over "
			"your assets from Unity!");
		return;
	}
	...
	for (const auto& dirEntry : fs::directory_iterator(audioFPath)) {
		...
		newSound.myAudioFile = KE_GLOBAL::audioWrapper.DoesSoundExist(
			newSound.mySoundFileName.c_str());

		if (newSound.myAudioFile == nullptr) {
			KE_GLOBAL::audioWrapper.AddSoundFromDisk(
				dirEntry.path().string().c_str(), newSound.myShouldLoop,
				newSound.mySoundFileName.c_str());
			newSound.myAudioFile = KE_GLOBAL::audioWrapper.DoesSoundExist(
				newSound.mySoundFileName.c_str());
		}
