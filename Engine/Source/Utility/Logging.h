...
#define Log(level, stream, msg, ...)                                       \
	...
	fprintf(stream,                                                        \
			...
			" IN ..\\%s\nIN FUNCTION: " __FUNCSIG__ "\nAT LINE: %i.\n" msg \
													"\n" LogDivider,       \
			relFilePath, __LINE__, __VA_ARGS__);                           \
	fflush(stream);

/*
  Change the color of the text, print stuff out, then restore the color to what
  it was. The scope is there to avoid polluting the calling code with names that
  will lead to a name collision and thus a compilation error.
*/
