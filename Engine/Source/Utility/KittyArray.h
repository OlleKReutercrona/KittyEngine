...
class KitSet {
public:
	inline static size_t GetFieldSize(size_t aElementCount) {
		return sizeof(size_t) +
			   sizeof(KitField) * (aElementCount / sizeof(KitField) + 1);
	}
...
public:
	LaserPtr(KittyArray<T>* aDataSource, size_t aIndex)
		: dataSource(aDataSource), index(aIndex) {}
...
private:
	...
	void Reallocate(size_t aNewCapacity) {
		...
		memmove((char*)newBlock + newFieldSize, (char*)newBlock + oldFieldSize,
				sizeof(T) * capacity);
...
public:
	KittyArray(size_t aCapacity = 1)
		: count(0), capacity(aCapacity), backFillIndices{} {
		Allocate(aCapacity);
	}
