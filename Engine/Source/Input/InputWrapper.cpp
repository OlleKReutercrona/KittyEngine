...
namespace KE {
	InputWrapper::InputWrapper() {
		...
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxA, XButtons.A));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxB, XButtons.B));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxX, XButtons.X));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxY, XButtons.Y));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxDPadUp, XButtons.DPadUp));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxDPadDown, XButtons.DPadDown));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxDPadLeft, XButtons.DPadLeft));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxDPadRight, XButtons.DPadRight));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxLShoulder, XButtons.LShoulder));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxRShoulder, XButtons.RShoulder));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxLThumbstick, XButtons.LThumbstick));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxRThumbstick, XButtons.RThumbstick));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxStart, XButtons.Start));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxBack, XButtons.Back));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxLeftTrigger, XButtons.LTrigger));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxRightTrigger, XButtons.RTrigger));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxLeftStick, XButtons.LStick));
		myInputEvents.push_back(
			ES::EventSystem::GetInstance().CreateNewEvent<InputEvent>(
				eInputType::XboxRightStick, XButtons.RStick));
	}
	...
	void InputWrapper::Update() {
		...
		for (InputEvent* action : myInputEvents) {
			...
			if (action->myKeyBindings.size() > 0) {
				for (int i = 0; i < action->myKeyBindings.size(); i++) {
					...
					if (PressedKeyThisFrame(tempKeys, key)) {
						...
						action->myMousePosition =
							myInputHandler.GetMousePosition();

						action->interactions.push_back(
							{eInteractionType::Pressed, key, GetKeyDesc(key)});
						action->interactions.back().myInputType =
							action->myInputType;
						action->interactions.back().myControllerID =
							GetKeyboardID(key);
						...
						action->interactions.push_back(
							{eInteractionType::Released, key, GetKeyDesc(key)});
						action->interactions.back().myInputType =
							action->myInputType;
						action->interactions.back().myControllerID =
							GetKeyboardID(key);
						...
						action->interactions.push_back(
							{eInteractionType::Held, key, GetKeyDesc(key)});
						action->interactions.back().myInputType =
							action->myInputType;
						action->interactions.back().myControllerID =
							GetKeyboardID(key);
			...
			// Gamepad
				...
				for (int i = 0; i < 4; ++i) {
					if (myGamepads[i].GetIsConnected()) {
						if (myGamepads[i].IsButtonPressed(
								action->myXboxButton)) {
							action->myInteractionType =
							...
							action->interactions.push_back(
								{eInteractionType::Pressed,
								 action->myXboxButton,
								 GetButtonName(
									 (eButtons)action->myXboxButton)});
							action->interactions.back().myInputType =
								action->myInputType;
							action->interactions.back().myControllerID =
								myGamepads[i].GetIndex();

							shouldQueue = true;
						} else if (myGamepads[i].IsButtonReleased(
									   action->myXboxButton)) {
							action->myInteractionType =
								eInteractionType::Released;
							action->myXboxButton = action->myXboxButton;
							action->interactions.push_back(
								{eInteractionType::Released,
								 action->myXboxButton,
							...
							shouldQueue = true;
						} else if (myGamepads[i].IsButtonHeld(
									   action->myXboxButton)) {
							action->myInteractionType = eInteractionType::Held;
							action->myXboxButton = action->myXboxButton;
							action->interactions.push_back(
								{eInteractionType::Held, action->myXboxButton,
								 GetButtonName(
									 (eButtons)action->myXboxButton)});
							action->interactions.back().myInputType =
								action->myInputType;
							...
							shouldQueue = true;
						} else if (action->myInputType ==
								   eInputType::XboxLeftStick) {
							if (myGamepads[i].IsLeftStickToggled()) {
								action->myInteractionType =
									eInteractionType::Pressed;
								action->myXboxButton = XButtons.LStick;
								action->interactions.push_back(
									{eInteractionType::Pressed, XButtons.LStick,
									 GetButtonName(
										 (eButtons)action->myXboxButton)});
								action->interactions.back().myStick = {
									myGamepads[i].GetLeftStickX(),
									myGamepads[i].GetLeftStickY()};
								action->interactions.back().myControllerID =
									myGamepads[i].GetIndex();
								action->interactions.back().myInputType =
									action->myInputType;
								...
								} else if (myGamepads[i]
											   .IsLeftStickToggledDown()) {
									action->interactions.back().myInputType =
										eInputType::XboxLeftTriggerToggledDown;
								} else if (myGamepads[i]
											   .IsLeftStickToggledLeft()) {
									action->interactions.back().myInputType =
										eInputType::XboxLeftTriggerToggledLeft;
								} else if (myGamepads[i]
											   .IsLeftStickToggledRight()) {
									action->interactions.back().myInputType =
							...
							if (!myGamepads[i].LStickInDeadzone()) {
								action->myInteractionType =
									eInteractionType::Held;
								action->myXboxButton = XButtons.LStick;
								action->interactions.push_back(
									{eInteractionType::Held, XButtons.LStick,
									 GetButtonName(
										 (eButtons)action->myXboxButton)});
								action->interactions.back().myStick = {
									myGamepads[i].GetLeftStickX(),
								...
								shouldQueue = true;
								   ...
								   eInputType::XboxRightStick) {
							if (!myGamepads[i].RStickInDeadzone()) {
								action->myInteractionType =
									eInteractionType::Held;
								action->myXboxButton = XButtons.RStick;
								action->interactions.push_back(
									{eInteractionType::Held, XButtons.RStick,
									 GetButtonName(
										 (eButtons)action->myXboxButton)});
								action->interactions.back().myStick = {
									myGamepads[i].GetRightStickX(),
									myGamepads[i].GetRightStickY()};
								action->interactions.back().myControllerID =
									myGamepads[i].GetIndex();
								action->interactions.back().myInputType =
									action->myInputType;
								...
								shouldQueue = true;
								   ...
								   eInputType::XboxLeftTrigger) {
							if (myGamepads[i].GetLeftTrigger() > 0.0f) {
								action->myInteractionType =
									eInteractionType::Held;
								action->myXboxButton = XButtons.LTrigger;
								action->interactions.push_back(
									{eInteractionType::Held, XButtons.LTrigger,
									 GetButtonName(
										 (eButtons)action->myXboxButton)});
								action->interactions.back().myTrigger =
									myGamepads[i].GetLeftTrigger();
								action->interactions.back().myControllerID =
									myGamepads[i].GetIndex();
								action->interactions.back().myInputType =
									action->myInputType;
								...
								shouldQueue = true;
								   ...
								   eInputType::XboxRightTrigger) {
							if (myGamepads[i].GetRightTrigger() > 0.0f) {
								action->myInteractionType =
									eInteractionType::Held;
								action->myXboxButton = XButtons.RTrigger;
								action->interactions.push_back(
									{eInteractionType::Held, XButtons.RTrigger,
									 GetButtonName(
										 (eButtons)action->myXboxButton)});
								action->interactions.back().myTrigger =
									myGamepads[i].GetRightTrigger();
								action->interactions.back().myControllerID =
									myGamepads[i].GetIndex();
								action->interactions.back().myInputType =
									action->myInputType;
	...
	bool InputWrapper::UpdateEvents(const UINT aMsg, const WPARAM aWParam,
	...
	bool InputWrapper::PressedKeyThisFrame(
		const std::vector<Key>& aMouseAndKeyboardKeys, Key& aOutKey) const {
		bool pressedKey = false;
	...
	bool InputWrapper::ReleasedKeyThisFrame(
		const std::vector<Key>& aMouseAndKeyboardKeys, Key& aOutKey) const {
	...
	bool InputWrapper::HeldKeyThisFrame(
		const std::vector<Key>& aMouseAndKeyboardKeys, Key& aOutKey) const {
		bool heldKey = false;
	...
	int InputWrapper::GetKeyboardID(Key& aKey) const {
		if (aKey == 'W' || aKey == 'A' || aKey == 'S' || aKey == 'D' ||
			aKey == VK_SPACE || aKey == 'J' || aKey == 'K' || aKey == 'L') {
			return 4;
		} else if (aKey == VK_UP || aKey == VK_DOWN || aKey == VK_LEFT ||
				   aKey == VK_RIGHT || aKey == VK_NUMPAD0 ||
				   aKey == VK_NUMPAD1 || aKey == VK_NUMPAD2 ||
				   aKey == VK_NUMPAD3) {
			return 5;
		}
