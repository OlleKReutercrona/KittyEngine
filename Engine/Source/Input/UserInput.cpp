	...
	void UserInput::OnReceiveEvent(ES::Event& aEvent) {
		...
		if (inputActionEvent != nullptr) {
			if (isMouseOverGUI) {
				...
				// inputActionEvent->myTriggerKeyName;
		...
		const MouseMoveEvent* mouseMoveEvent =
			dynamic_cast<MouseMoveEvent*>(&aEvent);
