...
namespace KE {
	...
	bool InputHandler::UpdateEvents(const UINT aMsg, const WPARAM aWParam,
			...
			case WM_INPUT: {
				{
					...
					if (GetRawInputData(reinterpret_cast<HRAWINPUT>(aLParam),
					...
					if (GetRawInputData(reinterpret_cast<HRAWINPUT>(aLParam),
										RID_INPUT, myRawBuffer.data(), &size,
					...
					auto& rawInput =
						reinterpret_cast<const RAWINPUT&>(*myRawBuffer.data());
					if (rawInput.header.dwType == RIM_TYPEMOUSE &&
						(rawInput.data.mouse.lLastX != 0 ||
						 rawInput.data.mouse.lLastY != 0)) {
						OnRawDelta(rawInput.data.mouse.lLastX,
	...
	bool InputHandler::IsMouseMoving() const {
		return myPreviousMousePos.x != myCurrentMousePos.x ||
	...
	void InputHandler::UpdateMousePosition() {
		...
		myMouseDelta = {myLiveMousePos.x - myPreviousMousePos.x,
						myLiveMousePos.y - myPreviousMousePos.y};
	}
