...
namespace KE {
	...
	void VFXManager::RenderInspectedVFX(int aVFXIndex, int aCurrentFrame,
										Transform* aTransform,
										eRenderLayers aLayer) {
	...
	void VFXManager::Init(Graphics* aGraphics) {
		...
		myVFXPostProcessing.Init(
			aGraphics->GetDevice().Get(), aGraphics,
		...
		*myVFXPostProcessing.GetPreProcessPS() =
			aGraphics->GetShaderLoader().GetPixelShader(
				SHADER_LOAD_PATH "BloomPreProcess_PS.cso");

		myVFXPostProcessing.SetPSShader(
			aGraphics->GetShaderLoader().GetPixelShader(
				SHADER_LOAD_PATH "PostProcessing_VFX_PS.cso"));
		myVFXPostProcessing.SetVSShader(
			aGraphics->GetShaderLoader().GetVertexShader(
				SHADER_LOAD_PATH "PostProcessing_VFX_VS.cso"));

		myVFXPostProcessing.ConfigureDownSampleRTs(
			aGraphics, aGraphics->GetRenderWidth(),
	...
	void VFXManager::RenderVFX(fxBuffer& fxb, ModelData* modelData) {
		...
		myGraphics->GetContext()->VSSetConstantBuffers(
			6u, 1u, pConstantBuffer.GetAddressOf());
		myGraphics->GetContext()->PSSetConstantBuffers(
			6u, 1u, pConstantBuffer.GetAddressOf());

		myGraphics->SetRasterizerState(KE::eRasterizerStates::FrontfaceCulling);
		myVFXRenderer.RenderModel(
			{nullptr, myGraphics->GetView(), myGraphics->GetProjection(), 0, 0},
			*modelData);
	...
	// Manager
	void VFXManager::Render(eRenderLayers aLayer,
		...
		for (auto& playedVFX : myRenderQueue) {
			for (auto& emitter : playedVFX.myEmitters) {
				...
				mySpriteManager->BindBuffers(
					*batch,
					myGraphics->GetCameraManager().GetHighlightedCamera());
				mySpriteManager->RenderBatch(*batch);
			}
		}

		for (VFXSequenceRenderPackage& package : myRenderPackages) {
			if (package.layer != aLayer) {
				continue;
			}
			...
			if (package.customBuffer.constantBuffer != nullptr) {
				...
				buf.constantBuffer->MapBuffer(buf.bufferData, buf.bufferSize,
											  myGraphics->GetContext().Get());
				buf.constantBuffer->BindForPS(buf.bufferSlot,
											  myGraphics->GetContext().Get());
			}
	...
	void VFXManager::Update(float aDeltaTime) {
		for (int i = 0; i < myRenderQueue.size(); i++) {
			...
			if (playerData.myTimer >
				myVFXSequences[playerData.mySequenceIndex].myDuration) {
			...
			for (auto& emitter : playerData.myEmitters) {
				emitter.myEmitter.Update(
					*playerData.myRenderInput.myTransform,
					playerData.myFrame >= emitter.myStartFrame &&
						playerData.myFrame <= emitter.myEndFrame);
			}
		}
		...
		// unfortunately we need to sort the render packages by distance to
		// camera
		Vector3f cameraPos =
			myGraphics->GetCameraManager()
				.GetHighlightedCamera()
				->transform.GetPosition();	// not great, might break
		std::sort(
			myRenderPackages.begin(), myRenderPackages.end(),
			[&cameraPos](const VFXSequenceRenderPackage& a,
						 const VFXSequenceRenderPackage& b) {
				float distA =
					(a.instanceTransform.GetPosition() - cameraPos).LengthSqr();
				float distB =
					(b.instanceTransform.GetPosition() - cameraPos).LengthSqr();
				return distA > distB;
			});
	}
	...
	void VFXManager::PrepareRenderData(VFXSequencePlayerData& aPlayerData) {
		...
		for (int ts = 0; ts < sq.myTimestamps.size(); ts++) {
			...
			if (vfxTS.myStartpoint > aPlayerData.myFrame ||
				vfxTS.myEndpoint < aPlayerData.myFrame) {
				continue;
			}
			if (vfxTS.myType == VFXType::VFXMeshInstance) {
				VFXSequenceRenderPackage& renderPackage =
					myRenderPackages.emplace_back();

				renderPackage.layer = aPlayerData.myLayer;
				renderPackage.modelData =
					sq.myVFXMeshes[vfxTS.myEffectIndex].GetModelData();

				renderPackage.bloom = aPlayerData.myRenderInput.bloom;

				renderPackage.instanceTransform =
					aPlayerData.myRenderInput.GetTransform() *
					sq.myVFXMeshes[vfxTS.myEffectIndex].myTransform;

				for (auto& modifier : vfxTS.myCurveDataSets) {
					if (!modifier.IsValid()) {
						continue;
					}
					const float mod = modifier.GetEvaluatedValue(
						aPlayerData.myFrame, vfxTS.myStartpoint,
						vfxTS.myEndpoint);
				...
				renderPackage.instanceTransform.TranslateLocal(
					renderPackage.attributes.translation);

				DirectX::XMMATRIX rotationMatrix =
					DirectX::XMMatrixRotationRollPitchYaw(rot.x, rot.y, rot.z);
				renderPackage.instanceTransform =
					rotationMatrix *
					renderPackage.instanceTransform.GetMatrix();
				...
				renderPackage.customBuffer =
					aPlayerData.myRenderInput.customBufferInput;
			} else if (vfxTS.myType == VFXType::ParticleEmitter) {
			}
		}
	}

	void VFXManager::InitializeVFXMesh(
		VFXMeshInstance& aMeshToInitialize) const {
		ModelData& modelData = *aMeshToInitialize.GetModelData();
		modelData.myTransform = &aMeshToInitialize.GetTransform()->GetMatrix();
		modelData.myMeshList = &myGraphics->GetModelLoader().Load(
			"Data/InternalAssets/cylinder.fbx");
		auto& rr = modelData.myRenderResources.emplace_back();
		rr.myMaterial =
			myGraphics->GetTextureLoader()
				.GetDefaultMaterial();	// TODO: FIX?? idk if i want to solve it
										// from elsewhere though, like being
										// able to replace a "normal" material
										// with a custom one from the editor
										// side
		rr.myPixelShader = myGraphics->GetShaderLoader().GetPixelShader(
			SHADER_LOAD_PATH "Model_VFX_PS.cso");
		rr.myVertexShader = myGraphics->GetShaderLoader().GetVertexShader(
			SHADER_LOAD_PATH "Model_VFX_VS.cso");
	}

	void VFXManager::InitializeParticleEmitter(
		ParticleEmitter& anEmitterToInitialize) const {
		anEmitterToInitialize.Init(myGraphics, 1024,
								   "Data/InternalAssets/coolest.png");	// temp!
	}

	void VFXManager::TriggerVFXSequence(int aVFXSequenceIndex,
										const VFXRenderInput& aRenderInput) {
		VFXSequencePlayerData& sqData =
			myRenderQueue.emplace_back(aRenderInput);
		...
		for (int i = 0;
			 i < myVFXSequences[aVFXSequenceIndex].myTimestamps.size(); ++i) {
			sqData.myTimestampTriggerInfo.push_back(false);
		}

		for (int i = 0;
			 i < myVFXSequences[aVFXSequenceIndex].myParticleEmitters.size();
			 ++i) {
			sqData.myEmitters.push_back(
				myVFXSequences[aVFXSequenceIndex].myParticleEmitters[i]);
		}
	}

	void VFXManager::StopVFXSequence(int aVFXSequenceIndex,
									 const VFXRenderInput& aRenderInput) {
		// this is stupid but fuck it, we look for a playerData with the same
		// index and transform pointer
		for (int i = 0; i < myRenderQueue.size(); i++) {
			if (myRenderQueue[i].mySequenceIndex == aVFXSequenceIndex &&
				myRenderQueue[i].myRenderInput.myTransform ==
					aRenderInput.myTransform) {
				myRenderQueue.erase(myRenderQueue.begin() + i);
				i--;
			}
		}
	...
	void VFXManager::SaveVFXSequence(VFXSequence* aSequence) {
		...
		for (auto& VFXMesh : sq.myVFXMeshes) {
			...
			mesh["albedo"] = VFXMesh.myModelData.myRenderResources[0]
								 .myMaterial->myTextures[0]
								 ->myMetadata.myFilePath;
			mesh["normal"] = VFXMesh.myModelData.myRenderResources[0]
								 .myMaterial->myTextures[1]
								 ->myMetadata.myFilePath;
			mesh["material"] = VFXMesh.myModelData.myRenderResources[0]
								   .myMaterial->myTextures[2]
								   ->myMetadata.myFilePath;
			mesh["effects"] = VFXMesh.myModelData.myRenderResources[0]
								  .myMaterial->myTextures[3]
								  ->myMetadata.myFilePath;

			mesh["vertexShader"] = VFXMesh.myModelData.myRenderResources[0]
									   .myVertexShader->GetName();
			mesh["pixelShader"] = VFXMesh.myModelData.myRenderResources[0]
									  .myPixelShader->GetName();
		...
		for (auto& emitter : sq.myParticleEmitters) {
			nlohmann::json em;
			em["particleCapacity"] =
				emitter.myEmitter.GetSpriteBatch()->myInstances.size();
			em["particleTexture"] =
				emitter.myEmitter.GetSpriteBatch()
					->myData.myTexture->myMetadata.myFilePath;
			em["particleMode"] =
				(int)emitter.myEmitter.GetSpriteBatch()->myData.myMode;
			...
			spa["startColor"] = {attr.startColor.x, attr.startColor.y,
								 attr.startColor.z, attr.startColor.w};
			spa["midColor"] = {attr.midColor.x, attr.midColor.y,
							   attr.midColor.z, attr.midColor.w};
			spa["endColor"] = {attr.endColor.x, attr.endColor.y,
							   attr.endColor.z, attr.endColor.w};
		...
		if (!file.is_open()) {
			KE_ERROR("Failed to save sequence %s (%i)", sq.myName.c_str(),
					 sq.myIndex);
			int i = 0;
			for (i = 0; i < 128 && !file.is_open(); i++) {
				out = VFX_SEQUENCE_FILE_LOCATION + sq.myName + "backupSave_" +
					  std::to_string(i) + ".kittyVFX";
				file.open(out);
			}
		}
	...
	void VFXManager::LoadVFXSequence(int aVFXSequenceIndex,
		...
		std::string fileToLoad =
			VFX_SEQUENCE_FILE_LOCATION + aFilePath + ".kittyVFX";
		std::ifstream file(fileToLoad);

		if (!file.is_open()) {
			// make a copy of the default file
			std::filesystem::copy(
				"Data/InternalAssets/VFXSequences/default.kittyVFX",
				fileToLoad);
			file.open(fileToLoad);

			if (!file.is_open()) {
				assert(false &&
					   "default.kittyVFX not found! attempted to load %s");
			}
		}
		...
		for (auto& mesh : input["meshes"]) {
			...
			meshData.myModelData.myMeshList =
				&myGraphics->GetModelLoader().Load(mesh["name"]);
			meshData.myModelData.myRenderResources.emplace_back();
			meshData.myModelData.myRenderResources[0].myMaterial =
				myGraphics->GetTextureLoader().GetCustomMaterial(
					mesh["albedo"], mesh["normal"], mesh["material"],
					mesh["effects"]);

			meshData.myModelData.myRenderResources[0].myVertexShader =
				myGraphics->GetShaderLoader().GetVertexShader(
					mesh["vertexShader"]);
			meshData.myModelData.myRenderResources[0].myPixelShader =
				myGraphics->GetShaderLoader().GetPixelShader(
					mesh["pixelShader"]);
			meshData.myModelData.myTransform =
				&meshData.myTransform.GetMatrix();
		}
		...
		for (auto& emitter : input["particleEmitters"]) {
			...
			em.Init(myGraphics, emitter["particleCapacity"],
					emitter["particleTexture"]);

			em.GetSpriteBatch()->myData.myMode =
				(SpriteBatchMode)emitter["particleMode"];
			...
			attr.startColor =
				Vector4f(spa["startColor"][0], spa["startColor"][1],
						 spa["startColor"][2], spa["startColor"][3]);
			attr.midColor = Vector4f(spa["midColor"][0], spa["midColor"][1],
									 spa["midColor"][2], spa["midColor"][3]);
			attr.endColor = Vector4f(spa["endColor"][0], spa["endColor"][1],
									 spa["endColor"][2], spa["endColor"][3]);
		...
		for (auto& timestamp : input["timestamps"]) {
			...
			if (ts.myType == VFXType::ParticleEmitter) {
				sq.myParticleEmitters[ts.myEffectIndex].myStartFrame =
					ts.myStartpoint;
				sq.myParticleEmitters[ts.myEffectIndex].myEndFrame =
					ts.myEndpoint;
			}

			for (auto& curve : timestamp["curves"]) {
				int index = (int)curve["curveAttribute"];
				if (index >= (int)VFXAttributeTypes::Count) {
					continue;
				}
