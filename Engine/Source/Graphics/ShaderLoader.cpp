...
#define SHADER_RECOMPILATION_SLEEP \
	5  // milliseconds to wait between a file change and recompilation
...
public:
	...
	(D3D_INCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData,
	 LPCVOID* ppData, UINT* pBytes) override {
		...
		// If the file was not found in the default directory, try other
		// directories within "Shaders"
		for (const auto& directoryEntry : fs::directory_iterator(shaderPath)) {
			if (directoryEntry.is_directory()) {
				std::string additionalDirectory =
					directoryEntry.path().string() + "/";
				if (fs::exists(additionalDirectory + fileName)) {
					// Read the contents of the file
					std::ifstream additionalFile(additionalDirectory +
												 fileName);
					std::string fileContents(
						(std::istreambuf_iterator<char>(additionalFile)),
						std::istreambuf_iterator<char>());
...
namespace KE {
	...
	VertexShader* ShaderLoader::CreateVertexShaderFromParsedScript(
		const std::string& aShaderName, const std::string& aParsedCode) {
		std::ofstream file;
		file.open("_VertexShaderStaging.txt");
		file << "#include "
				"\"Data/EngineAssets/LanguageDefinitions/hlslFunctions.hlsli\" "
				"\n";
		file << aParsedCode;
		file.close();
		...
		D3D11_INPUT_ELEMENT_DESC instancedLayout[] = {
			...
			{"INSTANCE_TRANSFORM", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1,
			 D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_INSTANCE_DATA, 1},
			{"INSTANCE_TRANSFORM", 1, DXGI_FORMAT_R32G32B32A32_FLOAT, 1,
			 D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_INSTANCE_DATA, 1},
			{"INSTANCE_TRANSFORM", 2, DXGI_FORMAT_R32G32B32A32_FLOAT, 1,
			 D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_INSTANCE_DATA, 1},
			{"INSTANCE_TRANSFORM", 3, DXGI_FORMAT_R32G32B32A32_FLOAT, 1,
			 D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_INSTANCE_DATA, 1},
			{"INSTANCE_ATTRIBUTES", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1,
			 D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_INSTANCE_DATA, 1},
		};
		...
		hr = myDevice->CreateVertexShader(compiledShader->GetBufferPointer(),
		...
		hr = myDevice->CreateInputLayout(
			instancedLayout, (UINT)std::size(instancedLayout),
	...
	VertexShader* ShaderLoader::CreateVertexShaderFromScript(
		const std::string& aShaderName, Script* aShaderScript) {
		auto* res = new ParsingOutput{aShaderScript};
		auto& result = *res;
		CodeScriptParser::Parse(aShaderScript, res);

		VertexShader* vs =
			CreateVertexShaderFromParsedScript(aShaderName, result.code);

		delete res;
		return vs;
	}

	PixelShader* ShaderLoader::CreatePixelShaderFromParsedScript(
		const std::string& aShaderName, const std::string& aParsedCode,
		const std::string& aEntryName) {
		Timer compileTimer;
		myPixelShaders[aShaderName] = PixelShader();
		myPixelShaders[aShaderName]
			.myPixelShaderSlots[0] = CreatePixelShaderFromCode(
			std::format(
				"{}\n{}\n{}\n{}",
				"#include "
				"\"Data/EngineAssets/LanguageDefinitions/hlslFunctions.hlsli\"",
				"#include "
				"\"Data/EngineAssets/LanguageDefinitions/hlslMath.hlsli\"",
				"#include "
				"\"Data/EngineAssets/LanguageDefinitions/hlslNoise.hlsli\"",
				aParsedCode),
	...
	PixelShader* ShaderLoader::CreatePixelShaderFromScript(
		const std::string& aShaderName, Script* aShaderScript,
		const std::string& aEntryName) {
		auto* res = new ParsingOutput{aShaderScript};
		...
		PixelShader* ps = CreatePixelShaderFromParsedScript(
			aShaderName, result.code, aEntryName);
		delete res;
		return ps;
	}
	...
	PixelShader* ShaderLoader::GetPixelShader(std::string aFilePath) {
		if (myPixelShaders.find(aFilePath) == myPixelShaders.end()) {
			...
			if (aFilePath.find("ScriptPS_") != std::string::npos) {
				const std::string scriptName = aFilePath.substr(
					aFilePath.find("ScriptPS_") + sizeof("ScriptPS_") - 1);

				auto* scriptMgr =
					KE_GLOBAL::blackboard.Get<ScriptManager>("scriptManager");
				CreatePixelShaderFromScript(
					aFilePath, scriptMgr->GetOrLoadScript(scriptName), "main");
	...
	ID3D11VertexShader* ShaderLoader::CreateVertexShaderFromFile(
	...
	ID3D11PixelShader* ShaderLoader::CreatePixelShaderFromFile(
		const wchar_t* aFilePath)  //, const char* anEntryPoint)
	{
		// Compile the shader from file
		ID3DBlob* compiledShader = nullptr;
		ID3DBlob* errorBlob = nullptr;
		...
		hr = myDevice->CreatePixelShader(compiledShader->GetBufferPointer(),
										 compiledShader->GetBufferSize(),
										 nullptr, &pixelShader);
	...
	ID3D11ComputeShader* ShaderLoader::CreateComputeShaderFromFile(
		const wchar_t* aFilePath) {
		// Compile the shader from file
		ID3DBlob* compiledShader = nullptr;
		ID3DBlob* errorBlob = nullptr;
		...
		hr = myDevice->CreateComputeShader(compiledShader->GetBufferPointer(),
										   compiledShader->GetBufferSize(),
	...
	ID3D11PixelShader* ShaderLoader::CreatePixelShaderFromCode(
		const std::string& aShaderCode, const std::string& aEntryName) {
		// Compile the shader from file
		ID3DBlob* compiledShader = nullptr;
		ID3DBlob* errorBlob = nullptr;
		...
		HRESULT hr = D3DCompile(
			aShaderCode.c_str(), aShaderCode.size(), nullptr, nullptr,
			D3D_COMPILE_STANDARD_FILE_INCLUDE, aEntryName.c_str(), "ps_5_0", 0,
			0, &compiledShader, &errorBlob);
	...
	bool ShaderLoader::RecompileShader(VertexShader* aVertexShader) {
		...
		for (const auto& entry :
			 std::filesystem::recursive_directory_iterator(HLSL_PATH)) {
		...
		ID3D11VertexShader* vertexShader =
			CreateVertexShaderFromFile(toWstr(filePath).c_str());
	...
	bool ShaderLoader::RecompileShader(PixelShader* aPixelShader) {
		...
		// recursively iterate through the HLSL_PATH folder to find the correct
		// file path
		for (const auto& entry :
			 std::filesystem::recursive_directory_iterator(HLSL_PATH)) {
			if (entry.path().filename().string() == fileName) {
		...
		ID3D11PixelShader* pixelShader =
			CreatePixelShaderFromFile(toWstr(filePath).c_str());
	...
	bool ShaderLoader::RecompileShader(ComputeShader* aComputeShader) {
		...
		ID3D11ComputeShader* computeShader =
			CreateComputeShaderFromFile(toWstr(filePath).c_str());
