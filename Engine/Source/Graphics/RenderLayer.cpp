...
void KE::RenderLayer::SetActive() {
	// myGBuffer.SetAsActiveTarget(myGraphics->GetContext().Get(),
	// myGBuffer.GetDepthStencilView());
}

void KE::RenderLayer::Render(KE::Camera* aCamera, KE::VertexShader* aVSOverride,
							 KE::PixelShader* aPSOverride) {
	if (!(flags & RenderLayerFlags_Active)) {
		return;
	}
...
void KE::RenderLayer::RenderDecals(Camera* aCamera, GBuffer* aMainGBuffer,
								   GBuffer* aCopyGBuffer,
								   DecalManager* aDecalManager) {
	if (!(flags & RenderLayerFlags_Active)) {
		return;
	}
	...
	aDecalManager->PrepareDecalRendering(myGraphics, aMainGBuffer,
										 aCopyGBuffer);

	aMainGBuffer->SetAsActiveTarget(context,
									aMainGBuffer->GetDepthStencilView());
...
void KE::RenderLayer::GenerateInstancingData(
	const ModelDataList& aModelDataList) {
	myInstancedRenderPackages.clear();
	myInstancedRenderer->GenerateInstancingData(
		myModelDataIndices, aModelDataList, &myInstancedRenderPackages);
