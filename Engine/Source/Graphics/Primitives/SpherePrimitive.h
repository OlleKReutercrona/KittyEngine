...
namespace KE {
	...
	public:
		...
		inline static const std::vector<VertexPoint>& GetVertices(
			...
			return mySphereData[anAmount].myVertices;
		};
		inline static const std::vector<unsigned int>& GetIndices(
			KE::SphereLOD anAmount = KE::SphereLOD::lowPoly) {
			return mySphereData[anAmount].myIndices;
		};

	private:
		void ContructSphere(KE::SphereLOD aLOD, const int aLat,
							const int aLong);
		void ContructSphere2(KE::SphereLOD aLOD, const int someStacks,
		...
		inline static std::unordered_map<KE::SphereLOD, KE::SphereData>
			mySphereData;
	};
}  // namespace KE
