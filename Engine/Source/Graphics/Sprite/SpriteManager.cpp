...
namespace KE {
	...
	void SpriteManager::Init(Graphics* aGraphics) {
		...
		SpriteVertex vertices[4] = {{-1.0f, -1.0f, 0.0f, 1.0f},
									{-1.0f, 1.0f, 0.0f, 1.0f},
									{1.0f, -1.0f, 0.0f, 1.0f},
									{1.0f, 1.0f, 0.0f, 1.0f}};
		...
		HRESULT result = device->CreateBuffer(&bufferDesc, &subresourceData,
											  &myVertexBuffer);
		if (FAILED(result)) {
		...
		result = device->CreateBuffer(&instanceBufferDesc, nullptr,
									  &myInstanceBuffer);
		if (FAILED(result)) {
		...
		result = device->CreateBuffer(&spriteRenderBufferDesc, nullptr,
									  &mySpriteRenderBuffer);
		if (FAILED(result)) {
		...
		result =
			device->CreateBuffer(&textRenderBufferDesc, nullptr, &myTextBuffer);
		if (FAILED(result)) {
		...
		result =
			device->CreateBuffer(&bufferDesc, &subresourceData, &myIndexBuffer);
		if (FAILED(result)) {
	...
	void SpriteManager::BindBuffers(SpriteBatch& aSpriteBatch,
									KE::Camera* aCamera) {
		// set topology
		...
		unsigned int instanceCount =
			(unsigned int)aSpriteBatch.myInstances.size();
		instanceCount;
		...
		switch (aSpriteBatch.myData.myMode) {
			case SpriteBatchMode::Default: {
				spriteRenderBuffer.completeTransform =
					myGraphics->myView * myGraphics->myProjection;
				break;
			}
			case SpriteBatchMode::Billboard: {
				spriteRenderBuffer.completeTransform =
					aCamera->transform.GetMatrix();
				break;
			}
			case SpriteBatchMode::Screen: {
				...
				spriteRenderBuffer.completeTransform *=
					DirectX::XMMatrixScaling(
						...
						1.0f / myGraphics->GetRenderHeight(), 1.0f);

				break;
			}
			case SpriteBatchMode::ScreenText: {
				const float rWidth = (float)myGraphics->GetRenderWidth(),
							rHeight = (float)myGraphics->GetRenderHeight();
				Camera camera;
				camera.transform.SetPosition(
					{rWidth / 2.0f, rHeight / 2.0f, 0.0f});
				...
				textStylingBufferData.textColour =
					aSpriteBatch.myTextStyling.text.colour;
				textStylingBufferData.strokeColour =
					aSpriteBatch.myTextStyling.stroke.colour;
				textStylingBufferData.strokeSize =
					aSpriteBatch.myTextStyling.stroke.width;
				textStylingBufferData.strokeSoftness =
					aSpriteBatch.myTextStyling.stroke.softness;

				D3D11_MAPPED_SUBRESOURCE textStylingMappedResource = {};
				context->Map(myTextBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
							 &textStylingMappedResource);
				memcpy(textStylingMappedResource.pData, &textStylingBufferData,
					   sizeof(TextStylingBufferData));
				context->Unmap(myTextBuffer, 0);

				myGraphics->GetContext()->PSSetConstantBuffers(8, 1,
															   &myTextBuffer);
				myGraphics->GetContext()->VSSetConstantBuffers(8, 1,
															   &myTextBuffer);
		...
		context->Map(mySpriteRenderBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
					 &spriteRenderMappedResource);
		memcpy(spriteRenderMappedResource.pData, &spriteRenderBuffer,
			   sizeof(SpriteRenderBuffer));
		context->Unmap(mySpriteRenderBuffer, 0);

		// TODO! define slot somewhere else :3
		myGraphics->GetContext()->PSSetConstantBuffers(3, 1,
													   &mySpriteRenderBuffer);
		myGraphics->GetContext()->VSSetConstantBuffers(3, 1,
													   &mySpriteRenderBuffer);

		// now, actually make my instancebuffer data the data from the
		// spritebatch.

		D3D11_MAPPED_SUBRESOURCE mappedResource = {};
		context->Map(myInstanceBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
					 &mappedResource);
		memcpy(mappedResource.pData, aSpriteBatch.myInstances.data(),
			   sizeof(Sprite) * instanceCount);
		context->Unmap(myInstanceBuffer, 0);

		myGraphics->GetContext()->IASetVertexBuffers(0, 2, bufferPointers,
													 strides, offsets);
		myGraphics->GetContext()->IASetIndexBuffer(myIndexBuffer,
												   DXGI_FORMAT_R32_UINT, 0);

		myGraphics->GetContext()->IASetInputLayout(
			myGraphics->GetShaderLoader()
				.GetVertexShader(SHADER_LOAD_PATH "Sprite_VS.cso")
				->GetInputLayout());
		myGraphics->GetContext()->VSSetShader(
			myGraphics->GetShaderLoader()
				.GetVertexShader(SHADER_LOAD_PATH "Sprite_VS.cso")
				->GetShader(),
			nullptr, 0);
		myGraphics->GetContext()->PSSetShader(
			myGraphics->GetShaderLoader()
				.GetPixelShader(SHADER_LOAD_PATH "Sprite_PS.cso")
				->GetShader(),
			nullptr, 0);
	}
