...
#pragma region EffectType
	enum class eEffectType	// Used together with min/max UVs in
							// SpriteRenderBuffer to create effects
	{
		None,
		Bars,
		Circle,
	...
	inline const char* EnumToString(const eEffectType aType) {
		switch (aType) {
			case eEffectType::None:
				return "None";
			case eEffectType::Bars:
				return "Bars";
			case eEffectType::Circle:
				return "Circle";
			case eEffectType::Cooldown:
				return "Cooldown";
			case eEffectType::JamesBond:
				return "JamesBond";
			case eEffectType::SpinningSquare:
				return "SpinningSquare";
...
#pragma region ProgressionDirection
	...
	inline eEffectType ProgressionDirectionToEffectType(
...
#pragma endregion
	...
	enum class SpriteBatchMode : int { Default, Billboard, Screen, ScreenText };

	struct SpriteData  // Data that is shared between all sprites of the same
					   // type
	{
	...
	struct Sprite {
		...
		void SetFillFactor(const float aFactor,
						   eProgressionDirection aDirection);
		...
		void RotateAroundCenter(const float aAngle) {
			...
			myAttributes.myTransform.SetPosition(
				...
				Vector3f(spriteWidth / 2.0f, spriteHeight / 2.0f, 0.0f));
			// rotate around z
	...
	inline void Sprite::SetFillFactor(
		const float aFactor,
		eProgressionDirection aDirection = eProgressionDirection::LeftToRight) {
		const float factor = std::clamp(aFactor, 0.0f, 1.0f);

		switch (aDirection) {
			case eProgressionDirection::None: {
				myAttributes.myUVs = {0.0f, 0.0f, 1.0f, 1.0f};
				break;
			}
			case eProgressionDirection::LeftToRight: {
				myAttributes.myUVs = {0.0f, 0.0f, factor, 1.0f};
				break;
			}
			case eProgressionDirection::RightToLeft: {
				myAttributes.myUVs = {1.0f - factor, 0.0f, 1.0f, 1.0f};
				break;
			}
			case eProgressionDirection::TopToBottom: {
				myAttributes.myUVs = {0.0f, 0.0f, 1.0f, factor};
				break;
			}
			case eProgressionDirection::BottomToTop: {
				myAttributes.myUVs = {0.0f, 1.0f - factor, 1.0f, 1.0f};
				break;
			}
			case eProgressionDirection::DiagonalLeftUp: {
				myAttributes.myUVs = {0.0f, 1.0f - factor, factor, 1.0f};
				break;
			}
			case eProgressionDirection::DiagonalLeftDown: {
				myAttributes.myUVs = {0.0f, 0.0f, factor, factor};
				break;
			}
			case eProgressionDirection::DiagonalRightUp: {
				myAttributes.myUVs = {1.0f - factor, 1.0f - factor, 1.0f, 1.0f};
				break;
			}
			case eProgressionDirection::DiagonalRightDown: {
				myAttributes.myUVs = {1.0f - factor, 0.0f, 1.0f, factor};
				break;
			}
			case eProgressionDirection::Circle: {
				myAttributes.myUVs = {factor, factor, factor, factor};
				break;
			}
			case eProgressionDirection::Cooldown: {
				myAttributes.myUVs = {1.0f - factor, factor, factor, factor};
				break;
			}
			case eProgressionDirection::JamesBond: {
				...
				myAttributes.myUVs = {x, y, 0.5f, 0.5f};
				break;
			}
			case eProgressionDirection::SpinningSquare: {
				float angle = factor * 2.0f * PI;
