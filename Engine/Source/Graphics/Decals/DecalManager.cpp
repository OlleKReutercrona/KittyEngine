...
int KE::DecalManager::CreateDecal(Material* aMaterial,
								  const Transform& aTransform) {
	if (myFreeDecalIndices.size() > 0) {
		int index = myFreeDecalIndices.back();
		myFreeDecalIndices.pop_back();
		myDecals[index] = {aMaterial,
						   aTransform,
						   DecalActiveState::Active,
						   {1.0f, 1.0f, 1.0f, 1.0f}};
		return index;
	}
...
void KE::DecalManager::DestroyDecal(int aIndex) {
	if (aIndex < 0 || aIndex >= myDecals.size()) {
		return;
	}
...
KE::Decal* KE::DecalManager::GetDecal(int aIndex) {
	...
	if (myDecals[aIndex].myActiveState == DecalActiveState::Destroyed) {
		return nullptr;
	}
...
void KE::DecalManager::PrepareDecalRendering(Graphics* aGraphics,
