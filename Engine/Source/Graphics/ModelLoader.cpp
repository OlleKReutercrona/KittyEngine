...
namespace KE {
	...
	void ModelLoader::Process(const aiNode* aNode, const aiScene* aScene,
		...
		for (unsigned int i = 0; i < aNode->mNumMeshes; i++) {
			...
			// Get Material from .fbx
			{
				...
				// std::cout << aiMesh->mName.C_Str() << " has material: " <<
			...
			for (unsigned int j = 0; j < aiMesh->mNumVertices; j++) {
				...
				if (aiMesh->HasTangentsAndBitangents()) {
					...
					// vertex.bw = 1.0f;
				} else	// if no tangents and bitangents, set them to zero (can
						// we not just calculate them?)
				{
					vertex.tx = 0.0f;
					vertex.ty = 0.0f;
					vertex.tz = 0.0f;
...

			{
				...
				vertexBufferDesc.ByteWidth =
					sizeof(Vertex) * (UINT)vertices.size();
				...
				HRESULT result = myDevice->CreateBuffer(
					&vertexBufferDesc, &vertexData, &mesh.myVertexBuffer);
...

			{
				...
				HRESULT result = myDevice->CreateBuffer(
					&indexBufferDesc, &indexData, &mesh.myIndexBuffer);
	...
	void ModelLoader::Process(const aiNode* aNode, const aiScene* aScene,
		...
		for (unsigned int i = 0; i < aNode->mNumMeshes; i++) {
			...
			for (unsigned int j = 0; j < aiMesh->mNumBones; j++) {
				for (unsigned int a = 0; a < aiMesh->mBones[j]->mNumWeights;
					 a++) {
					unsigned int vert =
						aiMesh->mBones[j]->mWeights[a].mVertexId;
			...
			SkeletalMesh& mesh =
				aSkeletalMeshListToFill.myMeshes.emplace_back();
			mesh.myBounds.max = max;
			mesh.myBounds.min = min;
			mesh.myBounds.myBoundingRadius = radius;
			mesh.myBounds.centerPoint = avrgPos;
...

			{
				...
				vertexBufferDesc.Usage = D3D11_USAGE_IMMUTABLE;
				vertexBufferDesc.ByteWidth =
					sizeof(BoneVertex) * (UINT)vertices.size();
				vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
				vertexBufferDesc.CPUAccessFlags = 0;
				vertexBufferDesc.MiscFlags = 0;
	...
	void ModelLoader::AssignBoneIndices(const aiScene* aScene,
										SkeletalMeshList& aMeshList) {
		// Go through all bones and assign indices through them.
		for (unsigned int i = 0; i < aScene->mNumMeshes; i++) {
			for (unsigned int j = 0; j < aScene->mMeshes[i]->mNumBones; j++) {
				for (unsigned int a = 0;
					 a < aScene->mMeshes[i]->mBones[j]->mNumWeights; a++) {
					unsigned int vert =
						aScene->mMeshes[i]->mBones[j]->mWeights[a].mVertexId;
					float weight =
						aScene->mMeshes[i]->mBones[j]->mWeights[a].mWeight;

					aMeshList.myMeshes[i].myVertices[vert].myBoneIndices[0] = j;
					aMeshList.myMeshes[i].myVertices[vert].myBoneWeights[0] =
						weight;
				}
			}
		}
			...
			for (auto& toLoad : myPreloadData) {
				...
				switch (toLoad.type) {
					...
					case PreloadData::Type::AnimatedMesh: {
						...
						LoadSkeletalModel(loadedSkeletalModel,
										  toLoad.filePaths[0]);
						...
						for (size_t i = 0; i < toLoad.filePaths.size(); i++) {
							...
							LoadAnimation(loadedSkeletalModel,
										  toLoad.filePaths[i]);
						}
						toLoad.loaded = true;
						break;
	...
	MeshList& ModelLoader::Load(const std::string& aFilePath) {
		...
		aiSetImportPropertyFloat(pStore, AI_CONFIG_GLOBAL_SCALE_FACTOR_KEY,
								 1.0f);
	...
	bool ModelLoader::IsModelSkeletal(const std::string& aFilePath) {
		...
		Assimp::Importer importer;
		const aiScene* scene = aiImportFileExWithProperties(
			aFilePath.c_str(), aiProcess_FindInvalidData, NULL, pStore);
	...
	void ModelLoader::LoadSkeletalModel(SkeletalModelData& aOutSkeletalModel,
		...
		if (mySkeletons.contains(aFilePath)) {
			...
			aOutSkeletalModel.myCombinedTransforms.resize(
				aOutSkeletalModel.mySkeleton->myBones.size());
			aOutSkeletalModel.myFinalTransforms.resize(
				aOutSkeletalModel.mySkeleton->myBones.size());
			aOutSkeletalModel.myWorldSpaceJoints.resize(
				aOutSkeletalModel.mySkeleton->myBones.size());

			for (unsigned int i = 0;
				 i < aOutSkeletalModel.mySkeleton->myBones.size(); i++) {
				aOutSkeletalModel.myCombinedTransforms[i] =
					aOutSkeletalModel.mySkeleton->myBones[i].myBindPose;
				aOutSkeletalModel.myFinalTransforms[i] =
					aOutSkeletalModel.mySkeleton->myBones[i].myBindPose;
				aOutSkeletalModel.myWorldSpaceJoints[i] =
					aOutSkeletalModel.mySkeleton->myBones[i].myBindPose;
			}
		...
		if (TGA::FBXImporter::LoadModel(aFilePath, tgaModel)) {
			...
			for (unsigned int i = 0; i < tgaModel.Skeleton.Bones.size(); i++) {
				...
				newBone.myBindPose = DirectX::XMMatrixSet(
					boneMatrix.Data[0], boneMatrix.Data[1], boneMatrix.Data[2],
					boneMatrix.Data[3], boneMatrix.Data[4], boneMatrix.Data[5],
					boneMatrix.Data[6], boneMatrix.Data[7], boneMatrix.Data[8],
					boneMatrix.Data[9], boneMatrix.Data[10],
					boneMatrix.Data[11], boneMatrix.Data[12],
					boneMatrix.Data[13], boneMatrix.Data[14],
					boneMatrix.Data[15]);

				// newBone.myBindPose = DirectX::XMMatrixScaling(0.01f, 0.01f,
				// 0.01f) * newBone.myBindPose;

				// Transpose the matrix
				newBone.myBindPose =
					DirectX::XMMatrixTranspose(newBone.myBindPose);
			...
			for (size_t i = 0; i < skeletalMeshList.myMeshes.size(); ++i) {
				...
				for (size_t v = 0; v < mesh.myVertices.size(); ++v) {
					...
					// Bone indices
					mesh.myVertices[v].myBoneIndices[0] =
						fbxMesh.Vertices[v].BoneIDs[0];
					mesh.myVertices[v].myBoneIndices[1] =
						fbxMesh.Vertices[v].BoneIDs[1];
					mesh.myVertices[v].myBoneIndices[2] =
						fbxMesh.Vertices[v].BoneIDs[2];
					mesh.myVertices[v].myBoneIndices[3] =
						fbxMesh.Vertices[v].BoneIDs[3];

					// Bone weights
					mesh.myVertices[v].myBoneWeights[0] =
						fbxMesh.Vertices[v].BoneWeights[0];
					mesh.myVertices[v].myBoneWeights[1] =
						fbxMesh.Vertices[v].BoneWeights[1];
					mesh.myVertices[v].myBoneWeights[2] =
						fbxMesh.Vertices[v].BoneWeights[2];
					mesh.myVertices[v].myBoneWeights[3] =
						fbxMesh.Vertices[v].BoneWeights[3];
				}

				{
					...
					vertexBufferDesc.Usage = D3D11_USAGE_IMMUTABLE;
					vertexBufferDesc.ByteWidth =
						sizeof(BoneVertex) * (UINT)mesh.myVertices.size();
					vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
					vertexBufferDesc.CPUAccessFlags = 0;
					vertexBufferDesc.MiscFlags = 0;
...

				{
					...
					indexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
					indexBufferDesc.ByteWidth =
						sizeof(unsigned int) * (UINT)mesh.myIndices.size();
					indexBufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
					indexBufferDesc.CPUAccessFlags = 0;
					indexBufferDesc.MiscFlags = 0;
					...
					std::string name =
						tgaModel.Materials[fbxMesh.MaterialIndex].MaterialName;
					FixMaterialName(name);

					aOutSkeletalModel.myMeshList->myMaterialNames.push_back(
						name);
				}
	...
	void ModelLoader::LoadAnimation(SkeletalModelData& aOutSkeletalModel,
									const std::string& aFilePath) {
		if (myAnimationClips.contains(aFilePath)) {
			aOutSkeletalModel
				.myAnimationClipMap[myAnimationClips[aFilePath].name] =
				&myAnimationClips[aFilePath];
			return;
		}

		std::vector<TGA::FBXAnimation> animation;

		if (TGA::FBXImporter::LoadAnimation(
				aFilePath, aOutSkeletalModel.mySkeleton->myBoneNames,
				animation)) {
			KE::Timer timer;
			myAnimationClips[aFilePath] = AnimationClip();
			AnimationClip& newAnimation = myAnimationClips[aFilePath];

			auto& anim = animation.back();	// this is kinda odd. Works for the
											// mixamo godman animations though.

			newAnimation.name = anim.Name;

			// Trim path from name
			const size_t index = newAnimation.name.find_last_of('/');
			newAnimation.name.erase(newAnimation.name.begin(),
									newAnimation.name.begin() + index + 1);
			...
			newAnimation.length =
				(float)anim.Length;	 // this might not be correct
			...
			for (size_t f = 0; f < newAnimation.keyframes.size(); f++) {
				newAnimation.keyframes[f].boneTransforms.resize(
					anim.Frames[f].LocalTransforms.size());

				for (size_t t = 0; t < anim.Frames[f].LocalTransforms.size();
					 t++) {
					DirectX::XMMATRIX localMatrix = {};
					memcpy(&localMatrix, &anim.Frames[f].LocalTransforms[t],
						   sizeof(float) * 16);

					//// Extract translation, rotation, and scaling components
					// DirectX::XMVECTOR translation, rotation, scale;
					// DirectX::XMMatrixDecompose(&scale, &rotation,
					// &translation, localMatrix);

					//// Scale the translation component
					// DirectX::XMMATRIX translationScaleMatrix =
					// DirectX::XMMatrixScaling(0.01f, 0.01f, 0.01f);
					// translation = DirectX::XMVector3Transform(translation,
					// translationScaleMatrix);

					//// Recreate the local matrix with the scaled translation
					// localMatrix =
					// DirectX::XMMatrixAffineTransformation(scale,
					// DirectX::XMVECTOR{0, 0, 0, 1}, rotation, translation);

					newAnimation.keyframes[f].boneTransforms[t] = localMatrix;
				}
			}

			aOutSkeletalModel.myAnimationClipMap[newAnimation.name] =
				&newAnimation;

			timer.UpdateDeltaTime();

			KE_LOG_CHANNEL("ModelLoader",
						   "Loaded animation [%s] in %.5f seconds",
						   newAnimation.name.c_str(), timer.GetDeltaTime());
		}
	}
	...
	void ModelLoader::Preload(const std::vector<std::string>& aFilePaths,
