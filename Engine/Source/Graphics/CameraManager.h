...
namespace KE {
	...
	public:
		...
		inline Camera* GetMainCamera() {
			return &cameras[KE_MAIN_CAMERA_INDEX];
		}
		inline Camera* GetHighlightedCamera() {
			return &cameras[highlightedCamera];
		}

		inline void SetHighlightedCamera(const int aIndex) {
			highlightedCamera = aIndex;
		}
	};
}  // namespace KE