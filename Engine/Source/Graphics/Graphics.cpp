...
namespace KE {
	Graphics::Graphics(HWND aHWnd, const UINT aWidth, const UINT aHeight)
		...
		// Swapchain setup
		{
			...
			scd.BufferDesc.ScanlineOrdering =
			...
			HRESULT swapResult = D3D11CreateDeviceAndSwapChain(
				...
				nullptr, 0, D3D11_SDK_VERSION, &scd, &mySwap, &myDevice,
				nullptr, &myContext);
		...
		// myDevice->QueryInterface(__uuidof(ID3D11Debug),
		...
		myDevice->CreateRenderTargetView(pBackBuffer.Get(), nullptr,
										 &myRenderTarget);
		...
		// Create depth stencil texture and view
		{
			...
			myDevice->CreateDepthStencilView(pDepthStencil.Get(), &descDSV,
											 &myDepthStencil);
		...
		// Default sampler state
		{
			...
			samplerDesc.AddressU = samplerDesc.AddressW = samplerDesc.AddressV =
		...
		// Fullscreen effect sampler state
		{
			...
			samplerDesc.MaxAnisotropy = D3D11_REQ_MAXANISOTROPY;
			myDevice->CreateSamplerState(&samplerDesc,
										 &myFullscreenEffectSampler);
		}
		...
		// Shadow sampler state
		{
			...
			samplerCompDesc.AddressU = samplerCompDesc.AddressW =
				samplerCompDesc.AddressV = D3D11_TEXTURE_ADDRESS_MIRROR;
			...
			myDevice->CreateSamplerState(&samplerCompDesc,
										 &myShadowCompSampler);
		}
		...
		myContext->PSSetSamplers(2, 1,
								 myFullscreenEffectSampler.GetAddressOf());
		...
		// Create Render Targets, TODO -> should probably use a member variable
		// or a #define instead of a random magic number
		for (int i = 0; i < 12; i++) {
			myRenderTargets.emplace_back();
			myRenderTargets.back().Init(myDevice.Get(), myContext.Get(),
										myWidth, myHeight);
		}
		...
		myDistanceFog.Init(myDevice.Get(), myContext.Get(), myWidth, myHeight,
						   myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
														 "DistanceFog_PS.cso"),
						   myShaderLoader.GetVertexShader(
							   SHADER_LOAD_PATH "FullscreenAsset_VS.cso"));

		hr = myDeferredLightManager.Init(myDevice.Get(), &myShaderLoader,
										 {(int)myWidth, (int)myHeight});
		if (FAILED(hr)) {
			assert(hr && "Failed to init DeferredLightManager");
		}

		myModelData.reserve(16384);	 // this way of doing stuff isnt very good,
									 // but it works for now
		mySkeletalModelData.reserve(1024);

		// <+--+> CREATE GBUFFER <+--+>
		myGBuffer = GBuffer::Create(
			{static_cast<int32_t>(myWidth), static_cast<int32_t>(myHeight)},
			myDevice.Get());
		mySecondaryGBuffer = GBuffer::Create(
			{static_cast<int32_t>(myWidth), static_cast<int32_t>(myHeight)},
			myDevice.Get());
		...
		// <+--+> COMMON BUFFER SETUP <+--+>
		{
			...
			CommonBuffer data = {};
			myCommonbuffer.MapBuffer(&data, sizeof(CommonBuffer),
									 myContext.Get());
		}
		...
		//___ GRID LINES ___ //
		{
			...
			lineData.myPS = myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
														  "DebugLine_PS.cso");
			lineData.myVS = myShaderLoader.GetVertexShader(SHADER_LOAD_PATH
														   "DebugLine_VS.cso");
			...
			for (int i = -gridHalfSize; i <= gridHalfSize; i++) {
				...
				lineData.myVertices.back().position = {
					(float)i, 0, (float)-gridHalfSize, 1.0f};
				...
				lineData.myIndices.push_back((int)lineData.myVertices.size());
				lineData.myVertices.emplace_back();
				lineData.myVertices.back().position = {(float)-gridHalfSize, 0,
													   (float)i, 1.0f};
				lineData.myVertices.back().colour = colour;
		...
		// <+--+> FULLSCREEN LINES <+--+>
		{
			...
			lineData.myVS = myShaderLoader.GetVertexShader(
				SHADER_LOAD_PATH "FullscreenDebugLine_VS.cso");
			lineData.myPS = myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
														  "DebugLine_PS.cso");
	...
	void Graphics::Init() {
		...
		// <+--+> FullScreen Asset <+--+>
		{
			...
			myFullscreenAsset.myVertexShader = myShaderLoader.GetVertexShader(
				SHADER_LOAD_PATH "OutlineRenderer_VS.cso");
			myFullscreenAsset.myPixelShader = myShaderLoader.GetPixelShader(
				SHADER_LOAD_PATH "OutlineRenderer_PS.cso");
		}

		// <+--+> Post Process <+--+>
		{
			myPostProcess.Init(myDevice.Get(), this,
							   myShaderLoader.GetVertexShader(
								   SHADER_LOAD_PATH "UpDownSample_VS.cso"),
							   myShaderLoader.GetPixelShader(
								   SHADER_LOAD_PATH "DownSample_PS.cso"),
							   myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
															 "UpSample_PS.cso"),
							   myShaderLoader.GetPixelShader(
								   SHADER_LOAD_PATH "Gaussian_PS.cso"));

			myPostProcess.SetPSShader(myShaderLoader.GetPixelShader(
				SHADER_LOAD_PATH "PostProcessing_PS.cso"));
			myPostProcess.SetVSShader(myShaderLoader.GetVertexShader(
				SHADER_LOAD_PATH "PostProcessing_VS.cso"));
		...
		for (int i = 0; i < static_cast<int>(eRenderLayers::Count); i++) {
			...
			myRenderLayers[i].AssignRenderers(
				&myDefaultRenderer, &mySkeletalRenderer, &myInstancedRenderer,
				&myDecalRenderer);

			myRenderLayers[i].SetDepthStencilState(
				KE::eDepthStencilStates::Write);
			myRenderLayers[i].SetBlendState(KE::eBlendStates::Disabled);
			myRenderLayers[i].SetRasterizerState(
				eRasterizerStates::BackfaceCulling);
		}

		myRenderLayers[static_cast<int>(eRenderLayers::Back)]
			.SetDepthStencilState(KE::eDepthStencilStates::ReadOnlyLessEqual);
		myRenderLayers[static_cast<int>(eRenderLayers::Back)]
			.SetRasterizerState(eRasterizerStates::FrontfaceCulling);

		myRenderLayers[static_cast<int>(eRenderLayers::Back)].flags =
			0 |
		...
		myRenderLayers[static_cast<int>(eRenderLayers::Main)].flags =
			0 | RenderLayerFlags_Active | RenderLayerFlags_ReceiveShadows |
			RenderLayerFlags_CastShadows | RenderLayerFlags_Lit |
			RenderLayerFlags_DrawModels | RenderLayerFlags_DrawSkeletal |
			RenderLayerFlags_DrawDecals | RenderLayerFlags_DrawSprites |
			RenderLayerFlags_DrawVFX
			//| RenderLayerFlags_SSAO
			;

		myRenderLayers[static_cast<int>(eRenderLayers::Front)].flags =
			0 |
			RenderLayerFlags_Active
			//| RenderLayerFlags_ReceiveShadows
			| RenderLayerFlags_CastShadows | RenderLayerFlags_Lit |
			...
			RenderLayerFlags_DrawSkeletal
			//| RenderLayerFlags_DrawDecals
			| RenderLayerFlags_DrawSprites | RenderLayerFlags_DrawVFX
			//| RenderLayerFlags_SSAO
			;

		myRenderLayers[static_cast<int>(eRenderLayers::UI)].flags =
			0 |
			RenderLayerFlags_Active
			//| RenderLayerFlags_ReceiveShadows
			//| RenderLayerFlags_CastShadows
		...
		// font = new
		// SpriteFont(FontLoader::GenerateMSDFFont("Data/InternalAssets/Dirga.ttf",
		// 64.0f, 3.0f, 2.0f, 1.0f));

		//
		for (int i = 1; i < 6; i++) {
			std::vector<std::string> loadPaths = {
				std::format("Data/Assets/Players/Player{}/sk_player0{}.fbx", i,
							i),
				std::format(
					"Data/Assets/Players/Player{}/anim_player0{}_idle.fbx", i,
					i),
				std::format(
					"Data/Assets/Players/Player{}/anim_player0{}_run.fbx", i,
					i),
				std::format(
					"Data/Assets/Players/Player{}/anim_player0{}_dash.fbx", i,
					i),
				std::format(
					"Data/Assets/Players/Player{}/anim_player0{}_throw.fbx", i,
					i),
				std::format(
					"Data/Assets/Players/Player{}/anim_player0{}_death.fbx", i,
					i),
				std::format(
					"Data/Assets/Players/Player{}/anim_player0{}_taunt.fbx", i,
					i),
				std::format(
					"Data/Assets/Players/Player{}/anim_player0{}_select.fbx", i,
					i),
				std::format("Data/Assets/Players/Player{}/"
							"anim_player0{}_victoryDance.fbx",
							i, i),
				std::format(
					"Data/Assets/Players/Player{}/anim_player0{}_podium.fbx", i,
					i),
			};
		...
		// myModelLoader.Preload({"Data\\EngineAssets\\SmoothSphere.fbx"},
		// PreloadData::Type::Mesh);
		// myModelLoader.Preload({"Data/EngineAssets/SmoothSphere.fbx"},
		// PreloadData::Type::Mesh);
	}
	...
	void Graphics::SetRenderResolution(int aWidth, int aHeight) {
		...
		pBackBuffer->Release();
		myDevice->CreateRenderTargetView(pBackBuffer, NULL,
		...
		descDepth.Usage = D3D11_USAGE_DEFAULT;
		descDepth.BindFlags = D3D11_BIND_DEPTH_STENCIL;
		myDevice->CreateTexture2D(&descDepth, nullptr,
		...
		myDevice->CreateDepthStencilView(
			pDepthStencil.Get(), &descDSV,
			myDepthStencil.ReleaseAndGetAddressOf());
		...
		for (int i = 0; i < KE_CAMERA_MAX; i++) {
			const KE::ProjectionData& data =
				myCameraManager.GetCamera(i)->GetProjectionData();
			if (myCameraManager.GetCamera(i)->GetType() ==
				KE::ProjectionType::Orthographic) {
				continue;
			}
			myCameraManager.GetCamera(i)->SetPerspective(
				(float)aWidth, (float)aHeight, data.perspective.fov,
				data.perspective.nearPlane, data.perspective.farPlane);
		}
	...
	void Graphics::SetRenderResolution(int aWidth, int aHeight) {
		...
		std::cout << "Graphics::SetRenderResolution CALLED with: " << aWidth
				  << ", " << aHeight << ", "
				  << (isFullscreen ? "true" : "false") << std::endl;

		int useWidth, useHeight;

		if (!isFullscreen)	// if windowed, we actually need to resize our swap
							// chain as the window resizes
		{
		...
		for (int i = 0; i < myRenderTargets.size(); i++) {
			if (i == 10 || i == 11) {
				continue;
			}  // don't resize the shell texturing RTs!
		...
		for (int i = 0; i < KE_CAMERA_MAX; i++) {
			const KE::ProjectionData& data =
				myCameraManager.GetCamera(i)->GetProjectionData();
			myCameraManager.GetCamera(i)->SetPerspective(
				(float)useWidth, (float)useHeight, data.perspective.fov,
				data.perspective.nearPlane, data.perspective.farPlane);
		}
	...
	void Graphics::SetViewport(int aWidth, int aHeight, float aTopLeftX,
							   float aTopLeftY) {
		D3D11_VIEWPORT vp;
	...
	int Graphics::GetRenderHeight() const {
		return myFullscreenRenderHeight > 0 ? myFullscreenRenderHeight
											: myHeight;
	}
	...
	void Graphics::BeginFrame() {
		...
		// Forward rendering
		{
			...
			myContext->ClearDepthStencilView(myDepthStencil.Get(),
	...
	void Graphics::SetCommonBuffer(Camera& aCamera) {
		...
		commonBuffer.clipToWorldMatrix =
			DirectX::XMMatrixInverse(nullptr, commonBuffer.worldToClipMatrix);

		commonBuffer.viewMatrix = myView;
		commonBuffer.projectionMatrix = myProjection;

		commonBuffer.projMatrixInverse =
			DirectX::XMMatrixInverse(nullptr, myProjection);
		commonBuffer.viewMatrixInverse =
			DirectX::XMMatrixInverse(nullptr, myView);

		Matrix4x4f camTransform = DirectX::XMMatrixInverse(nullptr, myView);

		commonBuffer.cameraPosition = {camTransform(4, 1), camTransform(4, 2),
									   camTransform(4, 3), camTransform(4, 4)};

#ifndef KITTYENGINE_SHIP
		commonBuffer.resolution = {(unsigned int)myWidth,
		...
		commonBuffer.camNearFar = {
			aCamera.GetProjectionData().perspective.nearPlane,
			aCamera.GetProjectionData().perspective.farPlane};

		myCommonbuffer.MapBuffer(&commonBuffer, sizeof(CommonBuffer),
								 myContext.Get());
	...
	void Graphics::RenderDebug() {
		...
		static PixelShader* behindLinePS = myShaderLoader.GetPixelShader(
			SHADER_LOAD_PATH "DebugLine_Behind_PS.cso");
	...
	void Graphics::DrawLayer(Camera* camera, eRenderLayers layer,
							 bool debugDraw) {
		const int layerIndex = static_cast<int>(layer);
		if (!(myRenderLayers[layerIndex].flags & RenderLayerFlags_Active)) {
			return;
		}
...
#pragma region MainRenderPass
		...
		if (mySkybox) {
			myContext->PSSetShaderResources(
				15, 1, mySkybox->myShaderResourceView.GetAddressOf());
		}

		myRenderLayers[layerIndex].NewFrame();
		// myGBuffer.ClearTextures(myContext.Get(),false);
		myGBuffer.SetAsActiveTarget(myContext.Get(),
									myGBuffer.GetDepthStencilView());
		...
		if (myCubemap) {
			myContext->PSSetShaderResources(
				15, 1, myCubemap->myShaderResourceView.GetAddressOf());
		}
...
#pragma region ShadowPass
		if (myRenderLayers[layerIndex].flags & RenderLayerFlags_CastShadows) {
			...
			const auto stateOld =
				myRenderLayers[layerIndex].mySettings.rasterizerState;
			myRenderLayers[layerIndex].SetRasterizerState(
				eRasterizerStates::FrontfaceCulling);
			myRenderLayers[layerIndex].Render(
				&myDeferredLightManager.myDirectionalLightCamera, nullptr,
				myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
											  "Model_Shadow_PS.cso"));
...
#pragma region DecalPass
		...
		if (myRenderLayers[layerIndex].flags & RenderLayerFlags_DrawDecals) {
			myRenderLayers[layerIndex].RenderDecals(
				camera, &myGBuffer, &mySecondaryGBuffer, &myDecalManager);
		} else {
...
#pragma region LightingPass
		...
		if (myActivePass == eDeferredPasses::Camera) {
			if (myRenderLayers[layerIndex].flags & RenderLayerFlags_SSAO) {
				...
				myRenderLayers[layerIndex].GetProcessedRender()->MakeActive(
					false);
				myGBuffer.SetAllAsResources(myContext.Get(), 0u);
			}
			...
			if (myRenderLayers[layerIndex].flags & RenderLayerFlags_Lit) {
				...
				myRenderLayers[layerIndex].GetProcessedRender()->CopyFrom(
					myGBuffer.GetTexture(
						static_cast<int>(GBuffer::GBufferTexture::Albedo)));
			}
			...
			myFullscreenAsset.Render(
				...
				myShaderLoader.GetVertexShader(SHADER_LOAD_PATH
											   "FullscreenAsset_VS.cso"),
				myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
											  "FullscreenAsset_PS.cso"));
		}
...
#pragma region SpritePass
		...
		if (myRenderLayers[layerIndex].flags & RenderLayerFlags_DrawSprites) {
			...
			myRenderLayers[layerIndex].GetProcessedRender()->MakeActive(
				true, myGBuffer.GetDepthStencilView());
...
#pragma region DebugDraw
		if (layer == eRenderLayers::Main &&
			debugDraw)	// until we have per-layer debug lines, this will do
		{
...
#pragma region VFX
		{
			...
			if (myRenderLayers[layerIndex].flags & RenderLayerFlags_DrawVFX) {
				...
				ID3D11RenderTargetView* rtvs[2] = {
					myRenderLayers[layerIndex]
						.GetProcessedRender()
						->GetRenderTargetView(),
					myRenderTargets[4].GetRenderTargetView()};
				myContext->OMSetRenderTargets(2, rtvs,
											  myGBuffer.GetDepthStencilView());
				myVFXManager.Render(layer, myGBuffer.GetDepthStencilView());
				mySecondaryGBuffer.ClearAllAsResourcesSlots(myContext.Get(),
															4u);
			}
...
#pragma endregion
	...
	void Graphics::CompositeLayers(KE::RenderTarget& workingRenderTarget,
								   eRenderLayers aHighestLayer) {
		KE::DebugTimeLogger::BeginLogVar("Layer Composition Pass");
		workingRenderTarget.MakeActive(false);
		for (int i = 0; i < static_cast<int>(eRenderLayers::Count); i++) {
			if (i == (int)aHighestLayer) {
				break;
			}

			auto& layer = myRenderLayers[i];
			if (!(layer.flags & RenderLayerFlags_Active)) {
				continue;
			}
			myFullscreenAsset.Render(
				this, layer.GetProcessedRender()->GetShaderResourceView(),
				myShaderLoader.GetVertexShader(SHADER_LOAD_PATH
											   "FullscreenAsset_VS.cso"),
				myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
											  "RenderLayerCompositor_PS.cso"));
		}
		KE::DebugTimeLogger::EndLogVar("Layer Composition Pass");
	}

	void Graphics::PerformPostProcess(KE::RenderTarget& workingRenderTarget) {
		// we post process the composited image, currently no reason to do it
		// per layer

		myPostProcess.PreProcessBloom(this, &workingRenderTarget);
		myVFXManager.GetPostProcessing().MockPreProcessBloom(
			&myRenderTargets[4]);
		if (workingRenderTarget.ShouldRenderPostProcessing() &&
			myActivePass == eDeferredPasses::Camera) {
	...
	void Graphics::PerformBackBufferRender() {
		...
		asset.Init(this);
		asset.myVertexShader = myShaderLoader.GetVertexShader(
			SHADER_LOAD_PATH "FullscreenAsset_VS.cso");
		asset.myPixelShader = myShaderLoader.GetPixelShader(
			SHADER_LOAD_PATH "FullscreenAsset_PS.cso");
	...
	void Graphics::PerformBackBufferRender() {
		...
		myFullscreenAsset.Render(
			this, myRenderTargets[0].GetShaderResourceView(),
			myShaderLoader.GetVertexShader(SHADER_LOAD_PATH
										   "FullscreenAsset_VS.cso"),
			myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
										  "FullscreenAsset_PS.cso"));
	...
	void Graphics::Render() {
		...
		myRenderTargets[4].Clear(
			(float*)vfxClearColour,
			true);	// this is the render target used for vfx blooming
...
#define DEFERRED_GRASS
			...
			if (shells.Size() > 0) {
				for (const auto& shell : shells) {
					...
					if (shell.displacementTexture) {
						myContext->PSSetShaderResources(
							5, 1,
							shell.displacementTexture->myShaderResourceView
								.GetAddressOf());
					}

					ShellTexturingRenderInput shellIn{
						camera->GetViewMatrix(), camera->GetProjectionMatrix(),
						shell.attributes, nullptr,
						myShaderLoader.GetPixelShader(
							SHADER_LOAD_PATH "DeferredShellTexturing_PS.cso")};
			...
			myDeferredLightManager.PrepareShadowPass(this);
			myRenderLayers[(int)eRenderLayers::Main].Render(
				&myDeferredLightManager.myDirectionalLightCamera, nullptr,
				myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
											  "Model_Shadow_PS.cso"));
			SetRasterizerState(eRasterizerStates::BackfaceCulling);
			...
			myGBuffer.SetAsActiveTarget(
				myContext.Get(), workingRenderTarget.GetDepthStencilView());
			myRenderLayers[(int)eRenderLayers::Front].Render(camera);

			// decal rendering

			myRenderLayers[(int)eRenderLayers::Main].RenderDecals(
				camera, &myGBuffer, &mySecondaryGBuffer, &myDecalManager);

			// SSAO START
			if (myRenderLayers[(int)eRenderLayers::Main].flags &
				RenderLayerFlags_SSAO) {
			...
			if (shells.Size() > 0) {
				...
				for (const auto& shell : shells) {
					...
					ShellTexturingRenderInput shellIn{
						camera->GetViewMatrix(), camera->GetProjectionMatrix(),
						shell.attributes, nullptr, nullptr};
			...
			if (myActivePass == eDeferredPasses::Camera) {
				...
				if (!myWaterModels.empty()) {
					...
					myGBuffer.SetAsResourceOnSlot(
						myContext.Get(), GBuffer::GBufferTexture::WorldPosition,
						6u);
					myRenderTargets[5].SetAsShaderResource(5);
					if (mySkybox) {
						myContext->PSSetShaderResources(
							15, 1,
							mySkybox->myShaderResourceView.GetAddressOf());
					}
					workingRenderTarget.MakeActive(
						true, myGBuffer.GetDepthStencilView());
				}
				...
				myContext->OMSetRenderTargets(2, rtvs,
											  myGBuffer.GetDepthStencilView());
				myVFXManager.Render(eRenderLayers::Main,
									myGBuffer.GetDepthStencilView());
				myVFXManager.Render(eRenderLayers::Front,
									myGBuffer.GetDepthStencilView());
				...
				// Distance Fog
				{
					...
					myDistanceFog.BindResources(&workingRenderTarget,
												myContext.Get());
				...
				myFullscreenAsset.Render(
					this,
					myGBuffer.GetShaderResourceViews()[(int)myActivePass - 1],
					myShaderLoader.GetVertexShader(SHADER_LOAD_PATH
												   "FullscreenAsset_VS.cso"),
					myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
												  "FullscreenAsset_PS.cso"));
			...
			if (myDrawSSAOPass > 0) {
				myFullscreenAsset.Render(
					this,
					myDrawSSAOPass == 1 ? mySSAO.GetBlurSRV()
										: mySSAO.GetSSAOSRV(),
					myShaderLoader.GetVertexShader(SHADER_LOAD_PATH
												   "FullscreenAsset_VS.cso"),
					myShaderLoader.GetPixelShader(SHADER_LOAD_PATH
	...
	ModelData* Graphics::CreateModelData(const std::string& aModelName) {
		...
		workingData.myRenderResources.resize(
			workingData.myMeshList->myMeshes.size());

		for (size_t i = 0; i < workingData.myMeshList->myMeshes.size(); i++) {
			workingData.myRenderResources[i].myVertexShader =
				myShaderLoader.GetDefaultVertexShader();
			workingData.myRenderResources[i].myPixelShader =
				myShaderLoader.GetDefaultPixelShader();

			std::string texturePath =
				workingData.myMeshList->myMaterialNames[i];
			KE::Material* material =
				myTextureLoader.GetMaterialFromPath(texturePath);
	...
	SkeletalModelData* Graphics::CreateSkeletalModelData(
		const std::string& aModelName, KE::SkeletalModelData* aModelData) {
		KE::SkeletalModelData*
		...
		for (size_t i = 0; i < workingData->myMeshList->myMeshes.size(); i++) {
			workingData->myRenderResources[i].myVertexShader =
				myShaderLoader.GetVertexShader(
					SHADER_LOAD_PATH "Model_Deferred_Skeletal_VS.cso");
			workingData->myRenderResources[i].myPixelShader =
				myShaderLoader.GetDefaultPixelShader();
	...
	void Graphics::SetBlendState(eBlendStates aBlendState) {
		myContext->OMSetBlendState(myBlendStates[(int)aBlendState].Get(),
								   nullptr, 0xffffffff);
	}

	void Graphics::SetDepthStencilState(
		eDepthStencilStates aDepthStencilState) {
		myContext->OMSetDepthStencilState(
			myDepthStencilStates[(int)aDepthStencilState].Get(), 0);
	}
	...
	void Graphics::SetRenderTarget(RenderTarget* aRenderTarget,
								   bool aUseDepth) {
		if (!aRenderTarget) {
			myContext->OMSetRenderTargets(1u, myRenderTarget.GetAddressOf(),
										  myDepthStencil.Get());
	...
	void Graphics::DrawLineCube(const Vector3f& aPosition,
	...
	void Graphics::DrawLineSphere(const Vector3f& aPosition,
								  const float aRadius,
								  const Vector4f& aColour) {
		myDebugLines.DrawSphere(aPosition, aRadius, aColour);
	}

	void Graphics::DrawCone(const Vector3f& aPosition,
							const Vector3f& aDirection, const float aLength,
							const float anOuterRadius,
							const float anInnerRadius,
							const Vector4f& aColour) {
		myDebugLines.DrawCone(aPosition, aDirection, aLength, anOuterRadius,
							  anInnerRadius, aColour);
	}

	void Graphics::AddScreenSpaceLine(const Vector2f& aFrom,
									  const Vector2f& aTo,
									  const Vector4f& aColour) {
		LineVertex p1;
		p1.position = {2.0f * (aFrom.x / myWidth) - 1.0f,
					   2.0f * ((myHeight - aFrom.y) / myHeight) - 1.0f, 0.0f,
					   1.0f};
		p1.colour = aColour;

		LineVertex p2;
		p2.position = {2.0f * (aTo.x / myWidth) - 1.0f,
					   2.0f * ((myHeight - aTo.y) / myHeight) - 1.0f, 0.0f,
					   1.0f};
		p2.colour = aColour;
	...
	void Graphics::AppendRenderTargetQueue(int aRenderTargetIndex,
										   int aCameraIndex) {
		// loop through queue and check if the render target is already in the
		// queue
	...
	void Graphics::DrawIndexed(UINT aIndexCount, UINT aStartIndexLocation,
							   UINT aBaseVertexLocation) {
		myContext->DrawIndexed(aIndexCount, aStartIndexLocation,
							   aBaseVertexLocation);
		myDrawCalls++;
	}
	...
	void Graphics::IncrementDeferredPass() {
		...
		current = ++current < static_cast<int>(eDeferredPasses::Count) - 1
					  ? current
					  : 0;
	...
	void Graphics::BindMaterial(const Material* aMaterial,
								unsigned aStartSlot) {
...
#pragma region State Creation
	bool Graphics::CreateBlendStates() {
		...
		hr = myDevice->CreateBlendState(
			&blendStateDesc, myBlendStates[(int)eBlendStates::Disabled]
		...
		blendStateDesc.RenderTarget[0].RenderTargetWriteMask =
			D3D11_COLOR_WRITE_ENABLE_ALL;
		hr = myDevice->CreateBlendState(
			&blendStateDesc, myBlendStates[(int)eBlendStates::AlphaBlend]
								 .ReleaseAndGetAddressOf());
		if (FAILED(hr)) {
			return false;
		}
		...
		blendStateDesc.RenderTarget[0].RenderTargetWriteMask =
			D3D11_COLOR_WRITE_ENABLE_ALL;
		hr = myDevice->CreateBlendState(
			&blendStateDesc, myBlendStates[(int)eBlendStates::VFXBlend]
								 .ReleaseAndGetAddressOf());
		if (FAILED(hr)) {
			return false;
		}
		...
		blendStateDesc.RenderTarget[0].RenderTargetWriteMask =
			D3D11_COLOR_WRITE_ENABLE_ALL;
		hr = myDevice->CreateBlendState(
			&blendStateDesc, myBlendStates[(int)eBlendStates::AdditiveBlend]
								 .ReleaseAndGetAddressOf());
		if (FAILED(hr)) {
			return false;
		}
		...
		blendStateDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;
		blendStateDesc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;
		blendStateDesc.RenderTarget[0].DestBlendAlpha =
			D3D11_BLEND_INV_SRC_ALPHA;
		blendStateDesc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
		blendStateDesc.RenderTarget[0].RenderTargetWriteMask =
			D3D11_COLOR_WRITE_ENABLE_ALL;
		hr = myDevice->CreateBlendState(
			&blendStateDesc, myBlendStates[(int)eBlendStates::TransparencyBlend]
								 .ReleaseAndGetAddressOf());
		if (FAILED(hr)) {
			return false;
		}
	...
	bool Graphics::CreateDepthStencilStates() {
		...
		hr = myDevice->CreateDepthStencilState(
			&dsDesc, myDepthStencilStates[(int)eDepthStencilStates::Write]
						 .ReleaseAndGetAddressOf());
		...
		hr = myDevice->CreateDepthStencilState(
			&dsDesc,
			myDepthStencilStates[(int)eDepthStencilStates::ReadOnlyGreater]
				.ReleaseAndGetAddressOf());
		...
		hr = myDevice->CreateDepthStencilState(
			&dsDesc,
			myDepthStencilStates[(int)eDepthStencilStates::ReadOnlyLess]
				.ReleaseAndGetAddressOf());
		...
		hr = myDevice->CreateDepthStencilState(
			&dsDesc,
			myDepthStencilStates[(int)eDepthStencilStates::ReadOnlyEmpty]
				.ReleaseAndGetAddressOf());
		...
		hr = myDevice->CreateDepthStencilState(
			&dsDesc, myDepthStencilStates[(int)eDepthStencilStates::Always]
						 .ReleaseAndGetAddressOf());
		...
		hr = myDevice->CreateDepthStencilState(
			&dsDesc,
			myDepthStencilStates[(int)eDepthStencilStates::ReadOnlyLessEqual]
				.ReleaseAndGetAddressOf());
	...
	bool Graphics::CreateRasterizerStates() {
		...
		hr = myDevice->CreateRasterizerState(
			&rasterizerDesc,
			&myRasterizerStates[(int)eRasterizerStates::FrontfaceCulling]);
		if (FAILED(hr)) {
			return false;
		}
		...
		hr = myDevice->CreateRasterizerState(
			&rasterizerDesc,
			&myRasterizerStates[(int)eRasterizerStates::NoCulling]);
		if (FAILED(hr)) {
			return false;
		}
		...
		hr = myDevice->CreateRasterizerState(
			&rasterizerDesc,
			&myRasterizerStates[(int)eRasterizerStates::Wireframe]);
		if (FAILED(hr)) {
			return false;
		}
		...
		// AA Lines
		{
			...
			hr = myDevice->CreateRasterizerState(
				&desc, &myRasterizerStates[(int)eRasterizerStates::AALines]);
			if (FAILED(hr)) {
				return false;
			}
		}
	...
	Graphics::FrustumPlanes Graphics::ExtractFrustumPlanes() const {
		...
		frustum.planes[0] = DirectX::XMFLOAT4(VP._14 + VP._11, VP._24 + VP._21,
											  VP._34 + VP._31, VP._44 + VP._41);
		// Left plane
		frustum.planes[1] = DirectX::XMFLOAT4(VP._14 - VP._11, VP._24 - VP._21,
											  VP._34 - VP._31, VP._44 - VP._41);
		// Right plane
		frustum.planes[2] = DirectX::XMFLOAT4(VP._14 - VP._12, VP._24 - VP._22,
											  VP._34 - VP._32, VP._44 - VP._42);
		// Top plane
		frustum.planes[3] = DirectX::XMFLOAT4(VP._14 + VP._12, VP._24 + VP._22,
											  VP._34 + VP._32, VP._44 + VP._42);
		// Bottom plane
		frustum.planes[4] =
			DirectX::XMFLOAT4(VP._13, VP._23, VP._33, VP._43);	// Near plane
		frustum.planes[5] = DirectX::XMFLOAT4(VP._14 - VP._13, VP._24 - VP._23,
											  VP._34 - VP._33, VP._44 - VP._43);
		// Far plane
		...
		for (auto& plane : frustum.planes) {
			...
			plane = DirectX::XMFLOAT4(plane.x / length, plane.y / length,
									  plane.z / length, plane.w / length);
		}

		return frustum;
	}

	bool Graphics::IsModelBoundsInFrustum(
		const Vector3f& aPosition, const ModelBounds& aModelBounds) const {
		...
		for (int i = 0; i < 6; ++i) {
			if (frustum.planes[i].x * min.x + frustum.planes[i].y * min.y +
					frustum.planes[i].z * min.z + frustum.planes[i].w >
				0.0f)
				continue;
			if (frustum.planes[i].x * max.x + frustum.planes[i].y * min.y +
					frustum.planes[i].z * min.z + frustum.planes[i].w >
				0.0f)
				continue;
			if (frustum.planes[i].x * min.x + frustum.planes[i].y * max.y +
					frustum.planes[i].z * min.z + frustum.planes[i].w >
				0.0f)
				continue;
			if (frustum.planes[i].x * max.x + frustum.planes[i].y * max.y +
					frustum.planes[i].z * min.z + frustum.planes[i].w >
				0.0f)
				continue;
			if (frustum.planes[i].x * min.x + frustum.planes[i].y * min.y +
					frustum.planes[i].z * max.z + frustum.planes[i].w >
				0.0f)
				continue;
			if (frustum.planes[i].x * max.x + frustum.planes[i].y * min.y +
					frustum.planes[i].z * max.z + frustum.planes[i].w >
			...
			// If the bounding box is completely outside any frustum plane, it
			// is not visible
			return false;
		}

		// If the bounding box is not completely outside any frustum plane, it
		// is visible
		return true;
	}
