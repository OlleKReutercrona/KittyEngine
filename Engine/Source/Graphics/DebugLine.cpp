...
namespace KE {
	...
	bool DebugLine::Initialize(LineRenderData aRenderData,
							   ID3D11Device* aDevice, bool isStatic) {
		myDevice = aDevice;
		...
		if (myIndices > 0) {
			HRESULT result;
			{
				...
				vertexBufferDesc.ByteWidth =
					sizeof(LineVertex) * (int)aRenderData.myVertices.size();
				...
				result = myDevice->CreateBuffer(&vertexBufferDesc, &vertexData,
												&myVertexBuffer);
				if (FAILED(result)) {
					return false;
				}
...

			{
				...
				indexBufferDesc.ByteWidth =
					sizeof(unsigned int) * (int)aRenderData.myIndices.size();
	...
	void DebugLine::Render(ID3D11DeviceContext* context,
						   VertexShader* aVSOverride,
						   PixelShader* aPSOverride) {
		...
		context->IASetVertexBuffers(0, 1, myVertexBuffer.GetAddressOf(),
									&stride, &offset);
	...
	void DebugLine::SetVertexes() {
		...
		D3D11_BUFFER_DESC vertexBufferDesc = {};
		vertexBufferDesc.ByteWidth =
			sizeof(LineVertex) * (int)myVertices.size();
		vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
		vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		vertexBufferDesc.MiscFlags = 0;
	...
	void DebugLine::AddLine(const LineVertex aStartPoint,
							const LineVertex anEndPoint) {
	...
	void DebugLine::DrawCube(const Vector3f& aPosition,
							 const Vector3f& aDimensions,
		...
		for (size_t i = 0; i < 8; i++) {
			myVertices.emplace_back(LineVertex({vertices[i].x, vertices[i].y,
	...
	void DebugLine::DrawCube(const Transform& aTransform,
							 const Vector3f& aDimensions,
							 const Vector4f& aColour) {
		auto vertices = myCube.GetVertices();
		auto indices = myCube.GetIndices();
		...
		int startIndex = static_cast<int>(myVertices.size());
		for (size_t i = 0; i < 8; i++) {
			Vector4f position = {vertices[i].x, vertices[i].y, vertices[i].z,
								 vertices[i].w};
	...
	void DebugLine::DrawSphere(const Vector3f& aPosition, const float aRadius,
							   const Vector4f& aColour) {
		...
		for (size_t i = 0; i < vertices.size(); i++) {
			myVertices.push_back((LineVertex({vertices[i].x, vertices[i].y,
											  vertices[i].z, vertices[i].w},
											 aColour) *
								  aRadius) +
								 aPosition);
	...
	void DebugLine::DrawSphere(const Transform& aTransform, const float aRadius,
							   const Vector4f& aColour) {
		auto vertices = mySphere.GetVertices(KE::SphereLOD::axisSphere);
		auto indices = mySphere.GetIndices(KE::SphereLOD::axisSphere);
	...
	void DebugLine::DrawCone(const Vector3f& aPosition,
		...
		myVertices.push_back(
			LineVertex(aPosition.x, aPosition.y, aPosition.z, 1.0f));
		myIndices++;
		...
		myVertices.push_back(LineVertex(to.position.x + anOuterRadius,
										to.position.y, to.position.z, 1.0f));
		...
		myVertices.push_back(LineVertex(to.position.x, to.position.y,
										to.position.z + anOuterRadius, 1.0f));
		...
		// myIndicesVector.push_back(sourceIndex);
		// myIndicesVector.push_back(static_cast<int>(myVertices.size()));
		// myVertices.push_back(LineVertex(dir.x + anOuterRadius, dir.y, dir.z +
		// anOuterRadius, 1.0f));
	...
	void DebugLine::DrawCapsule(const Vector3f& aPosition, const float aRadius,
								const float aLength, const Vector4f& aColour) {
		...
		for (int i = 0; i < vertices.size(); i++) {
			...
			myVertices.push_back(
				(LineVertex({vertices[i].x, vertices[i].y + heightModifier,
							 vertices[i].z, vertices[i].w},
							aColour) *
				 aRadius) +
				aPosition);
	...
	void DebugLine::DrawCapsule(const Transform& aTransform,
								const float aRadius, const float aLength,
								const Vector4f& aColour) {
		auto mesh = myCapsule.GetMesh();
		...
		for (int i = 0; i < vertices.size(); i++) {
			...
			Vector4f position({vertices[i].x, vertices[i].y + heightModifier,
							   vertices[i].z, vertices[i].w});
	...
	void DebugLine::CreateBuffers() {
		{
			...
			HRESULT result = myDevice->CreateBuffer(
				&vertexBufferDesc, &vertexData, &myVertexBuffer);
			if (FAILED(result)) {
				return;
			}
		}

		{
			...
			D3D11_BUFFER_DESC indexBufferDesc = {};
			indexBufferDesc.ByteWidth =
				sizeof(unsigned int) * (int)myIndicesVector.size();
			indexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
			indexBufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
			D3D11_SUBRESOURCE_DATA indexData = {0};
			indexData.pSysMem = myIndicesVector.data();
			// Create the index buffer.
			HRESULT result = myDevice->CreateBuffer(&indexBufferDesc,
													&indexData, &myIndexBuffer);
			if (FAILED(result)) {
				return;
			}
		}
