...
void KE::RenderTarget::Create() {
	...
	HRESULT hr = myDevice->CreateTexture2D(&textureDesc, nullptr,
	...
	hr = myDevice->CreateRenderTargetView(
		myTexture.Get(), nullptr, myRenderTargetView.ReleaseAndGetAddressOf());
	if (FAILED(hr)) {
	...
	hr = myDevice->CreateShaderResourceView(
		myTexture.Get(), nullptr,
		myShaderResourceView.ReleaseAndGetAddressOf());
	if (FAILED(hr)) {
	...
	hr = myDevice->CreateDepthStencilView(
		myDepthStencilTexture.Get(), nullptr,
...
void KE::RenderTarget::Init(ID3D11Device* aDevice,
...
void KE::RenderTarget::MakeActive(bool aUseDepth,
								  ID3D11DepthStencilView* aDSV) {
	// unbind shader resource views
	// ID3D11ShaderResourceView* nullSRV[1] = { nullptr };
	// myContext->PSSetShaderResources(0, 1, nullSRV);
	if (aDSV) {
		myContext->OMSetRenderTargets(1, myRenderTargetView.GetAddressOf(),
									  aDSV);
		return;
	}
	myContext->OMSetRenderTargets(
		1, myRenderTargetView.GetAddressOf(),
...
void KE::RenderTarget::CopyFrom(RenderTarget* aRenderTarget, bool aCopyDepth) {
	...
	if (aCopyDepth) {
		myContext->CopyResource(myDepthStencilTexture.Get(),
								aRenderTarget->myDepthStencilTexture.Get());
	}
}
...
void KE::RenderTarget::CopyFrom(ID3D11ShaderResourceView* aShaderResourceView) {
	...
	aShaderResourceView->GetResource(
