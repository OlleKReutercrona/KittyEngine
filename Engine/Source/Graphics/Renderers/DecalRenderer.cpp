...
void KE::DecalRenderer::Init(Graphics* aGraphics) {
	...
	myDecalMesh =
		&aGraphics->GetModelLoader().Load("Data/EngineAssets/Cube.fbx");
}

void KE::DecalRenderer::RenderDecal(const DecalRenderInput& aInput,
									const Decal& aDecal) {
	...
	myGraphics->GetContext()->IASetIndexBuffer(mesh.myIndexBuffer.Get(),
											   DXGI_FORMAT_R32_UINT, 0u);

	DecalObjectBufferData bufferData;
	bufferData.objectToWorld = aDecal.myTransform.GetMatrix();
	bufferData.worldToObject =
		DirectX::XMMatrixInverse(nullptr, bufferData.objectToWorld);
	bufferData.textureIntensities = aDecal.myTextureIntensities;

	myDecalBuffer.MapBuffer(&bufferData, sizeof(bufferData),
...
void KE::DecalRenderer::Render(const DecalRenderInput& aInput) {
	...
	graphicsContext->PSSetShader(
		shaderLoader.GetPixelShader(SHADER_LOAD_PATH "Decal_Deferred_PS.cso")
			->GetShader(),
		nullptr, 0u);
	graphicsContext->VSSetShader(
		shaderLoader.GetVertexShader(SHADER_LOAD_PATH "Decal_Deferred_VS.cso")
			->GetShader(),
		nullptr, 0u);
