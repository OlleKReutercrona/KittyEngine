...
namespace KE {
	...
	struct InstancedRenderPackage {
		...
		bool TryAddModelData(size_t aModelDataIndex,
							 const RenderResourceList& aRenderResourceSet) {
			if (aRenderResourceSet.size() != renderResources.size()) {
				return false;
			}

			for (size_t i = 0; i < aRenderResourceSet.size(); ++i) {
				if (aRenderResourceSet[i].myVertexShader !=
						renderResources[i].myVertexShader ||
					aRenderResourceSet[i].myPixelShader !=
						renderResources[i].myPixelShader ||
					aRenderResourceSet[i].myMaterial !=
						renderResources[i].myMaterial) {
					return false;
				}
			}
	...
	struct InstancedRenderData {
		...
		InstancedRenderData(const DirectX::XMMATRIX& aTransform)
			: myTransform(aTransform), attributes({0, 0, 1, 1}) {}
	};
	...
	private:
		...
		void RenderPackage(const InstancedRenderPackage& aPackage,
						   const InstanceRenderInput& aInput);

	public:
		void Init(Graphics* aGraphics);

		void SortRenderPackages(InstancedRenderPackageList* aPackageList);
		void GenerateInstancingData(
