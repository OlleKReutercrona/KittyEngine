...
void KE::SkeletalRenderer::Render(const SkeletalRenderInput& aInput) {
	...
	const SkeletalModelDataList& modelDatas =
		myGraphics->GetSkeletalModelData();
	for (const size_t& index : indices) {
		if (!modelDatas[index].myActiveStatus) {
			continue;
		}
...
void KE::SkeletalRenderer::RenderSkeletalModel(
	const SkeletalRenderInput& aInput, const SkeletalModelData& aModelData) {
	...
	for (unsigned int i = 0; i < (unsigned int)meshList->myMeshes.size(); i++) {
		...
		if (resource.myCBuffer) {
			if (resource.myCBufferVSSlot >= 0) {
				resource.myCBuffer->BindForVS(resource.myCBufferVSSlot,
											  graphicsContext.Get());
			}

			if (resource.myCBufferPSSlot >= 0) {
				resource.myCBuffer->BindForPS(resource.myCBufferPSSlot,
											  graphicsContext.Get());
			}
		}
		...
		graphicsContext->IASetVertexBuffers(
		...
		objectData.objectToClip = objectData.objectToWorld * aInput.viewMatrix *
								  aInput.projectionMatrix;

		myObjectBuffer.MapBuffer((void*)&objectData, sizeof(objectData),
								 graphicsContext.Get());
		myObjectBuffer.BindForVS(1, graphicsContext.Get());

		myAnimationBuffer.MapBuffer(
			(void*)aModelData.myFinalTransforms.data(),
			sizeof(*aModelData.myFinalTransforms.data()) *
				static_cast<int>(aModelData.myFinalTransforms.size()),
			graphicsContext.Get());
