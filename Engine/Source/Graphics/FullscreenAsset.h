...
namespace KE {
	...
	private:
		...
		void MakeUVTransform(Graphics* aGraphics, Vector2f aUVMin,
							 Vector2f aUVMax);
	...
	public:
		...
		inline void AssignTexture(Texture* aTexture) {
			myTexture = aTexture;
		}
		void Render(Graphics* aGraphics, const Vector2f& uvMin = {0.0f, 0.0f},
					const Vector2f& uvMax = {1.0f, 1.0f});
		void Render(Graphics* aGraphics,
					ID3D11ShaderResourceView* aShaderResourceView,
					VertexShader* aVS = nullptr, PixelShader* aPS = nullptr,
					const Vector2f& uvMin = {0.0f, 0.0f},
					const Vector2f& uvMax = {1.0f, 1.0f});
		void RenderWithoutSRV(Graphics* aGraphics, VertexShader* aVS = nullptr,
