...
namespace KE {
	...
	public:
		...
		void InitShadowCamera(const Vector2i aSize, const float aNear,
							  const float aFar);
		HRESULT Init(ID3D11Device* aDevice, ShaderLoader* aShaderLoader,
		...
		// This is a dangerous function. If a Vector of LightData resizes will
		// cause the pointers to break
	...
	private:
		...
		std::string myDirectionalLightPSName =
			"DeferredDirectionalLight_PS.cso";
