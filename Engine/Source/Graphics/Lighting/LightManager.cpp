...
namespace KE {
	LightManager::LightManager(ID3D11Device* aDevice, const UINT aSlot)
		...
		const HRESULT result =
			aDevice->CreateBuffer(&cbd, &csd, &myConstantBuffer);
		if (FAILED(result)) {
	...
	void LightManager::UpdateBuffer(ID3D11DeviceContext* aContext) {
		for (const LightComponentData& lightComponentData :
			 myLightComponentData) {
			...
			switch (lightComponentData.myLightType) {
				case eLightType::Directional: {
					const DirectionalLightData* data =
						static_cast<DirectionalLightData*>(myLightData[index]);
					myLightBuffer.myDirectionalLightData.myDirection =
						data->myDirection;
					myLightBuffer.myDirectionalLightData.myColour =
						data->myColour;
					myLightBuffer.myDirectionalLightData
						.myDirectionalLightIntensity =
						data->myDirectionalLightIntensity;
					myLightBuffer.myDirectionalLightData
						.myAmbientLightIntensity =
						data->myAmbientLightIntensity;
				} break;
				case eLightType::Point: {
					const PointLightData* data =
						static_cast<PointLightData*>(myLightData[index]);
					myLightBuffer.myPointLightData[typeIndex].myPosition =
						data->myPosition;
					myLightBuffer.myPointLightData[typeIndex].myColour =
						data->myColour;
					myLightBuffer.myPointLightData[typeIndex].myIntensity =
						data->myIntensity;
					myLightBuffer.myPointLightData[typeIndex].myRange =
						data->myRange;
					myLightBuffer.myPointLightData[typeIndex].isActive =
						data->isActive;
				} break;
				case eLightType::Spot: {
					const SpotLightData* data =
						static_cast<SpotLightData*>(myLightData[index]);
					myLightBuffer.mySpotLightData[typeIndex].myPosition =
						data->myPosition;
					myLightBuffer.mySpotLightData[typeIndex].myDirection =
						data->myDirection;
					myLightBuffer.mySpotLightData[typeIndex].myColour =
						data->myColour;
					myLightBuffer.mySpotLightData[typeIndex].myIntensity =
						data->myIntensity;
					myLightBuffer.mySpotLightData[typeIndex].myRange =
						data->myRange;
					myLightBuffer.mySpotLightData[typeIndex].myInnerAngle =
						data->myInnerAngle;
					myLightBuffer.mySpotLightData[typeIndex].myOuterAngle =
						data->myOuterAngle;
					myLightBuffer.mySpotLightData[typeIndex].isActive =
						data->isActive;
				} break;
				default:;
			}
		...
		aContext->Map(myConstantBuffer.Get(), 0u, D3D11_MAP_WRITE_DISCARD, 0u,
					  &msr);
	...
	void LightManager::BindBuffer(ID3D11DeviceContext* aContext) {
		...
		aContext->PSSetConstantBuffers(mySlot, 1u,
									   myConstantBuffer.GetAddressOf());
	}

	LightComponentData& LightManager::CreateLightData(
		const eLightType aLightType) {
		const UINT index = static_cast<UINT>(myLightComponentData.size());
		myLightComponentData.emplace_back();
#ifdef _DEBUG
		assert(index <= MAX_LIGHT_VECTOR_CAPACITY &&
			   "Light vector had to resize beyond max lights");
#endif
		...
		switch (aLightType) {
			case eLightType::Directional: {
				...
				myLightComponentData.back().myLightTypeName =
					"Directional Light";
				myLightComponentData.back().myIndex = index;
				myLightComponentData.back().myTypeIndex =
