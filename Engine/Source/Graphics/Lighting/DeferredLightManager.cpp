...
namespace KE {
	...
	void DeferredLightManager::InitShadowCamera(const Vector2i aSize,
												const float aNear,
												const float aFar) {
		//												aLeft,  aRight, aTop,
		// aBottom, aNear, aFar
	...
	HRESULT DeferredLightManager::Init(ID3D11Device* aDevice,
									   ShaderLoader* aShaderLoader,
									   const Vector2i aSize) {
		myLightVS =
			aShaderLoader->GetVertexShader(SHADER_LOAD_PATH + myLightVSName);

		myDirectionalLightPS = aShaderLoader->GetPixelShader(
			SHADER_LOAD_PATH + myDirectionalLightPSName);
		myPointLightPS = aShaderLoader->GetPixelShader(SHADER_LOAD_PATH +
													   myPointLightPSName);
		mySpotLightPS =
			aShaderLoader->GetPixelShader(SHADER_LOAD_PATH + mySpotLightPSName);
		...
		// ------ Initiate Meshes ------ //
		{
			// Quad
			{
				...
				HRESULT result;
				{
					...
					vertexBufferDesc.ByteWidth =
						sizeof(Vertex) * (int)myQuad.myVertices.size();
					...
					result = aDevice->CreateBuffer(
						&vertexBufferDesc, &vertexData, &myQuad.myVertexBuffer);
					if (FAILED(result)) {
						return result;
					}
				}

				{
					...
					indexBufferDesc.ByteWidth =
						sizeof(unsigned int) * (int)myQuad.myIndices.size();
			...
			// Sphere
			{
				...
				for (size_t i = 0; i < vertices.size(); i++) {
					// This is ASS, primitives should use the same vertex type
					// as Mesh - Olle
					Vertex v;
					v.x = vertices[i].x;
					v.y = vertices[i].y;
				...
				HRESULT result;
				{
					...
					D3D11_BUFFER_DESC vertexBufferDesc = {};
					vertexBufferDesc.ByteWidth =
						sizeof(Vertex) * (int)mySphere.myVertices.size();
					...
					result =
						aDevice->CreateBuffer(&vertexBufferDesc, &vertexData,
											  &mySphere.myVertexBuffer);
					if (FAILED(result)) {
						return result;
					}
				}

				{
					...
					indexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
					indexBufferDesc.ByteWidth =
						sizeof(unsigned int) * (int)mySphere.myIndices.size();
					indexBufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
					D3D11_SUBRESOURCE_DATA indexData = {0};
	...
	void DeferredLightManager::PrepareShadowPass(Graphics* aGraphics) {
		...
		aGraphics->SetProjection(
			myDirectionalLightCamera.GetProjectionMatrix());
		myDirectionalLightCamera.transform.SetDirection(
			{-myDirectionalLight.myDirection.x,
			 -myDirectionalLight.myDirection.y,
			 -myDirectionalLight.myDirection.z});
	...
	int DeferredLightManager::Render(Graphics* aGraphics) {
		...
		context->IASetVertexBuffers(
			0u, 1u, myQuad.myVertexBuffer.GetAddressOf(), &stride, &offset);
		context->IASetIndexBuffer(myQuad.myIndexBuffer.Get(),
		...
		// ------ Directional Light ------ //
		{
			...
			myTransformBuffer.MapBuffer(&transformBuffer,
										sizeof(ConstantBuffer), context.Get());
			myTransformBuffer.BindForVS(DEFERRED_LIGHT_BUFFER_SLOT,
										context.Get());

			// myDirectionalLight.myDirectionalLightCamera =
			// myDirectionalLightCamera.transform.GetMatrix();
			myDirectionalLight.myDirectionalLightCamera =
				myDirectionalLightCamera.GetViewMatrix() *
				myDirectionalLightCamera.GetProjectionMatrix();
			myDirectionalLightBuffer.MapBuffer(&myDirectionalLight,
											   sizeof(DirectionalLightData),
											   context.Get());
			myDirectionalLightBuffer.BindForPS(DEFERRED_LIGHT_BUFFER_SLOT,
											   context.Get());

			// Set Pixel Shader
			context->PSSetShader(myDirectionalLightPS->GetShader(), nullptr,
								 0u);
		...
		for (PointLightData lightData : myPointLights) {
			...
			myPointLightBuffer.MapBuffer(&lightData, sizeof(PointLightData),
										 context.Get());
			myPointLightBuffer.BindForPS(DEFERRED_LIGHT_BUFFER_SLOT,
										 context.Get());

			// Draw Mesh
			context->IASetIndexBuffer(mySphere.myIndexBuffer.Get(),
									  DXGI_FORMAT_R32_UINT, 0u);
			context->IASetVertexBuffers(0u, 1u,
										mySphere.myVertexBuffer.GetAddressOf(),
										&stride, &offset);
		...
		for (SpotLightData lightData : mySpotLights) {
			...
			mySpotLightbuffer.MapBuffer(&lightData, sizeof(SpotLightData),
										context.Get());
	...
	// TODO Importer needs to recieve a LightData& instead of a
	// LightComponentData&
	LightData* DeferredLightManager::CreateLightData(
		const eLightType aLightType) {
		switch (aLightType) {
	...
	void DeferredLightManager::RemoveLightData(eLightType aType,
	...
	void DeferredLightManager::AssignCubemap(Cubemap* aCubemap) {
		// myCubemap =
		// aTextureLoader->GetCubemapFromPath("Data/Assets/Materials/CubeMaps/Cubemap_test.dds");
		myCubemap = aCubemap;
	}
}  // namespace KE
