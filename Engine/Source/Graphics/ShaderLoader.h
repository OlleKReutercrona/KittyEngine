...
namespace KE {
	...
	public:
		...
		VertexShader* CreateVertexShaderFromParsedScript(
			const std::string& aShaderName, const std::string& aParsedCode);
		VertexShader* CreateVertexShaderFromScript(
			const std::string& aShaderName, Script* aShaderScript);

		PixelShader* CreatePixelShaderFromParsedScript(
			const std::string& aShaderName, const std::string& aParsedCode,
			const std::string& aEntryName = "main");
		PixelShader* CreatePixelShaderFromScript(
			const std::string& aShaderName, Script* aShaderScript,
			const std::string& aEntryName = "main");
		...
		const std::unordered_map<std::string, VertexShader>& GetVertexShaders()
			const {
			return myVertexShaders;
		}
		const std::unordered_map<std::string, PixelShader>& GetPixelShaders()
			const {
			return myPixelShaders;
		}
		const std::unordered_map<std::string, ComputeShader>&
		GetComputeShaders() const {
			return myComputeShaders;
		}
		...
		ID3D11VertexShader* CreateVertexShaderFromFile(
			const wchar_t* aFilePath);
		ID3D11PixelShader* CreatePixelShaderFromFile(const wchar_t* aFilePath);
		ID3D11ComputeShader* CreateComputeShaderFromFile(
			const wchar_t* aFilePath);

		ID3D11PixelShader* CreatePixelShaderFromCode(
			const std::string& aShaderCode,
			const std::string& aEntryName = "main");

		inline VertexShader* GetDefaultVertexShader() {
			return GetVertexShader(DEFAULT_VERTEX_SHADER);
		}
		inline PixelShader* GetDefaultPixelShader() {
			return GetPixelShader(DEFAULT_PIXEL_SHADER);
		}
