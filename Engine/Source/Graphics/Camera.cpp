...
void KE::Camera::SetFOV(const float aFOV) {
	SetPerspective(myProjectionData.perspective.width,
				   myProjectionData.perspective.height, aFOV,
				   myProjectionData.perspective.nearPlane,
				   myProjectionData.perspective.farPlane);
}

void KE::Camera::SetPerspective(float aWidth, float aHeight, float aFov,
	...
	// TODO This should be set from one and only one place, not in
	// Game/Cameramanager/Component etc...
	myProjectionData.perspective.farPlane = aFarPlane;
	myType = ProjectionType::Perspective;

	// our fov is horiontal, so we need to calculate the vertical fov to use in
	...
	myProjectionMatrix = DirectX::XMMatrixPerspectiveFovLH(
		verticalFov, aWidth / aHeight, aNearPlane, aFarPlane);
}

void KE::Camera::SetOrthographic(float aWidth, float aHeight, float aNearPlane,
								 float aFarPlane) {
	...
	myProjectionMatrix =
		DirectX::XMMatrixOrthographicLH(aWidth, aHeight, aNearPlane, aFarPlane);
}
...
Rayf KE::Camera::ScreenPointToRay(const Vector2f& aMousePosition) {
	...
	Vector3f dir =
		forward + mousePosDiv.x * rightScaled + mousePosDiv.y * upScaled;
	dir.Normalize();
...
bool KE::Camera::WorldToScreenPoint(const Vector3f& aWorldPoint,
									Vector2f& aOutScreenPoint,
									Vector2f aScreenDimensions) {
	Vector4f positonClip = {aWorldPoint.x, aWorldPoint.y, aWorldPoint.z, 1.0f};

	positonClip =
		positonClip * Matrix4x4f(GetViewMatrix() * GetProjectionMatrix());
	...
	if (aScreenDimensions.x <= 0.0f || aScreenDimensions.y <= 0.0f) {
		aScreenDimensions = {myProjectionData.perspective.width,
							 myProjectionData.perspective.height};
	}

	aOutScreenPoint.x = (positonClip.x + 1.0f) * 0.5f * aScreenDimensions.x;
	aOutScreenPoint.y = aScreenDimensions.y -
						(positonClip.y + 1.0f) * 0.5f * aScreenDimensions.y;

	if (aOutScreenPoint.x < 0.0f || aOutScreenPoint.x > aScreenDimensions.x ||
		aOutScreenPoint.y < 0.0f || aOutScreenPoint.y > aScreenDimensions.y) {
		return false;
	}
...
// bool KE::Camera::NDCtoWorldSpace(const Vector2f& aPoint, OUT Vector3f&
// aOutWP)
//{
