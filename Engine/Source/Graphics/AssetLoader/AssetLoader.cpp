...
void KE::AssetLoader::CopyAssets() {
	...
	if (std::filesystem::exists("../Engine/EngineAssets")) {
		Timer engineTimer;
		std::filesystem::copy("../Engine/EngineAssets", "Data/EngineAssets",
							  options);
	...
	if (std::filesystem::exists("../Engine/EngineAssets")) {
		Timer editorTimer;
		std::filesystem::copy("../Editor/EditorAssets", "Data/EditorAssets",
							  options);
	...
	if (std::filesystem::exists("../Engine/EngineAssets")) {
		Timer projectTimer;
		std::filesystem::copy("../Project/ProjectAssets", "Data/ProjectAssets",
							  options);
...
void KE::AssetLoader::TextureLoad() {
	...
	for (auto& entry : std::filesystem::recursive_directory_iterator("Data")) {
		...
		if (fileName.extension() == ".dds" || fileName.extension() == ".DDS" ||
			fileName.extension() == ".png" || fileName.extension() == ".PNG") {
			texLoader.GetTextureFromPath(path.string());
			std::cout << "TextureLoader thread loaded: " << path.string()
					  << std::endl;
		}
	}

	std::cout << "TextureLoader thread finished in " << timer.UpdateDeltaTime()
			  << " seconds (?)" << std::endl;
}
...
void KE::AssetLoader::ModelLoad() {
	...
	std::cout << "ModelLoader thread finished in " << timer.UpdateDeltaTime()
			  << " seconds (?)" << std::endl;
}
