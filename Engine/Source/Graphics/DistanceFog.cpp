...
void KE::DistanceFog::Init(ID3D11Device* aDevice, ID3D11DeviceContext* aContext,
						   const int aWidth, const int aHeight,
						   PixelShader* aPS, VertexShader* aVS) {
	...
	ADD_LAMBDA_WINDOW("Distance Fog",
					  std::bind(&KE::DistanceFog::DrawImGUI, this));
}
...
void KE::DistanceFog::BindResources(RenderTarget* aRTToCopy,
...
void KE::DistanceFog::Render(KE::Graphics* aGraphics,
							 KE::FullscreenAsset& aFullscreenAssetToRender) {
	// DrawImGUI();

	aFullscreenAssetToRender.RenderWithoutSRV(aGraphics, myVertexShader,
											  myPixelShader);
...
void KE::DistanceFog::DrawImGUI() {
	...
	ImGui::DragFloat("Max Distance", &myDistFogData.maxDistance, 0.1f, 0.0f,
					 100.0f);
	ImGui::DragFloat("Water Height", &myDistFogData.waterheight, 0.1f, -25.0f,
					 25.0f);
	ImGui::DragFloat("Fog Distance", &myDistFogData.fadeDistance, 0.1f, 0.0f,
					 100.0f);
