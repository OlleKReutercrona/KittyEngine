...
namespace KE {
	...
	struct Mesh {
		...
		inline unsigned int GetIndexCount() const {
			return (unsigned int)myIndices.size();
		}
	...
	struct ModelData  // Final data blob that graphics engine uses to render a
					  // mesh/modelcomponent :)
	{
		...
		// this is kinda evil, but everything inside render resources is a
		// pointer, so we can just memset it to 0.. i think
		~ModelData() {
			memset(&myRenderResources, 0, sizeof(myRenderResources));
		}
