...
void KE::DeferredRenderTarget::Create() {
	...
	textureDesc.Format =
		DXGI_FORMAT_R8G8B8A8_UNORM;	 // TODO: make this a parameter
	...
	for (int i = 0; i < KE_DEFERRED_RENDER_TARGET_COUNT; i++) {
		hr = myDevice->CreateTexture2D(&textureDesc, nullptr,
									   myTextures[i].GetAddressOf());
		if (FAILED(hr)) {
		...
		hr = myDevice->CreateRenderTargetView(
			myTextures[i].Get(), nullptr,
			myRenderTargetViews[i].GetAddressOf());
		if (FAILED(hr)) {
			KE_ERROR(
				"Failed to create render target view for render target, step "
				"%i.",
				i);
			return;
		}
		...
		hr = myDevice->CreateShaderResourceView(
			myTextures[i].Get(), nullptr,
			myShaderResourceViews[i].GetAddressOf());
		if (FAILED(hr)) {
			KE_ERROR(
				"Failed to create shader resource view for render target, step "
				"%i.",
				i);
			return;
		}
	...
	hr = myDevice->CreateTexture2D(&depthStencilDesc, nullptr,
								   myDepthStencilTexture.GetAddressOf());
	if (FAILED(hr)) {
	...
	depthStencilViewDesc.Format =
		DXGI_FORMAT_D32_FLOAT;	// TODO: make this a parameter
	depthStencilViewDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
	depthStencilViewDesc.Texture2D.MipSlice = 0;

	hr = myDevice->CreateDepthStencilView(myDepthStencilTexture.Get(),
	...
	hr = myDevice->CreateShaderResourceView(
		...
		myDepthStencilShaderResourceView.GetAddressOf());
	if (FAILED(hr)) {
...
KE::DeferredRenderTarget::DeferredRenderTarget(ID3D11Device* aDevice,
...
void KE::DeferredRenderTarget::MakeActive(bool aUseDepth) {
	...
	ID3D11RenderTargetView* renderTargets[KE_DEFERRED_RENDER_TARGET_COUNT] = {
		myRenderTargetViews[0].Get(), myRenderTargetViews[1].Get(),
		myRenderTargetViews[2].Get(), myRenderTargetViews[3].Get(),
		myRenderTargetViews[4].Get(),

	};

	myContext->OMSetRenderTargets(
		KE_DEFERRED_RENDER_TARGET_COUNT, renderTargets,
...
void KE::DeferredRenderTarget::Clear(float* aColour, bool aClearDepth) {
	if (aColour) {
		for (int i = 0; i < KE_DEFERRED_RENDER_TARGET_COUNT; i++) {
			myContext->ClearRenderTargetView(myRenderTargetViews[i].Get(),
											 aColour);
		}
	}

	if (aClearDepth) {
		myContext->ClearDepthStencilView(myDepthStencilView.Get(),
