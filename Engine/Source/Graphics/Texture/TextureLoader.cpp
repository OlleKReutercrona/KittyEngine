...
namespace KE {
	...
	void TextureLoader::Init(ID3D11Device* aDevice,
	...
	bool TextureLoader::SaveTextureToFile(const std::string& aOutputFilePath,
										  ID3D11ShaderResourceView* aTexture,
										  unsigned int aWidth,
										  unsigned int aHeight, Vector2f aUVMin,
										  Vector2f aUVMax) const {
		DirectX::Image img;
		...
		ComPtr<ID3D11Texture2D> stagingTexture = nullptr;
		{
			...
			myDevice->CreateTexture2D(&desc, nullptr,
		...
		ComPtr<ID3D11RenderTargetView> intermediateRT = nullptr;
		{
			...
			myDevice->CreateRenderTargetView(
				intermediateTexture.Get(), nullptr,
				intermediateRT.ReleaseAndGetAddressOf());
		}

		myContext->OMSetRenderTargets(1, intermediateRT.GetAddressOf(),
									  nullptr);
		...
		fs.Render(gfx, aTexture,
				  gfx->GetShaderLoader().GetVertexShader(
					  SHADER_LOAD_PATH "FullscreenAsset_VS.cso"),
				  gfx->GetShaderLoader().GetPixelShader(
		...
		myContext->CopyResource(stagingTexture.Get(),
								intermediateTexture.Get());
		myContext->Flush();

		// copy the pixels from the staging texture (a D3D11Texture2D) to
		// img.pixels
		D3D11_MAPPED_SUBRESOURCE mappedResource = {0};

		myContext->Map(stagingTexture.Get(), 0, D3D11_MAP_READ, 0,
					   &mappedResource);
		memcpy(img.pixels, mappedResource.pData, img.slicePitch);
		myContext->Unmap(stagingTexture.Get(), 0);

		std::wstring filePathW =
			std::wstring(aOutputFilePath.begin(), aOutputFilePath.end());

		// check if file is read only
		if (std::filesystem::exists(aOutputFilePath) &&
			std::filesystem::is_regular_file(aOutputFilePath) &&
			std::filesystem::status(aOutputFilePath).permissions() ==
				std::filesystem::perms::_File_attribute_readonly) {
			std::filesystem::permissions(
				aOutputFilePath, std::filesystem::perms::owner_write,
				std::filesystem::perm_options::replace);
		}

		HRESULT hr = DirectX::SaveToDDSFile(img, DirectX::DDS_FLAGS_NONE,
											filePathW.c_str());
		if (FAILED(hr)) {
			return false;
		}
	...
	Texture* TextureLoader::GetDefaultTexture(int anIndex) {
		switch (anIndex) {
			...
			default:
				// KE_WARNING("Invalid index for default texture request: %i",
				// anIndex);
				return &loadedTextures[myDefaultTexture_c];
		}
	}
	...
	bool TextureLoader::InitializeTexture(Texture* aTexture) {
		...
		if (filePath.find(".dds") != std::string::npos ||
			...
			hr = DirectX::CreateDDSTextureFromFileEx(
				myDevice, nullptr, wPath.c_str(), 0, D3D11_USAGE_DEFAULT,
				D3D11_BIND_SHADER_RESOURCE, /*D3D11_BIND_SHADER_RESOURCE |
											   D3D11_BIND_RENDER_TARGET*/
				0, 0, DirectX::DDS_LOADER_IGNORE_SRGB,
			...
			if (desc.ArraySize > 1) {
				std::cout << "Array size was greater than 0 for " << filePath
						  << std::endl;
			}
			...
			if (srvDesc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURECUBE) {
				aTexture->myMetadata.myTextureFormat =
					TextureFormat::TextureCube;
			}
			...
			hr = LoadFromWICFile(wPath.c_str(), DirectX::WIC_FLAGS_IGNORE_SRGB,
								 &metadata, image);
			...
			hr = CreateShaderResourceView(
				myDevice, image.GetImages(), image.GetImageCount(), metadata,
				aTexture->myShaderResourceView.ReleaseAndGetAddressOf());
		}
	...
	Texture* TextureLoader::GetTextureFromPath(const std::string& aFilePath,
											   bool loggingEnabled,
											   bool alwaysRegenerate) {
		// replace backslashes with forward slashes
		std::string reformattedString =
			std::filesystem::path(aFilePath).string();
		...
		workingTexture->myMetadata.myFileName =
			reformattedString.substr(reformattedString.find_last_of("/") + 1);
		workingTexture->myMetadata.failed = false;

		if (!InitializeTexture(workingTexture)) {
			if (loggingEnabled) {
				KE_LOG_CHANNEL(
					"TextureLoader", "Failed to load texture %s",
					reformattedString.c_str());	 // TODO: Add more info to this
												 // error message.
			}

			workingTexture->myShaderResourceView =
				GetDefaultTexture((int)workingTexture->myMetadata.myTextureType)
	...
	Texture* TextureLoader::CreateTextureFromData(
		const std::string& aTextureName, const unsigned char* aData,
		const int aWidth, const int aHeight, bool alwaysRegenerate) {
		...
		hr = myDevice->CreateShaderResourceView(
			tex.Get(), nullptr,
			workingTexture.myShaderResourceView.ReleaseAndGetAddressOf());
		...
		// Build the texture path by adding suffix to the FBX file name
		{
			for (int i = 0; i < PBR_TEXTURES; ++i) {
				std::string texturePath =
					"Data/Assets/Materials/" + aFilePath + SUFFIXES[i];

				workingMaterial->myTextures[i] =
					GetTextureFromPath(texturePath, i < 3);
				if (workingMaterial->myTextures[i] == GetDefaultTexture()) {
					// KE_ERROR("Failed to load texture %s",
					// texturePath.c_str()); // TODO: Add more info to this
					// error message.
	...
	Cubemap* TextureLoader::GetCubemapFromPath(const std::string& aFilePath) {
		...
		if (FAILED(hr)) {
			KE_ERROR(
				...
				aFilePath
					.c_str());	// TODO: Add more info to this error message.
			return nullptr;
		}

		hr = CreateShaderResourceView(myDevice, image.GetImages(),
									  image.GetImageCount(), metadata,
									  &workingCubemap->myShaderResourceView);

		if (FAILED(hr)) {
			KE_ERROR(
				"Failed to create shader resource view.");	// TODO: Add more
															// info to this
															// error message.
			return nullptr;
		}
	...
	Material* TextureLoader::GetCustomMaterial(const std::string& aAlbedoPath,
											   const std::string& aNormalPath,
											   const std::string& aMaterialPath,
											   const std::string& aFXPath) {
		const std::string aName =
			aAlbedoPath + aNormalPath + aMaterialPath + aFXPath;
	...
	DataMaterial* TextureLoader::CreateDataMaterialTextures(
		...
		std::string normalTexName =
			workingDataMaterial.myName + "_n.dds";	// not sure we need this.
		...
		unsigned char
			texureDataArray[textureDimensions * textureDimensions * 4];
		// heap allocate instead
		// unsigned char* texureDataArray = new unsigned char[textureDimensions
		// * textureDimensions * 4];
		...
		{  // ALBEO MAP
			...
			if (aDataMaterial.albedoMap.empty()) {
				// albedo tex
				for (int i = 0; i < textureDimensions * textureDimensions * 4;
					 i += 4) {
					texureDataArray[i + 0] =
						(unsigned char)(workingDataMaterial.albedo.x * 255);
					texureDataArray[i + 1] =
						(unsigned char)(workingDataMaterial.albedo.y * 255);
					texureDataArray[i + 2] =
						(unsigned char)(workingDataMaterial.albedo.z * 255);
					texureDataArray[i + 3] = 255;
				}
				workingDataMaterial.myTextures[0] = CreateTextureFromData(
					albedoTexName, texureDataArray, textureDimensions,
					textureDimensions, true);
			} else {
				workingDataMaterial.myTextures[0] =
					GetTextureFromPath(aDataMaterial.albedoMap);
			}
		}
		...
		{  // NORMAL MAP
			...
			if (!aDataMaterial.normalMap.empty()) {
				workingDataMaterial.myTextures[1] =
					GetTextureFromPath(aDataMaterial.normalMap);
			...
			if (!found) {
				...
				workingDataMaterial.myTextures[1] = CreateTextureFromData(
					normalTexName, texureDataArray, textureDimensions,
					textureDimensions, true);
			}
		}
		...
		{  // MATERIAL MAP
			...
			if (!aDataMaterial.materialMap.empty()) {
				workingDataMaterial.myTextures[2] =
					GetTextureFromPath(aDataMaterial.materialMap);
				found = !workingDataMaterial.myTextures[2]->myMetadata.failed;
			}

			if (!found) {
				// material tex, ao (unused for now) in r, roughness in g,
				// metallic in b
				for (int i = 0; i < textureDimensions * textureDimensions * 4;
					 i += 4) {
					texureDataArray[i + 0] = 0;
					texureDataArray[i + 1] =
						(unsigned char)(workingDataMaterial.roughness * 255);
					texureDataArray[i + 2] =
						(unsigned char)(workingDataMaterial.metallic * 255);
					texureDataArray[i + 3] = 255;
				}
				workingDataMaterial.myTextures[2] = CreateTextureFromData(
					materialTexName, texureDataArray, textureDimensions,
					textureDimensions, true);
			}
		}
		...
		{  // FX MAP
			...
			if (!aDataMaterial.effectsMap.empty()) {
				workingDataMaterial.myTextures[3] =
					GetTextureFromPath(aDataMaterial.effectsMap);
			...
			if (!found) {
				...
				for (int i = 0; i < textureDimensions * textureDimensions * 4;
					 i += 4) {
					texureDataArray[i + 0] =
						(unsigned char)(workingDataMaterial.emission * 255);
					texureDataArray[i + 1] = 0;
					texureDataArray[i + 2] = 0;
					texureDataArray[i + 3] = 255;
				}
				workingDataMaterial.myTextures[3] = CreateTextureFromData(
					fxTexName, texureDataArray, textureDimensions,
					textureDimensions, true);
			}
		}
	...
	DataMaterial* TextureLoader::LoadDataMaterial(const std::string& aFilePath,
												  bool alwaysRegenerate) {
		if (!alwaysRegenerate &&
			loadedDataMaterials.find(aFilePath) != loadedDataMaterials.end()) {
		...
		workingDataMaterial.albedo =
			Vector3f(jsonMaterial["albedo"]["r"].get<float>(),
	...
	void TextureLoader::SaveDataMaterial(const std::string& aFilePath,
										 const DataMaterial& aDataMaterial) {
		using nlohmann::json;
	...
	DataMaterial* TextureLoader::GetDataMaterial(
		const DataMaterial& aDataMaterial) {
	...
	void TextureLoader::RenderThumbnail(KE::Material* aMaterial,
		...
		modelData.myMeshList =
			&modelLoader.Load("Data/EngineAssets/SmoothSphere.fbx");

		modelData.myRenderResources.emplace_back();

		modelData.myRenderResources[0].myVertexShader =
			shaderLoader.GetVertexShader(SHADER_LOAD_PATH
										 "Model_Deferred_VS.cso");
		modelData.myRenderResources[0].myPixelShader =
			shaderLoader.GetPixelShader(SHADER_LOAD_PATH
										"Model_Deferred_PS.cso");
		...
		renderCamera.transform.SetRotation(
			{0.0f, angle * KE::DegToRadImmediate, 0.0f});
		renderCamera.transform.TranslateLocal({0.0f, 0.0f, -1.0f});
		// Vector3f horizAngle = { sin(angle * KE::DegToRadImmediate), 0.0f,
		// cos(angle * KE::DegToRadImmediate)};
		...
		buf.SetAsActiveTarget(gfx->GetContext().Get(),
							  buf.GetDepthStencilView());
		...
		rr.myVertexShader =
			shaderLoader.GetVertexShader(SHADER_LOAD_PATH "Skybox_VS.cso");
		rr.myPixelShader = shaderLoader.GetPixelShader(SHADER_LOAD_PATH
													   "Skybox_Preview_PS.cso");
		...
		KE::Cubemap* myCubemap = textureLoader.GetCubemapFromPath(
		...
		textureLoader.SaveTextureToFile(aThumbnailPath,
										rt->GetShaderResourceView(), 512, 512,
										uvMin, uvMax);

		gfx->SetViewport(rX, rY);
	}

	Texture* TextureLoader::ProjectCubemap(const std::string& aCubemapPath,
										   CubemapProjectionType aProjection) {
		...
		switch (aProjection) {
			case CubemapProjectionType::Equirectangular: {
				...
				gfx->GetFullscreenAsset()->Render(
					gfx, map->myShaderResourceView.Get(),
					gfx->GetShaderLoader().GetVertexShader(
						SHADER_LOAD_PATH "FullscreenAsset_VS.cso"),
					gfx->GetShaderLoader().GetPixelShader(
						SHADER_LOAD_PATH "CubemapProjection_PS.cso"),
					{0.0f, 0.0f}, {1.0f, 1.0f});
				...
				SaveTextureToFile("testOut.dds", rt->GetShaderResourceView(),
								  512, 512, uvMin, uvMax);
	...
	bool TextureLoader::ReloadTexture(Texture* aTexture) {
		if (aTexture == nullptr) {
			KE_LOG_CHANNEL("TextureLoader",
						   "ReloadTexture called with a nullptr texture.");
			return false;
		}
