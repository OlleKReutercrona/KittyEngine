...
class Transform {
public:
	Transform(Vector3f aPosition = Vector3f(0.0f, 0.0f, 0.0f),
	...
	Transform(const physx::PxTransform& apxTransform) {
		myMatrix = DirectX::XMMatrixRotationQuaternion(
			{apxTransform.q.x, apxTransform.q.y, apxTransform.q.z,
			 apxTransform.q.w});
	...
	operator physx::PxTransform() const {
		...
		return physx::PxTransform(pos.x, pos.y, pos.z,
								  {quat.m128_f32[0], quat.m128_f32[1],
								   quat.m128_f32[2], quat.m128_f32[3]});
	}

	inline void SetMatrix(const DirectX::XMMATRIX& aMatrix) {
		myMatrix = aMatrix;
	};
	...
	inline Matrix4x4f GetCUMatrix() const {
		return Matrix4x4f(myMatrix);
	};
	...
	bool operator!=(const Transform& aTransform) {
		return myMatrix.r[0].m128_f32[0] !=
				   aTransform.myMatrix.r[0].m128_f32[0] ||
			   myMatrix.r[0].m128_f32[1] !=
				   aTransform.myMatrix.r[0].m128_f32[1] ||
			   myMatrix.r[0].m128_f32[2] !=
				   aTransform.myMatrix.r[0].m128_f32[2] ||
			   myMatrix.r[0].m128_f32[3] !=
				   aTransform.myMatrix.r[0].m128_f32[3] ||
			   myMatrix.r[1].m128_f32[0] !=
				   aTransform.myMatrix.r[1].m128_f32[0] ||
			   myMatrix.r[1].m128_f32[1] !=
				   aTransform.myMatrix.r[1].m128_f32[1] ||
			   myMatrix.r[1].m128_f32[2] !=
				   aTransform.myMatrix.r[1].m128_f32[2] ||
			   myMatrix.r[1].m128_f32[3] !=
				   aTransform.myMatrix.r[1].m128_f32[3] ||
			   myMatrix.r[2].m128_f32[0] !=
				   aTransform.myMatrix.r[2].m128_f32[0] ||
			   myMatrix.r[2].m128_f32[1] !=
				   aTransform.myMatrix.r[2].m128_f32[1] ||
			   myMatrix.r[2].m128_f32[2] !=
				   aTransform.myMatrix.r[2].m128_f32[2] ||
			   myMatrix.r[2].m128_f32[3] !=
				   aTransform.myMatrix.r[2].m128_f32[3] ||
			   myMatrix.r[3].m128_f32[0] !=
				   aTransform.myMatrix.r[3].m128_f32[0] ||
			   myMatrix.r[3].m128_f32[1] !=
				   aTransform.myMatrix.r[3].m128_f32[1] ||
			   myMatrix.r[3].m128_f32[2] !=
				   aTransform.myMatrix.r[3].m128_f32[2] ||
			   myMatrix.r[3].m128_f32[3] !=
				   aTransform.myMatrix.r[3].m128_f32[3];
	}
	...
	inline void FucKShitUp(const float spriteWidth, const float spriteHeight,
						   const Vector3f& aRotation) {
		DirectX::XMMATRIX pivotTranslation =
			DirectX::XMMatrixTranslation(-spriteWidth, -spriteHeight, 0.0f);
		DirectX::XMMATRIX rotation = DirectX::XMMatrixRotationRollPitchYaw(
			aRotation.x, aRotation.y, aRotation.z);
		myMatrix =
			pivotTranslation * rotation *
			DirectX::XMMatrixTranslation(spriteWidth, spriteHeight, 0.0f) *
			myMatrix;
	}
	//
	...
	inline void LookAt(const Vector3f& aPosition) {
		...
		Vector3f right =
			Vector3f(0.0f, 1.0f, 0.0f).Cross(forward).GetNormalized();
	...
	// I don't really think you should use this one. It probably won't work as
	// you expect
	inline void SetRotation(const Vector3f& aRotation) {
		Vector3f pos = GetPosition();
		Vector3f scale = GetScale();

		myMatrix = DirectX::XMMatrixRotationRollPitchYaw(
			aRotation.x, aRotation.y, aRotation.z);
	...
	// This is sort of an operation that shouldn't be done, but it's here for
	// convenience
	inline Vector3f GetRotation() const {
		...
		rotation.y = atan2f(-forwardRotation.x,
							sqrtf(forwardRotation.y * forwardRotation.y +
								  forwardRotation.z * forwardRotation.z));
	...
	inline void TranslateLocal(const Vector3f& aMoveAmount) {
		...
		Vector3f moveAmount = forward * aMoveAmount.z + right * aMoveAmount.x +
							  up * aMoveAmount.y;
	...
	inline void RotateWorld(const Vector3f& aRotation) {
		...
		myMatrix = myMatrix * DirectX::XMMatrixRotationRollPitchYaw(
								  aRotation.x, aRotation.y, aRotation.z);
		SetPosition(pos);
	}
	...
	inline void SetRotation(float aOrientationRad) {
		...
		rotation = DirectX::XMQuaternionRotationAxis(
			DirectX::XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f), aOrientationRad);

		myMatrix = DirectX::XMMatrixAffineTransformation(
