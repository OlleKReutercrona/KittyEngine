...
namespace CommonUtilities {
	...
	class Sphere {
	public:
		// Default constructor: there is no sphere, the radius is zero and the
		...
		// Init the sphere with a center and a radius, the same as the
		// constructor above.
		void InitWithCenterAndRadius(const Vector3<T>& aCenter, T aRadius);

		// Returns whether a point is inside the sphere: it is inside when the
		// point is on the sphere surface or inside of the sphere.
	...
	template <class T>
	inline void Sphere<T>::InitWithCenterAndRadius(const Vector3<T>& aCenter,
												   T aRadius) {
		// Init the sphere with a center and a radius, the same as the
		// constructor above.
	...
	inline bool Sphere<T>::IsInside(const Vector3<T>& aPosition) const {
		...
		// - it is inside when the point is on the sphere surface or inside of
		// the sphere.
	...
	template <class T>
	inline Vector3<T> Sphere<T>::GetNormal(Vector3<T> aPosition) const {
		Vector3<T> aVector = aPosition - myPosition;
		aVector.Normalize();
		return aVector;
	}
