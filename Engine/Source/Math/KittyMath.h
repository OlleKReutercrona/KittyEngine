...
namespace KE {
	...
	template <typename T>
	constexpr T Interp(const T& aCurrent, const T& aTarget,
					   float aAlpha) noexcept {
	...
	constexpr T Gauss(T aX, T aSigma) noexcept {
		...
		return (static_cast<T>(1.0) /
				sqrt(static_cast<T>(2.0) * static_cast<T>(PI_D) * ss)) *
			   exp(-Square(aX) / (static_cast<T>(2.0) * ss));
	}
	...
	inline Vector3f SmoothLerp(Vector3f aStart, Vector3f aEnd,
							   float aPercentage) {
		float smoothstep = Smoothstep(aPercentage);

		return Vector3f(aStart.x + (aEnd.x - aStart.x) * smoothstep,
	...
	inline float GetRandomFloat(float aMin, float aMax,
								float aPrecision = 1000.0f) {
		if (aMin - aMax == 0.0f) {
			return aMin;
		}

		float random = static_cast<float>(
			rand() % static_cast<int>((aMax - aMin) * aPrecision));
