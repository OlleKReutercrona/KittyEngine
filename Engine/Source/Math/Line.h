...
namespace CommonUtilities {
	...
	public:
		...
		// Constructor that takes two points that define the line, the direction
		// is aPoint1 - aPoint0.
		Line<T>(const Vector2<T>& aPoint0, const Vector2<T>& aPoint1);
		// Init the line with two points, the same as the constructor above.
		void InitWith2Points(const Vector2<T>& aPoint0,
							 const Vector2<T>& aPoint1);
		// Init the line with a point and a direction.
		void InitWithPointAndDirection(const Vector2<T>& aPoint,
									   const Vector2<T>& ´aDirection);
		// Returns whether a point is inside the line: it is inside when the
		// point is on the line or on the side the normal is pointing away from.
	...
	// Constructor that takes two points that define the line, the direction is
	// aPoint1 - aPoint0.
	template <class T>
	...
	// Init the line with two points, the same as the constructor above.
	template <class T>
	inline void Line<T>::InitWith2Points(const Vector2<T>& aPoint0,
										 const Vector2<T>& aPoint1) {
		myPoint = aPoint0;
		myDirection = aPoint1 - aPoint0;
		myDirection.Normalize();
	...
	// Init the line with a point and a direction.
	template <class T>
	inline void Line<T>::InitWithPointAndDirection(
		const Vector2<T>& aPoint, const Vector2<T>& aDirection) {
		myPoint = aPoint;
	...
	template <class T>
	inline bool Line<T>::IsInside(const Vector3<T>& aPosition) const {
