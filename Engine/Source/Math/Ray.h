...
namespace CommonUtilities {
	...
	class Ray {
	public:
		// Default constructor: there is no ray, the origin and direction are
		// the zero vector.
		Ray();
		...
		// Constructor that takes two points that define the ray, the direction
		// is aDirectionm and the origin is aOrigin.
		Ray(const Vector3<T>& aOrigin, const Vector3<T>& aDirection);

		// Init the ray with two points, the same as the constructor above.
		void InitWith2Points(const Vector3<T>& aOrigin,
							 const Vector3<T>& aPoint);

		// Init the ray with an origin and a direction.
		void InitWithOriginAndDirection(const Vector3<T>& aOrigin,
										const Vector3<T>& aDirection);
	...
	template <class T>
	inline Ray<T>::Ray(const Vector3<T>& aOrigin,
					   const Vector3<T>& aDirection) {
		myOrigin = aOrigin;
	...
	template <class T>
	inline void Ray<T>::InitWith2Points(const Vector3<T>& aOrigin,
										const Vector3<T>& aPoint) {
		myOrigin = aOrigin;
	...
	template <class T>
	inline void Ray<T>::InitWithOriginAndDirection(
