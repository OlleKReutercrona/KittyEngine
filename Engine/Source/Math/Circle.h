...
namespace CommonUtilities {
	...
	class Circle {
	public:
		// Default constructor: there is no Circle, the radius is zero and the
		...
		// Init the Circle with a center and a radius, the same as the
		// constructor above.
		void InitWithCenterAndRadius(const Vector2<T>& aCenter, T aRadius);

		// Returns whether a point is inside the Circle: it is inside when the
		// point is on the Circle surface or inside of the Circle.
	...
	template <class T>
	inline Circle<T>::Circle(const Circle<T>& aCircle) {
		// Copy constructor.
	...
	template <class T>
	inline void Circle<T>::InitWithCenterAndRadius(const Vector2<T>& aCenter,
												   T aRadius) {
		// Init the Circle with a center and a radius, the same as the
		// constructor above.
	...
	inline bool Circle<T>::IsInside(const Vector2<T>& aPosition) const {
		...
		// - it is inside when the point is on the Circle surface or inside of
		// the Circle.
