...
namespace CommonUtilities {
	...
	public:
		...
		const T& operator[](const unsigned int aIndex) const {
			assert(aIndex < 3 &&
				   "Trying to access class member out of bounds in Vector3 ");
	...
	template <class T>
	Vector3<T> operator+(const Vector3<T>& aVector0,
						 const Vector3<T>& aVector1) {
	...
	template <class T>
	Vector3<T> operator/(const Vector3<T>& aVector, const T& aScalar) {
		// Returns the vector aVector divided by the scalar aScalar (equivalent
		// to aVector multiplied by 1 / aScalar)
	...
	bool operator!=(Vector3<T>& aVector0, Vector3<T>& aVector1) {
		return !(aVector0.x >= aVector1.x - 0.0001f &&
				 aVector0.x <= aVector1.x + 0.0001f &&
				 aVector0.y >= aVector1.y - 0.0001f &&
				 aVector0.y <= aVector1.y + 0.0001f &&
				 aVector0.z >= aVector1.z - 0.0001f &&
				 aVector0.z <= aVector1.z + 0.0001f);
	}
	...
	inline Vector3<T> Vector3<T>::Cross(const Vector3<T>& aVector) const {
		...
		Vector3<T> Temp((y * aVector.z) - (z * aVector.y),
	...
	template <class T>
	inline static Vector3<T> Lerp(const Vector3<T>& aStart,
								  const Vector3<T>& aEnd,
								  const float aPercentage) {
		Vector3<T> temp;
