...
namespace CommonUtilities {
	...
	class AABB3D {
	public:
		// Default constructor: there is no AABB, both min and max points are
		// the zero vector.
		AABB3D();
		...
		// Init the AABB with the positions of the minimum and maximum corners,
		// same as the constructor above.
		void InitWithMinAndMax(const Vector3<T>& aMin, const Vector3<T>& aMax);

		// Returns whether a point is inside the AABB: it is inside when the
		// point is on any of the AABB's sides or inside of the AABB.
		bool IsInside(const Vector3<T>& aPosition) const;
		bool IsInside(const Vector3<T>& aPosition,
					  const Vector3<T>& aNormal) const;
	...
	template <class T>
	inline void AABB3D<T>::InitWithMinAndMax(const Vector3<T>& aMin,
											 const Vector3<T>& aMax) {
		// Init the AABB with the positions of the minimum and maximum corners,
		// same as the constructor above.
		myMin = aMin;
		myMax = aMax;
...
#pragma region old_solution
		// if (!(aPosition.x < myMin.x) != !(aPosition.x > myMax.x)) return
		// false;
		//
		// if (!(aPosition.y < myMin.y) != !(aPosition.y > myMax.y)) return
		// false;
		//
		// if (!(aPosition.z < myMin.z) != !(aPosition.z > myMax.z)) return
		// false;
		//
		// return true;
#pragma endregion
	...
	template <class T>
	bool AABB3D<T>::IsInside(const Vector3<T>& aPosition,
							 const Vector3<T>& aNormal) const {
		return (((aPosition.x >= myMin.x) && (aPosition.x <= myMax.x) ||
				 aNormal.x != 0) &&
				((aPosition.y >= myMin.y) && (aPosition.y <= myMax.y) ||
