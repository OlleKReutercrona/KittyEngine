...
namespace CommonUtilities {
	...
	template <class T>
	Matrix3x3<T> operator+(const Matrix3x3<T>& aMatrix0,
						   const Matrix3x3<T>& aMatrix1) {
	...
	template <class T>
	Vector3<T> operator*(const Matrix3x3<T>& aMatrix,
	...
	void operator*=(Matrix3x3<T>& aMatrix0, const Matrix3x3<T>& aMatrix1) {
		...
		for (T row = 1; row <= 3; row++) {
			for (T column = 1; column <= 3; column++) {
				tempMatrix(row, column) =
					aMatrix0(row, 1) * aMatrix1(1, column) +
					aMatrix0(row, 2) * aMatrix1(2, column) +
					aMatrix0(row, 3) * aMatrix1(3, column);
			}
		}
		aMatrix0 = tempMatrix;
	}
	template <class T>
	bool operator==(const Matrix3x3<T>& aMatrix0,
					const Matrix3x3<T>& aMatrix1) {
		for (int row = 1; row <= 3; row++) {
			for (int column = 1; column <= 3; column++) {
	...
	inline Matrix3x3<T> Matrix3x3<T>::Transpose(
		const Matrix3x3<T>& aMatrixToTranspose) {
