...
// Good info about collision tests:
...
inline bool Intersect(const KE::Shape* aShape, Rayf& aRay,
					  Vector3f& aOutIntersectionPoint);
inline bool Intersect(const KE::Shape* aShape, Rayf& aRay);

// Ray Tests
inline bool IntersectBoxRay(const KE::Box& aBox, Rayf& aRay);
inline bool IntersectBoxRay(const KE::Box& aSource, Rayf& aRay,
							Vector3f& aOutIntersectionPoint);
inline bool IntersectionPlaneRay(const Plane& aPlane, const Rayf& aRay,
								 Vector3f& aOutIntersectionPoint);
...
inline bool IntersectBoxPoint(const KE::Box aBoxCollider,
...
inline bool IntersectSpherePoint(const KE::Sphere& aFirst,
								 const Vector3f& aSecond);

inline bool IntersectLineLine(Linef& aLine0, Linef& aLine1,
							  Vector2f& aOutIntersectPoint);
// inline bool IntersectLineLineTEST(Linef& aLine0, Linef& aLine1, Vector2f&
// aOutIntersectPoint);
///	\--------------------------------------------/
/// |	\\\\\\+------------------------+//////	 |
/// |	<<<<<<		   RAY TESTS		>>>>>>	 |
...
inline bool IntersectBoxRay(const KE::Box& aBox, Rayf& aRay) {
	...
	float t1 = (aBox.myMin.x + aBox.myPosition.x + aBox.myOffset.x -
				aRay.GetOrigin().x) *
			   dirfrac.x;
	float t2 = (aBox.myMax.x + aBox.myPosition.x + aBox.myOffset.x -
				aRay.GetOrigin().x) *
			   dirfrac.x;

	float t3 = (aBox.myMin.y + aBox.myPosition.y + aBox.myOffset.y -
				aRay.GetOrigin().y) *
			   dirfrac.y;
	float t4 = (aBox.myMax.y + aBox.myPosition.y + aBox.myOffset.y -
				aRay.GetOrigin().y) *
			   dirfrac.y;

	float t5 = (aBox.myMin.z + aBox.myPosition.z + aBox.myOffset.z -
				aRay.GetOrigin().z) *
			   dirfrac.z;
	float t6 = (aBox.myMax.z + aBox.myPosition.z + aBox.myOffset.z -
				aRay.GetOrigin().z) *
			   dirfrac.z;
...
inline bool IntersectBoxRay(const KE::Box& aSource, Rayf& aRay,
							Vector3f& aOutIntersectionPoint) {
	Plane planes[6];
	planes[0] = Plane(aSource.myMin + aSource.myPosition + aSource.myOffset,
					  Vector3f(-1, 0, 0));
	planes[1] = Plane(aSource.myMin + aSource.myPosition + aSource.myOffset,
					  Vector3f(0, -1, 0));
	planes[2] = Plane(aSource.myMin + aSource.myPosition + aSource.myOffset,
					  Vector3f(0, 0, -1));
	planes[3] = Plane(aSource.myMax + aSource.myPosition + aSource.myOffset,
					  Vector3f(1, 0, 0));
	planes[4] = Plane(aSource.myMax + aSource.myPosition + aSource.myOffset,
					  Vector3f(0, 1, 0));
	planes[5] = Plane(aSource.myMax + aSource.myPosition + aSource.myOffset,
					  Vector3f(0, 0, 1));
	...
	for (int i = 0; i < 6; i++) {
		if (IntersectionPlaneRay(planes[i], aRay, aOutIntersectionPoint)) {
			if (IntersectBoxPoint(aSource, aOutIntersectionPoint,
								  planes[i].GetNormal())) {
				...
				if (distance.Length() < intersectionDistance &&
					...
					intersectionDistance =
						static_cast<float>(distance.Length());
...
inline bool IntersectionPlaneRay(const Plane& aPlane, const Rayf& aRay,
	...
	float distance = (d - aRay.GetOrigin().Dot(aPlane.GetNormal())) /
	...
	aOutIntersectionPoint =
		aRay.GetOrigin() + aRay.GetDirection().GetNormalized() * distance;
	return true;
}
...
inline bool IntersectSphereRay(const KE::Sphere& aSphere, const Rayf& aRay) {
	...
	float midCornerToSphereCenter = rayToSphereCenter.Dot(aRay.GetDirection()) /
									aRay.GetDirection().Length();

	if (midCornerToSphereCenter < 0) {
		return false;
	}

	float rayOrginToSphereCenter =
		sqrt(rayToSphereCenter.LengthSqr() -
			 midCornerToSphereCenter * midCornerToSphereCenter);
...
inline bool IntersectSphereRay(const KE::Sphere& aSphere, const Rayf& aRay,
	...
	float interSectToMidCorner =
		sqrt(aSphere.myRadius * aSphere.myRadius -
			 rayOrginToSphereCenter * rayOrginToSphereCenter);

	aOutIntersectionPoint =
		aRay.GetOrigin() +
		aRay.GetDirection() * (midCornerToSphereCenter - interSectToMidCorner);
	return true;
}
	...
	switch (aFirst->myShapeData) {
		case eShapes::eBox: {
			switch (aSecond->myShapeData) {
				...
				case eShapes::eSphere: {
					...
					const KE::Sphere* sphere =
						dynamic_cast<const KE::Sphere*>(aSecond);
		...
		case eShapes::eSphere: {
			switch (aSecond->myShapeData) {
				case eShapes::eBox: {
					const KE::Sphere* sphere =
						dynamic_cast<const KE::Sphere*>(aFirst);
					const KE::Box* box = dynamic_cast<const KE::Box*>(aSecond);

					return Intersect(*box, *sphere);
				}
				case eShapes::eSphere: {
					const KE::Sphere* sphere1 =
						dynamic_cast<const KE::Sphere*>(aFirst);
					const KE::Sphere* sphere2 =
						dynamic_cast<const KE::Sphere*>(aSecond);
...
inline bool Intersect(const KE::Shape* aShape, Rayf& aRay,
	...
	switch (aShape->myShapeData) {
		...
		case eShapes::eSphere: {
			return IntersectSphereRay(*dynamic_cast<const KE::Sphere*>(aShape),
									  aRay, aOutIntersectionPoint);
		}
		default:
			break;
	}
...
inline bool Intersect(const KE::Box& aBox1, const KE::Box& aBox2) {
	bool xL = aBox1.myMax.x + aBox1.myPosition.x + aBox1.myOffset.x >
			  aBox2.myMin.x + aBox2.myPosition.x + aBox2.myOffset.x;
	bool xR = aBox1.myMin.x + aBox1.myPosition.x + aBox1.myOffset.x <
			  aBox2.myMax.x + aBox2.myPosition.x + aBox2.myOffset.x;
	bool yL = aBox1.myMax.y + aBox1.myPosition.y + aBox1.myOffset.y >
			  aBox2.myMin.y + aBox2.myPosition.y + aBox2.myOffset.y;
	bool yR = aBox1.myMin.y + aBox1.myPosition.y + aBox1.myOffset.y <
			  aBox2.myMax.y + aBox2.myPosition.y + aBox2.myOffset.y;
	bool zL = aBox1.myMax.z + aBox1.myPosition.z + aBox1.myOffset.z >
			  aBox2.myMin.z + aBox2.myPosition.z + aBox2.myOffset.z;
	bool zR = aBox1.myMin.z + aBox1.myPosition.z + aBox1.myOffset.z <
			  aBox2.myMax.z + aBox2.myPosition.z + aBox2.myOffset.z;
...
inline bool IntersectBoxPoint(const KE::Box aBoxCollider,
							  const Vector3f& aPosition) {
	return (((aPosition.x >= aBoxCollider.myMin.x) &&
			 (aPosition.x <= aBoxCollider.myMax.x)) &&
			((aPosition.y >= aBoxCollider.myMin.y) &&
			 (aPosition.y <= aBoxCollider.myMax.y)) &&
			((aPosition.z >= aBoxCollider.myMin.z) &&
			 (aPosition.z <= aBoxCollider.myMax.z)));
}
...
inline bool IntersectBoxPoint(const Vector2f& aBox, const Vector2f& aPoint) {
	...
	return (((aPoint.x >= minB.x) && (aPoint.x <= maxB.x)) &&
...
inline bool IntersectBoxPoint(const KE::Box aBoxCollider,
							  ...
							  const Vector3f& aNormal) {
	Vector3f low =
		aBoxCollider.myMin + aBoxCollider.myPosition + aBoxCollider.myOffset;
	Vector3f high =
		aBoxCollider.myMax + aBoxCollider.myPosition + aBoxCollider.myOffset;
	return (
...
inline bool Intersect(const KE::Sphere& aFirst, const KE::Sphere& aSecond) {
	...
	Vector3f distVector = (aFirst.myPosition + aFirst.myOffset) -
						  (aSecond.myPosition + aSecond.myOffset);
...
inline bool IntersectSpherePoint(const KE::Sphere& aSphere,
								 const Vector3f& aPoint) {
	/// https://math.stackexchange.com/questions/3118238/check-if-3d-point-is-inside-sphere

	double d =
		sqrt(pow(aPoint.x - aSphere.myPosition.x + aSphere.myOffset.x, 2) +
...
inline bool IntersectLineLine(Linef& aLine0, Linef& aLine1,
	...
	if (!std::isfinite(abs(m1)) && !std::isfinite(abs(m2))) {
		// Lines are both vertical (perpendicular), and they intersect at (x1,
		// y2) or (x2, y1).
	...
	if (!std::isfinite(abs(m1))) {
		// Line aLine0 is vertical, and the intersection is calculated using the
		// equation of line aLine1.
		aOutIntersectPoint.x = x1;
	...
	if (!std::isfinite(abs(m2))) {
		// Line aLine1 is vertical, and the intersection is calculated using the
		// equation of line aLine0.
