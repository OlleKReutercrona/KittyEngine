...
namespace KE {
	...
	bool RaycastHandler::RayCastMissed(Vector2f aMousePos,
									   Vector3f aPlayerPosition) {
		Vector3f position = {0, 0, 0};
		float distance = FLT_MAX;
		Rayf ray = mySceneCamera->GetRay(aMousePos);
	...
	Vector3f RaycastHandler::RaycastPlayerAction(Vector2f aMousePos,
												 Vector3f aPlayerPosition) {
		Vector3f position = {};
		// Rayf ray = mySceneCamera->GetRay(aMousePos);
		// myCollisionHandler->PlayerRayCast(ray, position, aPlayerPosition);
	...
	Vector3f RaycastHandler::RaycastEnemy(Vector2f aMousePos) {
		Vector3f position = {};
		// Rayf ray = mySceneCamera->GetRay(aMousePos);
		// KE::Collider* temp = myCollisionHandler->RayCast(ray, position,
		// static_cast<int>(KE::Collision::Layers::Enemy));
		return position;
	}
	...
	bool RaycastHandler::RayCastEnemyHit(Vector2f aMousePos) {
		...
		// Rayf ray = mySceneCamera->GetRay(aMousePos);
		// KE::Collider* temp = myCollisionHandler->RayCast(ray, position,
		// static_cast<int>(KE::Collision::Layers::Enemy)); if (temp != nullptr)
		//{
		//	return true;
		// }
