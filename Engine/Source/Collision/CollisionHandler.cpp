...
void KE::CollisionHandler::InitPhysX() {
	...
	myFoundation =
		PxCreateFoundation(PX_PHYSICS_FOUNDATION_VERSION,
						   myDefaultAllocatorCallback, myDefaultErrorCallback);
	...
	myPhysics = PxCreatePhysics(PX_PHYSICS_VERSION, *myFoundation,
								myToleranceScale, true, myPvd, 0);
	...
	if (pvdClient) {
		...
		pvdClient->setScenePvdFlag(PxPvdSceneFlag::eTRANSMIT_SCENEQUERIES,
								   true);
		pvdClient->updateCamera("test", PxVec3(0, 10, 10), PxVec3(0, 1, 0),
								PxVec3(0, 0, 0));
	} else {
...
void KE::CollisionHandler::UpdateGameObjects() {
	for (auto& physobj : myPhysObjects) {
		if (!physobj.second->myKECollider.isStatic) {
			...
			if (physobj.second->disableColliderMovementPrecedence) {
				continue;
			}

			physx::PxTransform trans = physobj.second->myActor->getGlobalPose();

			// Remove offset from transform since the physx transform is already
			// moved by the offset
			...
			// physobj.second->myKECollider.myComponent->GetGameObject().myTransform
			// = trans;
			if (!physobj.second->myKECollider.myComponent) {
				std::cout << "FIX THIS ISSUE\n";
			}

			physobj.second->myKECollider.myComponent->GetGameObject()
...
KE::Collider* KE::CollisionHandler::RayCast(const Rayf& aRay,
											Vector3f& anOutPoint,
	...
	if (myScene->raycast({origin.x, origin.y, origin.z},
						 {direction.x, direction.y, direction.z}, aDistance,
						 hitData, physx::PxHitFlag::eDEFAULT)) {
		for (int i = 0; i < (int)hitData.getNbTouches(); i++) {
			...
			if (!obj.myKECollider.isActive) {
				continue;
			}

			// Check if on layermask
			if (aLayerMask == 0 ||
				(int)obj.myKECollider.myComponent->GetGameObject().myLayer &
					aLayerMask) {
				Vector3f hitPosition =
					*(Vector3f*)&hitData.getTouch(i).position;
...
void KE::CollisionHandler::NavmeshRayCast(Rayf& aRay, Vector3f& aOutPoint,
	...
	for (NavNode& node : myNavmesh->myNodes) {
		...
		// If IntersectPoint is in triangle -> return true (Position is valid on
		// Navmesh)
...
std::vector<KE::Collider*> KE::CollisionHandler::BoxCast(const KE::Box& aBox,
														 int aLayerMask) {
	...
	physx::PxVec3 vecPos({aBox.myPosition.x + aBox.myOffset.x,
						  aBox.myPosition.y + aBox.myOffset.y,
						  aBox.myPosition.z + aBox.myOffset.z});
...
KE::Collider* KE::CollisionHandler::CreateBox(const Transform& aTransform,
											  const Vector3f& anOffset,
	...
	physx::PxShape* aBoxShape = myPhysics->createShape(
		physx::PxBoxGeometry(transformedSize.x, transformedSize.y,
							 transformedSize.z),
		*myMaterial);
...
KE::Collider* KE::CollisionHandler::CreateSphere(const Transform& aTransform,
												 const Vector3f& anOffset,
												 const float aRadius,
												 const Collision::Layers aLayer,
												 const bool aIsStatic,
	...
	physx::PxShape* sphere =
		myPhysics->createShape(physx::PxSphereGeometry(aRadius), *myMaterial);
...
KE::Collider* KE::CollisionHandler::CreateCapsule(
	const Transform& aTransform, const Vector3f& anOffset, const float aRadius,
	const float aHalfHeight, const Collision::Layers aLayer,
	const bool aIsKinematic, const bool aIsStatic) {
#ifndef USE_PHYSX
	return nullptr;
#else
	const int ID = GenerateID();
	...
	// 	physx::PxShape* aBoxShape =
	// myPhysics->createShape(physx::PxBoxGeometry(transformedSize.x,
	// transformedSize.y, transformedSize.z), * myMaterial);

	physx::PxShape* capsule = myPhysics->createShape(
		physx::PxCapsuleGeometry(aRadius, aHalfHeight), *myMaterial);
...
KE::Collider* KE::CollisionHandler::CreateMeshCollider(
	const Transform& aTransform, const std::vector<Vertex>& someVertices,
	const std::vector<unsigned int>& someIndices,
	const Collision::Layers aLayer, const bool aIsStatic,
	const bool aIsKinematic) {
#ifndef USE_PHYSX
	return nullptr;
#else
	...
	physObj.myActor = actor;
	physObj.myActor->userData = (void*)&physObj.ID;
	physObj.myActor->setActorFlag(physx::PxActorFlag::eDISABLE_SIMULATION,
								  true);
	...
	meshDesc.triangles.count =
		static_cast<physx::PxU32>(someIndices.size() / 3);
	...
	physx::PxTriangleMesh* triangleMesh = PxCreateTriangleMesh(
		param22, meshDesc, myPhysics->getPhysicsInsertionCallback());
	...
	physx::PxShape* shape =
		physx::PxRigidActorExt::createExclusiveShape(*actor, geom, *myMaterial);
...
KE::PhysicsCharacterController* KE::CollisionHandler::CreateController(
	...
	auto characterController = controller->Create(
		myPhysics, myControllerManager, aPhysxData, aControllerUserData);
...
void KE::CollisionHandler::RegisterPhysicsObject(
		...
		for (physx::PxU32 j = 0; j < nShapes; j++) {
			...
			switch (geom.getType()) {
				...
				case physx::PxGeometryType::eBOX: {
					Vector3f pos = Vector3f(tm.p.x, tm.p.y, tm.p.z);
					Vector3f size = Vector3f(geom.box().halfExtents.x * 2,
											 geom.box().halfExtents.y * 2,
											 geom.box().halfExtents.z * 2);
...
void KE::CollisionHandler::onContact(
	...
	if (!myPhysObjects.contains(colID1) || !myPhysObjects.contains(colID2)) {
		std::cout << "on contact trigger but couldnt find a matching actor ["
				  << colID1 << "] & [" << colID2 << "]" << std::endl;
		return;
	}
	...
	if (!col1.myKECollider.myComponent || !col2.myKECollider.myComponent) {
		...
		auto* testData =
			(CharacterControllerUserData*)(col1.myKECollider.myComponent)
				? col1.myActor->userData
				: col2.myActor->userData;

		if (testData) {
			CharacterControllerUserData* data =
				static_cast<CharacterControllerUserData*>(testData);

			std::cout << "Player\n";
		}

		PhysxShapeUserData* userData1 =
			static_cast<PhysxShapeUserData*>(shape1->userData);
		PhysxShapeUserData* userData2 =
			static_cast<PhysxShapeUserData*>(shape2->userData);

		if (userData1 && userData2) {
			// std::cout << "User: " << userData1->myID << " Collided with: " <<
			// userData2->myID << std::endl; std::cout << "COLLIDING WITH
			// PLAYER" << std::endl;
			//  [TODO] Set up a Common PhysxUserData as seen in Collider.h for
			//  all objects, including CCT's. [TODO] Player's CCT's
			//  physxObject's component is null, this should probably be set to
			//  some kind of component to avoid handling stuff in this IF
			//  statement. Ultimately we want to trigger each components
	...
	CollisionData data1(col1.myKECollider,
						col1.myKECollider.myComponent->GetGameObject());
	CollisionData data2(col2.myKECollider,
						col2.myKECollider.myComponent->GetGameObject());
	...
	if (!col1.myKECollider.isStatic) {
		...
		col1.myKECollider.myComponent->GetGameObject().myTransform.SetPosition(
			{shapeTransform.p.x, shapeTransform.p.y, shapeTransform.p.z});
	}
	...
	if (!col2.myKECollider.isStatic) {
		...
		col2.myKECollider.myComponent->GetGameObject().myTransform.SetPosition(
			{shapeTransform.p.x, shapeTransform.p.y, shapeTransform.p.z});
	}
...
void KE::CollisionHandler::onTrigger(physx::PxTriggerPair* pairs,
									 physx::PxU32 count) {
	std::cout << "TRIGGER DETECTED IN PHYSX!!!!" << std::endl;
}

physx::PxFilterFlags KE::CollisionHandler::CustomFilterShader(
	...
	// if (physx::PxFilterObjectIsTrigger(attribute0) ||
	// physx::PxFilterObjectIsTrigger(attribute1))
	//{
	...
	if ((filterData0.word0 & static_cast<int>(KE::Collision::Layers::Player) &&
		 filterData1.word0 &
			 static_cast<int>(KE::Collision::Layers::Projectile)) ||
	...
	// The words can be layers or IDs that can be used to compare with other
	// objects.

	// Trigger the contact callback for pairs (A, B) where
	// the filtermask of A contains the ID of B and vice versa.
	// if ((filterData0.word0 & filterData1.word1) || (filterData1.word0 &
	// filterData0.word1)) if ((filterData0.word1 & filterData1.word0))
	//{
	//	pairFlags |= physx::PxPairFlag::eNOTIFY_TOUCH_FOUND;
	//}
...
//{
//	// [NOTE] This needs to be optimized for future project. Prefferd with a
//[QuadTree] & [LayerMasks].
//
//	for (int i = 0; i < myColliders.size(); i++)
//	{
...
//
// bool KE::CollisionHandler::PlayerRayCast(Rayf& aRay, Vector3f& aOutPoint,
...
//}
//
// std::vector<KE::Collider*> KE::CollisionHandler::ColliderCast(const
// KE::Shape& aShape, const int aLayerMask)
//{
...
//}
//
// std::vector<KE::Collider*> KE::CollisionHandler::SphereCast(const KE::Sphere&
// aSphere, const int aLayerMask)
//{
//	std::vector<Collider*> collisions;
//
//	for (size_t i = 0; i < myColliders.size(); i++)
