...
namespace KE {
	...
	class CharacterControllerFilter : public physx::PxControllerFilterCallback,
	...
	public:
		...
		inline Vector3f& GetVelocityRef() {
			return myVelocity;
		}
		...
		inline MovementData& GetMovementData() {
			return myMovementData;
		}
		void SetCollisionWithLayer(Collision::Layers aLayer, bool aEnable);
		inline void SetDashThroughWalls(bool aDashThroughWalls) {
			myDashThroughWalls = aDashThroughWalls;
		}

		inline const Vector3f& GetVelocity() const {
			return myVelocity;
		}
		inline CharacterControllerUserData* GetUserData() {
			return &myUserData;
		}
		inline void ResetAttack() {
			myAttackUserData.myHitIDs.clear();
		}
		...
		inline void SetOrientation(float aOrientation) {
	...
	private:
		...
		float myCoyoteTime =
			0.25f;	// how long can we be in the air before we start falling
		...
		// Targets
		float targetRadius =
			0.01745f;  // Threashold for when rotation is good enough.
		float slowRadius =
			0.8f;  // Threashold for when to start slowing down our rotation.
