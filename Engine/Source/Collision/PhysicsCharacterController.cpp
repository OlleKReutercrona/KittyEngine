...
namespace KE {
	bool CharacterControllerFilter::filter(const physx::PxController& a,
										   const physx::PxController& b) {
		const auto& objAData =
			*static_cast<CharacterControllerUserData*>(a.getActor()->userData);
		const auto& objBData =
			*static_cast<CharacterControllerUserData*>(b.getActor()->userData);

		if (objAData.team == objBData.team) {
			return false;
		}
		if (objAData.objType == CharacterControllerUserData::Type::Boomerang &&
	...
	physx::PxQueryHitType::Enum CharacterControllerFilter::preFilter(
	...
	physx::PxQueryHitType::Enum CharacterControllerFilter::postFilter(
		...
		if (filterData.word0 &
			static_cast<int>(KE::Collision::Layers::PlayerAttack)) {
	...
	// void GameObjectCollisionInterface::OnCollision(GameObject* aGameObject,
	...
	std::pair<int, PhysicsObject*> PhysicsCharacterController::Create(
		...
		myMaterial = aPhysics->createMaterial(aData.physicsMaterial.x,
											  aData.physicsMaterial.y,
											  aData.physicsMaterial.z);
		...
		desc.position =
			physx::PxExtendedVec3(positionRef.x, positionRef.y, positionRef.z);
		desc.material = myMaterial;
		desc.climbingMode =
			physx::PxCapsuleClimbingMode::eCONSTRAINED;	 // TODO Expose this
		...
		filterData.word1 |=
			static_cast<int>(KE::Collision::Layers::MovingObject);

		myController->getActor()->setActorFlag(
			physx::PxActorFlag::eDISABLE_GRAVITY, true);
	...
	void PhysicsCharacterController::DashUpdate() {
		...
		if (myDashThroughWalls) {
			...
			const bool tp = myController->setFootPosition(dispPos);
			...
			tpFilter.flags = physx::PxQueryFlag::eSTATIC |
							 physx::PxQueryFlag::eDYNAMIC |
							 physx::PxQueryFlag::ePOSTFILTER;
			...
			bool overlap = myPhysicsObject.myActor->getScene()->overlap(
				shape->getGeometry(), myPhysicsObject.myActor->getGlobalPose(),
				hit, tpFilter, &myFilterCallback);

			myDashTimer += KE_GLOBAL::deltaTime;
			if (cappedTimer >= timeToTarget && !overlap) {
				myIsDashing = false;
				physx::PxExtendedVec3 finalPos =
					myController->getFootPosition();
				finalPos.y = 0.0f;
				myController->setFootPosition(finalPos);

				myTransform->SetPosition(
					{(float)finalPos.x, (float)finalPos.y, (float)finalPos.z});
			}

		} else {
			const physx::PxControllerCollisionFlags collisionFlags =
				myController->move(displacement * KE_GLOBAL::deltaTime, 0.0f,
								   KE_GLOBAL::deltaTime, myFilterData, NULL);
			...
			myTransform->SetPosition(
				{(float)newPos.x, (float)newPos.y - 0.5f, (float)newPos.z});
	...
	bool PhysicsCharacterController::CheckAgainstMovingObjects() {
		if (myIsDashing && myDashThroughWalls) {
			return false;
		}
		...
		tpFilter.data.word1 =
			static_cast<int>(KE::Collision::Layers::MovingObject);
		// tpFilter.data.word1 |=
		// static_cast<int>(KE::Collision::Layers::Ground);

		tpFilter.flags =
			physx::PxQueryFlag::eDYNAMIC | physx::PxQueryFlag::ePOSTFILTER;
		...
		while (iter < maxIter) {
			...
			myController->getActor()->getShapes(&shape, 1, 0);
			bool overlap = myPhysicsObject.myActor->getScene()->overlap(
				shape->getGeometry(), globalPose, hit, tpFilter,
				&myFilterCallback);
	...
	void PhysicsCharacterController::Update() {
		...
		myCoyoteTimer =
			myIsGrounded
				? 0.0f
				: myCoyoteTimer + (myIsDashing ? 0.0f : KE_GLOBAL::deltaTime);

		physx::PxVec3 displacement =
			myVelocity.LengthSqr() > 0.1f
				? physx::PxVec3{myVelocity.x, -5.0f, myVelocity.z}
				: physx::PxVec3{0.0f, -5.0f, 0.0f};

		displacement.y = myCoyoteTimer > myCoyoteTime || myIsGrounded
							 ? displacement.y
							 : 0.0f;
		...
		collisionFlags& physx::PxControllerCollisionFlag::eCOLLISION_DOWN
			? myIsGrounded = true
			: myIsGrounded = false;
		...
		if (myLinearDecayActive) {
			float decay =
				1.0f - (myMovementData.decayMagnitude * KE_GLOBAL::deltaTime);
			myVelocity *= decay;
		}

		if (mAngularDecayActive) {
			myRotation *= 1.0f - (myMovementData.angularDecayMagnitude *
								  KE_GLOBAL::deltaTime);
		}
	...
	void PhysicsCharacterController::Move(const Vector3f& aDirection,
										  float aSpeedScale) {
		if (!myIsGrounded) return;

		myLinearDecayActive = false;
		myVelocity += (aDirection * myMovementData.linearAcceleration) *
					  KE_GLOBAL::deltaTime;
	...
	void PhysicsCharacterController::Dash(const Vector3f& aDirection,
										  float aDistance) {
		myDashTimer = 0.0f;
		myIsDashing = true;

		Vector3f realDashTarget =
			myTransform->GetPosition() + (aDirection * aDistance);
		...
		if (GetDistanceToGround(&workingTarget) < 0.0f) {
			...
			for (float i = 0.0f; i < maxExtraDistance; i += 0.1f) {
				workingTarget =
					myTransform->GetPosition() + (aDirection * (aDistance + i));
				if (GetDistanceToGround(&workingTarget) >= 0.0f) {
					workingTarget = myTransform->GetPosition() +
									(aDirection * (aDistance + i + 0.2f));
					break;
				}
			}
		}
	...
	void PhysicsCharacterController::Rotate(const Vector3f& aDirection) {
		if (aDirection.GetNormalized().LengthSqr() < 0.1f) {
			return;
		}
		...
		float speed =
			rotationSize > slowRadius
				? myMovementData.angularAcceleration
				: myMovementData.maxRotation * (rotationSize / slowRadius);
	...
	/* When the character hits a shape (both static and dynamic), the onShapeHit
	 * callback is called. */
	void PhysicsCharacterController::onShapeHit(
		const physx::PxControllerShapeHit& aHit) {
	...
	/* When the character hits another character controller the onControllerHit
	 * callback is called. */
	void PhysicsCharacterController::onControllerHit(
		const physx::PxControllersHit& aHit) {
		const auto& hitUserData = *static_cast<CharacterControllerUserData*>(
			aHit.other->getActor()->userData);

		PhysXCollisionData collisionData;
		collisionData.objHit = hitUserData.gameObject;

		GameObjectCollisionInterface::OnCollision(myUserData.gameObject,
												  collisionData);
	}

	/* When the character hits a user-defined obstacle, the onObstacleHit
	 * callback is called. */
	void PhysicsCharacterController::onObstacleHit(
		const physx::PxControllerObstacleHit& aHit) {}

	void PhysicsCharacterController::SetCollisionWithLayer(
		Collision::Layers aLayer, bool aEnable) {
		if (aEnable) {
	...
	Vector3f PhysicsCharacterController::GetFootPositioin() {
		Vector3f result = {(float)myController->getFootPosition().x,
	...
	Vector3f PhysicsCharacterController::GetRegularPositioin() {
		Vector3f result = {(float)myController->getPosition().x,
	...
	float PhysicsCharacterController::GetDistanceToGround(Vector3f* aPosition) {
		...
		groundFilter.data.word0 =
			static_cast<int>(KE::Collision::Layers::Player);
		groundFilter.data.word1 =
			static_cast<int>(KE::Collision::Layers::Ground);

		const physx::PxU32 bufferSize = 256;  // [in] size of 'hitBuffer'
		physx::PxRaycastHit
			hitBuffer[bufferSize];	// [out] User provided buffer for results
		physx::PxRaycastBuffer buf(
			hitBuffer,
			bufferSize);  // [out] Blocking and touching hits stored here

		auto footPos = myController->getFootPosition();
		if (aPosition) {
			footPos = {static_cast<float>(aPosition->x),
					   static_cast<float>(aPosition->y),
					   static_cast<float>(aPosition->z)};
		}

		physx::PxVec3 wFootPos = {static_cast<float>(footPos.x),
								  static_cast<float>(footPos.y),
								  static_cast<float>(footPos.z)};

		bool overlap = myPhysicsObject.myActor->getScene()->raycast(
			wFootPos, physx::PxVec3(0.0f, -1.0f, 0.0f), 100.0f, buf,
	...
	void PhysicsCharacterController::AttackUpdate() {
		...
		bool overlap = myPhysicsObject.myActor->getScene()->overlap(
			...
			myPhysicsObject.myActor->getGlobalPose(), buf, tpFilter);

		const float maximumHitAngle = 1.0f;
		auto& dbg =
			KE_GLOBAL::blackboard.Get<KE::Graphics>()->GetDebugRenderer();

		Matrix3x3f leftRot =
			Matrix3x3f::CreateRotationAroundY(-maximumHitAngle);
		...
		for (physx::PxU32 i = 0; i < buf.nbTouches; i++) {
			...
			auto hitUserData =
				*static_cast<CharacterControllerUserData*>(hit.actor->userData);
			if (hitUserData.gameObject->myID == myUserData.gameObject->myID) {
				continue;
			}
			if (myAttackUserData.myHitIDs.find(hitUserData.gameObject->myID) !=
				myAttackUserData.myHitIDs.end()) {
				continue;
			}
			...
			Vector3f hitPosition = {hitPositionPX.x, hitPositionPX.y,
									hitPositionPX.z};

			Vector3f hitDirection =
				(hitPosition - myTransform->GetPosition()).GetNormalized();
			float angle = acosf(hitDirection.Dot(myTransform->GetForward()));
			if (angle > maximumHitAngle) {
				continue;
			}
		}
	}
