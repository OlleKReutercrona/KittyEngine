...
namespace KE {
	void LevelSettingsMeowFile::Save(const std::string& aFilePath) {
		...
		if (CreateDirectoryA(LEMEOW_PATH, NULL) ||
			ERROR_ALREADY_EXISTS == GetLastError()) {
			std::fstream file;
			file.open(LEMEOW_PATH + aFilePath, std::ios_base::trunc |
												   std::ios_base::binary |
												   std::ios_base::out);

			file.clear();

			file.write(reinterpret_cast<char*>(&myPPData),
					   sizeof(KE::PostProcessAttributes));
			file.close();
		} else {
