...
namespace KE {
	NavGrid::NavGrid(Vector2i aGridSize, float aCellSize)
		: myGridSize(aGridSize), myCellSize(aCellSize) {}

	bool NavGrid::Init(float aCellSize, Vector3f aMin, Vector3f aMax,
					   std::vector<KE::NavNode>& someNodes) {
		myCellSize = aCellSize;
		myMin = {aMin.x, aMin.z};
		myMax = {aMax.x, aMax.z};

		myGridSize = {(int)(abs(aMax.x - aMin.x) / myCellSize) + 1,
					  (int)(abs(aMax.z - aMin.z) / myCellSize) + 1};
		...
		for (int i = 0; i < someNodes.size(); i++) {
			...
			for (int y = 0; y < ySize; y++) {
				for (int x = 0; x < xSize; x++) {
					int index =
						(minCord.x + x) + (minCord.y + y) * myGridSize.x;
	...
	Vector2i NavGrid::GetCoordinate(Vector3f aPos) {
		...
		return Vector2i((int)(relativeX / myCellSize),
						(int)(relativeY / myCellSize));
	}
