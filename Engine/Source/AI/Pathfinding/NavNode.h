...
namespace KE {
	inline static float Sign(Vector2f aPoint0, Vector2f aPoint1,
							 Vector2f aPoint2) {
		return (aPoint0.x - aPoint2.x) * (aPoint1.y - aPoint2.y) -
			   (aPoint1.x - aPoint2.x) * (aPoint0.y - aPoint2.y);
	}
	inline static bool InsideTriangle(
		const Vector3f& aPoint, std::array<const Vector3f*, 3>& someVertices) {
		float d1, d2, d3;
	...
	inline static bool GetHeightInsideTriangle(
		Vector3f& aOutPos, const std::array<const Vector3f*, 3>& someVertices) {
		...
		if (!(has_neg && has_pos)) {
			...
			float height = u * someVertices[0]->y + v * someVertices[1]->y +
						   w * someVertices[2]->y;

			if (isnan(height)) {
				KE_ERROR(
					"GetHeightInsideTriangle gave nan-ind. Returning average "
					"triangle height");

				height = (someVertices[0]->y + someVertices[1]->y +
						  someVertices[2]->y) /
						 3.0f;
			}
	...
	enum class eNodeStatus { Unvisited, Open, Closed };

	struct NavNode {
		NavNode(int aID, std::array<unsigned int, 3> someIndices,
				std::array<const Vector3f*, 3>& someVertices)
...
#pragma region LineColliders
		...
		inline void ClampVelocity(const Vector3f& aPosition,
								  Vector3f& aVelocity) {
			...
			for (Linef& line : myLineColliders) {
				if (line.IsInside(aPosition + aVelocity)) {
					float dotProduct =
						velo_2D.GetNormalized().Dot(line.GetNormal());

					velo_2D -=
						(dotProduct * velo_2D.Length()) * line.GetNormal();
				}
			}
		...
		inline void ClampVelocity(const Vector3f& aPosition,
			...
			for (Linef& line : myLineColliders) {
				if (line.IsInside(pos)) {
					float dotProduct =
						aVelocity.GetNormalized().Dot(line.GetNormal());

					aVelocity -=
						(dotProduct * aVelocity.Length()) * line.GetNormal();
				}
			}
		}
#pragma endregion
