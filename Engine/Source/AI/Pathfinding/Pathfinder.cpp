...
namespace KE {
	...
	void Pathfinder::DebugRender(Graphics* aGraphics) {
		...
		if (myDebugPath_2D.size() > 0) {
			v1.position = {myDebugStart_2D.x, lineHeight, myDebugStart_2D.y,
						   1.0f};
			v2.position = {myDebugPath_2D.back().x, lineHeight,
						   myDebugPath_2D.back().y, 1.0f};
			aGraphics->AddLine(v1, v2);

			for (int i = 1; i < myDebugPath_2D.size(); i++) {
				v1.position = {myDebugPath_2D[i].x, lineHeight,
							   myDebugPath_2D[i].y, 1.0f};
				v2.position = {myDebugPath_2D[i - 1].x, lineHeight,
							   myDebugPath_2D[i - 1].y, 1.0f};
				aGraphics->AddLine(v1, v2);
			}
		}

		if (myDebugPath_3D.size() > 0) {
			v1.position = {myDebugStart_3D.x, myDebugStart_3D.y,
						   myDebugStart_3D.z, 1.0f};
			v2.position = {myDebugPath_3D.back().x, myDebugPath_3D.back().y,
						   myDebugPath_3D.back().z, 1.0f};
			aGraphics->AddLine(v1, v2);

			for (int i = 1; i < myDebugPath_3D.size(); i++) {
				v1.colour = {1, 0, 0, 1};
				v2.colour = {1, 0, 0, 1};
				v1.position = {myDebugPath_3D[i].x, myDebugPath_3D[i].y,
							   myDebugPath_3D[i].z, 1.0f};
				v2.position = {myDebugPath_3D[i - 1].x, myDebugPath_3D[i - 1].y,
							   myDebugPath_3D[i - 1].z, 1.0f};
				aGraphics->AddLine(v1, v2);
				...
				v1.position = {myDebugPath_3D[i].x, myDebugPath_3D[i].y,
							   myDebugPath_3D[i].z, 1.0f};
				v2.position = {myDebugPath_3D[i].x, myDebugPath_3D[i].y + 0.5f,
							   myDebugPath_3D[i].z, 1.0f};
				aGraphics->AddLine(v1, v2);
			}
		}
	...
	// <[ 2D Version ]> //
	std::vector<Vector2f> Pathfinder::FindPath_2D(Vector3f aStart,
												  Vector3f aEnd,
												  NavNode* aStartNode) {
		...
		int startIndex =
			aStartNode ? aStartNode->index : GetNodeIndexFromPoint(aEnd);
		int endIndex = GetNodeIndexFromPoint(aStart);

		// If either location is outside the Navmesh, try to get nearest point
		// on Navmesh.
		if (!ConstrainToNavmesh(aStart, endIndex)) {
			return empty;
		}
		if (!ConstrainToNavmesh(aEnd, startIndex)) {
			return empty;
		}
		...
		if (!AStar(startNode, endNode, aEnd)) {
			return empty;
		}
	...
	std::vector<Vector2f> Pathfinder::CreatePath_2D(FunnelData& aData,
													const Vector3f& aEnd) {
		...
		int targetCount = static_cast<int>(aData.leftTargets.size() +
										   aData.rightTargets.size());

		while (targetCount > 0) {
			Funneling_2D(aData, path);

			targetCount = static_cast<int>(aData.leftTargets.size() +
										   aData.rightTargets.size());
		}
	...
	void Pathfinder::Funneling_2D(FunnelData& aData,
								  std::vector<Vector2f>& aPath) {
		...
		if (aData.left.index != tempLeft.index) {
			...
			if (angle < 0) {
				Vector3f smoothedApex =
					GetSmoothedWaypoint(aData.right, aData.apex);
				...
				CollectFromSaved(aData.rightSaved, aData.rightTargets,
								 aData.right.index, aData.apex.index);
				CollectFromSaved(aData.leftSaved, aData.leftTargets,
								 apexNeighbour, aData.apex.index);
				...
				aData.angle =
					GetFunnelAngle(aData.left, aData.right, aData.apex);
		...
		if (aData.right.index != tempRight.index) {
			...
			if (angle < 0) {
				Vector3f smoothedApex =
					GetSmoothedWaypoint(aData.left, aData.apex);
				aPath.emplace_back(smoothedApex.x, smoothedApex.z);
				...
				CollectFromSaved(aData.leftSaved, aData.leftTargets,
								 aData.left.index, aData.apex.index);
				CollectFromSaved(aData.rightSaved, aData.rightTargets,
								 apexNeighbour, aData.apex.index);
	...
	std::vector<Vector3f> Pathfinder::CreatePath(
		FunnelData& aData, std::vector<Portal>& aPortalsToCross,
		Vector3f& aTarget) {
		...
		int targetCount =
			(int)aData.leftTargets.size() + (int)aData.rightTargets.size();
		while (targetCount > 0) {
	...
	void Pathfinder::Funneling(FunnelData& aData, std::vector<Vector3f>& aPath,
	...
	bool Pathfinder::LineIntersect(Vector2f& aOutLeft, Vector2f& aOutRight,
								   const Vector2f aStartPos,
								   const Vector2f aLineDir) {
		...
		// If none of the vertices lies infront of right intersectLine ->
		// continue;
		if (dotRightv0 < 0 && dotRightv1 < 0) {
			return false;
		}
		...
		bool isSameSide = (isSameSidev0 < 0 && isSameSidev1 < 0) ||
								  (isSameSidev0 >= 0 && isSameSidev1 >= 0)
							  ? true
							  : false;
		...
		// This way we can assure that v0_2D is always left and v1_2D is always
		// right
		if (isSameSidev0 < 0) {
	...
	void Pathfinder::AddPortalCrossings(const FunnelData& aData,
										const FunnelLocation& aSide,
										std::vector<Portal>& aTraversedPortals,
										std::vector<Vector3f>& aPath) {
		Vector3f result;
		Vector2f v0_2D;
		Vector2f v1_2D;
		...
		for (int i = (int)aTraversedPortals.size() - 1; i >= 0; i--) {
			...
			if (portal.indices[0] == aSide.index ||
				portal.indices[1] == aSide.index) {
				break;
			}
			...
			funnelLine.InitWith2Points(
				{aData.apex.position.x, aData.apex.position.z},
				{aSide.position.x, aSide.position.z});
			portalLine.InitWith2Points(v0_2D, v1_2D);

			if (!IntersectLineLine(funnelLine, portalLine,
								   outIntersectPoint2D)) {
				continue;
			}

			vecBetweenVertices = (portal.vertices[1] - portal.vertices[0]);
			scalar = (outIntersectPoint2D - v0_2D).Length() /
					 (v0_2D - v1_2D).Length();
	...
	void Pathfinder::AddEndCrossings(const FunnelData& aData,
									 const FunnelLocation& aSide,
									 std::vector<Portal>& aTraversedPortals,
									 std::vector<Vector3f>& aPath) {
		Vector3f result;
		...
		// If ApexIndex equals INT_MIN we have never assigned it. Meaning the
		// path is straight(No Funnels overlapped eachother).
	...
	void Pathfinder::RemoveExcessPortals(std::vector<Portal>& aTraversedPortals,
										 const Apex& aApex) {
		for (int i = 0; i < aTraversedPortals.size(); i++) {
			const Portal& portal = aTraversedPortals[i];

			if (portal.indices[0] == aApex.index ||
				portal.indices[1] == aApex.index) {
	...
	// <[ SETUP ]> //
	bool Pathfinder::AStar(NavNode* aStartNode, NavNode* aEndNode,
						   const Vector3f& aEnd) {
		ResetScore();
	...
	void Pathfinder::ScoreCell(NavNode& aFromNode, NavNode& aNodeToScore,
							   const Vector3f& aTarget) {
		// <[SOLUTION 3.0]> //
		...
		aNodeToScore.fromDist =
			(aFromNode.myScorePos - aNodeToScore.myScorePos).Length() +
			aFromNode.fromDist;
		aNodeToScore.toDist = (aNodeToScore.myScorePos - aTarget).Length();
		aNodeToScore.cost = aNodeToScore.fromDist + aNodeToScore.toDist;
	}
	void Pathfinder::ScoreNeighbours(NavNode& aNode,
									 std::priority_queue<NavNode>& aQueue,
									 const Vector3f& aTarget) {
	...
	void Pathfinder::CreateChannel(std::vector<NavNode*>& aChannel,
								   NavNode* aStartNode, NavNode* aEndNode) {
	...
	void Pathfinder::CreateFunnelLocations(
		std::vector<NavNode*>& aChannel, FunnelData& aData, Vector3f& aTarget,
		std::vector<Portal>* aCrossingPortals) {
		...
		for (int i = 0; i < aChannel.size() - 1; i++) {
			...
			Vector2f dir =
				Vector2f(v0.x - center.x, v0.z - center.z).GetNormalized();
			...
			aData.leftTargets.emplace_back(portal.vertices[left],
										   portal.indices[left]);
			aData.leftTargets.back().neighbourIndex = portal.indices[right];

			aData.rightTargets.emplace_back(portal.vertices[right],
											portal.indices[right]);
			aData.rightTargets.back().neighbourIndex = portal.indices[left];
		}
	}
	void Pathfinder::GeneratePortal(Portal& aOutPortal,
									const NavNode* aFromNode,
									const NavNode* aToNode) {
		int assignCount = 0;
		for (int i = 0; i < aFromNode->indices.size(); i++) {
	...
	// <[ COMMON ]> //
	Vector3f Pathfinder::GetSmoothedWaypoint(const FunnelLocation& aSide,
											 const Apex& aApex,
											 const float aSmoothFactor) const {
		Vector3f dirToPoint = (aSide.position - aApex.position).GetNormalized();
		Vector3f counterclockDir =
			Vector3f({-dirToPoint.z, aSide.position.y, dirToPoint.x});
		Vector3f smoothedApex =
			aSide.position + (counterclockDir * aSmoothFactor);
	...
	Vector3f Pathfinder::GetNearestNavmeshPos(const Vector3f& aTargetPos) {
		if (!myNavmesh || myNavmesh->myNodes.empty()) {
			KE_ERROR(
				...
				Returning input pos: (x %.2f, y %.2f, z %.2f)",
		...
		for (int i = 0; i < myNavmesh->myVertices.size(); i++) {
			if (myNavmesh->myIndiceNodeMap.find(i) ==
				myNavmesh->myIndiceNodeMap.end()) {
				continue;
			}

			float tempDistance =
				(myNavmesh->myVertices[i] - aTargetPos).Length();
		...
		for (int i = 0; i < indexToNodeMap->nodes.size(); i++) {
			...
			for (int j = 0; j < node.vertices.size(); j++) {
				...
				for (int k = 0; k < myIntersectionLines.size(); k++) {
					...
					// If none of the vertices lies on each side of the
					// intersectLine Direction -> Skip it Else the function
					// modifies v0 to be leftside and v1 to be rightside
					if (!LineIntersect(v0_2D, v1_2D, targetPos2D,
									   intersectLine.GetDirection())) {
						continue;
					}

					vertexLine.InitWith2Points(v0_2D, v1_2D);

					if (IntersectLineLine(intersectLine, vertexLine,
										  outIntersectPoint2D)) {
						Vector3f vecBetweenVertices =
							(*node.vertices[max] - *node.vertices[min]);
						float scalar = (outIntersectPoint2D - v0_2D).Length() /
									   (v0_2D - v1_2D).Length();
						...
						result = (*node.vertices[min]) +
								 (vecBetweenVertices * scalar);
		...
		if (intersectedLine != INT_MIN) {
			Vector2f offset =
				myIntersectionLines[intersectedLine].GetDirection() / 50.f;
	...
	float Pathfinder::GetFunnelAngle(const FunnelLocation& aLeft,
									 const FunnelLocation& aRight,
									 const Apex& aApex) const {
	...
	void Pathfinder::CollectFromSaved(std::vector<FunnelLocation>& aSavedList,
									  std::vector<FunnelLocation>& aTargetList,
									  const int aIndexToFind,
									  const int aIndexToAvoid) {
	...
	int Pathfinder::GetNodeIndexFromPoint(const Vector3f aPosition) {
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			if (!IntersectionPlaneRay(plane, ray, intersectPoint)) {
				continue;
			}
	...
	void Pathfinder::AlignTargets(
		std::vector<FunnelLocation>& aListToReduce,
		const std::vector<FunnelLocation>& aCompareList) {
	...
	NavNode* Pathfinder::GetNode(const Vector3f& aPosition) {
		if (!myNodes || !myNavmesh) {
			KE_ERROR(
				"Pathfinder: GetNode failed, myNodes or myNavmesh was nullptr");
			return nullptr;
		}

		// <<[Raycast from above the position]>> //
		Rayf ray =
			Rayf({aPosition.x, aPosition.y + 1.0f, aPosition.z}, {0, -1, 0});
		Plane plane;
		Vector3f intersectPoint;
		const Vector3f* v1;
		const Vector3f* v2;
