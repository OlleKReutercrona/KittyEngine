...
namespace KE {
	...
	bool KE::Navmesh::Init(Graphics* aGraphics, std::string& aObjFilepath) {
		...
		if (!meshList || meshList->myMeshes.empty()) {
			KE_ERROR("Navmesh failed to get MeshList from Graphics for file %s",
					 aObjFilepath.c_str());
			return false;
		}
	...
	void Navmesh::DebugRender(Graphics* aGraphics) {
		...
		for (int i = 0; i < myNodes.size(); i++) {
			...
			// <[Navmesh Triangles]> //
			{
				...
				v1.position = {node.vertices[0]->x, node.vertices[0]->y,
							   node.vertices[0]->z, 1.0f};
				v2.position = {node.vertices[1]->x, node.vertices[1]->y,
							   node.vertices[1]->z, 1.0f};
				aGraphics->AddLine(v1, v2);

				v1.position = v2.position;
				v2.position = {node.vertices[2]->x, node.vertices[2]->y,
							   node.vertices[2]->z, 1.0f};
				aGraphics->AddLine(v1, v2);

				v1.position = v2.position;
				v2.position = {node.vertices[0]->x, node.vertices[0]->y,
							   node.vertices[0]->z, 1.0f};
				aGraphics->AddLine(v1, v2);
			}
	...
	void Navmesh::CreateNodes(const Mesh& aMesh) {
		...
		for (int i = 0; i < aMesh.myVertices.size(); i++) {
			...
			myIndiceNodeMap.insert(std::pair<int, NodesAttachedToIndex>(
				i, NodesAttachedToIndex(i)));
		}
		for (unsigned int nodeIndex = 0; nodeIndex < aMesh.myIndices.size();
			 nodeIndex += 3) {
			int id = nodeIndex / 3;
			std::array<unsigned int, 3> indices = {
				aMesh.myIndices[nodeIndex + 0], aMesh.myIndices[nodeIndex + 1],
			...
			std::array<const Vector3f*, 3> vertices = {&myVertices[indices[0]],
	...
	void Navmesh::CreateNodes(const KE::KittyMesh& aMesh, Vector3f& aMin,
							  Vector3f& aMax) {
	...
	void Navmesh::GenerateLineColliders(NavNode* aNode) {
		...
		std::array<unsigned int, 2> line0 = {aNode->indices[0],
											 aNode->indices[1]};
		std::array<unsigned int, 2> line1 = {aNode->indices[1],
											 aNode->indices[2]};
		std::array<unsigned int, 2> line2 = {aNode->indices[2],
											 aNode->indices[0]};
		...
		if (!line0_shared) {
			...
			Vector2f v2_to_check = {myVertices[aNode->indices[2]].x,
									myVertices[aNode->indices[2]].z};
		...
		if (!line1_shared) {
			...
			Vector2f v2_to_check = {myVertices[aNode->indices[0]].x,
									myVertices[aNode->indices[0]].z};
		...
		if (!line2_shared) {
			...
			Vector2f v2_to_check = {myVertices[aNode->indices[1]].x,
									myVertices[aNode->indices[1]].z};
	...
	bool Navmesh::GotIndices(std::array<unsigned int, 2> aLine,
							 NavNode& aNode) {
		int count = 0;
