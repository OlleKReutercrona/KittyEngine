...
namespace AI {
	...
	AI::BehaviourTree* BehaviourTreeBuilder::GenerateBT(EnemyType aType) {
		...
		std::vector<KE::PinConnection>& rootConnections =
			data.connections[rootOutPinID];

		if (rootConnections.empty()) {
			KE_WARNING(
				"BT-Builder did not find any connections from root node.");
			return nullptr;
		}
	...
	KE::Script* BehaviourTreeBuilder::GetEnemyScript(const EnemyType aType) {
		auto* scriptManager =
			KE_GLOBAL::blackboard.Get<KE::ScriptManager>("scriptManager");
		std::string path = {};
	...
	KE::ScriptNode* BehaviourTreeBuilder::GetEditorRootNode(
		AI::RecursiveData& aData, std::string aRootName) {
	...
	TreeNode* BehaviourTreeBuilder::GetTreeNode(TreeNodeType aType) {
		...
		switch (aType) {
			...
			default:
				KE_LOG("BT-Builder(GetNode) did not find node: ",
					   typeNames[static_cast<int>(aType)].c_str());
				break;
		}
		return node;
	}

	bool BehaviourTreeBuilder::RecursiveAttach(KE::ScriptMemberID& aScriptNode,
											   AI::TreeNode* aTreeNode,
											   RecursiveData& aData) {
