...
namespace KE {
	...
	Window::WindowClass::WindowClass() : myHInst(GetModuleHandle(nullptr)) {
		...
		wc.hIcon = static_cast<HICON>(
			LoadImage(myWndClass.myHInst, L"ID", IMAGE_ICON, 32, 32, 0));
		wc.hCursor = nullptr;
		wc.hbrBackground = nullptr;
		wc.lpszMenuName = nullptr;
		wc.lpszClassName = GetName();
		wc.hIconSm = static_cast<HICON>(
			LoadImage(myWndClass.myHInst, L"ID", IMAGE_ICON, 16, 16, 0));
		RegisterClassEx(&wc);
	}
	...
	Window::Window(const int aWidth, const int aHeight, const wchar_t* aName)
		...
		myHWnd = CreateWindow(WindowClass::GetName(), aName,
		...
		// Event
		myResolutionEvent =
			ES::EventSystem::GetInstance().CreateNewEvent<ResolutionEvent>();
	...
	std::optional<int> Window::ProcessMessages() {
		...
		while (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
			// Check for quit because PeekMessage does not signal this via
			// return
			...
			TranslateMessage(
				&msg);	// Posts auxilliary WM_CHAR messages from key msg
			DispatchMessage(&msg);
		}
	...
	void Window::SetWindowDims(int aX, int aY, int aWidth, int aHeight,
							   bool aFullscreen) {
		aFullscreen;
		...
		if (aFullscreen) {
			...
			AdjustWindowRect(&wr, WS_CAPTION | WS_MINIMIZEBOX | WS_SYSMENU,
							 FALSE);
		}
	...
	void Window::SetRenderResolution(const int aWidth, const int aHeight) {
		std::cout << "Window::SetRenderResolution CALLED with: " << aWidth
				  << ", " << aHeight << ", isFullscreen "
	...
	void Window::SetFullscreen(bool aState, int aWidth, int aHeight) {
		std::cout << "Window::SetFullscreen CALLED with: "
				  << (aState ? "true" : "false") << std::endl;
	...
	LRESULT WINAPI Window::HandleMsgSetup(HWND aHWnd, UINT aUMsg,
										  WPARAM aWParam, LPARAM aLParam) {
		// Use create parameter passed in from CreateWindow() to store window
		// class pointer at WinAPI
		if (aUMsg == WM_NCCREATE) {
			// Extract ptr to window class from creation data
			const CREATESTRUCTW* const pCreate =
				reinterpret_cast<CREATESTRUCTW*>(aLParam);
			Window* const pWnd = static_cast<Window*>(pCreate->lpCreateParams);
			// Set WinAPI-managed user data to store ptr to window class
			SetWindowLongPtr(aHWnd, GWLP_USERDATA,
							 reinterpret_cast<LONG_PTR>(pWnd));
			// Sett kessage proc to normal (non-setup) handler now that setup is
			// finished
			SetWindowLongPtr(
				aHWnd, GWLP_WNDPROC,
		...
		// If we get a message before the WM_NCCREATE message, handle with
		// default handler
		return DefWindowProc(aHWnd, aUMsg, aWParam, aLParam);
	}

	LRESULT WINAPI Window::HandleMsgThunk(HWND aHWnd, UINT aUMsg,
										  WPARAM aWParam, LPARAM aLParam) {
		// Retrieve ptr to window class
		Window* const pWnd =
			reinterpret_cast<Window*>(GetWindowLongPtr(aHWnd, GWLP_USERDATA));
		// Forward message to window class handler
		return pWnd->HandleMsg(aHWnd, aUMsg, aWParam, aLParam);
	}

	LRESULT Window::HandleMsg(HWND aHWnd, UINT aUMsg, WPARAM aWParam,
							  LPARAM aLParam) {
		...
		if ((aUMsg >= WM_KEYDOWN && aUMsg <= WM_SYSKEYUP &&
			 ImGui::GetIO().WantCaptureKeyboard) ||
			(aUMsg >= WM_MOUSEMOVE && aUMsg <= WM_MOUSEWHEEL &&
			 ImGui::GetIO().WantCaptureMouse)) {
			// return DefWindowProc(aHWnd, aUMsg, aWParam, aLParam); //maybe
			// change this to just skipping the switch?
		}
		...
		switch (aUMsg) {
			...
			// Clear keystates when window loses focus to prevent input getting
			// stuck
			case WM_KILLFOCUS: {
			...
			case WM_SIZE: {
				if (myGraphics) {
					std::cout << "WM_SIZE CALLED with: " << LOWORD(aLParam)
							  << ", " << HIWORD(aLParam) << ", "
							  << (isFullscreen ? "true" : "false") << std::endl;

					myGraphics->SetRenderResolution(LOWORD(aLParam),
													HIWORD(aLParam));
					...
					myResolutionEvent->myFullscreen = isFullscreen;
					ES::EventSystem::GetInstance().SendEvent(
						*myResolutionEvent);
				}
				break;
			}
			case WM_ACTIVATE: {
				// Confine/free cursor on window to foreground/background if
				// cursor disabled
				if (!isCursorEnabled) {
			...
			case WM_DROPFILES: {
				...
				// for every file, run ReceiveDragDrop on the an std::string of
				// the file path
				...
				for (UINT i = 0; i < fileCount; i++) {
					...
					DragQueryFileA(hDrop, i, filePath.data(),
								   filePathLength + 1);
	...
	void Window::OnReceiveEvent(ES::Event& aEvent) {
		if (GUIResolutionEvent* resEvent =
				dynamic_cast<GUIResolutionEvent*>(&aEvent)) {
			std::cout << "Window GUIResolutionEvent Received with: "
					  << resEvent->myWidth << ", " << resEvent->myHeight << ", "
			...
			if (resEvent->myFullscreen && !isFullscreen) {
				...
				SetFullscreen(resEvent->myFullscreen, resEvent->myWidth,
							  resEvent->myHeight);
				SetRenderResolution(resEvent->myWidth, resEvent->myHeight);
			}
#endif
