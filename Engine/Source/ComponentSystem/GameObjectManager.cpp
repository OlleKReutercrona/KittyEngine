...
void KE::GameObjectManager::Unload() {
	for (int i = 0; i < myNewGameObjects.size(); i++) {
		if (myNewGameObjects[i]->myID ==
			ReservedGameObjects::eGameSystemManager) {
...
void KE::GameObjectManager::LoadTransforms(const LevelTransformFile& aFile) {
	...
	/*
		...
		Preferably we would want to pool up all the transforms elsewhere (maybe
	   as a component) where we can memcpy all the transforms from the file
	   directly to the component.
		- Olle
	*/

	for (int i = 0; i < aFile.myData.transforms.size(); i++) {
		CreateGameObject(aFile.myData.transformsIDs[i], nullptr,
						 aFile.myData.transforms[i]);
	}
}

KE::GameObject* KE::GameObjectManager::CreateGameObject(
	const int anID, const std::string* aName, const Transform& aTransform,
	const bool isStatic) {
...
void KE::GameObjectManager::DestroyGameObject(const int anID) {
	...
	myGameObjects.erase(
		std::remove(myGameObjects.begin(), myGameObjects.end(), GO),
		myGameObjects.end());
	myNewGameObjects.erase(
		std::remove(myNewGameObjects.begin(), myNewGameObjects.end(), GO),
		myNewGameObjects.end());
...
void KE::GameObjectManager::RemoveParent(GameObject& aChild) {
	...
	for (size_t i = 0; i < aChild.myParent->myChildren.size(); i++) {
		if (aChild.myParent->myChildren[i] == &aChild) {
			aChild.myParent->myChildren.erase(
				aChild.myParent->myChildren.begin() + i);
			break;
		}
	}
...
void KE::GameObjectManager::Init(Scene* aScene) {
	...
	for (unsigned int i = 0; i < myPersistantGameObjects.size(); i++) {
		...
		myMappedGameObjects.emplace(std::pair(myPersistantGameObjects[i]->myID,
											  myPersistantGameObjects[i]));
	}
...
const std::vector<KE::GameObject*>& KE::GameObjectManager::GetGameObjects()
	const {
	return myGameObjects;
}

const std::vector<KE::GameObject*>& KE::GameObjectManager::GetNewGameObjects()
	const {
