...
void KE::SceneTransitionComponent::OnTriggerEnter(
	const CollisionData& aCollisionData) {
	if (!KE::Collision::IsOnLayer(aCollisionData.hitGameObject.myLayer,
								  KE::Collision::Layers::Player))
		return;

	myGameObject.GetManager().GetScene()->SetLevelFromBuildIndex(mySceneIndex);
}

void KE::SceneTransitionComponent::OnCollisionEnter(
	const CollisionData& aCollisionData) {
	if (!KE::Collision::IsOnLayer(aCollisionData.hitGameObject.myLayer,
								  KE::Collision::Layers::Player))
		return;
...
void KE::SceneTransitionComponent::SetData(void* someData) {
	SceneTransitionComponentData* data =
		(SceneTransitionComponentData*)someData;
