...
namespace KE {
	LightComponent::LightComponent(GameObject& aGameObject)
		: Component(aGameObject) {}
	LightComponent::~LightComponent() = default;

	void LightComponent::SetData(void* aDataObject) {
		const LightComponentData* data =
			static_cast<LightComponentData*>(aDataObject);
			...
			case eLightType::Directional: {
				DirectionalLightData* lightData =
					static_cast<DirectionalLightData*>(
						myLightComponentData.myLightData);
				lightData->myDirection =
					myGameObject.myTransform.GetForward() * -1.0f;
			} break;
			case eLightType::Point: {
				// PointLightData* lightData =
				// static_cast<PointLightData*>(myLightComponentData.myLightData);
				// lightData->myPosition =
				// myGameObject.myWorldSpaceTransform.GetPosition();
				//
				// aDbg.RenderSphere(myGameObject.myWorldSpaceTransform.GetPositionRef(),
				// lightData->myRange);
			} break;
			case eLightType::Spot: {
				SpotLightData* lightData = static_cast<SpotLightData*>(
					myLightComponentData.myLightData);
				lightData->myDirection =
					myGameObject.myTransform.GetForward() * -1.0f;
				lightData->myPosition =
				...
				aDbg.RenderCone(
					myGameObject.myWorldSpaceTransform.GetPosition(),
					Vector3f(-lightData->myDirection.x,
							 -lightData->myDirection.y,
							 -lightData->myDirection.z),
					lightData->myRange, lightData->myOuterAngle,
	...
	void LightComponent::UpdateLightData() const {
		switch (myLightComponentData.myLightType) {
			...
			case eLightType::Point: {
				PointLightData* lightData = static_cast<PointLightData*>(
					myLightComponentData.myLightData);
				lightData->myPosition =
					myGameObject.myWorldSpaceTransform.GetPosition();
			} break;
	...
	void LightComponent::OnEnable() {
		if (myLightComponentData.myLightType == eLightType::Point) {
			...
			static_cast<SpotLightData*>(myLightComponentData.myLightData)
				->isActive = 1;
		}
	}
	...
	void LightComponent::OnDestroy() {
		myManager->RemoveLightData(myLightComponentData.myLightType,
								   myLightComponentData.myLightData);
	}
}  // namespace KE
