...
KE::BoxColliderComponent::BoxColliderComponent(GameObject& aGameObject)
...
void KE::BoxColliderComponent::Update() {
	...
	{
		myCollider->UpdatePosition(
			myGameObject.myWorldSpaceTransform.GetPositionRef());
	...
	//&myCollider->myCollisionData[i] }); 		if
	//(myCollider->myCollisionData[i].isTrigger)
	//		{
...
void KE::BoxColliderComponent::OnCollisionEnter(
	const CollisionData& aCollisionData) {
	aCollisionData;
	// std::cout << "Collision enter | ID " << myGameObject.myID << " collided
	// with " << aCollisionData.hitGameObject.myID << "\n";
}

void KE::BoxColliderComponent::OnCollisionStay(
	const CollisionData& aCollisionData) {
	aCollisionData;
	// std::cout << "Collision stay | ID " << myGameObject.myID << " collided
	// with " << aCollisionData.hitGameObject.myID << "\n";
}

void KE::BoxColliderComponent::OnCollisionExit(
	const CollisionData& aCollisionData) {
	aCollisionData;
	// std::cout << "Collision exit | ID " << myGameObject.myID << " collided
	// with " << aCollisionData.hitGameObject.myID << "\n";
}
...
void KE::BoxColliderComponent::DrawDebug(KE::DebugRenderer& aDbg) {
	...
	aDbg.RenderCube(temp, mySize, myIsHit ? myHitColour : myDefaultColour);
}
...
void KE::BoxColliderComponent::SetData(void* aDataObject) {
	...
	// myCollider->myShape = new Box(myMin, myMax,
	// myGameObject.myTransform.GetPositionRef());
