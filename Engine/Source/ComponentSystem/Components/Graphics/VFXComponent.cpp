...
KE::VFXComponent::VFXComponent(GameObject& aGameObject)
...
void KE::VFXComponent::TriggerVFX(int anIndex, bool aLooping,
								  bool aStationary) {
	if (myVFXSequenceIndices.size() <= anIndex) {
		KE_ERROR("VFXComponent::TriggerVFX: Index %i out of range", anIndex);
		return;
	}
	VFXRenderInput in(myGameObject.myWorldSpaceTransform, aLooping,
					  aStationary);
	myVFXManager->TriggerVFXSequence(myVFXSequenceIndices[anIndex], in);
}

void KE::VFXComponent::TriggerVFXAt(int anIndex, const Vector3f& aPosition,
									bool aLooping) {
	if (myVFXSequenceIndices.size() <= anIndex) {
...
void KE::VFXComponent::TriggerVFXCustom(int anIndex,
										const VFXRenderInput& anInput) {
	if (myVFXSequenceIndices.size() <= anIndex) {
		KE_ERROR("VFXComponent::TriggerVFXCustom: Index %i out of range",
				 anIndex);
		return;
	}
...
void KE::VFXComponent::StopVFXCustom(int anIndex,
									 const VFXRenderInput& anInput) {
	if (myVFXSequenceIndices.size() <= anIndex) {
...
void KE::VFXComponent::SetSequences(
	const std::vector<std::string>& someVFXSequenceNames) {
	myVFXSequenceIndices.clear();
	for (auto& name : someVFXSequenceNames) {
		myVFXSequenceIndices.push_back(
			myVFXManager->GetVFXSequenceFromName(name));
	}
}
