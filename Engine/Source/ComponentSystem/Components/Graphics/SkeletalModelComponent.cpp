...
KE::SkeletalModelComponent::SkeletalModelComponent(GameObject& aGameObject)
...
void KE::SkeletalModelComponent::Update() {
	...
	// RenderSkeleton();
	//  TODO Someone sets the scale to 1.0f somewhere so this is an ugly hack to
	//  fix that
...
void KE::SkeletalModelComponent::SetData(void* aDataObject) {
	SkeletalModelComponentData& data =
		*(SkeletalModelComponentData*)aDataObject;
