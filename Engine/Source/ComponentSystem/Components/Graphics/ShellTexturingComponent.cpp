...
KE::ShellTexturingComponent::ShellTexturingComponent(
	GameObject& aParentGameObject)
	: Component(aParentGameObject) {
...
void KE::ShellTexturingComponent::Refresh() {
	...
	const auto& shellTexturingComponentObjs =
		myGameObject.GetManager()
			.GetGameObjectsWithComponent<ShellTexturingComponent>();
	for (const auto& obj : shellTexturingComponentObjs) {
		ShellTexturingComponent& shellTexturingComponent =
			obj->GetComponent<ShellTexturingComponent>();
		if (&shellTexturingComponent == this) {
			continue;
		}

		shellTexturingComponent.primary =
			primary && shellTexturingComponent.primary
				? false
				: shellTexturingComponent.primary;
	...
	/*const auto& players =
	myGameObject.GetManager().GetGameObjectsWithComponent<P8::Player>(); const
	auto& boomers =
	myGameObject.GetManager().GetGameObjectsWithComponent<P8::BoomerangComponent>();
	...
	// myDisplacementObjects.push_back({myGameObject.GetManager().GetGameObject(4),
	// {1.0f, 1.0f, 1.0f} });
}
...
void KE::ShellTexturingComponent::CalculateBounds() {
	...
	Vector3f ownMinBounds =
		myGameObject.myTransform.GetPosition() - tScl / 2.0f;
	Vector3f ownMaxBounds =
		myGameObject.myTransform.GetPosition() + tScl / 2.0f;
	...
	for (const auto& shellTexturingComponent : otherShellTexturingComponents) {
		const Vector3f& pos =
			shellTexturingComponent->myGameObject.myTransform.GetPosition();
		const Vector3f& scl =
			shellTexturingComponent->myGameObject.myTransform.GetScale();
		sharedMinimumBounds = {
	...
	float minXProgress = (ownMinBounds.x - sharedMinimumBounds.x) /
						 (sharedMaximumBounds.x - sharedMinimumBounds.x);
	float minZProgress = (ownMinBounds.z - sharedMinimumBounds.z) /
						 (sharedMaximumBounds.z - sharedMinimumBounds.z);
	float maxXProgress = (ownMaxBounds.x - sharedMinimumBounds.x) /
						 (sharedMaximumBounds.x - sharedMinimumBounds.x);
	float maxZProgress = (ownMaxBounds.z - sharedMinimumBounds.z) /
						 (sharedMaximumBounds.z - sharedMinimumBounds.z);
...
void KE::ShellTexturingComponent::RenderDisplacement() {
	...
	myGraphics->GetShellTexturedRenderer()->FadeDisplacement(shellModelData,
															 1.0f, 1.0f);
	...
	myGraphics->GetShellTexturedRenderer()->RenderDisplacement(
		shellModelData, displacements, sharedBounds.min, sharedBounds.max,
		nullptr);
}
...
void KE::ShellTexturingComponent::SetDisplacementForLevel() {
	...
	std::string fileName = std::format(
		"Displacement_{}.dds", myGameObject.GetManager().GetScene()->sceneName);
	...
	if (!std::filesystem::exists(path)) {
		...
		std::filesystem::copy(
			zeroPath, path, std::filesystem::copy_options::overwrite_existing);
	}

	Texture* displacementTexture =
		myGraphics->GetTextureLoader().GetTextureFromPath(path, false, true);
...
void KE::ShellTexturingComponent::ClearDisplacement() {
	...
	std::filesystem::copy(zeroPath, path,
						  std::filesystem::copy_options::overwrite_existing);
	shellModelData->displacementTexture =
		myGraphics->GetTextureLoader().GetTextureFromPath(path, false, true);
}
...
void KE::ShellTexturingComponent::DrawDebug(KE::DebugRenderer& aDbg) {
	Vector3f pos =
		sharedBounds.min + (sharedBounds.max - sharedBounds.min) * 0.5f;
