...
namespace KE {
	GUIElement& GUIScene::CreateGUIElement(
		const Vector2f& aSize, const Vector2f& aOffset,
		const Vector2f& aScreenResolution, const bool aIsClickable,
		const eAlignType aAlignType, const eProgressionDirection aDirection) {
		float aPosX;
		float aPosY;
		...
		element.myBox.myOffsetResolutionFactor = {
			aOffset.x / aScreenResolution.x, aOffset.y / aScreenResolution.y};
		element.myBox.mySizeResolutionFactor = {aSize.x / aScreenResolution.x,
												aSize.y / aScreenResolution.y};
	...
	GUIElement& GUIScene::CreateGUIElement(
		const std::string& aName, const std::string& aTexturePath,
		...
		const Vector2i& aScreenResolution, const bool aIsButton) {
		float aPosX;
		float aPosY;
		...
		element.myBox.myOffset.x =
			aOffsetResolutionFactor.x * (float)aScreenResolution.x;
		element.myBox.myOffset.y =
			aOffsetResolutionFactor.y * (float)aScreenResolution.y;
		element.myBox.myWidth =
			aSizeResolutionFactor.x * (float)aScreenResolution.x;
		element.myBox.myHeight =
			aSizeResolutionFactor.y * (float)aScreenResolution.y;
	...
	GUIElement& GUIScene::DuplicateGUIElement(const GUIElement& aElement,
											  const Vector2f& aResolution) {
		std::string name = aElement.myName + "_Copy";
		return CreateGUIElement(
			{aElement.myBox.myWidth, aElement.myBox.myHeight},
			aElement.myBox.myOffset, aResolution,
			// aElement.myType,
			aElement.isButton, aElement.myAlignType,
	...
	void GUIScene::MoveElement(const size_t aIndex, const size_t aNewIndex) {
		if (aIndex < myGUIElements.size() &&
			aNewIndex < myGUIElements.size() + 1) {
			if (aIndex >= aNewIndex) {
				std::rotate(myGUIElements.rend() - aIndex - 1,
							myGUIElements.rend() - aIndex,
							myGUIElements.rend() - aNewIndex);
			} else if (aIndex < aNewIndex) {
				std::rotate(myGUIElements.begin() + aIndex,
							myGUIElements.begin() + aIndex + 1,
							myGUIElements.begin() + aNewIndex);
			}
		}
	}
	...
	// void GUIScene::Init(const eGUISceneType aType, const size_t
	// aNumberOfElementsToReserve)
	//{
	...
	void GUIScene::UpdateGUISize(const int aWidth, const int aHeight) {
		...
		for (GUIElement& element : myGUIElements) {
			...
			// Update tooltip position
			{
				...
				float posX =
					element.myBox.myScreenPosition.x + tooltip.myBox.myOffset.x;
				float posY =
					element.myBox.myScreenPosition.y + tooltip.myBox.myOffset.y;

				tooltip.myLinePoints[0] = {posX, posY};
				tooltip.myLinePoints[1] = {posX + tooltip.myBox.myWidth, posY};
				tooltip.myLinePoints[2] = {posX + tooltip.myBox.myWidth,
										   posY - tooltip.myBox.myHeight};
