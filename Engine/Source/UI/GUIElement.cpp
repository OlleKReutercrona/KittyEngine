...
namespace KE {
	...
	void GUIElement::SetFillFactor(const float aFactor) {
		...
		mySpriteBatch.myInstances[0].SetFillFactor(myFillFactor,
												   myProgressionDirection);
	}

	void GUIElement::Activate() {
		if (!myParentScene->IsActive()) {
			return;
		}
	...
	void GUIElement::UpdateSpriteScaleAndPosition(
		const Vector2f& aResolution, const bool aUpdateResolution) {
		float posX = 0.0f;
		float posY = 0.0f;
		...
		myBox.myOffsetResolutionFactor = {myBox.myOffset.x / aResolution.x,
										  myBox.myOffset.y / aResolution.y};
		myBox.mySizeResolutionFactor = {myBox.myWidth / aResolution.x,
		...
		if (!mySpriteBatch.myInstances.empty()) {
			mySpriteBatch.myInstances[0].myAttributes.myTransform.SetPosition(
				{posX, posY - myBox.myHeight, 0.0f});
			mySpriteBatch.myInstances[0].myAttributes.myTransform.SetScale(
				{myBox.myWidth, myBox.myHeight, 1.0f});
		}

		// mySprite->myAttributes.myTransform.SetScale({ myBox.myWidth,
		// myBox.myHeight, 1.0f });

		// Vector2f pos = myBox.myScreenPosition;
		// mySprite->myAttributes.myTransform.SetPosition({ pos.x, pos.y -
		// myBox.myHeight, 0.0f });
	}
