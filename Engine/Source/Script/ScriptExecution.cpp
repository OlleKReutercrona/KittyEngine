...
namespace KE {
	...
	PinValue NodeExecutor::GetPinValue(Pin* aPin) {
		...
		int pinIndex =
			connectedID.idParts.pinID - (int)node->GetInputPins().size();
	...
	void NodeExecutor::AddExecution(ScriptMemberID anID,
									int anExecutionContext) {
	...
	void NodeExecutor::ExecuteMacro(ScriptMacro* aMacro,
									ScriptMemberID aTriggeringNode) {
		ScriptRuntime subRuntime(aMacro->macroScript,
								 myRuntime->runningGameObject);
		subRuntime.macroExecutionData.macroExecutionNode =
			myScript->GetNodes()[aTriggeringNode];
		subRuntime.macroExecutionData.macroExecutor = this;
		subRuntime.Execute(aMacro->macroStartID);
	}

	PinValue NodeExecutor::GetMacroValue(ScriptMacro* aMacro,
										 ScriptMemberID aTriggeringNode,
										 int aPinIndex) {
		ScriptRuntime subRuntime(aMacro->macroScript,
								 myRuntime->runningGameObject);
		...
		return subRuntime.myExecutor.GetPinValue(
			&aMacro->macroScript->GetNodes()[aMacro->macroEndID]
				 ->GetInputPins()[aPinIndex]);
	}
	...
	void NodeExecutor::SetVariable(const std::string& aName,
								   const PinValue& aValue) {
	...
	void NodeExecutor::SetNodeVariable(ScriptMemberID anID,
									   const std::any& aValue) {
	...
	int NodeExecutor::GetContext(ScriptMemberID anID) {
		if (!myRuntime->executionContexts.contains(anID)) {
			return -1;
		}
		if (myRuntime->executionContexts[anID].size() == 0) {
			return -1;
		}
	...
	std::vector<Collider*> NodeExecutor::GetCollidersInBox(
		const Vector3f& aPosition, const Vector3f& aSize,
		const int aLayerMask) {
		Box box(aSize / -2.0f, aSize / 2.0f, aPosition);

		std::vector<Collider*> result =
			myRuntime->myCollisionHandler->BoxCast(box, aLayerMask);
	...
	void ScriptRuntime::TriggerEntryPoint(EntryPointType aType) {
		for (auto& entryPoint :
			 myScript->GetEntryPoints()[static_cast<int>(aType)]) {
			Execute(entryPoint);
		}
	}
	...
	void ScriptRuntime::Init(Script* aScript, GameObject* aGameObject) {
		myCollisionHandler =
			KE_GLOBAL::blackboard.Get<KE::CollisionHandler>("collisionHandler");
		myGameObjectManager = KE_GLOBAL::blackboard.Get<KE::GameObjectManager>(
			"gameObjectManager");
	...
	void ScriptRuntime::Execute(const ScriptMemberID& anExecutionPointID) {
		for (auto& connection :
			 myScript->GetConnections()[anExecutionPointID]) {
			...
			for (size_t i = 0; i < sizeDiff; i++) {
				...
				if (executionContexts.contains(fromNodeID) &&
					executionContexts[fromNodeID].size() > 0) {
