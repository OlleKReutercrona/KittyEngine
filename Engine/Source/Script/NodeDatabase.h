...
namespace KE {
	...
	public:
		...
		std::unordered_map<std::string, LanguageDefinitionNew*>
			myLanguageDefinitions;	// todo: this don't live here
		...
		std::unordered_map<ValueType, std::vector<NodePinDatabaseValue>>
			nodeInPinDatabase;
		std::unordered_map<ValueType, std::vector<NodePinDatabaseValue>>
			nodeOutPinDatabase;

		std::unordered_map<std::string, std::vector<NodePinDatabaseValue>>
			nodePinCodeInDatabase;
		std::unordered_map<std::string, std::vector<NodePinDatabaseValue>>
			nodePinCodeOutDatabase;
		...
		void CalculateNodeCategoryStack(const std::string& aNodeName,
										const char* aCategoryStack);
		void ProcessNodeData(const std::string& aNodeName);

		void RegisterNode(const char* aNodeName, NodeCategory aCategory,
						  const char* aDescription,
						  const NodeCreateFunction& aCreateFunction,
						  const char* aCategoryStack);
	...
	void NodeTypeDatabase::RegisterNode(const char* aCategoryStack) {
		...
		nodeTypes[nodeName] = {node.GetName(), node.GetCategory(),
							   node.GetDescription(),
							   [](Script* aScript) -> ScriptNode* {
								   ScriptNode* made = new T();
								   made->AssignScript(aScript);
								   return made;
							   }};
