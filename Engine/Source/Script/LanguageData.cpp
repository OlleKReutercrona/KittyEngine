...
bool KE::LanguageInspector::InspectType(const DataType& aType, void* aData,
										const char* aLabel) {
	if (aType.typeName == "float") {
		return ImGui::DragFloat(aLabel, (float*)aData);
	}
	if (aType.typeName == "float2") {
		return ImGui::DragFloat2(aLabel, (float*)aData);
	}
	if (aType.typeName == "float3") {
		return ImGui::DragFloat3(aLabel, (float*)aData);
	}
	if (aType.typeName == "float4") {
		return ImGui::DragFloat4(aLabel, (float*)aData);
	}

	if (aType.typeName == "int") {
		return ImGui::DragInt(aLabel, (int*)aData);
	}
	if (aType.typeName == "int2") {
		return ImGui::DragInt2(aLabel, (int*)aData);
	}
	if (aType.typeName == "int3") {
		return ImGui::DragInt3(aLabel, (int*)aData);
	}
	if (aType.typeName == "int4") {
		return ImGui::DragInt4(aLabel, (int*)aData);
	}

	if (aType.typeName == "bool") {
		return ImGui::Checkbox(aLabel, (bool*)aData);
	}
	if (aType.typeName == "uint") {
		return ImGui::DragInt(aLabel, (int*)aData, 1, 0, INT_MAX, "%d",
...
bool KE::LanguageDefiner::IsType(const std::string& aWord,
								 const LanguageDefinition& aLanguage) {
	if (aLanguage.dataTypes.contains(aWord)) {
		return true;
	}

	return false;  // TODO: this should be false, but I'm just testing the
				   // function right now
}
...
void KE::LanguageDefiner::EvaluateStructDefinition(
	const std::string& aLine, StructDefinition* aStruct,
	const LanguageDefinition& aWorkingLanguage) {}

void KE::LanguageDefiner::EvaluateFunctionDefinition(
	...
	std::string parameters =
		aLine.substr(openParen + 1, closeParen - openParen - 1);
	...
	for (const auto& parameter : splitParameters) {
		...
		aFunction->variants.back().parameters.push_back(
			{aWorkingLanguage.dataTypes.at(parameterParts[0]),
			 parameterParts[1]});
	}

	// identify function return type, it should be the first word in the line
	// that is a type
	std::vector<std::string> lineParts;
	SplitStringBy(aLine, ' ', lineParts);
	for (const auto& part : lineParts) {
		if (IsType(part, aWorkingLanguage)) {
			aFunction->variants.back().returnType =
				aWorkingLanguage.dataTypes.at(part);
			break;
		}
	}
...
void KE::LanguageDefiner::InterpretStructs(
	const char* aStructsFile, LanguageDefinition& aWorkingLanguage) {
	...
	// the file we're reading is just a source code file, so we'll have to do
	// some parsing to get the struct definitions
	std::string line;
		...
		if (!currentStruct) {
			...
			if (structInfoPosition != std::string::npos) {
				const size_t nameStart =
					structInfoPosition + sizeof("//@struct");
				const size_t nameEnd = line.find(' ', nameStart);

				const std::string structName =
					line.substr(nameStart, nameEnd - nameStart);

				aWorkingLanguage.dataTypes[structName] = {
					structName, {1.0f, 0.0f, 1.0f}, true};
			...
			if (scopeDepth > 0) {
				...
				if (parts.size() >= 2 && IsType(parts[0], aWorkingLanguage)) {
					currentStruct->members.push_back(
						{aWorkingLanguage.dataTypes.at(parts[0]), parts[1]});
				}
			}
...
void KE::LanguageDefiner::InterpretBuffers(
	const char* aBuffersFile, LanguageDefinition& aWorkingLanguage) {
	...
	while (std::getline(fileStream, line)) {
		if (!currentBuffer) {
			...
			if (bufferInfoPosition != std::string::npos) {
				const size_t nameStart =
					bufferInfoPosition + sizeof("//@buffer");
				const size_t nameEnd = line.find(' ', nameStart);

				const std::string bufferName =
					line.substr(nameStart, nameEnd - nameStart);

				aWorkingLanguage.dataTypes[bufferName] = {
					bufferName, {1.0f, 0.0f, 1.0f}, true};
			...
			if (instructLine == 0) {
				...
				if (slotPos != std::string::npos) {
					...
					const std::string slotStr =
						line.substr(slotStart, slotEnd - slotStart);
			...
			if (scopeDepth > 0) {
				...
				SplitStringBy(line, ' ', parts);
				if (parts.size() >= 2 && IsType(parts[0], aWorkingLanguage)) {
					currentBuffer->members.push_back(
						{aWorkingLanguage.dataTypes.at(parts[0]), parts[1]});
				}
			}
...
void KE::LanguageDefiner::InterpretFunctions(
	const char* aFunctionsFile, LanguageDefinition& aWorkingLanguage) {
	...
	// the file we're reading is just a source code file, so we'll have to do
	// some parsing to get the function definitions
	std::string line;
	// std::vector<std::string> lines;
	...
	while (std::getline(fileStream, line)) {
		...
		if (!currentFunction) {
			...
			if (functionInfoPosition != std::string::npos) {
				const size_t nameStart =
					functionInfoPosition + sizeof("//@function");
				const size_t nameEnd = line.find(' ', nameStart);
			...
			if (inFunctionLine == 0) {
				EvaluateFunctionDefinition(line, currentFunction,
										   aWorkingLanguage);
			}
...
void KE::LanguageDefiner::InterpretEntrypoints(
	const char* aEntrypointsFile, LanguageDefinition& aWorkingLanguage) {
	...
	while (std::getline(fileStream, line)) {
		if (!currentEntrypoint) {
			...
			if (entryPointPos != std::string::npos) {
				const size_t nameStart =
					entryPointPos + sizeof("//@entrypoint");
				const size_t nameEnd = line.find(' ', nameStart);
			...
			// replace parentheses with spaces so we can split the line by
			// spaces
			...
			for (size_t i = 0; i < words.size(); i++) {
				...
				if (IsType(word, aWorkingLanguage)) {
					if (!assignedReturnType) {
						currentEntrypoint->returnType =
							aWorkingLanguage.dataTypes.at(word);
						assignedReturnType = true;
					} else {
						currentEntrypoint->inputParameters.push_back(
							{aWorkingLanguage.dataTypes.at(word),
							 words[i + 1]});
					}
				}
...
void KE::LanguageDefiner::InterpretTextures(
	const char* aTexturesFile, LanguageDefinition& aWorkingLanguage) {
...
KE::LanguageDefinition KE::LanguageDefiner::DefineLanguage(
