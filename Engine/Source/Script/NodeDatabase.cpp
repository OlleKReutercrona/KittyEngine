...
KE::NodeTypeDatabase::NodeTypeDatabase() {
	...
	public:
		...
		virtual void Init() override {
			AddOutputPin(ValueType::Bool, PinType::Value, "Value");
		}
	};
	...
	public:
		...
		virtual void Init() override {
			AddOutputPin(ValueType::Int, PinType::Value, "Value");
		}
	};
	...
	public:
		...
		virtual void Init() override {
			AddOutputPin(ValueType::Float, PinType::Value, "Value");
		}
	};
	...
	private:
	public:
		DefineNode("Total Time", NodeCategory::Math,
				   "Outputs the seconds since the game started.");

		TotalTimeNode() : ScriptNode() {}

		virtual void Init() override {
			AddOutputPin(ValueType::Float, PinType::OutputValue, "Value");
		}

		virtual PinValue GetOutputPinValue(int aPinIndex,
										   NodeExecutor* anExecutor) override {
	...
	private:
	public:
		DefineNode("Delta Time", NodeCategory::Math,
				   "Outputs the seconds since the last frame.");
	...
	private:
	public:
		DefineNode("Awake", NodeCategory::Flow,
				   "Triggered when the object is awoken.");
	...
	private:
	public:
		DefineNode("Condition", NodeCategory::Flow,
				   "Branching flows based on condition.");
		...
		virtual void Execute(NodeExecutor* anExecutor) override {
			if (anExecutor->GetPinValue(&inputPins[1]).value.Bool) {
				anExecutor->AddExecution(outputPins[0].ID);
			} else {
	...
	public:
		DefineNode("Switch", NodeCategory::Flow,
				   "Branching flows based on defined cases.");
		...
		void AddCasePair() {
			...
			AddInputPin(inputPins[1].value.type, inputPins[1].type,
						caseName.c_str());
		...
		virtual void ExtraSerialize(void* aOutJson,
									Script* aLoadingScript) override {
			nlohmann::json* json = (nlohmann::json*)aOutJson;
			(*json)["casePairs"] = myCasePairs;
		}

		virtual void ExtraDeserialize(void* aOutJson,
									  Script* aLoadingScript) override {
			nlohmann::json* json = (nlohmann::json*)aOutJson;
			if (!json->contains("casePairs")) {
				return;
			}
			int casePairs = (*json)["casePairs"].get<int>();

			for (int i = 0; i < casePairs; i++) {
				AddCasePair();
			}
		}
	...
	public:
		...
		virtual void Execute(NodeExecutor* anExecutor) override {
			for (int i = (int)outputPins.size() - 1; i >= 0; i--) {
				anExecutor->AddExecution(outputPins[i].ID);
			}
		}
	};
	...
	private:
	public:
		DefineNode("Value Link", NodeCategory::Flow,
				   "Connects two value links, to improve structure.");
		...
		virtual bool TryAssignUnsupportedPinValue(int aPinIndex,
												  PinValue aValue) override {
		...
		virtual void BeginDrawNode() override {
			ImNodes::PushStyleVar(ImNodesStyleVar_NodePadding, ImVec2(4.0f, 0));
		}
		virtual void EndDrawNode() override {
			ImNodes::PopStyleVar();
		}
		...
		virtual void CustomRender() override {
			...
			drawList->AddRect(
				ImGui::GetCursorScreenPos() - ImVec2(4.0f, 1.0f),
	...
	private:
	public:
		DefineNode("Flow Link", NodeCategory::Flow,
				   "Connects two flow links, to improve structure.");
	...
	public:
		...
		PinValue GetOutputPinValue(int aPinIndex,
								   NodeExecutor* anExecutor) override {
			PinValue result = {ValueType::Float};
			result.value.Float +=
				anExecutor->GetPinValue(&inputPins[0]).value.Float;
			result.value.Float +=
				anExecutor->GetPinValue(&inputPins[1]).value.Float;
			return result;
		}
	};
	...
	private:
	public:
		DefineNode("Subtracts", NodeCategory::Math,
	...
	private:
	public:
		DefineNode("Multiply", NodeCategory::Math,
	...
	private:
	public:
		DefineNode("Sine", NodeCategory::Math,
		...
		PinValue GetOutputPinValue(int aPinIndex,
			...
			PinValue result = {ValueType::Float};
			result.value.Float =
				sinf(anExecutor->GetPinValue(&inputPins[0]).value.Float);
			return result;
		}
	};
	...
	private:
	public:
		DefineNode("Cosine", NodeCategory::Math,
		...
		PinValue GetOutputPinValue(int aPinIndex,
			...
			PinValue result = {ValueType::Float};
			result.value.Float =
				cosf(anExecutor->GetPinValue(&inputPins[0]).value.Float);
			return result;
		}
	};
	...
	private:
	public:
		DefineNode("NOT", NodeCategory::Math,
		...
		virtual PinValue GetOutputPinValue(int aPinIndex,
			...
			return {ValueType::Bool,
					!anExecutor->GetPinValue(&inputPins[0]).value.Bool};
		}
	};
	...
	private:
	public:
		DefineNode("AND", NodeCategory::Math,
		...
		virtual PinValue GetOutputPinValue(int aPinIndex,
			...
			return {ValueType::Bool,
					anExecutor->GetPinValue(&inputPins[0]).value.Bool &&
						anExecutor->GetPinValue(&inputPins[1]).value.Bool};
		}
	};
	...
	private:
	public:
		DefineNode("NAND", NodeCategory::Math,
				   "Returns true if any input is false.");
	...
	private:
	public:
		DefineNode("OR", NodeCategory::Math,
				   "Returns true if either inputs is true.");
	...
	private:
	public:
		DefineNode("NOR", NodeCategory::Math,
	...
	private:
	public:
		DefineNode("XOR", NodeCategory::Math,
	...
	private:
	public:
		DefineNode("XNOR", NodeCategory::Math,
	...
	private:
	public:
		DefineNode("Equals", NodeCategory::Math,
				   "Returns true if both inputs are the same.");
	...
	private:
	public:
		DefineNode("Greater", NodeCategory::Math,
	...
	private:
	public:
		DefineNode("Lesser", NodeCategory::Math,
	...
	public:
		...
		virtual void Execute(NodeExecutor* anExecutor) override {
			switch (anExecutor->GetPinValue(&inputPins[1]).type) {
				...
				case ValueType::Int:
					KE_LOG("%d",
						   anExecutor->GetPinValue(&inputPins[1]).value.Int);
					break;
				case ValueType::Bool:
					KE_LOG("%s",
						   anExecutor->GetPinValue(&inputPins[1]).value.Bool
				...
				case ValueType::Vector2:
					KE_LOG(
						"%.3f, %.3f",
						anExecutor->GetPinValue(&inputPins[1]).value.Vector2.x,
						anExecutor->GetPinValue(&inputPins[1]).value.Vector2.y);
					break;
				case ValueType::Vector3:
					KE_LOG(
						"%.3f, %.3f, %.3f",
						anExecutor->GetPinValue(&inputPins[1]).value.Vector3.x,
						anExecutor->GetPinValue(&inputPins[1]).value.Vector3.y,
						anExecutor->GetPinValue(&inputPins[1]).value.Vector3.z);
					break;
				case ValueType::Vector4:
					KE_LOG(
						"%.3f, %.3f, %.3f, %.3f",
						anExecutor->GetPinValue(&inputPins[1]).value.Vector4.x,
						anExecutor->GetPinValue(&inputPins[1]).value.Vector4.y,
						anExecutor->GetPinValue(&inputPins[1]).value.Vector4.z,
						anExecutor->GetPinValue(&inputPins[1]).value.Vector4.w);
					break;
				case ValueType::String:
					KE_LOG("%s",
						   anExecutor->GetPinValue(&inputPins[1]).value.String);
					break;
				default:
					KE_LOG("Unsupported type");
					break;
			}
			anExecutor->AddExecution(outputPins[0].ID);
		}
	...
	public:
		DefineNode("Layer Mask", NodeCategory::Math,
				   "Outputs a collision layer mask.");

		LayerMaskNode() : ScriptNode() {}

		virtual void Init() override {
			AddOutputPin(ValueType::Int, PinType::OutputValue, "Mask");
		}

		virtual void CustomRender() override {
			for (int i = 0; i < (int)KE::Collision::Layers::Count; i++) {
				bool selected = layerMask & (1 << i);
				if (ImGui::Checkbox(KE::Collision::GetLayerAsString(
										(KE::Collision::Layers)(1 << i))
										.c_str(),
									&selected)) {
					if (selected) {
						layerMask |= (1 << i);
	...
	private:
	public:
		DefineNode("Box Collision", NodeCategory::Logic,
				   "Returns if any object collides with the box.");
		...
		virtual void Init() override {
			...
			AddOutputPin(ValueType::FlowValue, PinType::Flow, "");
			AddOutputPin(ValueType::Container, PinType::OutputValue,
						 "Collisions");
		...
		virtual void Execute(NodeExecutor* anExecutor) override {
			std::vector<Collider*> collisionResult =
				anExecutor->GetCollidersInBox(
		...
		virtual PinValue GetOutputPinValue(int aPinIndex,
			...
			if (std::any* outValue = anExecutor->GetNodeVariable(ID)) {
				std::vector<GameObject*>* collisionResult =
					std::any_cast<std::vector<GameObject*>>(outValue);
				return {ValueType::Container,
						ContainerValue(collisionResult->data(),
									   (int)collisionResult->size(),
									   ValueType::GameObject)};
			}
			return {ValueType::Container,
					ContainerValue(nullptr, 0, ValueType::GameObject)};
		}
	};
	...
	private:
	public:
		DefineNode("Get Object", NodeCategory::Variable,
		...
		virtual void Init() override {
			...
			AddOutputPin(ValueType::GameObject, PinType::OutputValue,
						 "GameObject");
		}
	...
	private:
	public:
		DefineNode("This Object", NodeCategory::Variable,
				   "Gets the object running the script.");
	...
	private:
	public:
		DefineNode("Set Transform", NodeCategory::Variable,
				   "Sets a GameObject's transform.");
		...
		virtual void Execute(NodeExecutor* anExecutor) override {
			if (GameObject* object =
					anExecutor->GetPinValue(&inputPins[1]).value.GameObject) {
				object->myTransform =
					anExecutor->GetPinValue(&inputPins[2]).value.Transform;
				KE::BoxColliderComponent* box;
				if (object->TryGetComponent(box)) {
					box->GetCollider()->UpdatePosition(
						object->myTransform.GetPosition());
				}
			}
			anExecutor->AddExecution(outputPins[0].ID);
		}
	...
	private:
	public:
		DefineNode("Object Attributes", NodeCategory::Variable,
		...
		virtual void Init() override {
			...
			AddOutputPin(ValueType::Transform, PinType::OutputValue,
						 "Local Transform");
			AddOutputPin(ValueType::Transform, PinType::OutputValue,
						 "World Transform");
		}
		...
		PinValue GetOutputPinValue(int aPinIndex,
								   NodeExecutor* anExecutor) override {
			GameObject* object =
				anExecutor->GetPinValue(&inputPins[0]).value.GameObject;
			switch (aPinIndex) {
				case 0:
					return {ValueType::Int, object->myID};
				case 1:
					return {ValueType::String, object->GetName().c_str()};
				case 2:
					return {ValueType::Transform, object->myTransform};
				case 3:
					return {ValueType::Transform,
							object->myWorldSpaceTransform};
				default:
					return {};
			}
		}
	...
	private:
	public:
		DefineNode("For Each", NodeCategory::Flow,
				   "Executes a flow for each object in a list.");
		...
		virtual void Execute(NodeExecutor* anExecutor) override {
			auto container =
				anExecutor->GetPinValue(&inputPins[1]).value.Container;
			switch (container.type) {
				case ValueType::GameObject: {
					for (int i = 0; i < container.size; i++) {
						GameObject* gameObject =
							((GameObject**)container.data)[i];
				...
				case ValueType::GameObject: {
					...
					if (context >= 0) {
						val.value.GameObject =
							((GameObject**)container.data)[context];
					}
					break;
				}
				default:
	...
	public:
		DefineNode("For Loop", NodeCategory::Flow,
				   "Executes its connections once for every number between min "
				   "and max.");
	...
	private:
	public:
		DefineNode("Get Transform Attributes", NodeCategory::Variable,
				   "Gets attributes of a Transform.");
		...
		PinValue GetOutputPinValue(int aPinIndex,
			...
			const Transform& transform =
				anExecutor->GetPinValue(&inputPins[0]).value.Transform;
			switch (aPinIndex) {
				case 0:
					return {ValueType::Vector3, transform.GetPosition()};
				case 1:
					return {ValueType::Vector3, transform.GetRotation()};
				case 2:
					return {ValueType::Vector3, transform.GetScale()};
				case 3:
					return {ValueType::Vector3, transform.GetRight()};
				case 4:
					return {ValueType::Vector3, transform.GetUp()};
				case 5:
					return {ValueType::Vector3, transform.GetForward()};
				default:
					return {};
			}
		}
	...
	private:
	public:
		DefineNode("Set Transform Position", NodeCategory::Variable,
				   "Sets position of a Transform.");
	...
	private:
	public:
		DefineNode("Set Transform Rotation", NodeCategory::Variable,
				   "Sets Rotation of a Transform.");
	...
	private:
	public:
		DefineNode("Set Transform Scale", NodeCategory::Variable,
				   "Gets attributes of a Transform.");
	...
	private:
	public:
		DefineNode("Vector2", NodeCategory::Math,
				   "Outputs a 2D Vector constant.");
		...
		virtual PinValue GetOutputPinValue(int aPinIndex,
			...
			result.value.Vector2.x =
				anExecutor->GetPinValue(&inputPins[0]).value.Float;
			result.value.Vector2.y =
				anExecutor->GetPinValue(&inputPins[1]).value.Float;
			return result;
		}
	};
	...
	private:
	public:
		DefineNode("Vector3", NodeCategory::Math,
				   "Outputs a 3D Vector constant.");
		...
		virtual PinValue GetOutputPinValue(int aPinIndex,
			...
			result.value.Vector3.x =
				anExecutor->GetPinValue(&inputPins[0]).value.Float;
			result.value.Vector3.y =
				anExecutor->GetPinValue(&inputPins[1]).value.Float;
			result.value.Vector3.z =
				anExecutor->GetPinValue(&inputPins[2]).value.Float;
			return result;
		}
	};
	...
	private:
	public:
		DefineNode("Vector4", NodeCategory::Math,
				   "Outputs a 4D Vector constant.");
		...
		virtual PinValue GetOutputPinValue(int aPinIndex,
			...
			result.value.Vector4.x =
				anExecutor->GetPinValue(&inputPins[0]).value.Float;
			result.value.Vector4.y =
				anExecutor->GetPinValue(&inputPins[1]).value.Float;
			result.value.Vector4.z =
				anExecutor->GetPinValue(&inputPins[2]).value.Float;
			result.value.Vector4.w =
				anExecutor->GetPinValue(&inputPins[3]).value.Float;
			return result;
		}
	};
	...
	private:
	public:
		DefineNode("Deconstruct Vector2", NodeCategory::Math,
				   "Deconstructs a Vector2 to float outputs");
		...
		PinValue GetOutputPinValue(int aPinIndex,
			...
			switch (aPinIndex) {
				case 0:
					return {
						ValueType::Float,
						anExecutor->GetPinValue(&inputPins[0]).value.Vector2.x};
				case 1:
					return {
						ValueType::Float,
						anExecutor->GetPinValue(&inputPins[0]).value.Vector3.y};
				default:
					return {};
			}
		}
	...
	private:
	public:
		DefineNode("Deconstruct Vector3", NodeCategory::Math,
				   "Deconstructs a Vector3 to float outputs");
		...
		PinValue GetOutputPinValue(int aPinIndex,
			...
			switch (aPinIndex) {
				case 0:
					return {
						ValueType::Float,
						anExecutor->GetPinValue(&inputPins[0]).value.Vector3.x};
				case 1:
					return {
						ValueType::Float,
						anExecutor->GetPinValue(&inputPins[0]).value.Vector3.y};
				case 2:
					return {
						ValueType::Float,
						anExecutor->GetPinValue(&inputPins[0]).value.Vector3.z};
				default:
					return {};
			}
		}
	...
	private:
	public:
		DefineNode("Deconstruct Vector4", NodeCategory::Math,
				   "Deconstructs a Vector4 to float outputs");
		...
		PinValue GetOutputPinValue(int aPinIndex,
			...
			switch (aPinIndex) {
				case 0:
					return {
						ValueType::Float,
						anExecutor->GetPinValue(&inputPins[0]).value.Vector4.x};
				case 1:
					return {
						ValueType::Float,
						anExecutor->GetPinValue(&inputPins[0]).value.Vector4.y};
				case 2:
					return {
						ValueType::Float,
						anExecutor->GetPinValue(&inputPins[0]).value.Vector4.z};
				case 3:
					return {
						ValueType::Float,
						anExecutor->GetPinValue(&inputPins[0]).value.Vector4.w};
				default:
					return {};
			}
		}
	...
	private:
	public:
		DefineNode("Add Vector", NodeCategory::Math,
		...
		PinValue GetOutputPinValue(int aPinIndex,
			...
			result.value.Vector3 +=
				anExecutor->GetPinValue(&inputPins[0]).value.Vector3;
			result.value.Vector3 +=
				anExecutor->GetPinValue(&inputPins[1]).value.Vector3;
			return result;
		}
	...
	private:
	public:
		DefineNode("Subtract Vector", NodeCategory::Math,
				   "Subtracts input B from input A.");
	...
	private:
	public:
		DefineNode("Multiply Vector", NodeCategory::Math,
	...
	private:
	public:
		DefineNode("Divide Vector", NodeCategory::Math,
	...
	private:
	public:
		DefineNode("Normalize Vector", NodeCategory::Math,
				   "Normalizes a vector so its lenth is 1.");

		NormalizeVectorNode() : ScriptNode() {}

		virtual void Init() override {
			AddInputPin(ValueType::Vector3, PinType::Value, "Vector");
			AddOutputPin(ValueType::Vector3, PinType::OutputValue,
						 "Normalized");
		}
		...
		PinValue GetOutputPinValue(int aPinIndex,
								   NodeExecutor* anExecutor) override {
			PinValue result = {ValueType::Vector3};
			result.value.Vector3 = anExecutor->GetPinValue(&inputPins[0])
									   .value.Vector3.GetNormalized();
			return result;
		}
	};
	...
	private:
	public:
		DefineNode("Vector Length", NodeCategory::Math,
		...
		PinValue GetOutputPinValue(int aPinIndex,
			...
			PinValue result = {ValueType::Float};
			result.value.Float =
				anExecutor->GetPinValue(&inputPins[0]).value.Vector3.Length();
			return result;
		}
	};
	...
	public:
		DefineNode("Macro Input", NodeCategory::Macro,
				   "Input node for a macro definition.");
		...
		virtual void OnConnectPin(Pin* aFromPin, Pin* aToPin) override {
			...
			if (aFromPin->ID == outputPins.back().ID) {
				AddOutputPin(ValueType::Any, PinType::OutputValue, "");
			}
		}
		...
		virtual PinValue GetOutputPinValue(int aPinIndex,
										   NodeExecutor* anExecutor) override {
			return anExecutor->GetMacroExecutionData()
				->macroExecutor->GetPinValue(
					&anExecutor->GetMacroExecutionData()
						 ->macroExecutionNode->GetInputPins()[aPinIndex]);
		}
		...
		virtual void ExtraDeserialize(void* aOutJson,
			...
			nlohmann::json* json = (nlohmann::json*)aOutJson;
			strcpy_s(macroName,
					 (*json)["macroName"].get<std::string>().c_str());
		...
		virtual void* GetCustomData() override {
			return macroName;
		}
	...
	public:
		DefineNode("Macro Output", NodeCategory::Macro,
				   "Output node for a macro definition.");
		...
		virtual void OnConnectPin(Pin* aFromPin, Pin* aToPin) override {
			...
			if (aToPin->ID == inputPins.back().ID) {
				AddInputPin(ValueType::Any, PinType::Value, "");
			}
		}
		...
		virtual void Execute(NodeExecutor* anExecutor) {
			for (int i = 0; i < inputPins.size(); i++) {
				if (inputPins[i].type != PinType::Flow) {
					continue;
				}
				auto* execData = anExecutor->GetMacroExecutionData();
				execData->macroExecutor->AddExecution(
					execData->macroExecutionNode->GetOutputPins()[i].ID);
			}
		}
	...
	public:
		...
		virtual void ExtraDeserialize(void* aOutJson,
			...
			strcpy_s(macroName,
					 (*json)["macroName"].get<std::string>().c_str());
			for (size_t i = 0; i < (*json)["inPins"].size(); i++) {
				AddInputPin(ValueType::Any, PinType::Value, "");
			}
			for (size_t i = 0; i < (*json)["outPins"].size(); i++) {
				AddOutputPin(ValueType::Any, PinType::Value, "");
			}
		...
		virtual void SetCustomData(void* aData) override {
			...
			for (auto& ouValue : inData.macro->outputValues) {
				...
				outputPins.back().ID.idParts.pinID =
					(short)(inputPins.size() + outputPins.size()) - 1;
			}
		}
	};
	...
	private:
	public:
		DefineNode("Behaviour Root", NodeCategory::Flow,
				   "Root node for a behaviour tree.");
	...
	public:
		DefineNode("Composite", NodeCategory::AIFlow,
				   "Sequencer node for a behaviour tree.");
		...
		virtual void ExtraDeserialize(void* aOutJson,
			...
			if ((*json).find("type") == ((*json).end())) return;

			int value = (*json)["type"];
			type = static_cast<AI::TreeNodeType>(value);
		}

		void* GetCustomData() final override {
			return (void*)&type;
		}
		void CustomRender() override {
			ImGui::SetNextItemWidth(200.f);

			std::string infoLabel = type == AI::TreeNodeType::COUNT
										? "Select"
										: AI::typeNames[(int)type];

			if (ImGui::BeginCombo("##", infoLabel.c_str())) {
				for (int i = 0;
					 i < static_cast<int>(AI::TreeNodeType::FLOW_END); i++) {
	...
	public:
		...
		void CustomRender() override {
			...
			if (ImGui::BeginCombo("##", infoLabel.c_str())) {
				for (int i = static_cast<int>(AI::TreeNodeType::LEAF_START) + 1;
					 i < static_cast<int>(AI::TreeNodeType::LEAF_END); i++) {
					if (ImGui::Selectable(AI::typeNames[i].c_str())) {
						type = static_cast<AI::TreeNodeType>(i);
					}
				}
	...
	public:
		DefineNode("AI Decorator", NodeCategory::AIDecorator,
				   "AI Decorator Node");
		...
		void CustomRender() override {
			...
			if (ImGui::BeginCombo("##", infoLabel.c_str())) {
				for (int i =
						 static_cast<int>(AI::TreeNodeType::DECORATOR_START) +
						 1;
					 i < static_cast<int>(AI::TreeNodeType::DECORATOR_END);
					 i++) {
					if (ImGui::Selectable(AI::typeNames[i].c_str())) {
						type = static_cast<AI::TreeNodeType>(i);
					}
...
void KE::NodeTypeDatabase::RegisterLanguage(
	const std::string& aLanguageFilePath) {
	auto languageDef =
		LanguageDefinerNew<HLSLDefiner>::Define(aLanguageFilePath.c_str());
	myLanguageDefinitions["hlsl"] =
		new HLSLDefiner::LangDefinition(languageDef);

	for (const auto& function : languageDef.functions) {
		RegisterNode(
			function.second.name.c_str(), NodeCategory::CodeLogic, "",
			[function, languageDef](Script* aScript) -> ScriptNode* {
				CodeFunctionNode* node = new CodeFunctionNode();
				/*node->SetVariantIndex(0);*/
				node->SetVariantCount(
					static_cast<unsigned int>(function.second.variants.size()));
	...
	for (const auto& dataStruct : languageDef.structs) {
		RegisterNode(
			dataStruct.second.name.c_str(), NodeCategory::CodeLogic, "",
	...
	for (const auto& buffer : languageDef.buffers) {
		RegisterNode(
			buffer.second.name.c_str(), NodeCategory::CodeLogic, "",
	...
	for (const auto& texture : languageDef.textures) {
		RegisterNode(
			texture.second.name.c_str(), NodeCategory::CodeLogic, "",
	...
	for (const auto& entryPoint : languageDef.entrypoints) {
		...
		RegisterNode(
			inName.c_str(), NodeCategory::CodeLogic, "",
		...
		RegisterNode(
			outName.c_str(), NodeCategory::CodeLogic, "",
			[entryPoint, languageDef](Script* aScript) -> ScriptNode* {
...
void KE::NodeTypeDatabase::CalculateNodeCategoryStack(
	const std::string& aNodeName, const char* aCategoryStack) {
...
void KE::NodeTypeDatabase::ProcessNodeData(const std::string& aNodeName) {
	...
	for (unsigned int variantIter = 0; variantIter < iterCount; variantIter++) {
		...
		for (int i = 0; i < node->GetOutputPins().size(); i++) {
			...
			if (pin->codeData) {
				nodePinCodeOutDatabase[((CodePin*)pin->codeData)
										   ->value.type.typeName]
					.push_back(v);
			} else {
...
void KE::NodeTypeDatabase::RegisterNode(
	const char* aNodeName, NodeCategory aCategory, const char* aDescription,
	const NodeCreateFunction& aCreateFunction, const char* aCategoryStack) {
	nodeTypes[aNodeName] = {aNodeName, aCategory, aDescription,
							aCreateFunction};
