...
void KE::HLSLDefiner::RegisterAnnotations(
...
void KE::HLSLDefiner::Interpret(LangDefinition& aDefinition,
	...
	switch (aCode.annotation) {
		case Annotations::Entrypoint: {
			...
			entry.name =
				entrypointName;	 // use the annotated name, so it can be named
								 // independently of the output code :)

			for (int i = 2; i < entrypointWords.size(); i++) {
				if (!aDefinition.dataTypes.contains(entrypointWords[i])) {
					continue;
				}

				entry.inputParameters.push_back(
					{aDefinition.dataTypes.at(entrypointWords[i]),
					 entrypointWords[i + 1]});
			}
		...
		case Annotations::Function: {
			...
			function.name =
				aCode.annotationParameters[1];	// use the annotated name, so it
												// can be named independently of
												// the output code :)

			for (int i = 2; i < functionWords.size(); i++) {
				if (!aDefinition.dataTypes.contains(functionWords[i])) {
					continue;
				}

				variant.parameters.push_back(
					{aDefinition.dataTypes.at(functionWords[i]),
					 functionWords[i + 1]});
			}
			break;
		}
		case Annotations::Struct: {
			...
			aDefinition.dataTypes[structName] = {
			...
			for (const auto& line : aCode.code) {
				...
				if (lineParts.size() >= 2 &&
					aDefinition.dataTypes.contains(lineParts[0])) {
					currentStruct.members.push_back(
						{aDefinition.dataTypes.at(lineParts[0]), lineParts[1]});
				}
			}
		...
		case Annotations::Buffer: {
			...
			aDefinition.dataTypes[bufferName] = {
			...
			for (const auto& line : aCode.code) {
				...
				if (const size_t registerPos = line.find("register");
					registerPos != std::string::npos) {
					const size_t parenBegin =
						line.find_first_of('(', registerPos);
					const size_t parenEnd = line.find_first_of(')', parenBegin);

					currentBuffer.slot = std::stoi(line.substr(
						parenBegin + 2, (parenEnd - parenBegin) - 2));
				}
		...
		case Annotations::Texture2D: {
			...
			aDefinition.dataTypes[textureName] = {
				textureName, {1.0f, 0.0f, 1.0f}, true};
			...
			if (const size_t registerPos = line.find("register");
				...
				currentTexture.slot = std::stoi(
					line.substr(parenBegin + 2, (parenEnd - parenBegin) - 2));
			}
		...
		case Annotations::Intrinsic: {
			...
			for (int i = 1; i < outputTypes.size(); i++) {
				auto& function = aDefinition.functions[intrinsicName]
		...
		case Annotations::Operator: {
			...
			for (int i = 1; i < outputTypes.size(); i++) {
				auto& function =
					aDefinition.functions[operatorName].variants.emplace_back();
				function.returnType = aDefinition.dataTypes[outputTypes[i]];
