...
namespace KE {
	...
	struct ParsingData {
		...
		std::unordered_map<ScriptMemberID, std::vector<PinConnection>,
						   ScriptMemberID>& scriptConnections;

		std::vector<ParsedConnection> nodeFlow;
		std::vector<ValueNodeTree> nodeTrees;
		std::unordered_map<ScriptMemberID, int, ScriptMemberID>
			idToTreeIndexMap;
		std::unordered_map<ScriptMemberID, DependencyMapping, ScriptMemberID>
			dependencyMap;
		std::unordered_map<ScriptMemberID, std::vector<ScriptMemberID>,
						   ScriptMemberID>
			dependents;

		std::unordered_map<ScriptMemberID, std::string, ScriptMemberID>
			parsedNodeMap;
		std::unordered_map<ScriptMemberID, std::vector<GeneratedVariable>,
						   ScriptMemberID>
			generatedVariables;

		// new functionality
		std::unordered_map<ScriptMemberID, CodeVariable, ScriptMemberID>
			tempNewVariables;
		std::unordered_map<ScriptMemberID, CodeVariable, ScriptMemberID>
			newVariables;
		...
		std::unordered_map<ScriptMemberID, std::vector<CodeScope*>,
						   ScriptMemberID>
			evaluatedScopeData;
	...
	private:
		...
		static std::string EvaluateVariable(ParsingData& aData,
											const std::string& variableName,
											ScriptNode* aNode);
		static std::string ReplaceCodeVariableNames(ParsingData& aData,
													const std::string& nodeCode,
													ScriptNode* aNode);

		static void EvaluateScope(ParsingData& aData, ScriptMemberID aNodeID);
		static bool IsInScope(ParsingData& aData, ScriptMemberID aNodeID,
							  const ScopePinList& aScopeMemberList,
							  std::unordered_map<ScriptMemberID, bool,
												 ScriptMemberID>& aWorkingData);
		static std::string WriteScope(ParsingData& aData, CodeScope* aScope);
		static void CreateScopeStack(ParsingData& aData,
									 std::vector<CodeScope*>& aOutScopeStack);
		static void AddScopeIndenting(ParsingData& aData, std::string& aCode);

		static void FindDependentScopes(ParsingData& aData,
										ScriptMemberID aNodeID);
		static void CorrectUnscopedNodes(ParsingData& aData);

		static void DependencyHelper(
			ParsingData& aData, ScriptMemberID aNodeID,
			std::unordered_map<ScriptMemberID, bool, ScriptMemberID>&
				aWorkingData);
		static void EvaluateDependencies(ParsingData& aData);
		static void EvaluateNodeFlow(ParsingData& aData,
									 ScriptNode* aStartNode);

		static void AppendParsingOrder(ParsingData& aData,
									   ScriptMemberID aNodeID);
		static void ParsingOrderHelper(ParsingData& aData,
									   ScriptMemberID aNodeID);
		static void GenerateParsingOrder(ParsingData& aData);

		static std::string ParseCodeNode(ParsingData& aData,
	...
	public:
		...
		static void RenderPreviewImages(
			...
			Graphics* aGraphics, Material* aMaterial = nullptr);
		static bool Parse(Script* aScript, ParsingOutput*& anOutput);

		static void GetOutputRenderCode(
			const ParsingOutput& aData,
			std::vector<std::pair<ScriptMemberID, std::string>>&
				aOutputCodeList);

		static ScriptMemberID GetLineAuthor(ParsingData& aData,
											const std::string& aLine);
		static std::pair<size_t, size_t> GetCodeAreaFromAuthor(
