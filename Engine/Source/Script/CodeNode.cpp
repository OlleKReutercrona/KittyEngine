...
void KE::CodeFunctionNode::Init() {
	for (const auto& param :
		 myFunctionDefinition.variants[myVariantIndex].parameters) {
	...
	codePinData.push_back(
		{CodePinType::Value,
		 {myFunctionDefinition.variants[myVariantIndex].returnType}});

	for (int i = 0;
		 i < myFunctionDefinition.variants[myVariantIndex].parameters.size();
		 i++) {
...
KE::CommandList KE::CodeFunctionNode::GetCommands(
	const LanguageDefinitionNew& aLanguage) {
	CommandList commands;

	// generate the function call
	GenerateVariableCommand* functionCall =
	...
	if (myFunctionDefinition.operatorKey == "") {
		...
		for (int i = 0;
			 i <
			 myFunctionDefinition.variants[myVariantIndex].parameters.size();
			 i++) {
			functionCallIDS.push_back(inputPins[i].ID);
		}
		functionCall->data.assignmentInputs.push_back(
			{functionCallIDS, CodeOperation::CallFunction});
		functionCall->data.optionalData = myFunctionDefinition.name;
	} else {
		functionCall->data.assignmentInputs.push_back(
			{inputPins[0].ID,
			 StringToCodeMathOperation(myFunctionDefinition.operatorKey)});
		functionCall->data.assignmentInputs.push_back(
...
void KE::CodeFunctionNode::SetVariantIndex(unsigned aIndex) {
	...
	for (int i = 0; i < newFunctionDefinition.parameters.size(); i++) {
		((CodePin*)inputPins[i].codeData)->value.type =
			newFunctionDefinition.parameters[i].type;
	}

	((CodePin*)outputPins[0].codeData)->value.type =
		newFunctionDefinition.returnType;
}

void KE::CodeFunctionNode::ExtraSerialize(void* aOutJson,
										  Script* aLoadingScript) {
	nlohmann::json* json = (nlohmann::json*)aOutJson;
	(*json)["variantIndex"] = static_cast<int>(myVariantIndex);
}

void KE::CodeFunctionNode::ExtraDeserialize(void* aInJson,
											Script* aLoadingScript) {
...
KE::CommandList KE::CodeStructNode::GetCommands(
	const LanguageDefinitionNew& aLanguage) {
	CommandList commands;

	for (int i = 0; i < myStructDefinition.members.size(); i++) {
		GenerateVariableCommand* memberVariable =
			commands.Add<GenerateVariableCommand>();
...
KE::CommandList KE::CodeBufferNode::GetCommands(
	const LanguageDefinitionNew& aLanguage) {
	CommandList commands;
...
void KE::CodeTextureNode::CustomRender() {
	...
	// parentScript->GetCodeRenderingContext()->shaderTextures[myTextureDefinition.slot];
	// if (srv)
	//{
...
KE::CommandList KE::CodeTextureNode::GetCommands(
	const LanguageDefinitionNew& aLanguage) {
	CommandList commands;
...
auto KE::CodeEntryPointNode::GetParameters() const {
	std::vector<std::pair<const DataType*, std::pair<std::string, std::string>>>
		inputParams;
	for (const auto& param : myEntrypointDefinition.inputParameters) {
		if (param.type.isStruct) {
			auto* hlslDef = (HLSLDefiner::LangDefinition*)
								parentScript->GetLanguageDefinition();

			for (const auto& member :
				 hlslDef->structs[param.type.typeName].members) {
...
KE::CommandList KE::CodeEntryPointNode::GetCommands(
	const LanguageDefinitionNew& aLanguage) {
	CommandList commands;
	...
	for (int i = 0; i < myEntrypointDefinition.inputParameters.size(); i++) {
		...
		inputParams += std::format(
			"{} {}", myEntrypointDefinition.inputParameters[i].type.typeName,
			myEntrypointDefinition.inputParameters[i].name);
	}
	...
	std::string formated = std::format(
		...
		inputParams, "{", myEntrypointDefinition.returnType.typeName);
	...
	for (int i = 0; i < params.size(); i++) {
		GenerateVariableCommand* inputVariable =
			commands.Add<GenerateVariableCommand>();
		...
		inputVariable->data.optionalData =
			params[i].second.first == ""
				? params[i].second.second
				: std::format("{}.{}", params[i].second.first,
							  params[i].second.second);
	}
...
auto KE::CodeExitPointNode::GetParameters() const {
	...
	if (myEntrypointDefinition.returnType.isStruct) {
		auto* hlslDef =
			(HLSLDefiner::LangDefinition*)parentScript->GetLanguageDefinition();

		for (const auto& member :
			 hlslDef->structs[myEntrypointDefinition.returnType.typeName]
				 .members) {
...
KE::CommandList KE::CodeExitPointNode::GetCommands(
	const LanguageDefinitionNew& aLanguage) {
	CommandList commands;
	...
	for (int i = 0; i < params.size(); i++) {
		AssignValueCommand* assignValueCommand =
			commands.Add<AssignValueCommand>();
		assignValueCommand->assignFrom = inputPins[i + 1].ID;
		assignValueCommand->assignTo =
			std::format("{}.{}", "OUTPUT", params[i].second);
	}
