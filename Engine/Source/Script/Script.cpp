...
void KE::Script::RemoveNode(ScriptMemberID anID) {
	...
	// delete nodes[anID]; //TODO: make sure memory is cleared properly now that
	// we have an action system
	nodes.erase(anID);
...
void KE::Script::AddConnection(ScriptMemberID from, ScriptMemberID to,
							   bool aTwoWay) {
...
void KE::Script::SaveToFile(const std::string& aPath) {
	...
	for (auto& node : nodes) {
		...
		nodeJson["position"] = {nodePositions[node.second->ID].x,
								nodePositions[node.second->ID].y};
	...
	for (auto& comment : scriptComments) {
		...
		commentJson["colour"] = {comment.colour.x, comment.colour.y,
								 comment.colour.z, comment.colour.w};
	...
	if (std::filesystem::exists(aPath) &&
		...
		std::filesystem::status(aPath).permissions() ==
			std::filesystem::perms::_File_attribute_readonly) {
		std::filesystem::permissions(aPath, std::filesystem::perms::owner_write,
...
void KE::Script::LoadFromFile(const std::string& aPath) {
	...
	try	 // try parsing it, if it fails, save the file and try again. If it
		 // fails again, we have a problem.
	{
		file >> scriptJson;
		...
		LoadFromFile(aPath);  // cursed AF!
							  // if the script is empty, save it, then load it
							  // again, so that we have a valid script file.
		return;
	}
	...
	for (auto& comment : scriptJson["comments"]) {
		...
		commentData.colour = {comment["colour"][0], comment["colour"][1],
							  comment["colour"][2], comment["colour"][3]};
	...
	for (auto& nodeJson : scriptJson["nodes"]) {
		...
		const std::string& internalName =
			KE::nodeTypeDatabase.nodeNameMap[jsonName];

		if (!KE::nodeTypeDatabase.nodeTypes.contains(internalName)) {
			continue;
		}
		ScriptNode* node =
			KE::nodeTypeDatabase.nodeTypes[internalName].createFunction(this);
		node->ID = id;
		InsertNode(node);
		...
		if (nextNodeID <=
			id.idParts
				.nodeID)  // make sure we don't try making existing nodes later.
		{
	...
	for (auto& connectionJson : scriptJson["connections"]) {
		...
		if (!(nodes.contains(from.GetNodeID()) &&
			  nodes.contains(to.GetNodeID()))) {
			continue;
		}

		AddConnection(
			from, to,
			false);	 // one way, since the other way will be added later!
	}
}
	...
	for (auto& node : nodes) {
		...
		if (strcmp(node.second->GetName(), "Macro Input") == 0) {
			...
			for (auto& pin : node.second->GetOutputPins()) {
				if (&pin == &node.second->GetOutputPins().back()) {
					break;
				}  // skip the last pin, which is the macro end pin
			...
			for (auto& pin : node.second->GetInputPins()) {
				if (&pin == &node.second->GetInputPins().back()) {
					break;
				}  // skip the last pin, which is the macro end pin
...
void KE::Script::EvaluateEntryPoints() {
	...
	for (auto& node : nodes) {
		if (strcmp(node.second->GetName(), "Awake") == 0) {
			entryPoints[static_cast<int>(EntryPointType::Awake)].push_back(
				node.second->ID);
		} else if (strcmp(node.second->GetName(), "Update") == 0) {
			entryPoints[static_cast<int>(EntryPointType::Update)].push_back(
				node.second->ID);
		}
	}
}
