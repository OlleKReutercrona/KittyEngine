...
KE::ParsingData::ParsingData(Script* aScript)
	: script(aScript),
	  scriptNodes(aScript->GetNodes()),
	  scriptConnections(aScript->GetConnections()) {}

KE::Pin* KE::CodeScriptParser::GetUsedPin(ParsingData& aData,
										  ScriptMemberID aPinID) {
	...
	if (IsLinkNode(aData, aPinID.GetNodeID()) && isOutputPin) {
		auto linkConnections =
			aData.scriptConnections[aData.scriptNodes.at(aPinID.GetNodeID())
										->GetInputPins()[0]
										.ID];
		if (linkConnections.empty()) {
			return workingPin;
		}

		return GetUsedPin(
			aData,
			aData.scriptNodes.at(aPinID.GetNodeID())->GetInputPins()[0].ID);
	}
...
KE::PinValue KE::CodeScriptParser::GetPinValue(ParsingData& aData,
											   ScriptMemberID aPinID) {
	return GetUsedPin(aData, aPinID)->value;
}

std::string KE::CodeScriptParser::EvaluateVariable(
	ParsingData& aData, const std::string& variableName, ScriptNode* aNode) {
	std::string out = "";
...
std::string KE::CodeScriptParser::ReplaceCodeVariableNames(
	ParsingData& aData, const std::string& nodeCode, ScriptNode* aNode) {
	std::string out;
	char identifier = '$';
	...
	while (nodeCode.find(identifier, off) != std::string::npos) {
		...
		std::string variableName =
			nodeCode.substr(firstPos + 1, nextPos - firstPos - 1);

		const std::string replacedVariableName =
			EvaluateVariable(aData, variableName, aNode);
...
void KE::CodeScriptParser::EvaluateScope(ParsingData& aData,
										 ScriptMemberID aNodeID) {
	...
	for (auto& scope : aData.codeScopes) {
		...
		if (deepestScope && workingDepth >= scopeDepth) {
			continue;
		}
...
bool KE::CodeScriptParser::IsInScope(
	KE::ParsingData& aData, KE::ScriptMemberID aNodeID,
	const ScopePinList& aScopeMemberList,
	std::unordered_map<KE::ScriptMemberID, bool, KE::ScriptMemberID>&
		aWorkingData) {
	if (aNodeID.GetNodeID() == aScopeMemberList.pins.at(0).GetNodeID()) {
		return false;
	}  // we've reached the origin of the scope

	if (aWorkingData.contains(aNodeID)) {
		return false;  // we've already evaluated this node, so it should have
					   // returned true already if it was in scope.
	}
	...
	for (auto& pin : aData.scriptNodes[aNodeID]->GetInputPins()) {
		...
		if (aData.scriptConnections.contains(pin.ID)) {
			for (auto& connection : aData.scriptConnections[pin.ID]) {
				if (aScopeMemberList.Contains(
						connection.to))	 // this pin has a direct connection to
										 // the scope origin
				{
					return true;
				}
				...
				if (IsLinkNode(aData, connection.to.GetNodeID())) {
					auto linkConnections =
						aData.scriptConnections
							[aData.scriptNodes.at(connection.to.GetNodeID())
								 ->GetInputPins()[0]
								 .ID];
					if (linkConnections.empty()) {
						continue;
					...
					if (IsInScope(
							aData, connection.to.GetNodeID(), aScopeMemberList,
							aWorkingData))	// this pin has a connection to a
				...
				if (IsInScope(aData, connection.to.GetNodeID(),
							  aScopeMemberList,
							  aWorkingData))  // this pin has a connection to a
											  // node that is in the scope
				{
...
std::string KE::CodeScriptParser::WriteScope(ParsingData& aData,
											 CodeScope* aScope) {
	if (aScope->hasClosed) {
		return "";
	}
	...
	for (ScriptMemberID node : aData.parsingOrder) {
		if (std::ranges::find(aScope->scopeMembers, node) ==
			aScope->scopeMembers.end()) {
			continue;
		}
...
void KE::CodeScriptParser::CreateScopeStack(
	ParsingData& aData, std::vector<CodeScope*>& aOutScopeStack) {
	for (auto& scope : aData.codeScopes) {
		aOutScopeStack.push_back(&scope);
	}
}

void KE::CodeScriptParser::AddScopeIndenting(ParsingData& aData,
											 std::string& aCode) {
	std::string spacing;
	constexpr size_t spacingSpaceCount = 2;
	for (size_t i = 0; i < aData.currentScopeDepth * spacingSpaceCount; i++) {
		spacing += " ";
	}
	...
	// Read lines from the string stream and store them in the vector using a
...
void KE::CodeScriptParser::FindDependentScopes(ParsingData& aData,
											   ScriptMemberID aNodeID) {
	...
	for (auto& outputPin : node->GetOutputPins()) {
		for (auto& connection : aData.scriptConnections[outputPin.ID]) {
			const std::vector<CodeScope*>& connectedNodeScopes =
				aData.evaluatedScopeData[connection.to.GetNodeID()];
			...
			for (auto& scope : connectedNodeScopes) {
				...
				if (nodeScopes.empty() ||
					std::ranges::find(nodeScopes, scope) == nodeScopes.end()) {
...
void KE::CodeScriptParser::CorrectUnscopedNodes(ParsingData& aData) {
	// some nodes (those that only in- and output values) don't have a scope
	// yet. We need to find a place for them.
	for (ScriptMemberID nodeID : aData.unscopedNodes) {
		...
		CodeScope* sharedParentScope =
			aData.evaluatedScopeData[nodeID].empty()
				? &aData.codeScopes[0]
				: aData.evaluatedScopeData[nodeID][0]->parentScope;

		for (auto* scope : aData.evaluatedScopeData[nodeID]) {
			if (scope->parentScope != sharedParentScope) {
				matchingParentScopes = false;
			}
		}
...
void KE::CodeScriptParser::DependencyHelper(
	ParsingData& aData, ScriptMemberID aNodeID,
	std::unordered_map<ScriptMemberID, bool, ScriptMemberID>& aWorkingData) {
	if (aWorkingData[aNodeID]) {
		return;
	}
	if (!aData.scriptNodes[aNodeID]) {
		return;
	}
	...
	for (const auto& input : aData.scriptNodes[aNodeID]->GetInputPins()) {
		for (const auto& connection : aData.scriptConnections[input.ID]) {
			...
			if (IsLinkNode(aData, toNode)) {
				auto linkConnections =
					aData.scriptConnections
						[aData.scriptNodes.at(toNode)->GetInputPins()[0].ID];
				if (linkConnections.empty()) {
					continue;
				}
				...
				if (std::ranges::find(aData.dependencyMap[aNodeID].dependencies,
									  linkToNode) ==
					aData.dependencyMap[aNodeID].dependencies.end()) {
					aData.dependencyMap[aNodeID].dependencies.push_back(
						linkToNode);
				}
				// aData.dependencyMap[aNodeID].dependencies.push_back(linkToNode);

				// if dependents doesn't contain the linkToNode, add it
				if (std::ranges::find(aData.dependents[linkToNode].begin(),
									  aData.dependents[linkToNode].end(),
									  aNodeID) ==
					aData.dependents[linkToNode].end()) {
			...
			// aData.dependencyMap[aNodeID].dependencies.push_back(toNode);
			if (std::ranges::find(aData.dependents[toNode].begin(),
								  aData.dependents[toNode].end(),
								  aNodeID) == aData.dependents[toNode].end()) {
...
std::string KE::CodeScriptParser::ParseCodeNode(ParsingData& aData,
												ScriptNode* aNodeToParse) {
	if (aNodeToParse == nullptr) {
		return "not a node";
	}

	std::string out;
	KE::CodeScriptNode* codeNode =
		dynamic_cast<KE::CodeScriptNode*>(aNodeToParse);
	if (codeNode == nullptr) {
		return std::format("{} is not a code node!\n",
		...
		switch (parsedCommand.type) {
			case ParseResult::ResultType::CreateScope: {
				...
				if (!aData.evaluatedScopeData[aNodeToParse->ID].empty()) {
					const auto& nodeScope =
						aData.evaluatedScopeData[aNodeToParse->ID][0];
					newScope->parentScope = nodeScope;
					nodeScope->childScopes.push_back(newScope);
				}
				if (newScope->parentScope == nullptr) {
					newScope->parentScope = &aData.codeScopes[0];
				}
			...
			case ParseResult::ResultType::GenerateVariable: {
				...
				commandOut =
					ReplaceCodeVariableNames(aData, commandOut, aNodeToParse);

				if (variable->level == CodeVariableLevel::Define &&
					parsedCommand.variable->parsedType == "float4") {
					aData.outputRenderNodes.insert(
						{variable->originID,
						 parsedCommand.variable->parsedName});
				}
			...
			case ParseResult::ResultType::AssignValue: {
				commandOut +=
					std::format("{} = {};\n", parsedCommand.assign->assignTo,
								parsedCommand.assign->parsedAssignment);
				commandOut =
					ReplaceCodeVariableNames(aData, commandOut, aNodeToParse);
			}
			default:
...
std::string KE::CodeScriptParser::GenerateCode(ParsingData& aData) {
	...
	for (const auto& node : aData.parsingOrder) {
		aData.generatedCode[node.GetNodeID()] =
			ParseCodeNode(aData, aData.scriptNodes[node.GetNodeID()]);
	}
...
bool KE::CodeScriptParser::IsLinkNode(ParsingData& aData,
									  ScriptMemberID aNodeID) {
	if (!aData.scriptNodes.contains(aNodeID)) {
		return false;
	}
	if (!aData.scriptNodes[aNodeID]) {
		return false;
...
void KE::CodeScriptParser::GeneratePreviewShaders(
	Script* aScript, const ParsingOutput& aData, Graphics* aGraphics,
	std::unordered_map<ScriptMemberID, PixelShader*, ScriptMemberID>&
		generatedShaderMap) {
	...
	for (int i = 0; i < outputCodeList.size(); i++) {
		generatedShaderMap[outputCodeList[i].first] =
			aGraphics->GetShaderLoader().CreatePixelShaderFromParsedScript(
				std::format("shaderPreviewScript_{}_PS",
							outputCodeList[i].first.idParts.nodeID),
				outputCodeList[i].second, "mainPreview");
	}
}

void KE::CodeScriptParser::RenderPreviewImages(
	Script* aScript, const ParsingOutput& aData,
	std::unordered_map<KE::ScriptMemberID, int, KE::ScriptMemberID>&
		aNodeTextureLinkageMap,
	std::array<ID3D11ShaderResourceView*, 16>& aOutShaderTextures,
	Graphics* aGraphics, KE::Material* aPreviewRenderMaterial) {
	ModelData modelData;
	Transform t;

	modelData.myMeshList =
		&aGraphics->GetModelLoader().Load("Data/EngineAssets/SmoothSphere.fbx");
	...
	if (aPreviewRenderMaterial) {
		...
		modelData.myRenderResources[0].myMaterial =
			aGraphics->GetTextureLoader().GetDefaultMaterial();
	}

	if (aData.parsingData.codeScopes.size() > 1 &&
		aData.parsingData.codeScopes[1].scopeStart.find("VFX") !=
			std::string::npos) {
		modelData.myRenderResources[0].myVertexShader =
			aGraphics->GetShaderLoader().GetVertexShader(SHADER_LOAD_PATH
														 "Model_VFX_VS.cso");
	} else {
		modelData.myRenderResources[0].myVertexShader =
			aGraphics->GetShaderLoader().GetVertexShader(
				SHADER_LOAD_PATH "Model_Deferred_VS.cso");
	}
	...
	// this gives us a 16x16 grid of 256x256 previews, which should be enough
	// for now (that's 256 previews)

	Camera renderCamera;
	renderCamera.SetPerspective(previewSize, previewSize,
								90.0f * KE::DegToRadImmediate, 0.01f, 10.f);
	...
	rt->Resize(
		previewAtlasSize,
		previewAtlasSize);	// we really dont want to do this here in the future
	...
	for (int i = 0; i < outputCodeList.size(); i++) {
		modelData.myRenderResources[0].myPixelShader =
			aGraphics->GetShaderLoader().GetPixelShader(
				std::format("shaderPreviewScript_{}_PS",
							outputCodeList[i].first.idParts.nodeID));
		if (modelData.myRenderResources[0].myPixelShader ==
			aGraphics->GetShaderLoader().GetDefaultPixelShader()) {
			continue;
		}
	...
	aOutShaderTextures[0] = modelData.myRenderResources[0]
								.myMaterial->myTextures[0]
								->myShaderResourceView.Get();
	aOutShaderTextures[1] = modelData.myRenderResources[0]
								.myMaterial->myTextures[1]
								->myShaderResourceView.Get();
	aOutShaderTextures[2] = modelData.myRenderResources[0]
								.myMaterial->myTextures[2]
								->myShaderResourceView.Get();
	aOutShaderTextures[3] = modelData.myRenderResources[0]
								.myMaterial->myTextures[3]
								->myShaderResourceView.Get();
}
...
bool KE::CodeScriptParser::Parse(Script* aScript, ParsingOutput*& anOutput) {
	...
	anOutput->parsingData.codeScopes.push_back(
		CodeScope(INT_MAX, {}, {}, "", "", nullptr));
...
void KE::CodeScriptParser::GetOutputRenderCode(
	const ParsingOutput& aData,
	std::vector<std::pair<ScriptMemberID, std::string>>& aOutputCodeList) {
	for (const auto& node : aData.parsingData.outputRenderNodes) {
		std::string out;

		if (aData.parsingData.codeScopes[1].scopeStart.find("VFX") !=
			std::string::npos) {
		...
		for (const auto& parseID : aData.parsingData.parsingOrder) {
			...
			if (nodeCode.find("not a") != std::string::npos) {
				continue;
			}
			if (nodeCode.find("OUTPUT") != std::string::npos) {
				continue;
			}
			out += nodeCode;
			...
			if (parseID == node.first.GetNodeID()) {
				size_t varNameStart =
					nodeCode.find_first_of("float4") + strlen("float4");
				size_t varNameEnd =
					nodeCode.find_first_of(" ", varNameStart + 1);
				std::string varName = nodeCode.substr(
					varNameStart + 1, varNameEnd - varNameStart);
...
void KE::CodeScriptParser::AppendParsingOrder(ParsingData& aData,
											  ScriptMemberID aNodeID) {
	if (std::find(aData.parsingOrder.begin(), aData.parsingOrder.end(),
				  aNodeID) == aData.parsingOrder.end()) {
		aData.parsingOrder.push_back(aNodeID);
	}
}

void KE::CodeScriptParser::ParsingOrderHelper(ParsingData& aData,
											  ScriptMemberID aNodeID) {
...
KE::ScriptMemberID KE::CodeScriptParser::GetLineAuthor(
	ParsingData& aData, const std::string& aLine) {
	ScriptMemberID out;
	out.combinedID = INT_MIN;

	for (const auto& node : aData.scriptNodes) {
		if (aData.parsedNodeMap[node.first.GetNodeID()].find(aLine) !=
			std::string::npos) {
			out = node.first;
			break;
		}
...
std::pair<size_t, size_t> KE::CodeScriptParser::GetCodeAreaFromAuthor(
	const ParsingData& aData, ScriptMemberID anAuthor,
